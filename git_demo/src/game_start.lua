...
function restart(callback)
    GlobalAudio:getInstance():DeleteMe()
    ...
    EventManager:getInstance():UnBindAll() 
    GlobalReminder:getInstance():DeleteMe()
...
function uploadFile(realname, name, time)
    ...
    if type(str) ~= "string" or string.len(str) < 1000 then
        ...
        EventManager:getInstance():Fire(EventId.ON_VOICE_UPLOAD_RESULT, {false, filename, string.len(str)})
    else
