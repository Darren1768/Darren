...
function LevupgradeWindow:open_callback()
	...
	self.new_lev_num = CommonNum.new(107, self.main_container, 1, -6, cc.p(0.5, 0.5))
	self.new_lev_num:setPosition(cc.p(196,112))
...
function LevupgradeWindow:setAwardInfo( )
	...
	if self.award_data and next(self.award_data) ~= nil then
		...
		for i,v in ipairs(self.award_data) do
			...
			item:setPosition(cc.p(pos_x, panel_size.height/2))
		end
...
-- 		local path_icon = PathTool.getSingleImgForDownLoad("bigbg/battledrama", config.icon)
