...
function BrowsePanel:open_callback()
    ...
    main_container:getChildByName("Image_33"):getChildByName("Text_32"):setString(TI18N("½±ÀøÔ¤ÀÀ"))
    self.btn_con = main_container:getChildByName("btn_con")
    self.btn_con:getChildByName("Text_33"):setString(TI18N("È·ÈÏ"))
        ...
        for i=1, num do
    		...
    		if self.reward_list[i] then
                ...
                if data_reward[i].bid == 25 and data.is_holiday == 1 then
                    self.reward_list[i]:holidHeroExpeditTag(true, TI18N("ÏÞÊ±ÌáÉý"))
                else
