...
function WorldMapMainWindow:openRootWnd(data)
    ...
    if dungeon_data ~= nil then
        local dun_id = dungeon_data.last_dun_id
        if self.data and self.data.map_id ~=0 then
            dun_id = Config.DungeonData.data_drama_dungeon_getmap_fun(self.data.map_id).checkpoint_id[1][1]
        end
