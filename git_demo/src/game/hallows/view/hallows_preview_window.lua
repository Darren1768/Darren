...
function HallowsPreviewWindow:__init()
	...
	self.res_list = {
		...
		{path = PathTool.getSingleImgForDownLoad("bigbg","bigbg_69", true), type = ResourcesType.single},
		{path = PathTool.getSingleImgForDownLoad("bigbg","bigbg_70"), type = ResourcesType.single}
	}
...
function HallowsPreviewWindow:open_callback(  )
	...
	self.background:loadTexture(PathTool.getSingleImgForDownLoad("bigbg","bigbg_69",true), LOADTEXT_TYPE)
...
function HallowsPreviewItem:_onClickContainer(  )
	if self.hallows_status == HallowsConst.Status.close then
		...
		if controller:getHallowsRoot() then
			EventManager:getInstance():Fire(HallowsEvent.UndateHallowsInfoEvent, self.data.id)
		else
