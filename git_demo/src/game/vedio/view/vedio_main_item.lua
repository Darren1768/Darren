...
function VedioMainItem:configUI(  )
    ...
    self.pos_result_up = cc.p(self.result_win:getPosition())
    self.pos_result_down = cc.p(self.result_loss:getPosition())
    ...
    self.collect_text = self.collect_btn:getChildByName("Text_1")
    
    self.like_btn = image_panel:getChildByName("like_btn")
    self.like_btn:ignoreContentAdaptWithSize(true)
    self.like_text = self.like_btn:getChildByName("Text_1")
...
function VedioMainItem:register_event(  )
    ...
    registerButtonEventListener(self.play_btn, function (  )
        if self.data then
            ...
            if self.is_myself or self.is_collect then
                EventManager:getInstance():Fire(VedioEvent.UpdateVedioDataEvent, new_data)
            end
    ...
    registerButtonEventListener(self.collect_btn, function (  )
        if self.data then
            ...
            if self.data.is_collect == TRUE then
                ...
                self.collect_text:setString(TI18N('收藏'))
    ...
    registerButtonEventListener(self.like_btn, function (  )
        if self.data then
            ...
            self.like_text:setString(TI18N('已点赞'))
    ...
    EventManager:getInstance():Bind(VedioEvent.UpdateVedioDataEvent,self, function(data)
    ...
    EventManager:getInstance():Bind(VedioEvent.UpdateTodayLikeNum,self, function(data)
...
function VedioMainItem:setData( data )
    ...
    if data.is_collect == TRUE then
        self.collect_text:setString(TI18N('取消收藏'))
    else
        self.collect_text:setString(TI18N('收藏'))
    end
    ...
    if self.is_myself then
        self.like_btn:setTouchEnabled(false)
        setChildUnEnabled(true, self.like_btn)
        self.like_text:setString(TI18N('点赞'))
    ...
    if data.ret == 1 then
        self.result_win:setPosition(self.pos_result_up)
        self.result_loss:setPosition(self.pos_result_down)
    elseif data.ret == 2 then
        self.result_win:setPosition(self.pos_result_down)
        self.result_loss:setPosition(self.pos_result_up)
    else
    ...
    for i=1,9 do
        ...
        if role_data then
            if hero_item == nil then
                local node_parent = self.image_panel:getChildByName('image_icn_'..i)
                hero_item = self:createHeroExhibitionItemByIndex(i, node_parent)
                self.left_item_list[i] = hero_item
            else
                hero_item:setVisible(true)
            end
            hero_item:setRotation(45)
    ...
    for i=1,9 do
        ...
        if role_data then
            if hero_item == nil then
                local node_parent = self.image_panel:getChildByName('image_icn2_'..i)
                hero_item = self:createHeroExhibitionItemByIndex(i, node_parent)
...
function VedioMainItem:createHeroExhibitionItemByIndex(index, parent)
    ...
    local pos = cc.p(parent:getContentSize().width/2,parent:getContentSize().height/2)
