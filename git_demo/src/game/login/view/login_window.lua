...
function LoginWindow:openRootWnd(index)
	...
	delayOnce(function()
        GlobalAudio:getInstance():playMusic(GlobalAudio.AUDIO_TYPE.SCENE, "s_001", true)
    end,1)
...
function LoginWindow:preloadSomething(  )
	...
	if SOUND_EFFECT_LIST then 
		if GlobalAudio:getInstance().preLoadEffectByPath then
	...
	if SCENE_MUSIC_LIST then
		if GlobalAudio:getInstance().preLoadMusicByPath then
			for i,v in ipairs(SCENE_MUSIC_LIST) do
				GlobalAudio:getInstance():preLoadMusicByPath(v)
			end
		end
	end
