...
function FriendController:registerEvents()
    ...
    EventManager:getInstance():Bind(FriendEvent.REQUEST_LIST,self,function()
     ...
     EventManager:getInstance():Bind(FriendEvent.FRIEND_ADD,self,function(srv_id,rid)
            self:addOther(srv_id,rid)
        end)
    

    EventManager:getInstance():Bind(FriendEvent.FRIEND_QUERY_FIND,self,function(value)
            self:queryFind(value)
        end)
    

    EventManager:getInstance():Bind(FriendEvent.FRIEND_RECOMMEND,self,function()
            self:recommend()
        end)
    

      ---好友邮件界面
     EventManager:getInstance():Bind(FriendEvent.OPEN_FRIEND_INFOM,self,function(data,count,begin_pos,group_type)           
    ...
    EventManager:getInstance():Bind(FriendEvent.UPDATE_COUNT,self, function(type,group,list)
            self:updateFriendTishi()
        end)
...
function FriendController:friendListHandler( data )
    ...
    EventManager:getInstance():Fire(FriendEvent.FRIEND_LIST, self.model:getArray())
    self:apply()
    ...
    if old_index and  new_index and vo then
        if vo then
            if data.is_online == 1 then
                EventManager:getInstance():Fire(FriendEvent.FRIEND_UPDATE_ITEM, vo, 0)
...
function FriendController:applyListHandler( data )        
    ...
    EventManager:getInstance():Fire(FriendEvent.UPDATE_COUNT,2,#self.model.apply)--单个数据增加
    EventManager:getInstance():Fire(FriendEvent.FRIEND_APPLY)
...
function FriendController:refuseApplyListHandler(data)
    --Debug.info(data)
    if data.code == 1 then
        EventManager:getInstance():Fire(FriendEvent.UPDATE_APPLY_LIST)
        self.model.apply = {}
        self:updateFriendTishi()
    end
end
...
--     Debug.info(data)
    ...
    EventManager:getInstance():Fire(FriendEvent.UD_COMMEND_LIST,list)
end
...
function FriendController:queryFindHandler( data )
    ...
    EventManager:getInstance():Fire(FriendEvent.FRIEND_QUERY_RESULT, t)
end
...
function FriendController:batchStrengthHandler(data)
    --Debug.info(data)
    if data.code == 1 then
        -- EventManager:getInstance():Fire(FriendEvent.STRENGTH_UPDATE, data)
        EventManager:getInstance():Fire(FriendEvent.Update_Red_Point)
    end
    message(data.msg)
end
...
function FriendController:addToListHandler( data )
    ...
    if self.model then
        ...
        if self.model.apply then
         ...
         EventManager:getInstance():Fire(FriendEvent.UPDATE_APPLY)
...
function FriendController:delOtherHandler( data )
    ...
    if data.code == 1 then
        ...
        if old_data then
            ...
            EventManager:getInstance():Fire(FriendEvent.FRIEND_DELETE,old_data,self.group_type)
        end
        self:closeInfom()
    end
...
function FriendController:delMeHandler( data )
    ...
    if old_data then
        ...
        EventManager:getInstance():Fire(FriendEvent.UPDATE_GROUP_COUNT)
    end
end
...
function FriendController:deleteConnecter(vo)
    local confirm_handler = function()
        ...
        EventManager:getInstance():Fire(ContactEvent.CLOSE_TALK_INFO) 
        self:closeInfom()
        EventManager:getInstance():Fire(FriendEvent.FRIEND_DELETE,vo,FriendConst.FriendGroupType.communicate)
...
function FriendController:deleteBlackConnecter(srv_id,rid)
        ...
        EventManager:getInstance():Fire(ContactEvent.CLOSE_TALK_INFO) 
        self:closeInfom()
        EventManager:getInstance():Fire(FriendEvent.FRIEND_DELETE,{srv_id=srv_id,rid=rid},FriendConst.FriendGroupType.friend)
end
...
function FriendController:getRolesOnline(data)
    if data then
       EventManager:getInstance():Fire(FriendEvent.UPDATE_ONLINE,data.online_roles)
    end
end
...
function FriendController:getFriendWndDelStatus(  )
    if self.friend_manager_window and self.friend_manager_window:getDelFriendStatus() then
        return true
    end
    return false
end
...
--竖版好友管理界面
function FriendController:openFriendManagerWindow(bool)
    if bool == true then 
        if not self.friend_manager_window  then
            self.friend_manager_window = FriendManagerWindow.New()
        end
        self.friend_manager_window:open()
    else
        if self.friend_manager_window then 
            self.friend_manager_window:close()
            self.friend_manager_window = nil
        end
    end
end
...
function FriendController:addPrivateMsg(chat_vo)
    ...
    --         friend.gsrv_id=""
    --         friend.guild_name = ""
    --         EventManager:getInstance():Fire(FriendEvent.STRANGER_LIST, self.model:getArray() ,self.model.apply)
    --     end
    ...
    --         EventManager:getInstance():Fire(FriendEvent.UPDATE_COUNT,1,group_id,self.pri_list[group_id])--单个数据增加
    --     end
    --     EventManager:getInstance():Fire(FriendEvent.UPDATE_GROUP_COUNT)
    -- end
end
...
function FriendController:delPriCount( key )
    ...
    EventManager:getInstance():Fire(FriendEvent.UPDATE_COUNT,1,key,0)--单个数据增加
end
...
-- print("======handle13333======")
    if data.code == 1 then
        ...
        EventManager:getInstance():Fire(FriendEvent.FRIEND_DELETE,data,self.group_type or FriendConst.FriendGroupType.black_list) --类型2是黑名单分组
    end
end