...
function FriendCheckInfoWindow:open_callback(  )
    ...
    self.head:setPosition(cc.p(83, 170))
    ...
    -- self.vip_label = CommonNum.new(219, self.info_con, 0, -2, cc.p(0, 0.5))
    -- self.vip_label:setPosition(self.vip_icon:getPositionX() + 20, self.vip_icon:getPositionY())
    ...
    self.elfin_tree_lv:setString('神器')
    self.elfin_skill_panel = self.main_container:getChildByName("elfin_skill_panel")

    self.fight_label = CommonNum.new(101, self.main_container, 0, -2, cc.p(0, 0))
    self.fight_label:setPosition(410, 278)
    ...
    -- local nameArray={'jubao_btn','black_btn','pk_btn','friend_btn'}
    -- for i=1,4 do
    --     local btn=self.main_panel:getChildByName(nameArray[i])
    --     btn:loadTexture('hl_common_4_8.png',1)
    -- end
end
...
function FriendCheckInfoWindow:register_event()
    ...
    if self.head then
        self.head:addCallBack(function(sender, event_type)
                customClickAction(sender, event_type)
    ...
    --接受数据
    EventManager:getInstance():Bind(RoleEvent.DISPATCH_PLAYER_VO_EVENT,self,function ( data )
...
function FriendCheckInfoWindow:updateData( data )
    ...
    for i=1,1 do
        ...
        if not elfin_skill_item then
            elfin_skill_item = SkillItem.new(true, true, false, 0.6, true)
    ...
    for i,v in ipairs(temp) do
        delayRun(self.main_panel, i/60, function() 
                if self.item_list[i]==nil then 
                    ...
                    partner_item:setData(v,true)
                    -- partner_item:setBaseData(v)
