...
function AdventureEvtAnswerWindow:__init(data)
    ...
    self.res_list = {
        ...
        { path = PathTool.getSingleImgForDownLoad("bigbg", "bigbg_25"), type = ResourcesType.single },
    }
    self.btn_list = {}
    self.str = ""
end
function AdventureEvtAnswerWindow:open_callback()
    ...
    loadSpriteTexture(self.role_bg,PathTool.getSingleImgForDownLoad("bigbg", "bigbg_25"),LOADTEXT_TYPE)
...
function AdventureEvtAnswerWindow:register_event()
    ...
    if self.close_btn then
     ...
     EventManager:getInstance():Bind(AdventureEvent.Update_Evt_Answer_Info,self,function (data)
    ...
    if answer_config then
        ...
        for i = 1, num do
            if not self.btn_list[i] then
                ...
                local name = createLabel(33, 175, nil, 89, 45, "", btn:getRoot(), nil, cc.p(0, 0.5), TITLE_FONT)
                btn.name = name
