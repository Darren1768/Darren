...
function HomePetGoBackPanel:openRootWnd(setting)
    ...
    if self.config.s_id == 1 then --出行
        title_res = PathTool.getSingleImgForDownLoad("bigbg/homepet", "home_pet_go_out_bg", false)
    ...
    elseif self.config.s_id == 2 then -- 回来
        title_res = PathTool.getSingleImgForDownLoad("bigbg/homepet", "home_pet_go_back_bg", false)
