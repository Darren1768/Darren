...
-- --------------------------------------------------------------------
BattleFieldController = BattleFieldController or BaseClass(BaseController)

function BattleFieldController:config()
    self.dispather = EventManager:getInstance()
    self.allRedDatas = {}
end

function BattleFieldController:registerEvents()
end

function BattleFieldController:registerProtocals()
end

----------- @ 界面相关
-- 打开区场景
function BattleFieldController:openMainScene(status, data)
    if status == true then
        if not self.area_scene_wnd then
            self.area_scene_wnd = BattleFieldWindow.New(data)
        end
...
-- 引导需要
function BattleFieldController:getGhostdomSceneRoot(  )
    if self.area_scene_wnd then
...
-- function BattleFieldController:onClickGhostdomBuildById(area_type, bid)
...
function BattleFieldController:setBuildRedStatus( build_id, red_data )
...
function BattleFieldController:updateRedData( build_id, red_data )
    self.allRedDatas[build_id] = red_data
end

function BattleFieldController:getRedData( build_id )
    return self.allRedDatas[build_id]
end



function BattleFieldController:__delete()
    self.allRedDatas = {}
end