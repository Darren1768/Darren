...
function GuildwarAwardBoxWindow:__init()
	...
	self.res_list = {
		...
		{path = PathTool.getSingleImgForDownLoad("bigbg/guildwar","guildwar_3"), type = ResourcesType.single },
		{path = PathTool.getSingleImgForDownLoad("bigbg/guildwar","guildwar_4"), type = ResourcesType.single },
	}
end
	...
	if self.data.rid == 0 then
		...
		if not self.load_image_flag then
			...
			if self.data.status == GuildwarConst.box_type.gold then
				self.image_box:loadTexture(PathTool.getSingleImgForDownLoad("bigbg/guildwar","guildwar_3"), LOADTEXT_TYPE)
			elseif self.data.status == GuildwarConst.box_type.copper then
				self.image_box:loadTexture(PathTool.getSingleImgForDownLoad("bigbg/guildwar","guildwar_4"), LOADTEXT_TYPE)
			end
		end
	else
