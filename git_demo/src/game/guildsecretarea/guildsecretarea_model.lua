...
function GuildsecretareaModel:checkRedPoint(is_check_main)
    ...
    if is_check_main then
        ...
        GhostdomController:getInstance():setBuildRedStatus(GhostdomConst.Build_Type.guild,redpoint) 
    end
    EventManager:getInstance():Fire(GuildEvent.UpdateGuildRedStatus, GuildConst.red_index.guild_secret_area, status)
    return status
end
