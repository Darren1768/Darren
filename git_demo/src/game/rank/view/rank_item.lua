...
function RankItem:configUI( ... )
    ...
    local res = PathTool.getResFrame("common","hl_common_20")
    ...
    --选择框
    -- local line = createImage(self.back,PathTool.getResFrame("common","hl_common_15"),10,self.height/2,cc.p(0.5,0.5),true,0,false)
    -- line:setContentSize(cc.size(25, 7))
	...
	self.rank_index =  createLabel(40,cc.c3b(98,53,43),nil,60,self.height/2,"",self,0, cc.p(0.5,0.5))
...
function RankItem:setData( vo, index)
	...
	if self.index >= 1 and self.index <= 3 then
		...
		self.rank_icon:loadTexture(PathTool.getResFrame("common","hl_common_200"..(self.index-1)),LOADTEXT_TYPE_PLIST)
        -- self.background:loadTexture(PathTool.getResFrame("rank","hl_rank_14"..(self.index+2)),LOADTEXT_TYPE_PLIST)
		self.background:loadTexture(PathTool.getResFrame("rank","hl_rank_0001"),LOADTEXT_TYPE_PLIST)

        self.background:setCapInsets(cc.rect(20,20,1,1))
    else
        self.rank_index:setVisible(true)
        self.rank_icon:setVisible(false)
        -- self.background:loadTexture(PathTool.getResFrame("common","hl_common_20"),LOADTEXT_TYPE_PLIST)
		self.background:loadTexture(PathTool.getResFrame("rank","hl_rank_0001"),LOADTEXT_TYPE_PLIST)
...
function RankItem:updateDataByRankType()
	...
	if self.rank_type == RankConstant.RankType.union then 
		self.play_head = self:createPlayerHead(115,self.height/2)
		local face_id = self.data.leader_face
		local avatar_bid = self.data.leader_avatar_bid
	...
	elseif self.rank_type == RankConstant.RankType.union_union then 
			self.play_head = self:createPlayerHead(115,self.height/2)
			local face_id = self.data.leader_face
			local avatar_bid = self.data.leader_avatar_bid
			local vo = Config.AvatarData.data_avatar[avatar_bid]
			...
			self.play_head:setHeadRes(face_id, false, LOADTEXT_TYPE, self.data.face_file, self.data.face_update_time)
			self.play_head:setLev(self.data.leader_lev)
	else
		self.play_head = self:createPlayerHead(115,60)
	...
	if self.rank_type == RankConstant.RankType.power or self.rank_type == RankConstant.RankType.action_power then 
		local label = createLabel(26,cc.c3b(0x62,0x35,0x2b),nil,200,self.height/2,"",self.other_panel,0, cc.p(0,0.5))
		label:setString(role_name)

		local label = createRichLabel(24, cc.c3b(0xb2,0x77,0x42),cc.p(0.5,0.5), cc.p(480,self.height/2), 0, 0, 500)
	...
	elseif self.rank_type == RankConstant.RankType.union_boss then
		local label = createLabel(24, cc.c3b(0x62,0x35,0x2b), nil, 200, self.height / 2, '', self.other_panel, 0, cc.p(0, 0.5))
		label:setString(role_name)
		--战力
		local label = createRichLabel(24, cc.c3b(0x62,0x35,0x2b), cc.p(0.5, 0.5), cc.p(505, self.height / 2 + 30), 0, 0, 500)
		self.other_panel:addChild(label)
	...
	elseif self.rank_type == RankConstant.RankType.drama or self.rank_type == RankConstant.RankType.action_drama  then 
		...
		label:setString(role_name)
		--装备评分
		local label = createLabel(24,cc.c3b(0x62,0x35,0x2b),nil,495,self.height/2,"",self.other_panel,0, cc.p(0.5,0.5))
	...
	elseif self.rank_type == RankConstant.RankType.union then 
		...
		--名字
		local label = createLabel(24,cc.c3b(98,53,43),nil,204,80,"",self.other_panel,0, cc.p(0,0.5))
		label:setString(role_name)
		--宗主名字
		local label = createLabel(22,cc.c3b(129,104,65),nil,204,50,"",self.other_panel,0, cc.p(0,0.5))
		label:setString(TI18N("会长：")..(self.data.leader_name or ""))
		--等级
		-- local label = createLabel(24,cc.c3b(0x62,0x35,0x2b),nil,342,self.height/2,"",self.other_panel,0, cc.p(0.5,0.5))
		-- label:setString(self.data.lev or 313)
		--人数
		local label = createLabel(24,cc.c3b(98,53,43),nil,425,self.height/2,"",self.other_panel,0, cc.p(0.5,0.5))
		local num1 = self.data.members_num or 0
		local num2 = self.data.members_max or 0
		label:setString(num1.."/"..num2)

		--战力
		local power_bg =	createSprite(PathTool.getResFrame("common2", "hl_common_3079"), 530, self.height/2,self.other_panel, cc.p(0.5,0.5))

		 local power_11 =  createSprite(PathTool.getResFrame("common", "hl_common_28"), 20, 18, power_bg, cc.p(0.5,0.5))
		 power_11:setScale(0.8)
		local label = CommonNum.new(220, power_bg, num, -1, cc.p(0, 0))
		label:setPosition(32, 18)
        label:setNum(self.data.power or 0)
		label:setScale(0.8)
	...
	elseif self.rank_type == RankConstant.RankType.union_union then 
		...
		--宗主名字
		local label = createLabel(22,cc.c3b(129,104,65),nil,204,50,"",self.other_panel,0, cc.p(0,0.5))
		label:setString(TI18N("联盟长：")..(self.data.leader_name or ""))
		--等级
		-- local label = createLabel(24,cc.c3b(0x62,0x35,0x2b),nil,342,self.height/2,"",self.other_panel,0, cc.p(0.5,0.5))
		-- label:setString(self.data.lev or 313)
		--人数
	...
	elseif self.rank_type == RankConstant.RankType.tower or self.rank_type == RankConstant.RankType.action_tower then 
		...
		label:setString(SpiritController:getInstance():getStrByLevel(self.data.val1))
		--通关时间
		local label = createLabel(24,cc.c3b(0x62,0x35,0x2b),nil,500,self.height/2,"",self.other_panel,0, cc.p(0,0.5))
	...
	elseif self.rank_type == RankConstant.RankType.guild_war then 
		...
		label:setString(role_name)
		--星数
		local label = createLabel(24,cc.c3b(0x62,0x35,0x2b),nil,402,self.height/2,"",self.other_panel,0, cc.p(0,0.5))
	...
	elseif self.rank_type == RankConstant.RankType.elite then
        --名字
		local label = createLabel(24,cc.c3b(0x62,0x35,0x2b),nil,198,self.height/2+20,"",self.other_panel,0, cc.p(0,0.5))
        ...
        --积分
		local score = createLabel(24,cc.c3b(0x62,0x35,0x2b),nil,509,self.height/2,"",self.other_panel,0, cc.p(0,0.5))
		score:setString(self.data.score)
	elseif self.rank_type == RankConstant.RankType.ladder then --天梯排行 --lwc
		local label = createLabel(24,cc.c3b(0x62,0x35,0x2b),nil,200,self.height/2 + 18,"",self.other_panel,0, cc.p(0,0.5))
		label:setString(role_name)

		local name = self.data.guild_name 
		local label1 = createLabel(24,cc.c3b(0x62,0x35,0x2b),nil,480,self.height/2,"",self.other_panel,0, cc.p(0.5,0.5))
		...
		local label = createRichLabel(24, cc.c3b(0x62,0x35,0x2b),cc.p(0,0.5), cc.p(200,self.height/2 - 18), 0, 0, 500)
		self.other_panel:addChild(label)
		local power = self.data.val1 or 0
		local res = PathTool.getResFrame('common', 'hl_common_134')
		label:setString(string.format("<img src='%s' /> %s", res, power))
	...
	elseif self.rank_type == RankConstant.RankType.fans then 
		...
		local label = createLabel(24,cc.c3b(0x62,0x35,0x2b),nil,428,self.height/2,"",self.other_panel,0, cc.p(0,0.5))
...
function RankItem:addPraise(data)
	self._btnPraise = createTypeButton(self.other_panel, nil, 504,43, ButtonType.red)
	
	self._praise = createSprite(PathTool.getResFrame("common", "hl_common_125"), 31, 28, self._btnPraise, cc.p(0.5,0.5), LOADTEXT_TYPE_PLIST)
