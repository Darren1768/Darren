...
function RankController:registerProtocals()
    ...
    self:RegisterProtocal(12905, "handle_12905")  --请求联盟排行榜数据
...
function RankController:handle_12900( data )
    ...
    if data then
        EventManager:getInstance():Fire(RankEvent.RankEvent_Get_Rank_data,data)
    end
end
...
function RankController:handle_12901( data )
    ...
    EventManager:getInstance():Fire(RankEvent.RankEvent_Get_Time_event,data)
end
...
function RankController:handle_12902( data )
    EventManager:getInstance():Fire(RankEvent.RankEvent_Get_First_data,data)
end
--请求公会排行榜数据
...
--请求联盟排行榜数据
function RankController:send_12905(is_cluster)
    --请求排行榜
     local cluster = 0
    if is_cluster == true then
        cluster = 1
    ...
    protocal.num = 100
    protocal.is_cluster = cluster
    self:SendProtocal(12905,protocal)
end
function RankController:handle_12905( data )
    EventManager:getInstance():Fire(RankEvent.RankEvent_Get_Rank_data,data)
end
