...
BattleNationalWarTwoWindow = BattleNationalWarTwoWindow or BaseClass(BaseView)

function BattleNationalWarTwoWindow:__init()
    
    self.win_type = WinType.Full
    self.layout_name = "battle_national_war/battle_national_war_two_window"
    --self.background_path = PathTool.getSingleImgForDownLoad("bigbg","hl_bigbg_55", false)
    self.res_list = {
        {path = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_102",false), type = ResourcesType.single},
        { path = PathTool.getPlistImgForDownLoad("nationalwar","nationalwar"), type = ResourcesType.plist}
    }
    
    self.config = BattleNationalWarConst.btn_data2[1]
...
function BattleNationalWarTwoWindow:open_callback( )
...
-- 更新位置
function BattleNationalWarTwoWindow:updateMainScene(x, y)
    y = y or self.init_y
    for i=1, self.layer_num do
...
function BattleNationalWarTwoWindow:register_event()
...
function BattleNationalWarTwoWindow:openRootWnd()
...
-- 移动场景
function BattleNationalWarTwoWindow:moveMainScene(x)
...
-- 位置校准
function BattleNationalWarTwoWindow:scaleCheckPoint( _x )
    if _x > 0 then
        _x = 0
...
function BattleNationalWarTwoWindow:setVisible(bool)
...
-- 创建建筑
function BattleNationalWarTwoWindow:createBtnList()
...
function BattleNationalWarTwoWindow:updateItemList()
     for i , build_item in pairs(self.btn_item_list) do 
        --  build_item:setBaseData() 
...
function BattleNationalWarTwoWindow:addRoleToLayer1(obj)
    if  not obj then
        return
    end 
...
-- 更新或创建地图和建筑
function BattleNationalWarTwoWindow:setMapAndBuild()
...
-- 初始化地图数据
function BattleNationalWarTwoWindow:initMapCacheData()
    self.map_data = {}
    --local map_res = _string_format("resource/area_scene/map/%s", self.config.res_id)
    for i = 1, self.layer_num  do
        local res_path = "resource/bigbg/hl_bigbg_102.png"
...
-- 创建地图
function BattleNationalWarTwoWindow:createMap()
...
-- 创建数字展示
function BattleNationalWarTwoWindow:createShowResultView()
...
-- 创建建筑点击按钮
function BattleNationalWarTwoWindow:createBuildBtn()
...
-- 创建攻击按钮
function BattleNationalWarTwoWindow:createAttackBtnView()
...
-- 添加编队
function BattleNationalWarTwoWindow:createTeamView()
...
function BattleNationalWarTwoWindow:clearTeamView()
    self._team_title_list ={}
...
function BattleNationalWarTwoWindow:createQiZhiProgress()
...
function BattleNationalWarTwoWindow:showProgress()
...
function BattleNationalWarTwoWindow:hideProgress()
...
-- 倒计时
function BattleNationalWarTwoWindow:countDownTime(less_time)
...
function BattleNationalWarTwoWindow:setTimeFormatString(time)
    if time > 0 then
        self.time_num:setString(TimeTool.GetTimeFormatDayIIIIIII2(time))
    else
        self.time_num:setString("00:00")
    end
end
...
function BattleNationalWarTwoWindow:onClickCloseBtn()
   
    _ctrl:openNationalTwoScene(false)
end

function BattleNationalWarTwoWindow:onClickAutoBtn()
    _ctrl:openNationalTwoResultScene(true)
end


function BattleNationalWarTwoWindow:close_callback()
