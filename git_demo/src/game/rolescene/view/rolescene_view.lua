...
function RoleScene:playBackgroundMusic()
	...
	-- if GlobalAudio then
	-- 	GlobalAudio:getInstance():playMusic(GlobalAudio.AUDIO_TYPE.SCENE, misc_music, true)
	-- end
end
...
function RoleScene:registerEvent()
	...
	local function onTouchEnded( touch, event )
		if self.hero == nil then return end
		EventManager:getInstance():Fire(NpcEvent.CloseNpcTalkViewEvent) --移除npc对话面板
		if self.model then
    ...
    self.root_wnd:getEventDispatcher():addEventListenerWithSceneGraphPriority(self.touch_listener, self.root_wnd)
     ...
     EventManager:getInstance():Bind(RolesceneEvent.UPDATE_RED_INFO,self, function(params, from_type) 
...
function RoleScene:startWalk(sp, ep)
	...
	if self.findVo and self.findVo.sceneId == self.model:getSceneId() then
		if MapUtil.isOnRange(heroPos, target, self.model.unitRange) then
            EventManager:getInstance():Fire(RolesceneEvent.SCENE_WALKEND, self.findVo)
			return
		end
	end
...
function RoleScene:cancelWalk(is_auto)
	...
	self.hero:stopMove()
	-- EventManager:getInstance():Fire(SceneEvent.FINDING_STOP_WALK)
