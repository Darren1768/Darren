...
function RolesceneModel:registerEvent()
     EventManager:getInstance():Bind(SceneEvent.SCENE_WALKNEXT,self, function(pos)
			if self.scene then
				self.scene:walkNext(pos)
			end
		end)

     EventManager:getInstance():Bind(SceneEvent.SCENE_WALKEND,self, function(pos)
			if self.scene then
            	self.astar:reset()
			end
     ...
     EventManager:getInstance():Bind(RolesceneEvent.SCENE_WALKEND,self, function(vo)
            if self.scene then
                self.astar:reset()
            end
	...
	EventManager:getInstance():Bind(SceneEvent.SCENE_PLAYER_CLICK, self, clickPlayer)
	...
	EventManager:getInstance():Bind(SceneEvent.SCENE_UNIT_CLICK, self, clickUnit)
...
function RolesceneModel:clickSceneUnit(vo)
    ...
    if config ~= nil then
        ...
        if unit ~= nil then
		    EventManager:getInstance():Fire(SceneEvent.SCENE_NEAR_NPC, {vo.id, vo.battle_id, vo.base_id})
        end
    end
end
...
function RolesceneModel:__delete()
    ...
    -- 移除掉npc对话的
    EventManager:getInstance():Fire(NpcEvent.CloseNpcTalkViewEvent)

    EventManager:getInstance():Fire(RolesceneEvent.SCENE_CHANGE)
    --移除首席
...
function RolesceneModel:changeScene(data)
    ...
    self:sceneStart()
    EventManager:getInstance():Fire(RolesceneEvent.SCENE_CHANGE, self.scene_data)
end
