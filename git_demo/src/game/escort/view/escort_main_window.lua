...
function EscortMainWindow:__init()
	...
	self.res_list = {
        {path = PathTool.getSingleImgForDownLoad("bigbg", "bigbg_53", true), type = ResourcesType.single},
...
function EscortMainWindow:open_callback()
	...
	self.background:loadTexture(PathTool.getSingleImgForDownLoad("bigbg", "bigbg_53", true), LOADTEXT_TYPE)
...
function EscortMainWindow:register_event()
	...
	EventManager:getInstance():Bind(EscortEvent.UpdateEscortBaseEvent,self, function()
			self:initPlunderList()
		end)
	

	EventManager:getInstance():Bind(EscortEvent.UpdateEscortMyInfoEvent,self, function() 
			self:updateMyInfo(true)
		end)
	

	EventManager:getInstance():Bind(EscortEvent.UpdateEscortPlayerList,self, function(data)
...
function EscortMainWindow:updateMyBaseCount()
	...
	EventManager:getInstance():Fire(ActivityEvent.EscortCount)
end
