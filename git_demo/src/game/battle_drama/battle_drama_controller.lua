...
function BattleDramaController:handle13000(data)
    print("-------------handle13000---------------------")
--     ] dun_id = 0,
-- [LUA-print] cool_time = 0,
-- [LUA-print] max_dun_id = 0,
-- [LUA-print] status = 2,
-- [LUA-print] auto_num_max = 10,
-- [LUA-print] mode = 1,
-- [LUA-print] auto_num = 0,
-- [LUA-print] chapter_id = 2,
-- [LUA-print] map_id = 20001,
-- [LUA-print] is_first = 1,
-- [LUA-print] mode_list = {
-- [LUA-print] },
    printLuaTable(data)
    if data then
...
function BattleDramaController:send13003(dun_id)
    local protocal = {
        dun_id = dun_id or 0
    }
...
function BattleDramaController:handle13015( data )
    if data and data.dungeon_replay_log then
        EventManager:getInstance():Fire(Battle_dramaEvent.UpdatePassVedioDataEvent, data.dungeon_replay_log)
    end
end
...
function BattleDramaController:openDramHookRewardView(value, data)
    if value == false then
        ...
        if self.battle_drama_hook_reward_view == nil then
         ...
         self.battle_drama_hook_reward_view = BattlDramaQuickBattleWindow.New()
        end
...
function BattleDramaController:changeBattleTimeType(type)
    if self.battle_drama_ui then
        self.battle_drama_ui:setTimeType(type)
    end
end
--==============================--
--desc:世界等级tips
...
function BattleDramaController:setMaplayerShow( is_show )
    if self.battle_drama_ui and not tolua.isnull(self.battle_drama_ui) then
        self.battle_drama_ui:setMaplayerShow(is_show)
    end
end
...
function BattleDramaController:handle13019(data)
    ...
    local confirm_callback = function (  )
        ...
        self.hook_alert = nil
        EventManager:getInstance():Fire(Battle_dramaEvent.Close_Hook_Alert_Event)
    end
...
function BattleDramaController:handle13020( data )
    if data.val then
        EventManager:getInstance():Fire(Battle_dramaEvent.UpdateDramaProgressDataEvent, data.val)
    end
end
