...
function BattleDramaUnlockChapterView:__init()
    ...
    self.res_list = {
        ...
        { path = PathTool.getSingleImgForDownLoad("bigbg/battledrama", "battledrama_unlock_icon"), type = ResourcesType.single },
        { path = PathTool.getSingleImgForDownLoad("bigbg/battledrama", "txt_cn_battledrama_unlock_title"), type = ResourcesType.single },
...
function BattleDramaUnlockChapterView:open_callback()
    ...
    self.star_name = createLabel(40, Config.ColorData.data_color4[197], Config.ColorData.data_color4[196], self.main_panel:getContentSize().width / 2, 360, "", self.main_panel, 2, cc.p(0.5, 0), TITLE_FONT)
...
function BattleDramaUnlockChapterView:updateDesc(config)
    self.config = config
    if config then
        loadSpriteTexture(self.title_sp, PathTool.getSingleImgForDownLoad("bigbg/battledrama", "txt_cn_battledrama_unlock_title"), LOADTEXT_TYPE)
        loadSpriteTexture(self.icon, PathTool.getSingleImgForDownLoad("bigbg/battledrama", "battledrama_unlock_icon"), LOADTEXT_TYPE)
...
function BattleDramaUnlockChapterView:close_callback()
    EventManager:getInstance():Fire(Battle_dramaEvent.BattleDrama_Drama_Unlock_View, self.config)
    controller:openBattleDramaUnlockChapterView(false)
    GlobalTimeTicket:getInstance():remove("close_unlock_view")
