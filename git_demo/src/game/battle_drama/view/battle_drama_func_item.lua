...
function BattleDramaFuncItem:ctor(is_bool, is_bools, size)
    ...
    self.root_wnd = createCSBNote(PathTool.getTargetCSB('battledrama/battle_drama_open_func_item'))
...
function BattleDramaFuncItem:setData(data)
    if data then
        ...
        local title_id = PathTool.getSingleImgForDownLoad('bigbg/battledrama', data.unlock_icon)
