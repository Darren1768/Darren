...
function BattlDramaSwapRewardWindow:open_callback()
    ...
    self.image_top_bg = createScale9Sprite(PathTool.getSingleImgForDownLoad("bigbg", "bigbg_12"), self.back_panel:getContentSize().width / 2, self.back_panel:getContentSize().height, LOADTEXT_TYPE, self.back_panel)
    self.image_top_bg:setAnchorPoint(cc.p(0.5, 1))
    self.image_top_bg:setContentSize(cc.size(SCREEN_WIDTH + 100, 252.5))
    self.image_bottom_bg = createScale9Sprite(PathTool.getSingleImgForDownLoad("bigbg", "bigbg_12"), self.back_panel:getContentSize().width / 2, 0, LOADTEXT_TYPE, self.back_panel)
