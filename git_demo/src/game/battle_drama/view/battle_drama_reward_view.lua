...
function BattleDramaRewardWindow:__init(dun_id)
    ...
    self.res_list = {
        {path = PathTool.getSingleImgForDownLoad("bigbg", "bigbg_61"), type = ResourcesType.single },
...
function BattlDramaRewardItem:setData(data)
    ...
    if cur_dun >= drama_dungeon_info.floor then
        if is_received then
            ...
            self.comfirm_btn:loadTexture(PathTool.getResFrame("common", "hl_common_1002"), LOADTEXT_TYPE_PLIST)
            self.comfirm_btn_label:setString(TI18N("领取"))
    	    self.comfirm_btn_label:setColor(cc.c4b(98,54,43,0xff))
        end
    else 
        --前往
        self.img_received:setVisible(false)
        self.comfirm_btn:setVisible(true)
        self.comfirm_btn:loadTexture(PathTool.getResFrame("common2", "hl_common_1003"), LOADTEXT_TYPE_PLIST)
