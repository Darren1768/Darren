...
function GodbattleModel:updateTotalScore(data)
    ...
    EventManager:getInstance():Fire(GodbattleEvent.UpdateTotalScoreEvent, data)
end
...
function GodbattleModel:recalcRedPoint()
    ...
    EventManager:getInstance():Fire(GodbattleEvent.UpdateRewardRedPoint, num)
end
...
function GodbattleModel:updateRoleMoveData(data)
	...
	if data ~= nil and next(data.role_move_list) ~= nil then
		...
		EventManager:getInstance():Fire(GodbattleEvent.MoveRoleEvent, role_list)
	end
end
...
function GodbattleModel:updateRoleData(data)
    ...
    for i,v in ipairs(data.role_list) do
        ...
        if role_vo ~= nil and getNorKey(role_vo.rid, role_vo.srv_id) == getNorKey(v.rid, v.srv_id) then
            ...
            EventManager:getInstance():Fire(GodbattleEvent.UpdateSelfDataEvent, v.camp, v.score, v.win_acc, v.win_best)
        end
    ...
    EventManager:getInstance():Fire(GodbattleEvent.AddRoleDataEvent, data.type, role_list)
end
...
function GodbattleModel:updateGuardData(data)
    ...
    if next(guard_list) then
        EventManager:getInstance():Fire(GodbattleEvent.AddGuardDataEvent, data.type, guard_list)
    end
end
...
function GodbattleModel:updateRoleSkillData(data)
    ...
    EventManager:getInstance():Fire(GodbattleEvent.UpdateSkillListEvent, data.skill_list)
end
