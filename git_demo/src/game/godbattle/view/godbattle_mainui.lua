...
function GodBattleMainUI:registerEvent()    
    ...
    -- 添加排行榜数据处理
     EventManager:getInstance():Bind(GodbattleEvent.AddRoleDataEvent,self, function(type, role_list) 
            self:addRoleInfoList(type,role_list)
        end)

    -- 更新战场总积分
     EventManager:getInstance():Bind(GodbattleEvent.UpdateTotalScoreEvent,self, function(data) 
            self:updateTotalScore(data)
        end)

    -- 更新技能数据
     EventManager:getInstance():Bind(GodbattleEvent.UpdateSkillListEvent,self, function(list)
            self:updateSkillList(list)
        end)

    -- 战场倒计时
     EventManager:getInstance():Bind(GodbattleEvent.UpdateTimeCountDown,self, function(time) 
            self:timeCountDown(time)
        end)

    -- 实时更新个人击杀和积分数据
     EventManager:getInstance():Bind(GodbattleEvent.UpdateSelfDataEvent,self, function(camp, score, win_acc, win_best) 
            self:updateSelfData(camp, score, win_acc, win_best)
        end)

    -- 实时更新个人获得奖励数据
     EventManager:getInstance():Bind(GodbattleEvent.UpdateRoleAwards,self, function(list) 
    ...
    -- 更新守卫信息
     EventManager:getInstance():Bind(GodbattleEvent.AddGuardDataEvent,self, function(type, data_list)
    ...
    -- 红点更新
     EventManager:getInstance():Bind(GodbattleEvent.UpdateRewardRedPoint,self, function(num)
