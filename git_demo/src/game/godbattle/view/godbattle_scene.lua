...
function GodBattleScene:register_event()
     EventManager:getInstance():Bind(GodbattleEvent.AddRoleDataEvent,self, function(type, data_list)
            self:updateRoleListData(type, data_list)
        end)

     EventManager:getInstance():Bind(GodbattleEvent.MoveRoleEvent,self, function(role_list)
