...
function SeerpalaceSummonPanel:configUI(  )
    ...
    -- 预加载音效
    GlobalAudio:getInstance():preLoadEffect(GlobalAudio.AUDIO_TYPE.COMMON, 'c_xianzhizhaohuan')
    ...
    self.btn_summon_label = createRichLabel(30, cc.c3b(0xFF,0xD5,0x85), cc.p(0.5, 0.5), cc.p(btn_size.width/2, btn_size.height/2))
...
function SeerpalaceSummonPanel:updateSummonBtnLabel(  )
    if self.select_card then
        ...
        if summon_cost then
            ...
            if item_config then
                self.btn_summon_label:setString(string.format(TI18N("<img src='%s' scale=0.3 /><div outline=2,#4a2606>%d 炼灵</div>"), PathTool.getItemRes(item_config.icon), num))
            end
        end
    else
        self.btn_summon_label:setString(TI18N("<div outline=2,#4a2606>炼灵</div>"))
    end
end
...
function SeerpalaceSummonItem:configUI(  )
    ...
    self.base_pic = container:getChildByName("base_pic")
    self.shadow = container:getChildByName("shadow")
end
...
function SeerpalaceSummonItem:setIndex( index )
    ...
    if config and config.item_once then
        ...
        self:handleCardEffect(true, index)
    end
...
-- 卡牌特效
function SeerpalaceSummonItem:handleCardEffect( status, index )
    if status == false then
    ...
    if self.name == nil then
        self.name = createImage(self.effect_node, PathTool.getResFrame("pray","hl_pray_"..(index+2)), 0,-7, cc.p(0.5,0.5),true)
    end
end
...
function SeerpalaceSummonItem:setSelectStatus( status )
    if status == true then
        self.summon_layer:setPositionY(20)
        self.check_layer:setPositionY(20)

        self.effect_node:setPositionY(118)
        self.base_pic:setPositionY(118)
        self.base_pic:loadTexture(PathTool.getResFrame("pray", "hl_pray_2_2"), LOADTEXT_TYPE_PLIST)
        self.base_pic:setContentSize(cc.size(158,218))
        self.shadow:setVisible(false)
    else
        self.summon_layer:setPositionY(0)
        self.check_layer:setPositionY(0)
        self.effect_node:setPositionY(98)
        self.base_pic:setPositionY(98)
        self.shadow:setVisible(true)
        self.base_pic:loadTexture(PathTool.getResFrame("pray", "hl_pray_2"), LOADTEXT_TYPE_PLIST)
        self.base_pic:setContentSize(cc.size(124,182))
    end
