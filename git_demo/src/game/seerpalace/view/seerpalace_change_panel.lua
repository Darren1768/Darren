...
function SeerpalaceChangePanel:register_event(  )
	...
	for index,object in ipairs(self.camp_list) do
     ...
     EventManager:getInstance():Bind(SeerpalaceEvent.Change_Role_Info_Event,self,function (data)
			self:setData(data)
		end)

     EventManager:getInstance():Bind(SeerpalaceEvent.Change_Role_Success,self,function ()
	...
	if (self.cur_role_vo and next(self.cur_role_vo) ~= nil) or self.change_partner_id ~= 0 then
		...
		if left_role_vo and next(left_role_vo) ~= nil then
		    ...
		    if role_star > 0 and role_star <= 5 then
		        local res = PathTool.getResFrame("common","hl_common_508")
		    ...
		    elseif role_star >= 10 then
		    	local res = PathTool.getResFrame("common","hl_common_507")
