...
function SeerpalaceMainWindow:__init()
	...
	self.res_list = {
        ...
        {path = PathTool.getPlistImgForDownLoad("pray", "pray"), type = ResourcesType.plist},
		{path = PathTool.getSingleImgForDownLoad("bigbg","hl_bigbg_22", false), type = ResourcesType.single },
		{path = PathTool.getSingleImgForDownLoad("bigbg","hl_bigbg_23", false), type = ResourcesType.single },
		{path = PathTool.getSingleImgForDownLoad("bigbg","hl_bigbg_24",false), type = ResourcesType.single },
		{path = PathTool.getSingleImgForDownLoad("bigbg","hl_bigbg_25",false), type = ResourcesType.single },
	}
...
function SeerpalaceMainWindow:open_callback(  )
    ...
    for i=1,2 do
        ...
        if tab_btn then
            ...
            local lable1 = object.unselect_bg:getChildByName("title")
            local lable2 = object.select_bg:getChildByName("title")
            lable1:setString(tab_name_list[i])
            lable2:setString(tab_name_list[i])
...
function SeerpalaceMainWindow:changeSelectedTab( index )
	...
	if index == SeerpalaceConst.Tab_Index.Summon then
		...
		self.background:loadTexture(PathTool.getSingleImgForDownLoad("bigbg","bigbg_66",true), LOADTEXT_TYPE)
	elseif index == SeerpalaceConst.Tab_Index.Change then
		self.tips_label:setVisible(false)
		self.background:loadTexture(PathTool.getSingleImgForDownLoad("bigbg","bigbg_67",true), LOADTEXT_TYPE)
	end
...
function SeerpalaceMainWindow:initBtnGift()
    local id = 101 --先知礼包的id  一般不会改了.改了这里跳转也要跟着改 
