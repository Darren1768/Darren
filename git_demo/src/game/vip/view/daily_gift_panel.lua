...
function DailyGiftPanel:registerEvents(  )
	EventManager:getInstance():Bind(VipEvent.DAILY_GIFT_INFO,self,function ( )
			self:setData()
		end)
	

	-- 每日礼领取状态更新
	EventManager:getInstance():Bind(WelfareEvent.Update_Daily_Awawd_Data,self,function ( )
...
function DailyGiftItem:setData( data )
	...
	-- 背景
	local gift_res = PathTool.getSingleImgForDownLoad("bigbg", self.gift_config.bg_res)
