...
function HeroVo:__init()
    ...
    self.star           = 0    --星数
    self.star_skill_lev           = 0    --星技等级 5 0 --- 

    spiritbody_level    =0    --灵技等级
    spiritbody_star     =0    --灵技星级
    self.quality        = 0    --品质
    self.looks          = {}
    self.body_res       = ""
    self.res_id         = ""
...
function HeroVo:updateArtifactList(list)
    ...
    for i=1,5 do
        ...
        self.artifact_list[i] =artifact_data
    end
end
...
function HeroVo:checkResonateHeroByFormType(form_type, not_tips)
    ...
    if self:isResonateHero() then
        if form_type == PartnerConst.Fun_Form.Drama or     --剧情
            form_type == PartnerConst.Fun_Form.Spirit or   --试练塔
