...
function HeroController:registerEvents()
    ...
    if self.re_link_game_event == nil then
        self.re_link_game_event = self.dispather:Bind(LoginEvent.RE_LINK_GAME, self, function()
...
function HeroController:registerProtocals()
    ...
    self:RegisterProtocal(11049, "handle11049")     --觉醒
...
function HeroController:handle11025(data)
    ...
    EventManager:getInstance():Fire(HeroEvent.All_Hero_Base_Info_Event)
end
...
function HeroController:handle11026(data)
    ...
    EventManager:getInstance():Fire(HeroEvent.All_Hero_Detail_Info_Event)
end
...
function HeroController:handle11001( data )
    ...
    EventManager:getInstance():Fire(HeroEvent.Hero_Data_Add, data_list)
end
...
function HeroController:handle11003( data )
    ...
    if data.result == TRUE then
        --升级成功
        EventManager:getInstance():Fire(HeroEvent.Hero_Level_Up_Success_Event)
    end
end
...
function HeroController:handle11005( data )
    message(data.msg)
    if data.result == TRUE then
        EventManager:getInstance():Fire(HeroEvent.Upgrade_Star_Success_Event)
    else
...
function HeroController:handle11009(data)
    ...
    if data.result == TRUE then
        self.model:setHeroMaxCount(data.num)
        self.model:setHeroBuyNum(data.buy_num)
        EventManager:getInstance():Fire(HeroEvent.Buy_Hero_Max_Count_Event)
    end
end
--获取下一阶 信息
...
function HeroController:handle11016(data)
    EventManager:getInstance():Fire(HeroEvent.Next_Break_Info_Event, data)
end
...
function HeroController:handle11040( data )
    ...
    EventManager:getInstance():Fire(HeroEvent.Get_Had_Hero_Star_Event)
end
...
function HeroController:handle11075( data )
    if data.code == TRUE then
        EventManager:getInstance():Fire(HeroEvent.Hero_Reset_Look_Event, data)
    end
end
-- 请求英雄分解材料
...
function HeroController:handle11087( data )
    if data.code == TRUE then
        EventManager:getInstance():Fire(HeroEvent.Hero_Reset_Star_Event, data)
    end
end
...
function HeroController:handle11012( data )
    ...
    EventManager:getInstance():Fire(HeroEvent.Equip_Update_Event)

    self.model.is_equip_redpoint_hero_update = true
    self.model:checkEquipRedPointUpdate()
end 
function HeroController:sender11013(partner_id, item_id)
    local protocal ={}
    protocal.partner_id = partner_id
    protocal.item_id = item_id
    self:SendProtocal(11013,protocal)
end
function HeroController:handle11013( data )
    message(data.msg)
    if data.result == TRUE then
     
    end
end
function HeroController:sender11014(partner_id, item_id,count)
    local protocal ={}
    protocal.count = count
    protocal.partner_id = partner_id
    protocal.item_id = item_id

    self:SendProtocal(11014,protocal)
end
function HeroController:handle11014( data )
    message(data.msg)
    if data.result == TRUE then
...
function HeroController:handle11015( data )
    ...
    if data.result == TRUE then
        ...
        EventManager:getInstance():Fire(HeroEvent.Hero_Lock_Event)
    end
end
---------------------------装备相关结束------------------------------------
...
--灵身激活
function HeroController:sender11032(partner_id)
    local protocal ={}
    protocal.partner_id = partner_id
...
function HeroController:handle11032( data )
    ...
    if data.result == 1 then
        -- EventManager:getInstance():Fire(HeroEvent.Artifact_UpStar_Event,data)
    end
end
--灵身强化
function HeroController:sender11033(partner_id, time)
    local protocal ={}
    protocal.partner_id = partner_id
    protocal.strengthen_time = time
...
--灵身升星
function HeroController:sender11034(partner_id)
    local protocal ={}
    protocal.partner_id = partner_id
...
function HeroController:handle11035( data )
    message(data.msg)
    if data.spiritbody_skills then
        self.model:updateHeroVo(data)
    end
end
...
function HeroController:handle11036( data )
    message(data.msg)
    if data.result == 1 then
        EventManager:getInstance():Fire(HeroEvent.Artifact_Compound_Event, data.flag)
    end
end
...
function HeroController:handle11037( data )
    if data and data.lucky then
        ...
        EventManager:getInstance():Fire(HeroEvent.Artifact_Lucky_Event)
    end
end
...
function HeroController:handle11048( data )
    if data and data.artifact_ref_count then
        ...
        EventManager:getInstance():Fire(HeroEvent.Artifact_Recast_Count_Event)
    end
end
function HeroController:sender11049(partner_id, item_id)
    local protocal ={}
    protocal.partner_id = partner_id
    protocal.item_id = item_id
     
    self:SendProtocal(11049,protocal)
end
function HeroController:handle11049( data )
    message(data.msg)
    if data.result == TRUE then
...
function HeroController:handle11211(data)
    ...
    EventManager:getInstance():Fire(HeroEvent.Update_Fun_Form, data) 
end
...
function HeroController:handle11212(data)
    if data.code == TRUE then
        ...
        elseif data.type == PartnerConst.Fun_Form.ArenaTeam then
            self:sender11211(data.type)
            EventManager:getInstance():Fire(ArenateamEvent.ARENATEAM_REFRESH_TEAM_INFO_EVENT, data)
        ...
        EventManager:getInstance():Fire(HeroEvent.Update_Save_Form, data)
    else
        message(data.msg)
    end
end
...
function HeroController:handle11096(data)
    if data.result == TRUE then
        ...
        EventManager:getInstance():Fire(HeroEvent.Hero_Learn_Talent_Event, data)
    else
        message(data.msg)
    end
end
...
function HeroController:handle11097(data)
    if data.result == TRUE then
        self.model:updateHeroVoTalent({data}, true)
        EventManager:getInstance():Fire(HeroEvent.Hero_Level_Up_Talent_Event, data)
    else
        message(data.msg)
    end
end
...
function HeroController:handle11098(data)
    if data.result == TRUE then
        self.model:updateHeroVoTalent({data}, true)
        EventManager:getInstance():Fire(HeroEvent.Hero_Forget_Talent_Event, data)
    else
        message(data.msg)
    end
end
...
function HeroController:handle11099(data)
    ...
    EventManager:getInstance():Fire(HeroEvent.Hero_Get_Talent_Event, data.partner_ids)
end
---------------------------天赋相关结束-----------------------------------------
---------------------------神装相关结束-----------------------------------------
--神装保存重置技能"
...
function HeroController:handle11091(data)
    ...
    EventManager:getInstance():Fire(HeroEvent.Holy_Equipment_Update_Event, data)
end
--请求神装穿戴信息""
...
function HeroController:handle11092(data)
    ...
    EventManager:getInstance():Fire(HeroEvent.Hero_Get_Holy_Equipment_Event, data.partner_ids)
end
...
function HeroController:handle11088(data)
    message(data.msg)
    EventManager:getInstance():Fire(HeroEvent.Hero_Sell_Holy_Equipment_Res_Event, data)
end
...
function HeroController:handle25220(data)
    ...
    EventManager:getInstance():Fire(HeroEvent.Hero_Get_Holy_Equipment_Plan_Event, data)
end
...
function HeroController:handle25223(data)
    ...
    if data.code == TRUE then
        ...
        EventManager:getInstance():Fire(HeroEvent.Hero_Open_Holy_Equipment_Cell_Event, data.num)
    end
end
...
function HeroController:handle11019(data)
    ...
    EventManager:getInstance():Fire(HeroEvent.Hero_Skin_Used_Event, data)
end
...
function HeroController:handle11020(data)
    ...
    EventManager:getInstance():Fire(HeroEvent.Hero_Skin_Info_Event, data)
end
...
function HeroController:handle26400(data)
    ...
    EventManager:getInstance():Fire(HeroEvent.Hero_Resonate_Info_Event, data)
end
...
function HeroController:handle26410(data)
    ...
    EventManager:getInstance():Fire(HeroEvent.Hero_Resonate_Extract_Event, data)
end
...
function HeroController:handle26420(data)
    EventManager:getInstance():Fire(HeroEvent.Hero_Resonate_Empowerment_Event, data)
end
...
function HeroController:handle26421(data)
    message(data.msg)
    if data.result == TRUE then
        EventManager:getInstance():Fire(HeroEvent.Hero_Resonate_Empowerment_Success_Event, data)
    end
end
...
function HeroController:handle26422(data)
    EventManager:getInstance():Fire(HeroEvent.Hero_Resonate_Skill_List_Event, data)
end
...
function HeroController:handle26423(data)
    EventManager:getInstance():Fire(HeroEvent.Hero_Resonate_Hero_Power_Event, data)
end
...
function HeroController:handle26425(data)
    ...
    EventManager:getInstance():Fire(HeroEvent.Hero_Resonate_Crystal_Info_Event, data)
end
...
function HeroController:sender26426(partner_id, pos)
    ...
    EventManager:getInstance():Fire(HeroEvent.Hero_Resonate_Crystal_Can_List_Event, false)
end
...
function HeroController:handle26426(data)
    ...
    if data.result == TRUE then
        ...
        EventManager:getInstance():Fire(HeroEvent.Hero_Resonate_Crystal_Update_One_Event, self.put_pos)
    end
    self.put_pos = nil
end
...
function HeroController:handle26427(data)
    ...
    if data.result == TRUE then
        HeroCalculate.checkAllHeroRedPoint()
        EventManager:getInstance():Fire(HeroEvent.Hero_Resonate_Crystal_Update_One_Event, self.down_pos)
    end
    self.down_pos = nil
end
...
function HeroController:handle26428(data)
    message(data.msg)
    if data.result == TRUE then
        EventManager:getInstance():Fire(HeroEvent.Hero_Resonate_Crystal_Update_One_Event, data.pos)
    end
end
--解锁共鸣槽位"
...
function HeroController:handle26429(data)
    ...
    if data.result == TRUE then
        EventManager:getInstance():Fire(HeroEvent.Hero_Resonate_Crystal_Update_One_Event, data.pos)
        EventManager:getInstance():Fire(HeroEvent.Hero_Resonate_Crystal_Update_One_Event, data.pos + 1)
    end
end
--水晶升级(成功推送26425)"
...
function HeroController:handle26431(data)
    message(data.msg)
    if data.result == TRUE then
        EventManager:getInstance():Fire(HeroEvent.Hero_Resonate_Crystal_Power_Event, data)
    end
end
--请求水晶突破
...
function HeroController:handle26432(data)
    message(data.msg)
    if data.result == TRUE then
        EventManager:getInstance():Fire(HeroEvent.Hero_Resonate_Break_Event, data)
    end
end
---------------------------共鸣协议结束-----------------------------------------
...
function HeroController:handle11065(data)
    if data then
        ...
        EventManager:getInstance():Fire(HeroEvent.HERO_RESET_EVENT, data)
    end
...
function HeroController:handle11066(data)
    ...
    if data and data.code == TRUE then
        EventManager:getInstance():Fire(HeroEvent.HERO_RESET_ITEM_EVENT, data)
    end
end
...
function HeroController:handle11067(data)
    ...
    -- EventManager:getInstance():Fire(HeroEvent.HERO_RESET_100_LEVEL_EVENT, data)
end
...
--根据 fun_form_type 获得配置中的信息
function HeroController:getIndexByFormType(fun_form_type)
    local config = Config.PartnerData.data_partner_form
    local cur_config_info = config[1]
    for i,v in ipairs(config)do
        if v.form_type == fun_form_type then
            cur_config_info = v
            break
        end
    end
    return cur_config_info
end
...
--@ form_type --来源位置  1: 表示融合祭坛 2: 表示升星界面的 3:圣物
--@ setting.is_master 是否是主卡(融合祭坛专用)
function HeroController:openHeroUpgradeStarSkillEffectPanel(status, select_data, dic_other_selected, form_type, setting)
    if status == true then
        if not self.upgrade_star_skill_effect_panel then 
            self.upgrade_star_skill_effect_panel = HeroUpgradeStarSkillEffectPanel.New()
            self.upgrade_star_skill_effect_panel:open(select_data, dic_other_selected, form_type, setting)
        else
            if self.upgrade_star_skill_effect_panel and self.upgrade_star_skill_effect_panel:isOpen() == false then
                self.upgrade_star_skill_effect_panel:open()
                self.upgrade_star_skill_effect_panel:openRootWnd(select_data, dic_other_selected, form_type, setting)
            end
        end
    else 
        
        if self.upgrade_star_skill_effect_panel then 
            print("----------------upgrade_star_skill_effect_panel----------------------------",self.upgrade_star_skill_effect_panel)
            self.upgrade_star_skill_effect_panel:close()
            self.upgrade_star_skill_effect_panel = nil
        end
    end
end
...
function HeroController:updateSkillItem(skill_items,index, config, parent, pos,pos1,pos2,color1,color2)
   skill_items = skill_items or {}
    local skill = skill_items[index]
    if skill == nil then
        skill = SkillItem.new(true, true, true, 0.8,nil,nil,true)
        parent:addChild(skill)
        skill_items[index] = skill

        local name = createLabel(24,color1, nil, pos1.x, pos1.y, "", skill, 1, cc.p(0, 0),TITLE_FONT)
        local desc = createRichLabel(20, color2, cc.p(0, 1), pos2, 4, nil, 300)
        skill:addChild(desc)
        name:setScale(1.2)
        desc:setScale(1.1)
        skill.nameAdd = name
...
function HeroController:openArtifactTipsWindow(bool,data,open_type,partner_id,pos, is_off_tips, setting)
    if bool == true then
        if data == nil  then
            -- message(TI18N("数据异常"))or data.config == nil
            return 
        end
...
function HeroController:setExpend(csume,lables,items)
    if csume then
        for i = 1, #csume do
            local cObj = csume[i]
            ...
            if item_config then
                if not items[i].item_icon then
                 
                    local res = PathTool.getItemRes(item_config.icon)
                    items[i]:loadTexture(res, LOADTEXT_TYPE)
                    items[i]:setScale(0.25)
                else
                    items[i]:setData(item_config)
                end
                if total_num >= needNum then
                    lables[i]:setString(string_format("%d/%s", needNum, MoneyTool.GetMoneyString(total_num) ))
                else

                    lables[i]:setString(string_format("<div fontcolor=#ffffff outline=1,#62352b>%d/</div><div fontcolor=#bd5110  outline=1,#62352b>%d</div> ", needNum, total_num))
                end
            end
        end
    end 
...
-- 获取灵身ID   
function HeroController:getHero_spiritbody_id(hero_vo )
    local config = Config.PartnerData.data_partner_base[hero_vo.bid]
    if  config and  config.spiritbody_id >0  then
        return config.spiritbody_id
    end
    return nil
...
function HeroController:onPlayHeroVoice(vocie_res, time, audio_type)
    if audio_type == nil or audio_type == "" then
        audio_type = GlobalAudio.AUDIO_TYPE.DUBBING
    end
    --默认4秒
    ...
    self.hero_music = GlobalAudio:getInstance():playEffect(audio_type, vocie_res, false)
