...
function HeroMainInfoWindow:__init()
    ...
    self.res_list = {
        ...
        {path = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_50", false), type = ResourcesType.single},
        {path = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_47", false), type = ResourcesType.single},
        {path = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_41", false), type = ResourcesType.single},
        {path = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_46", false), type = ResourcesType.single},
        {path = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_33", false), type = ResourcesType.single},
        {path = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_42", false), type = ResourcesType.single},
        {path = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_43", false), type = ResourcesType.single},
        {path = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_44", false), type = ResourcesType.single},
        {path = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_53", false), type = ResourcesType.single},
    }
...
function HeroMainInfoWindow:open_callback()
    ...
    self.left_btn = self.root_wnd:getChildByName("left_btn")
    self.right_btn = self.root_wnd:getChildByName("right_btn")
    self.tab_type_list = {true,false,false,false,false,false}
    self.tab_btn_x = {}
    for i=1,5 do
        local tab_btn = {}
        ...
        self.tab_btn_x[i] = tab_btn.btn:getPositionX()
    end
    ...
    self.top_hero_container = self.main_container:getChildByName("top_hero_container")
...
function HeroMainInfoWindow:updateHero()
    self:updateHeroBg()
...
function HeroMainInfoWindow:updateScrollviewList()
    if self.child_scrollview == nil then
        local scroll_view_size = self.top_hero_container:getContentSize()
        local setting = {
            start_x = 0,                     -- 第一个单元的X起点
            space_x = 0,                     -- x方向的间隔
            start_y = 0,                     -- 第一个单元的Y起点
            space_y = 0,                     -- y方向的间隔
            item_width = 190,                -- 单元的尺寸width
        ...
        self.child_scrollview = CommonScrollViewSingleLayout.new(self.top_hero_container, cc.p(0,0) , ScrollViewDir.horizontal, ScrollViewStartPos.top, scroll_view_size, setting, cc.p(0, 0))
...
function HeroMainInfoWindow:createNewCell(width, height)
    local cell = HeroMainInfoHeroItem.new()
...
--获取数据数量
function HeroMainInfoWindow:numberOfCells()
...
--inde :数据的索引
function HeroMainInfoWindow:updateCellByIndex(cell, index)
    if not self.hero_list then return end
    cell.index = index
    cell.selected = false
    self.hero_list[index].info_select = false
    if index == self.select_index then 
        self.hero_list[index].info_select = true
    end 
...
function HeroMainInfoWindow:onCellTouched(cell)
    if not self.hero_list then return end
    if self.select_index == cell.index then return end 

    local last_index = self.select_index or 1
    self.select_index  =  cell.index

    self:updataChildScrollViewByIndex(last_index,self.select_index)
    self:updateData()
end


function HeroMainInfoWindow:updataChildScrollViewByIndex(last_index ,index)
    self.child_scrollview:resetItemByIndex(last_index)
    self.select_hero_vo = self.hero_list[index]
    self.child_scrollview:resetItemByIndex(index)
    self.child_scrollview:jumpToMoveByIndexHorizontal(index)
    role_vo:setCareer(self.select_hero_vo.type)
end
--查看立绘
...
function HeroMainInfoWindow:showSharePanel(bool)
    ...
    if not self.share_panel then 
        ...
        local res = PathTool.getResFrame("common","hl_common_4.1")
        local btn1 =  createTypeButton(self.share_panel, TI18N("世界频道"), size.width/2, 115, ButtonType.yellow)
        ...
        local btn2 =  createTypeButton(self.share_panel, TI18N("公会频道"), size.width/2, 45, ButtonType.yellow)
        ...
        local btn3 =  createTypeButton(self.share_panel, TI18N("跨服频道"), size.width/2, 185, ButtonType.yellow)
...
function HeroMainInfoWindow:changeTabType(index, check_repeat_click, sender)
    ...
    if not self.tab_type_list[index] then return end
    ...
    if self.pre_panel.setData then
...
-- function HeroMainInfoWindow:updateLeftRightBtns()

--     self.left_btn:setVisible(false)
--     self.right:setVisible(false)
--     if self.select_hero_vo then
--         if self.select_hero_vo > 1 then
--             self.left_btn:setVisible(true)
--         end
--         if next(self.select_hero_vo) then
...
-- end
function HeroMainInfoWindow:changeNextHero(num)
...
function HeroMainInfoWindow:createSubPanel(index)
    ...
    if panel == nil then
        ...
        elseif index == HeroConst.MainInfoTab.eMainBody then
            panel = HeroMainTabBodyPanel.new()

        elseif index == HeroConst.MainInfoTab.eMainTalent then
            panel = HeroMainTabArtifactsPanel.new()

        elseif index == HeroConst.MainInfoTab.eMainHolyEquipment then --神装
            panel = HeroMainTabHolyequipmentPanel.new()

        elseif index == HeroConst.MainInfoTab.eMainEquipment then

            panel = HeroMainTabEquipPanel.new()
        end
    ...
    if index == HeroConst.MainInfoTab.eMainTrain then
        ...
        bg_res  = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_47", false)
    elseif index == HeroConst.MainInfoTab.eMainUpgradeStar then

        bg_res  = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_50", false)
    elseif index == HeroConst.MainInfoTab.eMainBody then

        bg_res  = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_46", false)
    elseif index == HeroConst.MainInfoTab.eMainTalent then

        bg_res  = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_33", false)
    elseif index == HeroConst.MainInfoTab.eMainHolyEquipment then --神装

        bg_res  = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_41", false)
    elseif index == HeroConst.MainInfoTab.eMainEquipment then
...
function HeroMainInfoWindow:showLevelUpAction()
    if self.effect_parent == nil then
        ...
        self.main_panel:addChild(self.effect_parent, 2)
...
function HeroMainInfoWindow:openRootWnd(select_hero_vo, hero_list, setting)
    ...
    EventManager:getInstance():Fire(MainuiEvent.HEAD_UPDATE_WEALTH_EVENT, 2, Config.ItemData.data_assets_label2id.hero_exp)
end
...
function HeroMainInfoWindow:updateData()
  ...
  --  self:updateHero()
    ...
    EventManager:getInstance():Fire(HeroEvent.Hero_Data_Update, self.select_hero_vo)
end
...
function HeroMainInfoWindow:updatePageInfo()
    if self.show_model_type == HeroConst.BagTab.eBagHero then
        ...
        if self:checkStarTabShow(star) then
            ...
            self.tab_type_list[HeroConst.MainInfoTab.eMainUpgradeStar] = true
        end
        --table_insert(self.tab_type_list, HeroConst.MainInfoTab.eMainBody)
        local spiritbody_id=controller:getHero_spiritbody_id(self.select_hero_vo)
        if spiritbody_id then
            self.tab_type_list[HeroConst.MainInfoTab.eMainBody] = true
        else
            self.tab_type_list[HeroConst.MainInfoTab.eMainBody] = false
        end
  
        if self:checkTalentTabShow(star) then
            --table_insert(self.tab_type_list, HeroConst.MainInfoTab.eMainTalent)
            --self.tab_type_list[HeroConst.MainInfoTab.eMainTalent] = true
        end
        self.tab_type_list[HeroConst.MainInfoTab.eMainTalent] = true
        --table_insert(self.tab_type_list, HeroConst.MainInfoTab.eMainEquipment)
        self.tab_type_list[HeroConst.MainInfoTab.eMainEquipment] = true
        ...
        if self:checkHolyequipmentTabShow() then
            ...
            self.tab_type_list[HeroConst.MainInfoTab.eMainHolyEquipment] = true
        end
        local showIndex = 1
        for i,tab_btn in ipairs(self.tab_list) do
            if self.tab_type_list[i] then
                ...
                tab_btn.btn:setPositionX(self.tab_btn_x[showIndex]) 
    ...
    if self.cur_tab_index == nil then
        ...
        --如果显示的页签是锁住的那么默认1
        if self.cur_tab_index == HeroConst.MainInfoTab.eMainUpgradeStar and self.is_star_lock then
            self.cur_tab_index = 1     
        elseif self.cur_tab_index == HeroConst.MainInfoTab.eMainHolyEquipment and self.is_holy_equp_lock then
            self.cur_tab_index = 1 
        end
    end
...
function HeroMainInfoWindow:checkTabShow()
    ...
    for i,v in ipairs(self.tab_type_list) do
        if v then
            if i == HeroConst.MainInfoTab.eMainUpgradeStar then
                isTabUpgradeStar = true
            elseif i == HeroConst.MainInfoTab.eMainTalent then
                isTabTalent = true
            elseif i ==  HeroConst.MainInfoTab.eMainHolyEquipment then
                isTabHolyequip = true
            end
        end
...
function HeroMainInfoWindow:close_callback()
    ...
    EventManager:getInstance():Fire(MainuiEvent.HEAD_UPDATE_WEALTH_EVENT, 2, Config.ItemData.data_assets_label2id.gold)
    if self.spine then
        self.spine:DeleteMe()
        self.spine = nil
    end
    ...
    controller:openHeroMainInfoWindow(false)
    MainuiController:getInstance():setIsShowMainUIBottom(true) -- 显示底部UI
end
