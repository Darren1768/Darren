...
function FormationSelectPanel:setData(formation_type)
    ...
    self.time_ticket = GlobalTimeTicket:getInstance():add(function()
        if tolua.isnull(self.main_container) then return end
...
function FormationSelectPanel:createItem(config, item_width, index, role_lev)
    ...
    item.con = con
      ...
      --选择框
      local select_res = PathTool.getResFrame("form", "hl_form_icon_7")
      item.select_img = createImage(item.con, select_res,  size.width/2,size.height/2+20, cc.p(0.5,0.5), true, 0, true)
      -- item.select_img:setContentSize(size)

    --阵法icon
    local res = PathTool.getResFrame("form", "hl_form_icon_"..config.type)
    ...
    if is_select == true then
        item.select_img:setVisible(true)
        --当前背景
        local cur_res = PathTool.getResFrame("common2", "hl_common_3020")
        item.cur_img = createImage(item.con, cur_res,  10,48, cc.p(0.5,0.5), true, 0, false)
        local cur_name = createLabel(20,cc.c3b(239,212,174),nil, 11, 35, TI18N("当\n前"), item.cur_img, nil, cc.p(0.5,0.5),nil)
    ...
    local name_res = PathTool.getResFrame("common2","hl_common_0023")
    local name_bg = createImage(item.con,name_res,size.width/2,-20,cc.p(0.5,0.5),true,0,true)
    name_bg:setContentSize(cc.size(150,40))
    --阵法名字 --68452a
    item.name = createLabel(22,cc.c3b(239,212,174),nil, item_width * 0.5,-20 ,config.name, item.con,nil, cc.p(0.5,0.5),nil)

    --锁
    if role_lev < config.need_lev and self.team_index ~= 999 then --999 新手训练营特殊类型
        local res = PathTool.getResFrame("common","hl_common_92")
        item.lock_icon =createImage(item.con, res, size.width/2, 50, cc.p(0.5,0.5),true,0,false)
        local str = string_format("%s%s", config.need_lev, TI18N("解锁"))
        createLabel(22, cc.c3b(255,248,231), nil , size.width/2,10,str, item.con, 0, cc.p(0.5,0))
