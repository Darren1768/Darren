...
function HeroMainTabTalentPanel:layoutUI()
    ...
    if self.skill_item_list[3] then
        ...
        local bg_res = PathTool.getSingleImgForDownLoad("bigbg/hero","hero_talent_bg_1", false)
        self.box_item_load = loadSpriteTextureFromCDN(bg_box, bg_res, ResourcesType.single, self.box_item_load) 
        local bg_res = PathTool.getSingleImgForDownLoad("bigbg/hero","hero_talent_bg", false)
    ...
    local bg_res = PathTool.getSingleImgForDownLoad("bigbg/hero","hero_bag_talent_bg", false)
...
function HeroMainTabTalentPanel:registerEvents()
    ...
    -- 获取到天赋信息
    EventManager:getInstance():Bind(HeroEvent.Hero_Get_Talent_Event,self, function(list)
    ...
    -- 穿戴天赋技能返回
    EventManager:getInstance():Bind(HeroEvent.Hero_Learn_Talent_Event,self, function(data)
    ...
    -- 升级天赋技能返回
    EventManager:getInstance():Bind(HeroEvent.Hero_Level_Up_Talent_Event,self, function(data)
            if not data then return end
            if not self.hero_vo then return end
            if data.partner_id == self.hero_vo.partner_id then
    ...
    -- 升级天赋技能返回
    EventManager:getInstance():Bind(HeroEvent.Hero_Forget_Talent_Event,self, function(data)
            if not data then return end
            if not self.hero_vo then return end
            if data.partner_id == self.hero_vo.partner_id then
                self:initSkillItemList()
