...
function SkillItem:ctor(play_start, click, is_show_tips, scale, show_lev, swallow_touch,isCircle,show_type)
    ...
    self.isCircle = isCircle or false
    self.show_type = show_type or false -- 普 主 被
...
function SkillItem:configUICircle()
  
    --背景
    local res  = PathTool.getResFrame("common2", "hl_common_3016")
    self.background = createImage(self, res,  self.size.width/2, self.size.height/2, cc.p(0.5,0.5), true, 0, true)
    self.background:setContentSize(self.size)
    ...
    --技能等级
    if self.show_lev then
        local res_lev  = PathTool.getResFrame("common2","hl_common_3013")
        self.level_bg = createImage(self, res_lev, self.size.width/2 ,0, cc.p(0.5,0.5), true, 1, false)
        self.num_label = createLabel(20,cc.c4b(239,212,174,0xff),nil,self.level_bg:getContentSize().width/2-2,self.level_bg:getContentSize().height/2-1,"00",self.level_bg,1, cc.p(0.5,0.5))
        self.num_label:setZOrder(1)
    end
    ...
    if self.show_type then 
        local res_type  = PathTool.getResFrame("common2","hl_common_3010")
        self.type_bg = createSprite(res_type, self.size.width/2, self.size.height-2, self, cc.p(0.5, 0.5))
        self.type_bg:setZOrder(1)
    end 

    self.vSize = cc.size(96,96)
    -- local bgRes = PathTool.getResFrame("common","hl_common_39")
    local bgRes  = PathTool.getResFrame("common2", "hl_common_3015")
    self:buildLayout(self.vSize,bgRes)

     --背景框
     local res_kuang  = PathTool.getResFrame("common2", "hl_common_3016")
     self.background_kuang = createSprite(res_kuang, self.size.width/2, self.size.height/2, self, cc.p(0.5, 0.5))


end

-- 裁剪 切割
function SkillItem:buildLayout(size, bgRes)
...
function SkillItem:setData(data)
    ...
    if self.data == nil then
        ...
        if self.isCircle then 
            self.item_icon:loadTexture(skill_icon, LOADTEXT_TYPE)
        else 
        ...
        if skill_config.type == "passive_skill" then
            -- background_res_id = PathTool.getQualityBg(skill_config.level)
        end
        ...
        -- 显示等级,现在只有天赋技能显示等级
        local skill_level = data.skill_up_level and data.skill_up_level+1 or skill_config.level  --当前技能的等级 = 技能升了几级+1
        if self.show_lev and self.num_label then
            ...
            elseif skill_level and skill_level > 0 then
                self.level_bg:setVisible(true)
                self.num_label:setVisible(true)
                self.num_label:setString(skill_level)
            else
                self.num_label:setVisible(false)
        ...
        -- 普 主 被
        if  self.type_bg then 
            if data.type == "active_skill" then 
                if data.is_normal_skill == 1 then 
                    loadSpriteTexture(self.type_bg,PathTool.getResFrame("common2","hl_common_3010"), LOADTEXT_TYPE_PLIST)
                else
                    loadSpriteTexture(self.type_bg,PathTool.getResFrame("common2","hl_common_3011"), LOADTEXT_TYPE_PLIST)
                end 
            else 
                loadSpriteTexture(self.type_bg,PathTool.getResFrame("common2","hl_common_3012"), LOADTEXT_TYPE_PLIST)
            end
        end 
...
function SkillItem:showLockIcon(bool,str,tips)
    ...
    if not self.artifact_lock then 
        ...
        self.artifact_lock = createImage(self, res, 50,50, cc.p(0.5,0.5), true, 1, false)
        self.artifact_lock:setScale(1.5)
...
function SkillItem:showUnusualIcon( status ,type)--type 1：稀有  2：强力
    ...
    if not self.unusual_icon then  
        self.unusual_icon = createSprite(nil,0,95,self,cc.p(0,1),nil,2)
        local _label = createLabel(20,cc.c3b(239,212,174), nil,10,36,"首\n通", self.unusual_icon, nil, cc.p(0.5,0.5))
        _label:setName("un_label")
    end

    if status == true then
        local res = PathTool.getResFrame("common2","hl_common_3020")
        local _str = "稀\n有"
        if type and type == 2 then
            res = PathTool.getResFrame("common2","hl_common_3021")
            _str = "强\n力"
        end
        loadSpriteTexture(self.unusual_icon, res, LOADTEXT_TYPE_PLIST)
        self.unusual_icon:getChildByName("un_label"):setString(_str)
    end
    ...
    if partner_awakening_skill_config then
        if status then
            if self.skill_profession_icon == nil then
                ...
                local res = PathTool.getSingleImgForDownLoad("bigbg/hero", "talent_profession")
...
function SkillItem:showLevel( status )
    ...
    if status == true then
        ...
        if not self.num_label then
            self.num_label = createLabel(20,cc.c4b(239,212,174,0xff),nil,self.level_bg:getContentSize().width/2,self.level_bg:getContentSize().height/2,"00",self.level_bg,1, cc.p(0.5,0.5))
            self.num_label:setZOrder(1)
        end
        self.level_bg:setVisible(true)
        self.num_label:setVisible(true)
