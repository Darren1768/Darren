...
function HeroResonateTabResonatePanel:registerEvents()
    ...
    --共鸣信息更新
    EventManager:getInstance():Bind(HeroEvent.Hero_Resonate_Crystal_Info_Event,self, function(data)
    ...
    --能否更新list
    EventManager:getInstance():Bind(HeroEvent.Hero_Resonate_Crystal_Can_List_Event,self, function(status)
            self.can_update_list = status 
        end)

    --更新单个英雄信息
    EventManager:getInstance():Bind(HeroEvent.Hero_Resonate_Crystal_Update_One_Event,self, function(pos)
            self.can_update_list = true
    ...
    EventManager:getInstance():Bind(HeroEvent.Hero_Resonate_Crystal_Power_Event,self, function(data)
            if not data then return end
...
function HeroResonateTabResonatePanel:updateCellByIndex(cell, index)
    ...
    if data and data.id ~= 0 then
        ...
        elseif index > (self.open_pos_count + 1) then
            cell:showLockIcon(true, "", {res = PathTool.getResFrame("common","hl_common_92"), is_unenabled_bg = true})
        else
            cell:showLockIcon(false)
