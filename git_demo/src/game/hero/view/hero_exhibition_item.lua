...
HeroExhibitionItem.Width = 105
HeroExhibitionItem.Height = 133
...
function HeroExhibitionItem:ctor(scale, can_click, click_delay, can_effect, not_swallow)
    ...
    local res = PathTool.getResFrame("common", "hl_common_91")
    ...
    self:buildLayout(cc.size(105,128))
    self.background_box = createImage(self, nil, self.size.width / 2, self.size.height / 2, cc.p(0.5, 0.5), true, 1, false)
    -- self.num_label = createLabel(24, cc.c4b(0xFF, 0xFF, 0xFF, 0xFF), cc.c4b(0x00, 0x00, 0x00, 0xFF), 96, 96, "", self, 1, cc.p(1, 0))
    -- self.num_label:setZOrder(3)
    -- self.num_label:setVisible(false)

    self.num_label =  CommonNum.new(102, self, 1, -2, cc.p(1, 0.5))
    self.num_label:setZOrder(3)
    self.num_label:setVisible(false)
    self.num_label:setPosition(cc.p(96,125))
...
-- 裁剪 切割
function HeroExhibitionItem:buildLayout(size)
    self.vSize =size 
	self.mask_res = mask_res or PathTool.getResFrame("common2", "hl_common_553_2") --方
	...
	self.head_panel:setPosition(self.vSize.width/2, self.vSize.height/2+6)
	self.head_panel:setCascadeOpacityEnabled(true)
	self:addChild(self.head_panel,1)
...
function HeroExhibitionItem:initPartnerType()
    if self.partner_type == nil then
        self.partner_type = createSprite(PathTool.getPartnerTypeIcon(hero_type), 16, 116, self, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST, 1)
        self.partner_type:setVisible(false)
        self.partner_type:setScale(0.4)
    end
end
...
function HeroExhibitionItem:setUnitData(id)
    ...
    if config == nil then
        ...
        self.num_label:setNum("")

        self:setRectQualityImg(0)
        ...
        if monster_id then
            ...
            if monster_config then
                ...
                -- 设置品质框
                self:setRectQualityImg(monster_config.star)
                -- 设置阵营
...
--@return
--==============================--
function HeroExhibitionItem:setData(data, isRect)
    if isRect then
        self.size = cc.size(105, 134)
        self:setAnchorPoint(cc.p(0.5, 0.5))
        self:setContentSize(self.size)
    end
    self:unBindEvent()
    ...
    if data == nil then
        ...
        self:updateData(data, isRect)
    end
end

--@info 数据 可以是
function HeroExhibitionItem:updateData(info, isRect)
    if not info then
        return
    end
    local data
    ...
    --背景框
    if isRect ~= true then
        print("=================setRectQualityImg=============",data.quality)

        -- self:setQualityImg(data.quality)
        self:setRectQualityImg(data.quality)

        -- if self.background_box then
        --     self.background_box:removeFromParent()
...
function HeroExhibitionItem:setRectQualityImg(quality)
    local res_id = PathTool.getRectQualityBg(quality)
    if self.record_res_id == nil or self.record_res_id ~= res_id then
        self.record_res_id = res_id
        self.background:loadTexture(res_id, LOADTEXT_TYPE_PLIST)
    end
    if self.background_box then
        res_id = PathTool.getRectQualityBox(quality)
        self.background_box:loadTexture(res_id, LOADTEXT_TYPE_PLIST)
    end
end
...
function HeroExhibitionItem:addVoBindEvent()
    ...
    if type(self.data) == "table" and self.data and self.data.id ~= nil and self.data.Bind then
        if self.item_update_event == nil then
            self.item_update_event =
                self.data:Bind(
                HeroVo.UPDATE_Partner_ATTR,
                function(hero_vo)
                    --延迟一秒
                    delayRun(
                        self,
...
function HeroExhibitionItem:createStar(num, star_con, star_setting)
    ...
    if star_setting.star_list3 == nil then
        star_setting.star_list3 = {}
    end
    ...
    for i, v in pairs(star_setting.star_list3) do
        v:setVisible(false)
    end
    ...
    if num > 0 and num <= 5 then
        local res = PathTool.getResFrame("common", "hl_common_1007")
        _cStar(num, res, star_setting.star_list)
    elseif num >= 6 and num <= 9 then
        local res = PathTool.getResFrame("common", "hl_common_1007_2")
        local count = num - 5
    ...
    elseif num >= 10 then
        ...
        -- end
        local res = PathTool.getResFrame("common", "hl_common_1007_1")
        _cStar(new_num + 1, res, star_setting.star_list3)
...
function HeroExhibitionItem:setSelected(status)
    ...
    --         self.lay_select:setContentSize(cc.size(self.size.width / 4, self.size.height / 4))

    --         self.lay_select:setTouchEnabled(false)
    --         showLayoutRect(self.lay_select, 150)
    --         local res = PathTool.getResFrame("common", "hl_common_591")
    --         createImage(self.lay_select, res, self.size.width / 8, self.size.height / 8, cc.p(0.5, 0.5), true, 1, false)
    --         self.lay_select:setPosition(self.size.width / 8 * 7, self.size.height / 8 * 7)
    ...
    if status then
        if self.lay_select == nil then
            self.lay_select = ccui.Layout:create()
            self.lay_select:setAnchorPoint(cc.p(0.5, 0.5))
            self.lay_select:setContentSize(cc.size(0, 0))

            self.lay_select:setTouchEnabled(false)
            showLayoutRect(self.lay_select, 0)

            local res_1 = PathTool.getResFrame("common2", "hl_common_0007")
            local res_2 = PathTool.getResFrame("common2", "hl_common_0008")
            local hp_frame = createScale9Sprite(res_1, 0, 1, LOADTEXT_TYPE_PLIST, self.lay_select)
            hp_frame:setCapInsets(cc.rect(3, 4, 1, 1))
            hp_frame:setContentSize(cc.size(103,  128))

            local hp_frame1 = createScale9Sprite(res_2, 0, 1, LOADTEXT_TYPE_PLIST, hp_frame)
            hp_frame1:setCapInsets(cc.rect(5, 5, 5, 5))
            hp_frame1:setContentSize(cc.size(36,  90))
            hp_frame1:setPosition(self.size.width/2 ,self.size.height/2-4)

            local lable_title = createLabel(20, cc.c3b(250,232,150), nil,0, 0, "已\n选\n择", hp_frame1, nil, cc.p(0.5, 0.5))
            lable_title:setPosition(18 ,45)

            self.lay_select:setPosition(self.size.width/2 ,self.size.height/2)
            self:addChild(self.lay_select, 4)
...
-- 已支援
function HeroExhibitionItem:setSupportSelected(status)
    if status then
        if self.lay_select == nil then
            self.lay_select = ccui.Layout:create()
            self.lay_select:setAnchorPoint(cc.p(0.5, 0.5))
...
function HeroExhibitionItem:showLockIcon(bool, str, setting)
    if bool == false and not self.lock_icon then
        return
    end
    local setting = setting or {}
    local res = setting.res
    if not self.lock_icon then
        local res = res or PathTool.getResFrame("common", "hl_common_92")
...
function HeroExhibitionItem:showFightImg(bool)
    if bool == false and not self.fight_img then
        return
    end
    if not self.fight_img then
        local res = PathTool.getResFrame("common", "txt_cn_common_90001")
        self.fight_img = createImage(self, res, 100, 18, cc.p(0.5, 0.5), true, 999, false)
    end
    self.fight_img:setVisible(bool)
end

function HeroExhibitionItem:showAddIcon(bool, res)
    if bool == false and not self.add_btn then
        return
    end
    if not self.add_btn then
        local res = res or PathTool.getResFrame("common", "hl_common_1011")
    ...
    self.background_box:setVisible(false)
    self.background:setVisible(true)
    -- self.background:loadTexture(PathTool.getResFrame("common2", "hl_common_3058"), LOADTEXT_TYPE_PLIST)
    self.background:loadTexture(PathTool.getResFrame("common2", "hl_common_3058"), LOADTEXT_TYPE_PLIST)
	-- self.mask_res = mask_res or PathTool.getResFrame("common2", "hl_common_553_2") --方

    self.background:setContentSize(cc.size(self.size.width,self.size.height))
...
function HeroExhibitionItem:showAddIconII(bool, str)
    ...
    if not self.add_btn then
        local res = PathTool.getResFrame("common", "hl_common_1011")
...
function HeroExhibitionItem:showUnknownIcon(status)
    if status == false and not self.unknow_icon then
        return
    end
...
function HeroExhibitionItem:showRedPoint(bool, offset_x, offset_y)
    ...
    if not self.red_point then
        ...
        local res = PathTool.getResFrame("common", "hl_common_3")
        self.red_point = createImage(self, res, 85 + offset_x, 118 + offset_y, cc.p(0.5, 0.5), true, 10, false)
...
function HeroExhibitionItem:showHelpImg(bool)
    if bool == false and not self.help_img then
        return
    end
    if not self.help_img then
        local res = PathTool.getResFrame("common2", "hl_common_0006") --支援
        self.help_img = createSprite(res, 95, 20, self, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST, 3)
        self.help_img:setScale(0.5)

        local x,y = self.help_img:getPosition()
        self.lbl = createLabel(24,cc.c3b(255,238,202),nil,x-1,y+1,TI18N("援"),self,nil,cc.p(0.5,0.5))
        self.lbl:setLocalZOrder(3)
    end
    self.help_img:setVisible(bool)
    self.lbl:setVisible(bool)
end

--显示是否共鸣英雄图片
function HeroExhibitionItem:showResonateImg(bool)
    if bool == false and not self.resonate_img then
        return
    end
...
function HeroExhibitionItem:showResonateCrystalTime(bool, time_str)
    if bool == false and not self.resonate_crystal_img then
        return
    end
...
function HeroExhibitionItem:showSelfMarkImg(bool)
    if bool == false and not self.selfmark_img then
        return
    end
...
function HeroExhibitionItem:shwoExtendPower(status, power)
    if status == false then
        ...
        self.extend_power:setString(string.format("<div><img src='%s' scale=1 />%s</div>", PathTool.getResFrame("common", "hl_common_134"), power))
    end
end
...
--@return
--==============================--
function HeroExhibitionItem:showStrTips1(status, str)
  
    if status then
        if self.lay_select_str == nil then
            self.lay_select_str = ccui.Layout:create()
            self.lay_select_str:setAnchorPoint(cc.p(0.5, 0.5))
            self.lay_select_str:setContentSize(cc.size(0, 0))

            self.lay_select_str:setTouchEnabled(false)
            showLayoutRect(self.lay_select_str, 0)

            local res_1 = PathTool.getResFrame("common2", "hl_common_0007")
            local res_2 = PathTool.getResFrame("common2", "hl_common_0008")
            local hp_frame = createScale9Sprite(res_1, 0, 1, LOADTEXT_TYPE_PLIST, self.lay_select_str)
            hp_frame:setCapInsets(cc.rect(3, 4, 1, 1))
            hp_frame:setContentSize(cc.size(103,  128))
            ...
            local lable_title = createLabel(20, cc.c3b(250,232,150), nil,0, 0,str , hp_frame1, nil, cc.p(0.5, 0.5))
            lable_title:setPosition(18 ,45)

            self.lay_select_str:setPosition(self.size.width/2 ,self.size.height/2)
            self:addChild(self.lay_select_str, 4)

           
        else
            self.lay_select_str:setVisible(true)
        end
    else
        if self.lay_select_str then
            self.lay_select_str:setVisible(false)
        end
    end
end
...
--设置布阵界面显示名称和ss
function HeroExhibitionItem:showNameAndType(data)
    if not data then
        return
    end
    if self.form_name_bg == nil then
        local _res = PathTool.getFromQualityBg(data.quality)
        self.form_name_bg = createSprite(_res, -15, 55, self, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST, 1)
        self.form_name_label = createLabel(20, cc.c4b(0xe5, 0xdb, 0xcd, 0xff), nil, 13.5, 100, StringUtil.branchStr(data.name), self.form_name_bg, nil, cc.p(0.5, 1))
        -- self.form_type_bg = createSprite(PathTool.geHeroS_bg(data.quality), 13.5, 140, self.form_name_bg, cc.p(0.5, 1), LOADTEXT_TYPE_PLIST, 1)
        self.form_type = createSprite(PathTool.geHeroS(data.quality), 13.5, 150, self.form_name_bg, cc.p(0.5, 1), LOADTEXT_TYPE_PLIST, 1)
        self.form_type:setScale(0.5)
    else
        self.form_name_label:setString(StringUtil.branchStr(data.name))
        loadSpriteTexture(self.form_type, PathTool.geHeroS(data.quality))
        loadSpriteTexture(self.form_name_bg, PathTool.getFromQualityBg(data.quality))
