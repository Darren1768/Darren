...
function HeroMainTabHolyequipmentPanel:layoutUI()
    ...
    if self.holy_equip_bg then
        local camp_res = PathTool.getSingleImgForDownLoad("bigbg/hero","hero_holy_equip_bg", false)
...
function HeroMainTabHolyequipmentPanel:registerEvents()
    ...
    registerButtonEventListener(self.dungeon_btn_2, function(param, sender, event_type) 
     ...
     EventManager:getInstance():Bind(HeroEvent.Hero_Get_Holy_Equipment_Event,self, function(list)
     ...
     EventManager:getInstance():Bind(HeroEvent.Holy_Equipment_Update_Event,self, function(hero_vo)
