...
function HeroBreakPanel:open_callback()
    ...
    self.title:setString(TI18N("灵  升"))
    ...
    -- self.attr_label_key_list[1]:setString(TI18N("等级上限提升"))

    self.first_content = createRichLabel(22, cc.c4b(98,53,43,0xff), cc.p(0, 0.5), cc.p(70.5, 212), 5, nil, 540)
    attr_panel:addChild(self.first_content)
    ...
    self.break_btn:getChildByName("label"):setString(TI18N("灵升"))
end
...
function HeroBreakPanel:setData(hero_vo, next_data)
    ...
    self.first_content:setString(string.format("等级上限提升 %d级 至<div fontcolor=#49842f>  %s</div>级",break_config.lev_max,next_break_config.lev_max))

    --技能
--    if next_break_config.skill_num > break_config.skill_num then

      local new_kill_list=next_break_config.skill_up_list
    local old_kill_list=break_config.skill_up_list

    --查找是否有解锁新技能
    if  #new_kill_list > #old_kill_list then
        --说明有解锁技能
        ...
        local star_config = Config.PartnerData.data_partner_star(key)
            ...
            if skill_id ~= nil then
                ...
                self.skill_item = SkillItem.new(true,true,nil,1, true,nil,true,true)
                local size = self.skill_panel:getContentSize()
                self.skill_item:setPosition(cc.p(size.width/2, size.height/2+8))
...
function HeroBreakPanel:ShowNoneSkillInfo( ... )
    ...
    self.skill_name:setString(TI18N(""))
    local size = self.skill_panel:getContentSize()
    -- local bg = createImage(self.skill_panel, self.empty_res, size.width/2, size.height/2, cc.p(0.5,0.5), false)
    -- bg:setScale(0.6)
    -- commonShowEmptyIcon(self.skill_panel, true, {font_size = 22,scale = 0.7, text = TI18N("什么都没有")})
    -- self.skill_panel:setPositionY(self.skill_panel:getPositionY())
