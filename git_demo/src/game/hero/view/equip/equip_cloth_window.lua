...
function EquipClothWindow:register_event()
    ...
    --     self.put_equip_event = EventManager:getInstance():Bind(PartnerEvent.Equip_Update_Event,self,function()
...
function EquipClothWindow:updateEquipList()
    local scroll_view_size = cc.size(620,950)
    --有穿戴的要创建穿戴的
    ...
    --     end
    --     scroll_view_size = cc.size(620,950)
    -- end
    ...
    if self.list_view == nil then
        ...
        self.list_view = CommonScrollViewSingleLayout.new(self.main_panel, cc.p(12, 0) , ScrollViewDir.vertical, ScrollViewStartPos.top, scroll_view_size, setting)
...
function EquipClothWindow:createNewCell(width, height)
    ...
    print("---------------------self.cloth_data.all_score--------------",self.cloth_data.all_score)
...
function EquipClothItem:layoutUI()
    ...
    self.equip_career  = createLabel(24,Config.ColorData.data_color4[156],nil,300,70,"",self.main_panel,0, cc.p(0,0))
...
function EquipClothItem:setData(data)
    ...
    self.equip_career:setString("")
    if data.config.career then 
        local _str_care = TI18N("职业：")
        local _str_job = ""
        local color = Config.ColorData.data_color4[156]
        local type = role_vo:getCareer()
        local isContainer = false  --是否满足当前职业
        for i,v in pairs(data.config.career) do
            if _str_job ~= "" then 
                _str_job = _str_job..","
            end 
            _str_job = _str_job..HeroConst.CareerName[v]

            if v == type then
                isContainer = true
            end   
        end
        color = not isContainer and cc.c4b(239,58,58,255) or Config.ColorData.data_color4[156]
        _str_care = _str_care.._str_job
        self.equip_career:setString(_str_care)
        self.equip_career:setTextColor(color)
    end 
    -- 职业 career
    -- self.equip_career
    ...
    if self.data.config.sub_type == BackPackConst.item_tab_type.HOLYEQUIPMENT then --神装
        ...
        if self.equip_lev == nil then
            self.equip_lev = createLabel(24,Config.ColorData.data_color4[156],nil,150,69,"",self.main_panel,0, cc.p(0,0))
        end
        --评分
        if self.equip_score == nil then
            self.equip_score = createLabel(24,cc.c4b(0xa9,0x5a,0x1d,0xff),nil,300,28,"",self.main_panel,0, cc.p(0,0))
        end
...
function EquipClothItem:updateAttrList()
    ...
    for i,v in pairs(main_attr) do
        if not self.attr_list[index] then 
            local label = createLabel(24,Config.ColorData.data_color4[156],nil,150*index,28,"",self.main_panel,0, cc.p(0,0))--createRichLabel(22, Config.ColorData.data_color4[156], cc.p(0,0), cc.p(150*index,30), 0, 0, 400)
