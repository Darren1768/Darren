...
function HeroUpgradeStarSelectPanel:__init()
    ...
    self.res_list = {
        { path = PathTool.getSingleImgForDownLoad("hero","txt_cn_hero_temp_01"), type = ResourcesType.single },
...
function HeroUpgradeStarSelectPanel:open_callback()
    ...
    self.title:setString(TI18N("请选择材料武灵"))
    ...
    self.label_tip:setString(TI18N("(100%返还材料武灵灵升消耗的灵力、灵源和灵丹)"))
...
function HeroUpgradeStarSelectPanel:onClickBtnRight()
    ...
    EventManager:getInstance():Fire(HeroEvent.Upgrade_Star_Select_Event)
    self:onClickBtnClose()
end

--@select_data 选择的数据
--@select_data.is_ignore_hero_hun 是否忽略武灵魂(圣物用)
--@ dic_other_selected 其他已选的武灵数据 dic_other_selected[partner_id] = hero_vo 模式
                ...
                for _,v in pairs(list) do
                    if v.config and v.config.camp_type == self.select_data.camp_type and v.config.eqm_jie == self.select_data.star then
                        for i=1, v.quantity do
                            local hero_vo = {}
                            hero_vo.id = -(i + v.config.camp_type * 10)  --因为武灵id是正序的..这里就负序
