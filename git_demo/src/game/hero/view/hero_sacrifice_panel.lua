...
function HeroSacrificePanel:loadResources()
    self.res_list = {
        { path = PathTool.getSingleImgForDownLoad("bigbg","hl_bigbg_80", false), type = ResourcesType.single },
        { path = PathTool.getSingleImgForDownLoad("bigbg","hl_bigbg_81", false), type = ResourcesType.single },
        { path = PathTool.getSingleImgForDownLoad("bigbg","hl_bigbg_82", false), type = ResourcesType.single },
        { path = PathTool.getSingleImgForDownLoad("bigbg","bigbg_3", false), type = ResourcesType.single },
...
function HeroSacrificePanel:configUI()
    ...
    self.background:loadTexture(PathTool.getSingleImgForDownLoad("bigbg","hl_bigbg_80", false), LOADTEXT_TYPE)
    ...
    -- self.main_panel:getChildByName("win_title"):setString(TI18N("英雄炼灵"))
    ...
    --炼灵
    self.disband_btn = self.main_panel:getChildByName("disband_btn")
...
function HeroSacrificePanel:register_event()
    ...
    for select_camp, v in pairs(self.camp_btn_list) do
     ...
     EventManager:getInstance():Bind(BackpackEvent.ADD_GOODS,self, function(bag_code, add_list)
    ...
    -- 删除一个物品更新,也需要判断当前标签页类型
     EventManager:getInstance():Bind(BackpackEvent.DELETE_GOODS,self, function(bag_code,del_list)
        if bag_code ~= BackPackConst.Bag_Code.BACKPACK then
            return 
        end
...
function HeroSacrificePanel:_onClickBtnPutIn()
    ...
    self.is_put_in= true
    self.putin_btn_label:setString("取消放入")
end
...
function HeroSacrificePanel:changeSelectedTab()
    --elseif index == HeroConst.SacrificeType.eHeroSacrifice then   --英雄炼灵
...
function HeroSacrificePanel:updateHeroList(select_camp, reset)
    ...
    if not self.list_view then
        local scroll_view_size = cc.size(620,480)
        self.hero_setting = {
        ...
        local img_box_1 = self.disband_container:getChildByName("img_box_1")
        local x, y = img_box_1:getPosition()
    ...
    self.lab_have_count:setString(string_format(TI18N("可炼灵英雄:%s"), #self.show_list))
...
function HeroSacrificePanel:playEffect(status)
    ...
    --     GlobalAudio:getInstance():playEffect(GlobalAudio.AUDIO_TYPE.COMMON,"c_xianji")
