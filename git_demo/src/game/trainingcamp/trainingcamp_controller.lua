...
function TrainingcampController:registerEvents()
	--新获得伙伴弹窗
    EventManager:getInstance():Bind(MainuiEvent.CLOSE_ITEM_VIEW,self, function()
...
function TrainingcampController:handle27600( data )
	if data then
		...
		EventManager:getInstance():Fire(TrainingcampEvent.Update_Trainingcamp_Data_Event)
	end
end
...
function TrainingcampController:handle27602( data )
	if data then
		...
		if data.flag == 1 and data.is_first == 1 then
			EventManager:getInstance():Fire(TrainingcampEvent.Update_Trainingcamp_Tips_Event)
		end
		EventManager:getInstance():Fire(TrainingcampEvent.Is_Show_Formation_Event,data.is_formation)
	end
end
