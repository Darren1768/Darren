...
function TrainingcampTipsWindow:__init()
    ...
    self.res_list = {
        ...
        { path = PathTool.getSingleImgForDownLoad("bigbg","bigbg_93"), type = ResourcesType.single },
        { path = PathTool.getSingleImgForDownLoad("bigbg","bigbg_94"), type = ResourcesType.single },
	}
    ...
    self.background:addTouchEventListener(
        function(sender, event_type)
            if event_type == ccui.TouchEventType.ended then
                ...
                delayOnce(function()
                    EventManager:getInstance():Fire(TrainingcampEvent.Show_Close_Effect_Event)
...
function TrainingcampTipsWindow:createTitleContent(data)
   ...
   local index = string.find(data, "img")
    if index then
        ...
        local res = PathTool.getSingleImgForDownLoad('bigbg/trainingcamp', 'trainingcamp_'..bid)
