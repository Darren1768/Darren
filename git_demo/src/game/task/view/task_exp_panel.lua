...
function TaskExpPanel:configUI( )
    ...
    local res = PathTool.getResFrame("taskexp", "hl_task_20")
    for i=1,4 do
        local tab_btn = {}
...
function TaskExpPanel:registerEvent()
    ...
    --新的任务更新
    EventManager:getInstance():Bind(TaskEvent.TASK_EXP_UPDATE_EVENT,self,function()
            self:updateExpList(true)
        end)

    EventManager:getInstance():Bind(TaskEvent.TASK_EXP_UPDATE_TINE_EVENT,self,function()
...
function TaskExpPanel:updateExpList(is_keep_pos)
    ...
    if not self.list_view then
        local scroll_view_size = cc.size(620,676)
        local setting = {
...
function taskExpItem:configUI(width, height)
    ...
    local label=self.comfirm_btn:getChildByName("label")
    label:setFontSize(22)
    label:setString(TI18N("领取"))
    label:setTextColor(cc.c3b(189,81,16))
    ...
    self.title:setFontSize(22)
    self.title:setFontName('fonts/mini.ttf')
    self.title:setTextColor(cc.c3b(250,232,150))
    --时间
    ...
    self.recrod_time:setFontSize(20)
    self.recrod_time:setFontName('fonts/mini.ttf')
    self.recrod_time:setTextColor(cc.c3b(255,255,255))
    self.recrod_time:align(cc.p(0,0.5),179,116)

    --道具
    self.item_list = {}
    for i=1,2 do
        local item_node = main_container:getChildByName("item_node_"..i)
        self.item_list[i] = BackPackItem.new(false, true, false, 0.7, false, true, true)
        item_node:addChild(self.item_list[i])
    end
    self.task_desc = createRichLabel(20, cc.c3b(98,53,43), cc.p(0, 1), cc.p(26, 85), nil, nil, 270)
...
function taskExpItem:updateUnFinish()
    ...
    local res = PathTool.getResFrame("task","hl_task_14")
    self.title_bg:loadTexture(res, LOADTEXT_TYPE_PLIST)
end
...
function taskExpItem:updateAllFinish()
    ...
    local res = PathTool.getResFrame("task","hl_task_16")
    self.title_bg:loadTexture(res, LOADTEXT_TYPE_PLIST)
end
...
function taskExpItem:showProgress(status, percent, label)
    if status then
        if self.comp_bar == nil then
            local size = cc.size(95, 12)
            local res = PathTool.getResFrame("task","hl_task_7")
            local res1 = PathTool.getResFrame("task","hl_task_6")
            local bg,comp_bar = createLoadingBar(res, res1, size, self.main_container, cc.p(0.5,0.5), 552, 54, true, true)
            self.comp_bar_bg = bg
        ...
        if not self.comp_bar_label then
            local size = cc.size(95, 12)
            local text_color = cc.c3b(0x00,0x00,0x00)
            self.comp_bar_label = createLabel(20, text_color, nil, size.width/2, size.height/2 + 30, "", self.comp_bar, 1, cc.p(0.5, 0.5),'fonts/mini.ttf')
        end
