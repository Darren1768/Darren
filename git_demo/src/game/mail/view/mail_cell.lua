...
function MailCell:configUI()
	...
	self:setContentSize(cc.size(627,139))
...
function MailCell:changeIcon( status )
	...
	if status == 1 then --已读
		...
		if #self.data.assets>0 or #self.data.items>0 then --读了没领
			...
			loadSpriteTexture(self.icon, PathTool.getResFrame("common", "hl_common_130"), LOADTEXT_TYPE_PLIST)
	...
	elseif status == 0 then --未读
		if #self.data.assets>0 or #self.data.items>0 then --有物品
			...
			loadSpriteTexture(self.icon, PathTool.getResFrame("common", "hl_common_129"), LOADTEXT_TYPE_PLIST)
		else
