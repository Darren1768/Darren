...
function MonopolyMainScene:__init()
	...
	self.res_list = {
		...
		{path = PathTool.getSingleImgForDownLoad("monopoly", "dim_bg", true), type = ResourcesType.single},
	}
...
function MonopolyMainScene:MoveRoleByNum(num)
	...
	if self.move_tips_sound ~= nil then
        GlobalAudio:getInstance():removeEffectByData(self.move_tips_sound)
    end
    self.move_tips_sound = GlobalAudio:getInstance():playEffect(GlobalAudio.AUDIO_TYPE.COMMON, 'c_arenasettlement', false)
...
function MonopolyMainScene:checkShowTips()
	...
	if wait_show_tips_data and wait_show_tips_data[1] then -- 有待显示的tips则先显示tips
		if not self.wait_tips_bg then
			...
			self.wait_tips_txt = createLabel(40, cc.c4b(255,253,217,255), cc.c4b(167,80,3,255), 360, 54, "", self.wait_tips_bg, 2, cc.p(0.5, 0.5), TITLE_FONT)
		end
