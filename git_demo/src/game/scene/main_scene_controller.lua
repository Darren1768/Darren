...
function MainSceneController:registerEvents()
    if self.init_main_event == nil then
        self.init_main_event = EventManager:getInstance():Bind(EventId.ROLE_CREATE_SUCCESS, self, function()
                EventManager:getInstance():UnBindEvent(self.init_main_event)
                self.init_main_event = nil
                RenderMgr:getInstance():start()
                EventManager:getInstance():Fire(SceneEvent.FIRST_TIME_LOAD_FINISH)
    ...
    EventManager:getInstance():Bind(SceneEvent.MoveToBuildEvent, self, function(bid, show_figer, delay)
    ...
    EventManager:getInstance():Bind(SceneEvent.ENTER_FIGHT, self, function(combat_type)
            ...
            self:handleSceneStatus(false,true)
        end)
...
--@return 
--==============================--
function MainSceneController:handleSceneStatus(status, is_show_bottom)
    if self.main_scene == nil then return end
    
    is_show_bottom = is_show_bottom or status
    
    self.main_scene:setVisible(status)
    if status == true then
        self.main_scene:playBackgroundMusic()
    end
    MainuiController:getInstance():openMainUI(status,is_show_bottom)
end
...
-- 主城场景形象更换
function MainSceneController:changeSceneSpine(  )
    if self.main_scene then
        self.main_scene:createSpineInScene()
    end
end
...
function MainSceneController:openBuild(bid, extend)
    ...
    elseif bid == CenterSceneBuild.spirit then           -- 星命塔
    ...
    elseif bid == CenterSceneBuild.luckytreasure then                -- 幸运探宝
        ActionController:getInstance():openLuckyTreasureWin(true)
    elseif bid == CenterSceneBuild.hecheng then                -- 合成 
        ForgeHouseController:getInstance():openForgeHouseView(true, ForgeHouseConst.Tab_Index.Equip)
    elseif bid == CenterSceneBuild.xilingchuansong then                -- 洗灵传送
...
function MainSceneController:createBuildVo()
    ...
    EventManager:getInstance():Fire(SceneEvent.CreateBuildVoOver)
end
...
function MainSceneController:getBuildIdByCombat(combat_type)
    ...
    elseif combat_type == BattleConst.Fight_Type.Spirit then
        return CenterSceneBuild.spirit
