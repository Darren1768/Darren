...
function BuildItem:createBuild()
	...
	local temp_res = PathTool.getResFrame("mainui", "hl_mainui_chat_33")
    ...
    createLabel(28,cc.c3b(0xab,0x4a,0x27),nil,28,83,self.data.config.name,self.item_icon_bg,nil,cc.p(0.5, 0.5))
...
function BuildItem:clickNpcHandler()
	...
	playOtherSound(self.data.name,GlobalAudio.AUDIO_TYPE.COMMON)
...
function BuildItem:setRedPoint()
	...
	if status == false then
		...
		if self.tips_icon == nil then
			self.tips_icon = createSprite(PathTool.getResFrame("centerscene", "scene_0"), 46, 176, self.item_icon_bg, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST)
		end
...
function BuildItem:setGuideEffect()
	...
	if group_id == 0 then
		...
		if self.guide_effect == nil then
			...
			self.item_icon_bg:addChild(self.guide_effect, 10)
		end
	end
end
...
function BuildItem:fightStatus()
	...
	if status == false then
		...
		if self.fight_effect == nil then
			...
			self.item_icon_bg:addChild(self.fight_effect, 10)
		end
...
function BuildItem:setLockStatus()
	...
	if self.build_lock_status then
		...
		-- 	self.item_icon_bg:addChild(self.lock_icon)
		-- end
	else
		if self.data.config.bid == CenterSceneBuild.crossshow then
