...
function CenterCityScene:createScene()
    ...
    if build_list == nil or next(build_list) == nil then
        EventManager:getInstance():Bind(SceneEvent.CreateBuildVoOver,self, function() 
...
---创建主场景spine
function CenterCityScene:createSpineInScene()
    local role_vo = RoleController:getInstance():getRoleVo()
    local _config = Config.LooksData.data_data[role_vo.look_id]
    local ico_id = "H204010"  --H303010 精卫  洛神H103010 哪吒H105010 盘丝 H304020 紫霞 H103030 华佗 H204010
    if  _config then 
         ico_id=  _config.ico_id
    end 
    if self.look_spine then
        self.look_spine:removeFromParent()
    end 
    if self._btn_layout then
        self._btn_layout:removeFromParent()
    end 
    self._look_spine_index =0
    self.look_spine = createSpineByName(ico_id,"backstand",nil,nil,nil,nil,true)
    self.look_spine:setAnimation(0, "backstand", true)
    self.look_spine:setPosition(cc.p(500,450))
    self.look_spine:setAnchorPoint(cc.p(0.5,0.5))
    self["map_layer1"]:addChild(self.look_spine)


    delayRun(self.look_spine,1/60,function()
        local size_bound = self.look_spine:getBoundingBox()
        self._btn_layout  = ccui.Layout:create()
        self._btn_layout:setPosition(cc.p(500,450))
        self._btn_layout:setTouchEnabled(true)
        self._btn_layout:setContentSize(size_bound)
        self._btn_layout:setAnchorPoint(0.5,0)
        self["map_layer1"]:addChild( self._btn_layout)
        self._btn_layout:addTouchEventListener(function(sender,event_type)
            if event_type == ccui.TouchEventType.ended then
                print("----------self._look_spine_index-------------",self._look_spine_index)
                if  self.look_spine then 
                    local function animationCompleteFunc(event)
                        if event.animation == "backstand" then 
                            return
                        end 
                        if event.animation == "scene1" then 
                            self.look_spine:setAnimation(0, "scene2", false)
                            self.look_spine:setToSetupPose()
                            return
                        end 
                        if event.animation == "scene2" then 
                            self.look_spine:setAnimation(0, "scene1", false)
                            self.look_spine:setToSetupPose()
                            return
                        end 
                    end
                    self.look_spine:registerSpineEventHandler(animationCompleteFunc, sp.EventType.ANIMATION_COMPLETE)

                    if self._look_spine_index  ==0 then 
                        self._look_spine_index  = 1
                        self.look_spine:setAnimation(0, "scene1", false)
                         -- sence1
                    elseif self._look_spine_index  ==1  then 
                        self._look_spine_index  = 2
                        -- sence2
                        self.look_spine:setAnimation(0, "scene2", false)

                    elseif self._look_spine_index  ==2  then 
                        self._look_spine_index  = 0
                        -- backstand
                        self.look_spine:setAnimation(0, "backstand", true)
                    end 
    ...
    -- local ico_id =  "E204011"
    -- if self.look_spine then
    --     self.look_spine:removeFromParent()
    -- end 
    -- self.look_spine = createEffectSpine(ico_id, cc.p(500, 600), cc.p(0.5, 0.5), true, "animation")
    -- self.look_spine:setAnimation(0, "animation", true)
    -- self.look_spine:setAnchorPoint(cc.p(0.5,0.5))
    -- self["map_layer1"]:addChild(self.look_spine)
...
function CenterCityScene:quequeCreateEffect()
    ...
    if self.map_create_end == true then
        ...
        if effect_config ~= nil then
            ...
            local js_path, atlas_path, png_path = PathTool.getSpineByName(effect_config.res,nil) 
