...
function PartnerSummonGainWindow:__init(is_call)
    ...
    self.res_list = {
        ...
        -- { path = PathTool.getSingleImgForDownLoad("bigbg/partnersummon", "partnersummon_call_bg_100"), type = ResourcesType.single},
        { path = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_20", false), type = ResourcesType.single },
        { path = PathTool.getSingleImgForDownLoad("bigbg", "bigbg_12"), type = ResourcesType.single },
        { path = PathTool.getSingleImgForDownLoad("bigbg", "txt_cn_bigbg_18"), type = ResourcesType.single },
        { path = PathTool.getSingleImgForDownLoad("bigbg/partnersummon", "hl_bigbg_partnersumm_1"), type = ResourcesType.single },
        { path = PathTool.getSingleImgForDownLoad("bigbg/partnersummon", "hl_bigbg_partnersumm_2"), type = ResourcesType.single },
        { path = PathTool.getSingleImgForDownLoad("bigbg/partnersummon", "hl_bigbg_partnersumm_3"), type = ResourcesType.single },
        { path = PathTool.getSingleImgForDownLoad("bigbg/partnersummon", "hl_bigbg_partnersumm_4"), type = ResourcesType.single },
        { path = PathTool.getSingleImgForDownLoad("bigbg/partnersummon", "hl_bigbg_partnersumm_5"), type = ResourcesType.single },
...
function PartnerSummonGainWindow:createRootWnd()
    ...
    self.image_bg1 = createSprite(PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_21"), SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, self.root_wnd, cc.p(0.5, 0.5), LOADTEXT_TYPE)
    
    self.image_mark_bg = createScale9Sprite(PathTool.getResFrame("common", "hl_common_0"), SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 , LOADTEXT_TYPE_PLIST, self.root_wnd)
    self.image_mark_bg:setColor(cc.c3b(0,0,0))
    self.image_mark_bg:setContentSize(self.size)
    self.image_mark_bg:setOpacity(171)
  

    -- 预加载音效
    GlobalAudio:getInstance():preLoadEffect(GlobalAudio.AUDIO_TYPE.Recruit, "recruit_action")
    GlobalAudio:getInstance():preLoadEffect(GlobalAudio.AUDIO_TYPE.Recruit, "recruit_action2")
    GlobalAudio:getInstance():preLoadEffect(GlobalAudio.AUDIO_TYPE.Recruit, "result_01")
    GlobalAudio:getInstance():preLoadEffect(GlobalAudio.AUDIO_TYPE.COMMON, 'c_get')
    ...
    self.image_top_bg = createScale9Sprite(PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_31"), SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 -70 , LOADTEXT_TYPE, self.source_container)
    self.image_top_bg:setAnchorPoint(cc.p(0.5, 0.5))
    self.image_top_bg:setVisible(false)
    self.image_top_bg:setContentSize(cc.size(SCREEN_WIDTH,1513))
	-- self.image_top_bg:setCapInsets(cc.rect(0, 0, 360, 1000))
    self.image_top_bg:setCapInsets(cc.rect(0, 600, 0,900))


    self.image_bottom_bg = createScale9Sprite(PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_31"),SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 + 55, LOADTEXT_TYPE, self.source_container)
    self.image_bottom_bg:setContentSize(cc.size(0, 0))
    self.image_bottom_bg:setScaleY(-1)
    self.image_bottom_bg:setVisible(false)
    self.image_bottom_bg:setAnchorPoint(cc.p(0.5, 0))

    self.title_bg = createSprite(PathTool.getResFrame("common", "hl_common_598"), SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 +210, self.source_container, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST)
    self.title_bg:setVisible(false)
    self.title_bg:setScale(1)
    ...
    self.item_container:setPosition(cc.p(self.size.width / 2, SCREEN_HEIGHT / 2-70))
    ...
    self.comfirm_next_btn = createButton(self.btn_container, TI18N("离开"), 555, 120, cc.size(220, 77), PathTool.getResFrame("partnersummon", "partnersummon_btn"), 24)
    self.comfirm_next_btn:setRichText(TI18N("<div fontColor=#E7D5CA fontsize=26 ,#823705>离开</div>"))
    ...
    if self.is_call == TRUE then
        ...
        self.item_label_bg = createSprite(PathTool.getResFrame("common2", "hl_common_0023"),160, 60, self.item_container ,cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST)

        self.again_btn = createButton(self.item_container,TI18N("再幻一次"),165,0,cc.size(173,57),PathTool.getResFrame("common","hl_common_4_5"))
        self.again_btn:setRichText(TI18N("<div fontColor=#ffeeca fontsize=26>再幻一次</div>"))



        self.item_label = createRichLabel(22, 1, cc.p(0.5, 0.5), cc.p(80,90),0,0,500)
        self.item_label:setString("")
        self.again_btn:addChild(self.item_label)



        self.comfirm_btn = createButton(self.item_container,TI18N("离开"),560,0,cc.size(173,57),PathTool.getResFrame("common","hl_common_4_6"))
        self.comfirm_btn:setRichText(TI18N("<div fontColor=#E7D5CA fontsize=26>离开</div>"))
...
function PartnerSummonGainWindow:openRootWnd(data,is_call)
    ...
    if self.group_id ~= PartnersummonConst.Summon_Type.Score and config then
        ...
        elseif self.type_flag == 2 or self.type_flag == 3 then --精英幻灵
            self.one_icon_item = config.loss_item_once[1][1]
            self.five_icon_item = config.loss_item_ten[1][1]
        end
        self.again_btn:setVisible(true)
        self.comfirm_btn:setPosition(cc.p(560, 0))
    else
        self.again_btn:setVisible(false)
        self.comfirm_btn:setPosition(cc.p(self.item_container:getContentSize().width/2,0))
    end
    -- self.bg_res_id = PathTool.getSingleImgForDownLoad("bigbg/partnersummon", "partnersummon_call_bg_300", true)
    ...
    for i, v in ipairs(data.partner_bids) do
        ...
        if config and config.show_effect == 1 then
            ...
            if config.voice ~= "" then
                GlobalAudio:getInstance():preLoadEffect(GlobalAudio.AUDIO_TYPE.DUBBING, config.voice)
            end
...
function PartnerSummonGainWindow:updateEffectAction()
    ...
    self.recuit_music = GlobalAudio:getInstance():playEffect(GlobalAudio.AUDIO_TYPE.Recruit, music_name, false)
    ...
    if self.type_flag == 0 then
        if ccexp.VideoPlayer then
            ...
            self.first_effect_spine = self:playEffect("E0000000",PlayerAction.action,360,640, 99, self.first_container)
...
--开始播放视频
function PartnerSummonGainWindow:playVidoPlayer()
    if not self.videoPlayer then self:completeCallback() return end
    self.videoPlayer:setVisible(true)
    self.videoPlayer:play()     
end
--视频播放完成回调
function PartnerSummonGainWindow:completeCallback()
    self.image_bg:setVisible(true)
    if not self.is_show_item then
        if next(self.role_list or {}) == nil and self.is_call == TRUE then --表示这个没有英雄抽出
            self:updateItemData(self.reward_list)
...
--创建视频播放器
function PartnerSummonGainWindow:createVideoPlayer()
    print("=============开始创建视频播放器==========")
    if  device.platform == "android" or device.platform == "ios" then
        if not ccexp.VideoPlayer then return end
        ...
        if self.videoPlayer == nil then
            ...
            local size = self.first_container:getContentSize()
            local path = cc.FileUtils:getInstance():fullPathForFilename("res/video/draw_card_video.mp4")

            videoPlayer:setPosition(cc.p(size.width/2, size.height/2))
            videoPlayer:setAnchorPoint(cc.p(0.5, 0.5))
            videoPlayer:setContentSize(size)
            videoPlayer:setFileName(path)    --资源文件位置
            ...
            videoPlayer:addTouchEventListener(function(sender, eventType)
                    if eventType == ccui.TouchEventType.ended then
                        onVideoEventCallback(sender,ccexp.VideoPlayerEvent.COMPLETED)
                    end
                end)
            self.first_container:addChild(videoPlayer)
...
function PartnerSummonGainWindow:handleTimeSummonEffect( status )
    if status == true then
        ...
        if not self.time_summon_bg then
            ...
            local res_id = PathTool.getSingleImgForDownLoad("bigbg/timesummon",bg_res,true)
...
function PartnerSummonGainWindow:showHeroDetailInfo(  )
    ...
    if hero_cfg then
        ...
        -- 
    -- { path = PathTool.getSingleImgForDownLoad("bigbg/partnersummon", "hl_bigbg_partnersumm_1"), type = ResourcesType.single },
    loadSpriteTexture(self.background, PathTool.getSingleImgForDownLoad("campbg",hero_cfg.camp_type,false), LOADTEXT_TYPE)

        -- loadSpriteTexture(self.background, PathTool.getSingleImgForDownLoad("bigbg/partnersummon","hl_bigbg_partnersumm_"..hero_cfg.camp_type,false), LOADTEXT_TYPE)
        -- 名称
        ...
        -- 品质
        -- self.hero_quality_sprite
        loadSpriteTexture(self.hero_quality_sprite, PathTool.geHeroS(hero_cfg.quality), LOADTEXT_TYPE_PLIST)
        ...
        -- 星数
        local bg_res_id = PathTool.getSingleImgForDownLoad("bigbg/partnersummon", "partnersummon_call_bg_300", true)
        -- if hero_cfg.star >= 5 then
        --     bg_res_id = PathTool.getSingleImgForDownLoad("bigbg/partnersummon", "partnersummon_call_bg_400", true)
        ...
        if hero_cfg.draw_res ~= "" then
            local draw_res_path = PathTool.getSingleImgForDownLoad("herodraw/herodrawres", hero_cfg.draw_res, false)
...
function PartnerSummonGainWindow:quickShowHeroInfo(  )
    ...
    self.hero_name_txt:setPosition(cc.p(91.03, 908.92))
    self.hero_type_bg:stopAllActions()
    self.hero_type_bg:setPosition(cc.p(153.56, 868.59))
    self.hero_desc_bg:stopAllActions()
    self.hero_desc_bg:setScale(1)
    self.hero_desc_bg:setOpacity(255)
    self.hero_desc_bg:setPosition(cc.p(369.25, 187.09))
    self.hero_skill_btn:stopAllActions()
    self.hero_skill_btn:setPosition(cc.p(153.38, 770))
...
function PartnerSummonGainWindow:createHeroInfoNode(  )
    self.hero_info_node = createCSBNote(PathTool.getTargetCSB("newpartnersummon/newpartnersummon_hero_info"))
    ...
    self.background= self.hero_info_node:getChildByName("background")
    ...
    self.hero_new_sp:setVisible(false)
    self.hero_name_bg =   container:getChildByName("hero_name_bg")
    self.hero_quality_sprite =   self.hero_name_bg:getChildByName("hero_quality_sprite")
    ...
    self.type_sp_txt = self.hero_type_sp:getChildByName("type_sp_txt")
...
function PartnerSummonGainWindow:showHeroInfoEnterAni(  )
    ...
    self.card_music = GlobalAudio:getInstance():playEffect(GlobalAudio.AUDIO_TYPE.Recruit, "result_01", false)
    ...
    self.hero_name_txt:runAction(cc.Spawn:create(cc.FadeIn:create(0.25), cc.MoveTo:create(0.25, cc.p(91.03, 908.92))))
    -- local camp_pos_x = 360 - self.hero_name_txt:getContentSize().width*0.5 - 25
    ...
    self.hero_skill_btn:runAction(cc.MoveTo:create(0.3, cc.p(153.38, 770)))
    self.hero_type_bg:runAction(cc.MoveTo:create(0.3, cc.p(153.56, 868.59)))
    self.hero_desc_bg:runAction(cc.Sequence:create(cc.DelayTime:create(0.3), cc.Spawn:create(cc.FadeIn:create(0.1), cc.MoveTo:create(0.1, cc.p(369.25, 187.09)), cc.ScaleTo:create(0.1, 1.2)), cc.ScaleTo:create(0.05, 1.0), cc.CallFunc:create(function ()
        if self.cur_show_hero_cfg then
            if self.cur_show_hero_cfg.voice ~= "" then
                self.quick_show_voice = false
                HeroController:getInstance():onPlayHeroVoice(self.cur_show_hero_cfg.voice, self.cur_show_hero_cfg.voice_time)
                if self.hero_music ~= nil then
                    GlobalAudio:getInstance():removeEffectByData(self.hero_music)
                    self.hero_music = nil
                end
                self.hero_music = GlobalAudio:getInstance():playEffect(GlobalAudio.AUDIO_TYPE.DUBBING,self.cur_show_hero_cfg.voice,false)
            end
            if self.cur_show_hero_cfg.rare_flag and self.cur_show_hero_cfg.rare_flag == 1 then
                self:showRareEffect(true)
            end
...
function PartnerSummonGainWindow:updateItemData(data)
    ...
    for i, v in ipairs(data) do
        ...
        local _y = self.start_y - math.floor((i - 1) / col) * (BackPackItem.Height + self.space-10) 
    ...
    GlobalAudio:getInstance():playEffect(GlobalAudio.AUDIO_TYPE.COMMON, 'c_get', false)
    delayOnce(function ()
    ...
    if self.is_call == TRUE then
        if self.tiems == 10 and self.item_label and self.five_icon_item then
            self.again_btn:setRichText(TI18N("<div fontColor=#ffeeca fontsize=26>再幻十次</div>"))
...
function PartnerSummonGainWindow:updateTenSummon(  )
    if self.type_flag == 1 and self.item_label then
        ...
        if summon_have_num >= self.tiems then
            ...
            self.item_label:setString(string.format(TI18N("<img src=%s visible=true scale=0.5 /><div fontColor=#6FAF49>%d</div><div fontColor=#efd4ae>/%d</div>"), PathTool.getItemRes(item_icon), summon_have_num, self.tiems))
        else
            self._item_enough = false
            self.item_label:setString(string.format(TI18N("<img src=%s visible=true scale=0.5 /><div fontColor=#d6453b>%d</div><div fontColor=#efd4ae>/%d</div>"), PathTool.getItemRes(item_icon), summon_have_num, self.tiems))
        end
        return
    elseif (self.type_flag == 2 or self.type_flag == 3) and self.item_label then --精英幻灵
        local summon_have_num = BackpackController:getInstance():getModel():getItemNumByBid(self.five_icon_item)
        local item_icon = Config.ItemData.data_get_data(self.five_icon_item).icon
        if summon_have_num >= self.tiems then
            self._item_enough = true
    ...
    local str = string.format(TI18N("<img src=%s visible=true scale=0.5 /><div fontColor=#6faf49 >%d</div><div fontColor=#efd4ae>/%d</div>"), PathTool.getItemRes(item_icon), have_count, need_count)
    if have_count < need_count then
        str = string.format(TI18N("<img src=%s visible=true scale=0.5 /><div fontColor=#d6453b >%d</div><div fontColor=#efd4ae>/%d</div>"), PathTool.getItemRes(item_icon), have_count, need_count)
    end
...
function PartnerSummonGainWindow:updateSingleSummon()
    ...
    elseif config then
        ...
        if self.one_icon_num < need_count and (one_icon_special_num < special_need_count or special_need_count == 0) then --特殊道具和道具数量不够
            ...
            desc_str = string.format(TI18N("<img src=%s visible=true scale=0.5 /><div fontColor=#d6453b >%d</div><div fontColor=#efd4ae >/%d</div>"), PathTool.getItemRes(item_icon), self.one_icon_num, need_count)          
        elseif one_icon_special_num >= special_need_count and special_need_count ~= 0 then
            ...
            desc_str = string.format(TI18N("<img src=%s visible=true scale=0.5 /><div fontColor=#35ff14 >%d</div><div fontColor=#efd4ae >/%d</div>"), PathTool.getItemRes(special_icon), self.one_icon_num, special_need_count)
        else
            self.single_status = PartnersummonConst.Status.Item
            desc_str = string.format(TI18N("<img src=%s visible=true scale=0.5 /><div fontColor=#35ff14 >%d</div><div fontColor=#efd4ae >/%d</div>"), PathTool.getItemRes(item_icon), self.one_icon_num, need_count)
        end
    end
...
function PartnerSummonGainWindow:showAlert(num, item_icon_2, val_str, val_num, call_num,call_back)
    ...
    local str_ = str .. string.format(TI18N("<div fontColor=#764519>购买</div><div fontColor=#289b14 fontsize= 26>%s</div><div fontColor=#764519>%s(同时附赠</div><div fontColor=#289b14 fontsize= 26>%s</div><div fontColor=#764519>次幻灵)</div>"), val_num, val_str, call_num)
    ...
    --[[if self.role_list and next(self.role_list or {}) ~= nil then
        ...
        for i, v in ipairs(self.role_list) do
            if not self.role_spine_list[i] then
                ...
                local spine_sp = createSprite(PathTool.getSingleImgForDownLoad("bigbg/partnercard", "partnercard_10101"), size.width / 2, size.height / 2 + 170, self.effect_container, cc.p(0.5, 0.5), LOADTEXT_TYPE)
...
function PartnerSummonGainWindow:resgiter_event()
                ...
                if self.type_flag == 0 then
                    ...
                    if self.tiems == 1 then
                        ...
                        local call_back = function ()
                            ...
                            elseif self.type_flag == 2 then --精英幻灵
                                EliteSummonController:getInstance():send23221(1,3)
                            elseif self.type_flag == 3 then --自选精英幻灵
    ...
    if self.comfirm_btn then
     ...
     EventManager:getInstance():Bind(PartnersummonEvent.updateSummonDataEvent,self, function(data)
            if data then
                self:updateSingleSummon()
            end
        end)
     EventManager:getInstance():Bind(PartnersummonEvent.updateSummonShareDataEvent,self, function()
...
function PartnerSummonGainWindow:showSingleAlert()
    ...
    if config and self.group_id == PartnersummonConst.Summon_Type.Advanced then
        ...
        if self.type_flag == 0 then
            ...
            local str = str_ .. string.format(TI18N("<div fontColor=#764519>购买</div><div fontColor=#289b14 fontsize= 26>%s</div><div fontColor=#764519>%s(同时附赠</div><div fontColor=#289b14 fontsize= 26>%s</div><div fontColor=#764519>次幻灵)</div>"), val_num, val_str, call_num)   
...
function PartnerSummonGainWindow:close_callback()
    ...
    if self.card_music ~= nil then
        GlobalAudio:getInstance():removeEffectByData(self.card_music)
    end
    self:handleTimeSummonEffect(false)
    self:showDrawEffect1(false)
    self:showDrawEffect2(false)
...
function PartnerSummonGainWindow:showHeroAction()
    ...
    if self.temp_sp then
        ...
        if self.temp_sp and self.temp_sp.data then
            if not self.is_show_item then
                local res_id = PathTool.getSingleImgForDownLoad("bigbg/partnercard", "partnercard_" .. self.temp_sp.data.bid)
                ...
                local function animationEventFunc(event)
                    if event.eventData.name == "appear" then
                        if self.recuit_music ~= nil then
                            GlobalAudio:getInstance():removeEffectByData(self.recuit_music)
                        end
                        self.card_music = GlobalAudio:getInstance():playEffect(GlobalAudio.AUDIO_TYPE.Recruit, "result_01", false)
                ...
                local function animationCompleteFunc()
                    ...
                    if self.temp_sp.data.voice then
                        self.hero_music = GlobalAudio:getInstance():playEffect(GlobalAudio.AUDIO_TYPE.DUBBING,self.temp_sp.data.voice,false)
                    end
                end
...
function PartnerSummonGainWindow:updateNewDesc(data, base_data)
    if not self.name then
        self.name = createLabel(70, 1, 2, self.new_desc_container:getContentSize().width/2,475, data.name, self.new_desc_container, nil, cc.p(0.5, 0.5), TITLE_FONT)
        self.hero_pos = createLabel(40, 198,199, self.new_desc_container:getContentSize().width/2,415, data.hero_pos, self.new_desc_container, nil, cc.p(0.5, 0.5), TITLE_FONT)
    end
    if self.name then
