...
function PartnerSummonGainShowWindow:__init(bg_type)
    ...
    --[[if bg_type and bg_type == 2 then
        self.partnersummon_call_bg_res = PathTool.getSingleImgForDownLoad("bigbg/partnersummon", "partnersummon_call_bg_200")
    else
        self.partnersummon_call_bg_res = PathTool.getSingleImgForDownLoad("bigbg/partnersummon", "partnersummon_call_bg_100")
    end--]]
    --self.partnersummon_call_bg_res = PathTool.getSingleImgForDownLoad("bigbg/partnersummon", "partnersummon_call_bg_300", true)
    self.res_list = {
...
function PartnerSummonGainShowWindow:createRootWnd()
    ...
    self.title_bg = createSprite(PathTool.getSingleImgForDownLoad("bigbg", "txt_cn_bigbg_18"), SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 + 525, self.source_container, cc.p(0.5, 0.5), LOADTEXT_TYPE)
    ...
    if hero_cfg then
        ...
        if self.show_type == PartnersummonConst.Gain_Show_Type.Skin_show then
            ...
            if skin_cfg and skin_cfg.draw_res ~= "" then
                ...
                local draw_res_path = PathTool.getSingleImgForDownLoad("herodraw/herodrawres", skin_cfg.draw_res, false)
                self.hero_draw_load = loadSpriteTextureFromCDN(self.hero_draw_sp, draw_res_path, ResourcesType.single, self.hero_draw_load)
            end
            -- 皮肤立绘缩放和位置偏移
            if skin_cfg.scale_2 and  skin_cfg.scale_2 ~=0 then
        ...
        if self.temp_spine and self.temp_spine.data then
            if not self.is_show_item then
                ...
                if self.show_type == PartnersummonConst.Gain_Show_Type.Skin_show then
                    ...
                    if skin_config then
                        res_id = PathTool.getSingleImgForDownLoad("bigbg/partnercard", "partnercard_"..skin_config.head_card_id)
                    else
                        res_id = PathTool.getSingleImgForDownLoad("bigbg/partnercard", "partnercard_2001")
                    end
                else --PartnersummonConst.Gain_Show_Type.Common_Show
                    res_id = PathTool.getSingleImgForDownLoad("bigbg/partnercard", "partnercard_" .. self.temp_spine.data.bid)
                end
                ...
                local function animationEventFunc(event)
                    if event.eventData.name == "appear" then
                        GlobalAudio:getInstance():playEffect(GlobalAudio.AUDIO_TYPE.Recruit, "result_01", false)
                ...
                local function animationCompleteFunc()
                    if self.temp_spine and self.temp_spine.data.voice and self.temp_spine.data.voice ~= " " then 
                        self.hero_music = GlobalAudio:getInstance():playEffect(GlobalAudio.AUDIO_TYPE.DUBBING, self.temp_spine.data.voice, false)
                    end
                end
