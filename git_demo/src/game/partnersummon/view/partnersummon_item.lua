...
function PartnerSummonItem:ctor(index)
    ...
    self.root_wnd = createCSBNote(PathTool.getTargetCSB("partnersummon/partnersummon_item"))
...
function PartnerSummonItem:registerEvent()
    ...
    -- 某一卡库数据更新
     EventManager:getInstance():Bind(PartnersummonEvent.updateSummonSingleDataEvent,self,function(group_ids)
    ...
    --召唤5星
     EventManager:getInstance():Bind(PartnersummonEvent.SummonMustFiveStarEvent,self, function()
...
function PartnerSummonItem:setData(i,data)
    if data then
        ...
        -- 背景
        local res_id = PathTool.getSingleImgForDownLoad("bigbg/partnersummon", self.info_data.card_bg_res)
