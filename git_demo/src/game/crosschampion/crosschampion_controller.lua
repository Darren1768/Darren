...
function CrosschampionController:registerEvents()
	-- 点赞事件监听，判断红点
	EventManager:getInstance():Bind(RoleEvent.WorshipOtherRole,self, function(rid, srv_id, idx, _type) 
...
function CrosschampionController:handle26213( data )
	if data then
		...
		EventManager:getInstance():Fire(CrosschampionEvent.UpdateChampionTop3Event, data.rank_list)
	end
end
