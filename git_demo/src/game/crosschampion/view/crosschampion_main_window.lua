...
function CrosschampionMainWindow:__init()
    ...
    self.res_list = {
        ...
        { path = PathTool.getSingleImgForDownLoad("bigbg/crosschampion","hl_crosschampion_1000"), type = ResourcesType.single }
    }
    self.background_path = "resource/bigbg/crosschampion/hl_crosschampion_1000.png"
...
function CrosschampionMainWindow:open_callback(  )
    ...
    self.rank_btn:getChildByName("label"):setString(TI18N("排名"))
    self.shop_btn = stage_panel:getChildByName("shop_btn")
    self.shop_btn:getChildByName("label"):setString(TI18N("冠军商店"))
    self.reward_btn = stage_panel:getChildByName("reward_btn")
    self.reward_btn:getChildByName("label"):setString(TI18N("奖励"))
    self.guess_btn = stage_panel:getChildByName("guess_btn")
    self.guess_btn:getChildByName("label"):setString(TI18N("布阵"))
    ...
    dec_container:getChildByName("label_5"):setString(TI18N("提示:"))

    -- 当前赛况
    self.match_step = createRichLabel(22, 1, cc.p(0, 0.5), cc.p(163, 155), nil, nil, 560) 
    dec_container:addChild(self.match_step)

    -- 赛季时间
    self.match_desc = createRichLabel(22, 1, cc.p(0, 0.5), cc.p(163, 66), nil, nil, 560)
...
function CrosschampionMainWindow:_onClickShopBtn(  )
    ...
    MallController:getInstance():openMallPanel(true, MallConst.MallType.CrosschampionShop)
end

function CrosschampionMainWindow:_onClickRewardBtn(  )
    ArenaController:getInstance():openArenaChampionRankAwardsWindow(true, ArenaConst.champion_type.cross) 
end
