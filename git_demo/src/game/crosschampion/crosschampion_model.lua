...
function CrosschampionModel:updateChampionBaseInfo( data )
	self.base_info = data
	EventManager:getInstance():Fire(ArenaEvent.UpdateChampionBaseInfoEvent, data)
end
...
function CrosschampionModel:setRoleInfo( data )
	self.role_info = data
    EventManager:getInstance():Fire(ArenaEvent.UpdateChampionRoleInfoEvent, data)
end
...
function CrosschampionModel:updateCrosschampionRedStatus( bid, status )
    ...
    GhostdomController:getInstance():setBuildRedStatus(GhostdomConst.Build_Type.battle, {bid = CrossgroundConst.Red_Type.crosschampion, status = red_status})
    EventManager:getInstance():Fire(CrosschampionEvent.Update_Red_Status_Event, bid, status)
end
