...
function GuildskillModel:initGuildCareerSkill(data)
    ...
    if config == nil then
        ...
        EventManager:getInstance():Fire(GuildskillEvent.UpdateGuildSkillEvent, data.career)
    end
end
...
function GuildskillModel:resetCareerSkillInfo(career)
    ...
    if object then
        ...
        EventManager:getInstance():Fire(GuildskillEvent.ResetGuildSkillEvent, career)
    end 
end
...
function GuildskillModel:updateGuildCareerSkill(career, skill_id)
    ...
    if object and object.skill_ids then
        ...
        EventManager:getInstance():Fire(GuildskillEvent.UpdateSkillStatusEvent, career, skill_id)
    end
end
...
function GuildskillModel:upgradeGuildCareerSkill(career, group_id)
    ...
    EventManager:getInstance():Fire(GuildskillEvent.UpdateSkilUpgradeEvent, career, group_id)
end
...
function GuildskillModel:updateGuildPvpRedStatus(bid, status)
    ...
    local bool, desc = RoleController:getInstance():checkEnterIsLock(GhostdomConst.Build_Type.guild)
    ...
    if self.had_send_mainui == false then
        ...
        GhostdomController:getInstance():setBuildRedStatus(GhostdomConst.Build_Type.guild, {bid = bid, status = status})
    end

    -- 事件用于同步更新公会主ui的红点
    EventManager:getInstance():Fire(GuildEvent.UpdateGuildRedStatus, bid, status)
end 
--==============================--
--desc:清楚主界面上面的红点
--@return 
...
function GuildskillModel:clearGuildSkillIconRed()
    ...
    if is_red == true then
        ...
        GhostdomController:getInstance():setBuildRedStatus(GhostdomConst.Build_Type.guild, data)
    end
end
