...
function RefController:buildItemMsg(item_bid, srv_id, share_id, count)
    ...
    if config_data then
        return string.format("<div fontcolor=%s href=%s|%s|%s>[%s]</div>",  --"<div fontcolor=%s href=%s|%s|%s>[%s]</div>"
...
function RefController:handle10535(data)
    if data.flag == 1 then
        EventManager:getInstance():Fire(EventId.CHAT_SELECT_ITEM, data, self:getFrom())
    else
        message(data.msg)
    end
end
