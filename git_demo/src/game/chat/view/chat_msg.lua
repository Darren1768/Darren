...
function ChatMsg:createRelationIcon(data,scale,x,y)
    local scale = scale or 1
    local data = data or self.dataObj
    local common_relation_node = CommonRelationIconNode.new(data,scale)
    local relation_node_size = common_relation_node:getContentSize()
    local off_x = (self.vip_image~=nil and self.vip_image:isVisible()) and 30 or 0
    local x,y = x or self.player_name:getPositionX()+self.player_name:getContentSize().width-5+off_x , y or 25 --x or self.head_icon:getPositionX()+15+off_x , y or 25 --在名字前面显示


    if self.head_root:getChildByName("common_relation_node") then
        self.head_root:getChildByName("common_relation_node"):removeFromParent()
    end
    self.head_root:addChild(common_relation_node)
    common_relation_node:setPosition(x,y)
    common_relation_node:setName("common_relation_node")


    return relation_node_size.width
end

function ChatMsg:setSimple()
    if self.head_icon then
        self.head_icon:setScale(0.4)
    end
    if self.talk_time then
        self.talk_time:setVisible(false)
    end
    if self.chat_bg then
        self.chat_bg:setVisible(false)
    end

    self.head_icon:setPositionX(50)
    self.head_root:setPosition(cc.p(0,20))
    ...
    self.head_icon:setPosition(cc.p(30,0))

    local x,y = self.head_icon:getPosition()
    local relation_width = self:createRelationIcon(nil,0.7,x+15,y-38)
    relation_width = relation_width==0 and relation_width or relation_width-5
    --print("=========关系图标的宽==relation_width============",relation_width)

    self.player_name_simple = createLabel(20,cc.c4b(239,212,174,255),nil,109,94,"",self.root_wnd,nil,nil)
    self.player_name_simple:enableOutline(cc.c4b(49,41,39,128), 0)
    self.player_name_simple:setAnchorPoint(0,1)
    self.player_name_simple:setPosition(cc.p(relation_width+50,20))
    self.player_name_simple:setString(self.player_name:getString()..": ")

    if self.is_mine then 
        self.player_name_simple:setColor(cc.c3b(211,112,52))
        self.player_name_simple:enableOutline(cc.c4b(49,41,39,170),0)
    end 

    -- self.head_root.b 
    self.contentSimple = createRichLabel(20, cc.c4b(255,255,255,255), cc.p(0, 1), cc.p(190, 31), 2, -1, self.view_width)
    self.root_wnd:addChild(self.contentSimple)
    self.contentSimple:setMaxWidth(500, false)
    self.contentSimple:addTouchLinkListener(function(type, value, sender)
                ChatHelp.OnChatTouched(type, value, sender, self.dataObj)
            end, {"click","href"})
    local temp_str = string.format("<div fontsize=20 fontcolor=efd4ae outline=0,#312927>%s</div>",self.dataObj.msg)
    self.contentSimple:setString(temp_str)

    if self.vip_image then 
        self.vip_image:setVisible(false)
    end
    if self.site_txt then
        self.site_txt:setVisible(false)
    end
    self.player_name:setVisible(false)
    self.content:setVisible(false)
    
    local total_height = self.contentSimple:getSize().height + 12
    self.head_root:setPositionY(total_height - 20)
    self.player_name_simple:setPositionY(total_height - 9)
    self.contentSimple:setPositionY(total_height - 9)

    self.contentSimple:setPositionX(self.player_name_simple:getContentSize().width + self.player_name_simple:getPositionX())
...
function ChatMsg:showHead(data)
    ...
    if tolua.isnull(self.head_icon)  then
        ...
        if vo then
            ...
            -- local res = PathTool.getResFrame("mainui", "hl_mainui_chat_32", false, false)

            self.head_icon:showBg(res, nil, false, vo.offy)
            -- self.head_icon.headbg:setScale(3)
        end
        ...
        --名字
        self.player_name = createLabel(24,cc.c4b(79,51,57,255),nil,109,94,"",self.head_root,nil,nil,TITLE_FONT)
        ...
        --聊天内容 54f1ff
        self.content = createRichLabel(22, cc.c4b(79,51,57,255), cc.p(0, 0.5), cc.p(20, 64), 8, -1, self.view_width)
        self.chat_pic:addChild(self.content)
        self.content:addTouchLinkListener(function(type, value, sender)
                ChatHelp.OnChatTouched(type, value, sender, self.dataObj )
            end, {"click","href"})
        self.content:setMaxWidth(310, false)
    end
    ...
    local _content_gap = 32
    local _height = size.height + _content_gap

    local v_width = math.max(36+self.content:getContentSize().width,64)
    ...
    local res
    if sex == 1 then
        res = PathTool.getResFrame("common", "hl_common_8")
    else
        res = PathTool.getResFrame("common", "hl_common_9")
    end
    if data.sex ~= 2 then 
        self.sex_icon:setVisible(true)
        self.sex_icon:loadTexture(res, LOADTEXT_TYPE_PLIST)
    else 
        self.sex_icon:setVisible(false)
    end
    --区分跨服名字
    ...
    local headY = self.head_icon:getPositionY() + 10

    self.player_name:setPosition(cc.p(offx+guild_x+3,headY+20))
    local vip_x = 0
    if self.vip_image then 
    ...
    if self.site_txt then
        ...
        self.site_txt:setPosition(cc.p(self.player_name:getPositionX()+self.player_name:getContentSize().width+4+40,headY+20))
    end
    ...
    self.chat_pic:setPositionY(headY+offy-2)
    self.chat_bg:setPositionY(headY+offy-2)
    self.talk_time:setPositionY(headY+43)
    local offy = 0
...
function ChatMsg:setMyNotice(data)
    ...
    self.chat_pic:setPositionY(headY-2)
    self.chat_bg:setPosition(cc.p(35, headY-2))
    ...
    local res = PathTool.getResFrame("mainui","hl_chat_2")
    self.chat_bg:loadTexture(res,LOADTEXT_TYPE_PLIST)
    self.chat_bg:setScale9Enabled(true)
    self.chat_bg:setCapInsets(cc.rect(20,40,1,1))
    ...
    self.player_name:setPosition(cc.p(20,headY+20))
    ...
    if self.site_txt then
        ...
        self.site_txt:setPosition(cc.p(self.player_name:getPositionX()-self.player_name:getContentSize().width+2-20,headY+20))
    end
    --[[if self.msg_time then
...
function ChatMsg:showNotice(data)
    ...
    elseif data.channel == ChatConst.Channel.Team or ChatConst.Channel.Team_Sys or ChatConst.Channel.MyTeam then
        str = "<div>".. data.msg .. "</div>"
    end
...
function ChatMsg:setChannel(channel)
    ...
    if self.channel ~= channel then
        self.channel_bg:loadTexture(PathTool.getResFrame("mainui","hl_txt_cn_chat_icon_"..ChatConst.ChannelRes[channel]),LOADTEXT_TYPE_PLIST)
...
function ChatMsg:setVoiceListener(bool)
    if bool then
        self.voice_evt = EventManager:getInstance():Bind(VoiceMgr.NewMsg,self, function(name, msg)
        ...
        if self.voice_evt then
            EventManager:getInstance():UnBindEvent(self.voice_evt)
            self.voice_evt = nil
        end
    end
end
...
function ChatMsg:setVoicePlayListener(bool)
    if bool then
        self.play_evt = EventManager:getInstance():Bind(VoiceMgr.Played,self, function(bool)
        ...
        if self.play_evt then
            EventManager:getInstance():UnBindEvent(self.play_evt)
            self.play_evt = nil
        end
    end
end
...
function ChatMsg:onTranslateEnd(voice_name, voice_msg)
    if self.dataObj and self.dataObj.len==1 and self.dataObj.msg==voice_name then
        ...
        EventManager:getInstance():Fire(ChatConst.Voice_Translate_Panel)
    end
end
