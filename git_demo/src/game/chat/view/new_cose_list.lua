    ...
    for index=0, self.init_len-1 do             
        ...
        delayRun(self, 0.01*(index), function ()
            ...
            if index == self.init_len-1 then
                ...
                self.is_bottom = true
                self:updateMsg()
                EventManager:getInstance():Fire(ChatEvent.EndCallBack)
            end
        end)
    end
...
--更新聊天数据
function NewCoseList:updateMsg(force,is_simple)
...
function NewCoseList:showMonkey(bool, close_tips)
    ...
    commonShowEmptyIcon(self,bool,{text = close_tips or TI18N("暂时没有人说话")})
