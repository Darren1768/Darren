...
function ChatController:config()
    ...
    self.dispather:Bind(EventId.ROLE_CREATE_SUCCESS, self, function()
...
function ChatController:initChatMsgData(  )
    ...
    self.stack_list =
    {
        ...
        [ChatConst.Channel.MyTeam]  = Array.New(),
    }
    -- 未读数量
    self.cache_msg_list = {}
end
...
function ChatController:handle12721(data)
    ...
    EventManager:getInstance():Fire(EventId.CHAT_UPDATE_SELF, chatVo)
    EventManager:getInstance():Fire(FriendEvent.UPDATE_GROUP_COUNT)

     -- 玩家世界聊天和公会聊天看看需要不需要缓存起来
    if self.cur_channel ~= ChatConst.Channel.Friend or not self.chat_ui then
...
function ChatController:handle12761__(data, bool, is_login_content)
    ...
    if not FriendController:getInstance():getModel():isBlack(vo.rid, vo.srv_id) then
        ...
        if bool == nil then
            ...
            if is_myself == false then
                if channel == ChatConst.Channel.World and (self.cur_channel ~= ChatConst.Channel.World or not self.chat_ui) then
                    self:accumulateChannelNum(ChatConst.Channel.World)
                elseif channel == ChatConst.Channel.Gang and (self.cur_channel ~= ChatConst.Channel.Gang or not self.chat_ui) then
                    self:accumulateChannelNum(ChatConst.Channel.Gang)
                elseif channel == ChatConst.Channel.Cross and (self.cur_channel ~= ChatConst.Channel.Cross or not self.chat_ui) then
                    self:accumulateChannelNum(ChatConst.Channel.Cross)
                elseif channel == ChatConst.Channel.Province and (self.cur_channel ~= ChatConst.Channel.Province or not self.chat_ui) then
    ...
    if not FriendController:getInstance():getModel():isBlack(vo.rid, vo.srv_id) or
       channel == ChatConst.Channel.Team_Sys then --组队聊天也要此事件
        EventManager:getInstance():Fire(EventId.CHAT_UDMSG_WORLD, channel, is_self, is_login_content)
    end
    --记录自动播放的语音
...
function ChatController:pushAssetsMsg(text)
    ...
    EventManager:getInstance():Fire(EventId.CHAT_UDMSG_ASSETS)
end
...
function ChatController:openChatPanel(channel,form,data)
    ...
    if not self.chat_ui then
        self.chat_ui = ChatWindowLayer.New(self)--ChatWindow.New(self)
    end
    if form == "friend" then
        local roleVo = RoleController:getInstance():getRoleVo()
        if data then
...
function ChatController:setLastChannel(channel)
    ...
    -- 清除掉也要通知一下吧
    EventManager:getInstance():Fire(EventId.CHAT_NEWMSG_FLAG, channel)
end
...
function ChatController:openShowDramaTimer( status )
    if status == true then
        EventManager:getInstance():Fire(ChatEvent.AdjustMainChatBtnPos, true)
            ...
            self.showDramaTimer = GlobalTimeTicket:getInstance():add(function()
                ...
                if drama_content then
                    if self.drama_time >= self.interval_time then
                        ...
                        self.drama_time = 0
                        EventManager:getInstance():Fire(EventId.CHAT_UDMSG_WORLD)
                    end
                else
                    EventManager:getInstance():Fire(ChatEvent.AdjustMainChatBtnPos, false)
