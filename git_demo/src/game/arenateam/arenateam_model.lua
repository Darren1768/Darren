...
function ArenateamModel:checkArenateamTotalRedPoint( need_check)
    ...
    if self.my_team_info then 
        ...
        if need_check then
            EventManager:getInstance():Fire(ArenateamEvent.ARENATEAM_ALL_RED_POINT_EVENT)
            ...
            GhostdomController:getInstance():setBuildRedStatus(GhostdomConst.Build_Type.battle, {bid = CrossgroundConst.Red_Type.Arenateam, status = redpoint})
        end
...
function ArenateamModel:startTimeTicket()
    ...
    local _callback = function()
        ...
        EventManager:getInstance():Fire(ArenateamEvent.ARENATEAM_TIMER_EVENT, self.time_step)
