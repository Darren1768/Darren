...
function ArenateamHallTapTeamPanel:layoutUI()
    ...
    --  -- 标题
    -- local res = PathTool.getSingleImgForDownLoad("bigbg/rolepersonalspace", "role_honor_wall_bg", false)
    ...
    local res = PathTool.getResFrame("common", "hl_chat_3")
...
function ArenateamHallTapTeamPanel:registerEvents()
    ...
    registerButtonEventListener(self.refresh_btn, function() self:onRefreshBtn()  end ,true, REGISTER_BUTTON_SOUND_BUTTON_TYPY,nil,nil, 1)
     ...
     EventManager:getInstance():Bind(ArenateamEvent.ARENATEAM_HALL_MAIN_EVENT,self,function (data)
    ...
    --搜索返回
     EventManager:getInstance():Bind(ArenateamEvent.ARENATEAM_SEARCH_TEAM_EVENT,self,function (data)
            if not data then return end
            self:setSearchBack(data)
        end)
    --一键申请
     EventManager:getInstance():Bind(ArenateamEvent.ARENATEAM_KEY_APPLY_TEAM_EVENT,self,function (data)
            if not data then return end
            self:keyApplyTeam(data)
        end)
    --申请单个
     EventManager:getInstance():Bind(ArenateamEvent.ARENATEAM_APPLY_TEAM_EVENT,self,function (data)
            if not data then return end
