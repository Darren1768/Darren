...
function ArenateamHallTapMyTeamPanel:registerEvents()
    ...
    if self.team_item_list then
     ...
     EventManager:getInstance():Bind(ArenateamEvent.ARENATEAM_MY_TEAM_INFO_EVENT,self,function (data)
            if not data then return end
            self:setData(data)
        end)
    --报名参赛
     EventManager:getInstance():Bind(ArenateamEvent.ARENATEAM_JOIN_GAME_EVENT,self,function ()
            --状态改变
            if self.scdata then 
    ...
    --报名参赛
     EventManager:getInstance():Bind(ArenateamEvent.ARENATEAM_LEAVE_GAME_EVENT,self,function ()
            --状态改变
             if self.scdata then 
     ...
     EventManager:getInstance():Bind(ArenateamEvent.ARENATEAM_CHANGE_NAME_CALLBACK,self,function (name)
            self.record_name = name
        end)
     EventManager:getInstance():Bind(ArenateamEvent.ARENATEAM_TEAM_SET_CALLBACK,self,function (limit_lev, limit_power)
            self.record_limit_lev = limit_lev or 0
            self.record_limit_power = limit_power or 0
        end)
    --改名
     EventManager:getInstance():Bind(ArenateamEvent.ARENATEAM_CHANGE_NAME_EVENT,self,function ()
    ...
    --队伍设置
     EventManager:getInstance():Bind(ArenateamEvent.ARENATEAM_TEAM_SET_EVENT,self,function ()
            if self.scdata then
    ...
    --有红点
     EventManager:getInstance():Bind(ArenateamEvent.ARENATEAM_APPLY_RED_POINT_EVENT,self,function ()
            if self.scdata then
                self:updateRedPoint()   
            end
        end)
    --红点发生变化
     EventManager:getInstance():Bind(ArenateamEvent.ARENATEAM_ALL_RED_POINT_EVENT,self,function ()
            if self.scdata then
                self:updateRedPoint()   
            end
        end)
