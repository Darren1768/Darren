...
function LadderRankWindow:open_callback(  )
    ...
    self.no_rank_pic = self.my_rank:getChildByName("no_rank_pic")
    self.no_rank_pic:setVisible(false)
    ...
    self.my_head:setPosition(230, 45)
    ...
    self.rank_scrollview = CommonScrollViewLayout.new(self.rank_panel, cc.p(0,0) , ScrollViewDir.vertical, ScrollViewStartPos.top, scroll_view_size, setting)
...
function LadderRankWindow:register_event(  )
	...
	registerButtonEventListener(self.background, handler(self, self._onClickBtnClose), false, 2)
     ...
     EventManager:getInstance():Bind(LadderEvent.UpdateLadderRankData,self, function ( data )
            self:setData(data)
        end)
end
...
function LadderRankWindow:setData( data )
	...
	if not data.rank or data.rank == 0 then
		self.no_rank_pic:setVisible(true)
	...
	if not data.rank_list or next(data.rank_list) == nil then
        commonShowEmptyIcon(self.rank_panel, true, {text = TI18N("暂无排行")})
...
function LadderRankItem:configUI(  )
    ...
    self.role_head:setPosition(195, 56)
