...
function StoryTalk:openRootWnd(type, bid, actiontype, name, msg, res_type, res_rid, action_name ,left_right)
    print("---------------------type-----------------------",type)
...
--@return 
--==============================--
function StoryTalk:setData(type, bid, actiontype, name, msg, res_type, res_rid, action_name ,left_right)
...
function StoryTalk:showRoleSpine1( dirType, effid, actionName )
    local action = actionName --self:getSpineActionName(actionType)
    print("---------------dirType-------------------",actionName)
    if dirType == 0 then
        self.leftSpine = nil
        local pos_left = self.container:getChildByName("pos_left")
        local pos_right = self.container:getChildByName("pos_right")
...
-- 显示骨骼动画 dirType: 0为左边 1为右边
function StoryTalk:showRoleSprine( dirType, effid )
    local action = actionName --self:getSpineActionName(actionType)
