...
MainuiController.UPDATE_ICON_EVENT = "MainuiController.UPDATE_ICON_EVENT"
...
function MainuiController:registerEvents()
    ...
    EventManager:getInstance():Bind(Battle_dramaEvent.BattleDrama_Update_Max_Id,self, function(max_id)
            self:updateMainBtnStatus(max_id)
        end)


    -- 请求进入竞技场之前，需要告诉服务器，我要进去了，有没有真是战斗，快点告诉我
    EventManager:getInstance():Bind(BattleEvent.COMBAT_TYPE_BACK,self, function(combat_type, type)
    ...
    --代金券的检测(协议的推送)
    EventManager:getInstance():Bind(ActionEvent.ACTION_PERFER_ISOPEN,self, function(status)
            --IOS不展示
...
--@return 
--==============================--
function MainuiController:openMainUI(status,is_show_bottom)
    ...
    if status == true and in_fight == false then 
        ...
        if self.mainui then
            self.mainui:close()
            if is_show_bottom == true or is_show_bottom == false then
                self.mainui:setShowBottomUI(is_show_bottom)
            end
        end
    end
end
...
-- 设置主城顶部UI显示状态
function MainuiController:setIsShowMainUITop( status )
    if self.mainui then
        self.mainui:setShowTopUI(status)
    end
end
...
function MainuiController:setFunctionTipsStatus(id, data)
    ...
    if vo then
        ...
        EventManager:getInstance():Fire(MainuiController.UPDATE_ICON_EVENT,id) 
    end
    -- 检查红点状态
                    ...
                    if activate and activate[1] and activate[2] then
                        ...
                        if activate_condition == "lev" and lev >= activate_value then
                            ...
                            if function_vo ~= nil then
                                ...
                                EventManager:getInstance():Fire(MainuiEvent.UPDATE_FUNCTION_STATUS, config.id, true)
                            end
                        end
                    end
                end
            ...
            if activate[1] == nil or activate[2] == nil then
                ...
                if (activate_condition == "lev" and role_vo and role_vo.lev >= activate_value ) or (activate_condition == "dun" and drama_data and drama_data.max_dun_id >= activate_value ) then
                    ...
                    if function_vo ~= nil then
                        ...
                        -- 缓存图标创建成功之后
                        EventManager:getInstance():Fire(MainuiEvent.UPDATE_FUNCTION_STATUS, k, true)
                    end
                end
            end
        end
...
function MainuiController:createFunctionVo(config)
    ...
    if config.type == 0 then return end
...
function MainuiController:addFunctionIconById(id, ...)
    ...
    if (activate_condition == "lev" and role_vo and role_vo.lev >= activate_value ) or (activate_condition == "dun" and drama_data and drama_data.max_dun_id >= activate_value ) then
        ...
        if function_vo ~= nil then
            ...
            EventManager:getInstance():Fire(MainuiEvent.UPDATE_FUNCTION_STATUS, id, true)
        end
    else
...
function MainuiController:removeFunctionIconById(id)
    ...
    EventManager:getInstance():Fire(MainuiEvent.UPDATE_FUNCTION_STATUS, id, false)
end
...
function MainuiController:iconClickHandle(id,item,action_id)
    ...
    elseif id == MainuiConst.icon.backpack then

        if BackpackController:getInstance():getModel():checkEquipsIsFull() == true then
            BackpackController:getInstance():openMainView(true,BackPackConst.item_tab_type.EQUIPS)
        else
            BackpackController:getInstance():openMainView(true)
        end
    elseif id == MainuiConst.icon.community then
...
function MainuiController:setBtnRedPoint(id,data)
    ...
    if self.mainui then
        ...
        -- 设置背包是否已满
        if id == MainuiConst.icon.backpack then
...
function MainuiController:requestOpenBattleRelevanceWindow(battle_type, params)
    ...
    print("----------------------requestOpenBattleRelevanceWindow-------------------------------")
...
function MainuiController:openRelevanceWindowAtOnce(combat_type)
    ...
    elseif combat_type == BattleConst.Fight_Type.Spirit then 
    ...
    elseif combat_type == BattleConst.Fight_Type.LeagueExplore then -- 探索历练
        LeagueExploreController:getInstance():openMainView(true)
    end
...
function MainuiController:showTeacherPanel()
    if self.mainui then
        self.mainui:showTeacherPanel()
    end
end
