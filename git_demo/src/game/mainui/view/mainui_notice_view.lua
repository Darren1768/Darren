...
function MainUiNoticeView:registerEvent()
    EventManager:getInstance():Bind(TaskEvent.UpdateTaskList,self, function(is_new, task_list)
            self:fillFinishData(task_list, TaskConst.type.quest)
        end)
    

    EventManager:getInstance():Bind(TaskEvent.UpdateFeatList,self, function(feat_list)
            self:fillFinishData(feat_list, TaskConst.type.feat)
        end)
    

    EventManager:getInstance():Bind(TaskEvent.TASK_EXP_FINISH_TIPS_EVENT,self, function(feat_list)
...
function MainUiNoticeView:doMoveFinishItem()
    ...
    if self.cur_info and self.cur_info.config then
        ...
        self.load_resources = createResourcesLoad(PathTool.getSingleImgForDownLoad("bigbg/quest", res_name), ResourcesType.single, function() 
            loadSpriteTexture(self.task_img, PathTool.getSingleImgForDownLoad("bigbg/quest", res_name), LOADTEXT_TYPE)
