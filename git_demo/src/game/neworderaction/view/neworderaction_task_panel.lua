...
function NewOrderActionTaskPanel:layoutUI()
    self.root_wnd = createCSBNote(PathTool.getTargetCSB("neworderaction/task_panel"))
...
function NewOrderActionTaskPanel:registerEvents()
    EventManager:getInstance():Bind(NeworderactionEvent.OrderAction_TaskGet_Event,self,function()
        self:setTaskGetStatus()
    end)
end
...
function NewOrderActionTaskItem:configUI()
    self.root_wnd = createCSBNote(PathTool.getTargetCSB("neworderaction/task_item"))
