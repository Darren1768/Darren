...
function NewOrderactionWindow:__init()
    ...
    self.res_list = {
        ...
        {path = PathTool.getSingleImgForDownLoad("bigbg", "bigbg_99", true), type = ResourcesType.single}
    }
...
function NewOrderactionWindow:open_callback()
    ...
    if self.background ~= nil then
        self.background:loadTexture(PathTool.getSingleImgForDownLoad("bigbg", "bigbg_99", true), LOADTEXT_TYPE)
    end
...
function NewOrderActiodRewardItem:configUI()
    self.root_wnd = createCSBNote(PathTool.getTargetCSB("neworderaction/neworderaction_reward_item"))
