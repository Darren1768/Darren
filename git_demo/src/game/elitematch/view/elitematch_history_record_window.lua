...
function ElitematchHistoryRecordWindow:__init()
    ...
    self.res_list = {
        ...
        {path = PathTool.getSingleImgForDownLoad("bigbg/elitematch", "elitematch_history_bg", true), type = ResourcesType.single}
    }
    --站台数据
    self.station_list = {}
...
function ElitematchHistoryRecordWindow:open_callback()
    ...
    self.background:loadTexture(PathTool.getSingleImgForDownLoad("bigbg/elitematch", "elitematch_history_bg", true), LOADTEXT_TYPE)
    ...
    for i=1,6 do
        ...
        station_item.fight_label = CommonNum.new(220, power_click, 0, - 2, cc.p(0, 0.5))
...
function ElitematchHistoryRecordWindow:updateStationInfoByPos(pos_index, sever_data)
    ...
    if sever_data == nil then
        ...
        if honor_data and station_item.title_img and station_item.item_load == nil then
            local res = PathTool.getSingleImgForDownLoad("honor","txt_cn_honor_"..honor_data.res_id,false)
