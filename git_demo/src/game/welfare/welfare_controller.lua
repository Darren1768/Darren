...
function WelfareController:registerEvents()
    ...
    EventManager:getInstance():Bind(ActionEvent.UPDATA_WELFARE_FUND__STATUS_EVENT,self, function(data)
            local id_list = {}
...
function WelfareController:handle10989( data )
    if data then
        ...
        EventManager:getInstance():Fire(WelfareEvent.Update_Subscribe_data, data)
    end
end
...
function WelfareController:setWelfareStatus(bid, status)
    if not self:checkCanAdd(bid) then          -- 如果不可以开启,则这样处理吧
        ...
        -- 福利标签的面板
        EventManager:getInstance():Fire(WelfareEvent.UPDATE_WELFARE_TAB_STATUS, vo1)
    end
end
...
function WelfareController:handle24601(data)
    EventManager:getInstance():Fire(WelfareEvent.Get_SureveyQuest_Basic, data)
end
--获取答卷题目信息
...
function WelfareController:handle24602(data)
    EventManager:getInstance():Fire(WelfareEvent.Get_SureveyQuest_Topic_Content, data)
end
--答卷
...
function WelfareController:handle24604(data)
    message(data.msg)
    if data.code == 1 then
        EventManager:getInstance():Fire(WelfareEvent.Get_SureveyQuest_Get_Reward, data)
    end
end
--周、月礼包
...
function WelfareController:handle21007(data)
    EventManager:getInstance():Fire(WelfareEvent.Updata_Week_Month_Data, data)
end

function WelfareController:handle21008( data )
    self.model:setDailyAwardStatus(data.status)
    EventManager:getInstance():Fire(WelfareEvent.Update_Daily_Awawd_Data)
end
...
function WelfareController:handle16636(data)
    ...
    if data.code ~= 0 then
        ...
        EventManager:getInstance():Fire(WelfareEvent.UpdateBindPhoneStatus, self.bind_phone_data)
    end
end
