...
function ArenaManyPeopleMatchingWindow:__init( )
    ...
    self.res_list = {
        {path = PathTool.getSingleImgForDownLoad("arenampmatch","arenampmatch_bg"), type = ResourcesType.single },
    ...
    if self.data then
        ...
        for i,v in ipairs(atk_team_members) do
            if self.left_panel_list[i] then
                ...
                local res = PathTool.getSingleImgForDownLoad("arenampmatch/arenampmatch_icon", "amp_icon_"..v.score_lev)
