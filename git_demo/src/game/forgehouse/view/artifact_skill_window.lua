...
function ArtifactSkillWindow:open_callback(  )
    ...
    local title_list ={[1]="无暇灵魄技",[2]="完美灵魄技",[3]="绝品灵魄技"}
	...
	for i=1,3 do
        ...
        if tab_btn then
            local title = tab_btn:getChildByName("label")
...
function ArtifactSkillWindow:changeSelectedTab( index )
	...
	if self.tab_object then
        ...
        self.tab_object.normal_img:setVisible(true)
        self.tab_object.select_img:setVisible(false)
		self.tab_object.label:setTextColor(cc.c3b(168, 150, 121))
		self.tab_object = nil
	end
	self.tab_object = self.tab_list[index]
	if self.tab_object then

        self.tab_object.normal_img:setVisible(false)
        self.tab_object.select_img:setVisible(true)
		self.tab_object.label:setTextColor(cc.c4b(250, 232, 150))
	end
...
function ArtifactSkillItem:setData( skill_config )
	...
	if not self.skill_item then
		self.skill_item = SkillItem.new(true,true,true,nil,nil,false,true)
