...
function BackPackWindow:__init(sub_type)
    ...
    self.res_list = {
        ...
        { path = PathTool.getSingleImgForDownLoad("bigbg","hl_bigbg_26",false), type = ResourcesType.single },
...
function BackPackWindow:open_callback()
    ...
    -- if image_1 then
    --     local res = PathTool.getSingleImgForDownLoad("bigbg","pattern/pattern_3")
    --     if res ~= nil then
    ...
    tab_setting.tab_size = cc.size(128, 50)
    --tab_setting.select_color = cc.c4b(0xfa,0xe8,0x96,0xff)
    --tab_setting.select_outline = cc.c4b(0x6d,0x35,0x07,0xff)
    --tab_setting.normal_color = cc.c4b(0xa8,0x96,0x79,0xff)
    ...
    tab_setting.img_rect = cc.rect(50, 20, 60, 2)
    ...
    self.item_scrollview = CommonScrollViewLayout.new(self.item_container, cc.p(0, 0), ScrollViewDir.vertical, ScrollViewStartPos.top, scroll_view_size, setting)
...
function BackPackWindow:setPanelData()
    ...
    elseif self.cur_index == BackPackConst.item_tab_type.HERO then --英雄
        ...
        show_quick_sell = true
        --英雄碎片需要取消红点
        --MainuiController:getInstance():setBtnRedPoint(MainuiConst.icon.backpack, false)
        MainuiController:getInstance():setFunctionTipsStatus(MainuiConst.icon.backpack, false)
