...
function BackpackController:handle10515(data)
    ...
    if data.flag == TRUE then
        ...
        EventManager:getInstance():Fire(BackpackEvent.BACKPACK_USE_ITEM_EVENT, data)
    end
end
...
function BackpackController:handle10521(data)
    if data.items then
        -- 请求出售获得成功之后打开出售确认面板
        EventManager:getInstance():Fire(BackpackEvent.BACKPACK_SELL_CONFIRM, data)
    end
end 
...
function BackpackController:handle10522(data)
	...
	if data.flag == TRUE then
        ...
        self:closeGiftSelectPanel()
        EventManager:getInstance():Fire(BackpackEvent.Sell_Goods_Success)
	end
end 
...
function BackpackController:handle10523(data)
    message(data.msg)
    if data.flag == 1 then
        EventManager:getInstance():Fire(BackpackEvent.Compose_Goods_Success)
...
function BackpackController:handle10530(data)
    ...
    local function fun()
        MainuiController:getInstance():changeMainUIStatus(MainuiConst.icon.backpack,BackPackConst.item_tab_type.EQUIPS)
    end
...
function BackpackController:handle11008(data)
    EventManager:getInstance():Fire(BackpackEvent.Compose_BackPack_Success)
    message(data.msg)
    TipsManager:getInstance():showBackPackCompTips(false)
