...
function BackpackModel:updateExpansionInfo(data)
    ...
    EventManager:getInstance():Fire(BackpackEvent.UpdateEquipSize, data.type)
end
...
function BackpackModel:initItemList(data)
    ...
    EventManager:getInstance():Fire(BackpackEvent.GET_ALL_DATA, bag_code)
end
--计算碎片红点
function BackpackModel:getHeroChipRedPoint()
    ...
    --MainuiController:getInstance():setBtnRedPoint(MainuiConst.icon.backpack, status)
    MainuiController:getInstance():setFunctionTipsStatus(MainuiConst.icon.backpack, status)
end
...
function BackpackModel:addItemInBagCode(data)
        ...
        if config ~= nil then
            ...
            if config.sub_type == BackPackConst.item_tab_type.HERO then
                if self:checkHeroChipRedPoint(temp_item) then
                    --MainuiController:getInstance():setBtnRedPoint(MainuiConst.icon.backpack, true)
                    MainuiController:getInstance():setFunctionTipsStatus(MainuiConst.icon.backpack, true)
                end
            end
        end
    end
    ...
    EventManager:getInstance():Fire(BackpackEvent.ADD_GOODS, bag_code, temp_add)
end
...
function BackpackModel:deleteBagItems(data)
    ...
    EventManager:getInstance():Fire(BackpackEvent.DELETE_GOODS, bag_code, temp_tab)
end
...
function BackpackModel:updateBagItemsNum(data)
    ...
    bag_code = bag_code or BackPackConst.Bag_Code.BACKPACK
    EventManager:getInstance():Fire(BackpackEvent.MODIFY_GOODS_NUM, bag_code, change_list)
end
