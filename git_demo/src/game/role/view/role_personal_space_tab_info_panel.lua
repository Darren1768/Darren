...
function RolePersonalSpaceTabInfoPanel:registerEvents()
    ...
    for i,v in ipairs(self.set_btn_list) do
     ...
     EventManager:getInstance():Bind(NoticeEvent.All_Feedback_Event_Data,self, function()  --
    ...
    if self.parent.role_type == RoleConst.role_type.eMySelf then
        if self.role_vo then
            ...
            --获取称号
     EventManager:getInstance():Bind(RoleEvent.GetTitleList,self,function ( data )
                if not data then return end
                self:updateHonorInfo(data.base_id)
            end)
        --使用称号事件
     EventManager:getInstance():Bind(RoleEvent.UseTitle,self,function ( base_id )
        ...
        if not self.update_fun_form then 
            self.update_fun_form =EventManager:getInstance():Bind(HeroEvent.Update_Fun_Form,self,function(data)
        ...
        if not self.role_fans_rank_event then 
            self.role_fans_rank_event =EventManager:getInstance():Bind(RoleEvent.ROLE_FANS_RANK_EVENT,self,function(data)
                if not data then return end
        ...
        if not self.role_city_event then 
            self.role_city_event =EventManager:getInstance():Bind(RoleEvent.ROLE_CITY_EVENT,self,function(data)
                if not data then return end
        ...
        if not self.role_follow_event then 
            self.role_follow_event =EventManager:getInstance():Bind(RoleEvent.ROLE_FOLLOW_EVENT,self,function(data)
                if not data then return end
