...
function RoleTitlePanel:layoutUI()
    ...
    self.use_btn = createTypeButton(self.main_panel, TI18N("更 换"), self.size.width/2, -36, ButtonType.yellow)
...
function RoleTitlePanel:registerEvents()
    ...
    --拥有列表
     ...
     EventManager:getInstance():Bind(RoleEvent.UpdataTitleList,self,function ( data )
            self:updateHaveList(data,true)
            self:createItemList()
        end)

    --使用称号
     EventManager:getInstance():Bind(RoleEvent.UseTitle,self,function ( id )
            if id then
                self.use_id = id
                self:createItemList()
            end
...
function RoleTitlePanel:titleItemClick(face_item,vo)
    ...
    if is_has == 3 then
        self.use_btn:setBtnLabel(TI18N("前往获取1")) 
