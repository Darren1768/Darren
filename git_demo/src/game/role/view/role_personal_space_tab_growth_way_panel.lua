...
function RolePersonalSpaceTabGrowthWayPanel:registerEvents()
    ...
    if self.parent.role_type == RoleConst.role_type.eMySelf then
     EventManager:getInstance():Bind(RoleEvent.ROLE_MYSELF_GROWTH_WAY_EVENT,self,function (data)
     ...
     EventManager:getInstance():Bind(RoleEvent.ROLE_OHTER_GROWTH_WAY_EVENT,self,function (data)
                if not data then return end
                self.is_init = true
                if data.start ~= 0 then
                    self:addGrowthWayData()
