...
OrganizeVo = OrganizeVo or BaseClass(EventDispatcher)

OrganizeVo.UPDATE_FRIEND_ATTR_LOGIN_OUT_TIME = "UPDATE_FRIEND_ATTR_LOGIN_OUT_TIME"

function OrganizeVo:__init( ... )
	-- body
	self.srv_id = ""
	self.rid = 0
	self.name = ""
...
function OrganizeVo:setData( data )
	-- body
...
function OrganizeVo:update( key,value )
	-- body
	if self[key] then
		self[key] = value

		if key == "login_out_time" then
			self:Fire(OrganizeVo.UPDATE_FRIEND_ATTR_LOGIN_OUT_TIME,self)
		end
	end

end
function OrganizeVo:setKey(key,value)
   self[key] = value
end
function OrganizeVo:__delete( ... )
	-- body
end