...
function ElfinRousePanel:registerEvent(  )
    ...
    -- 古树信息
    EventManager:getInstance():Bind(ElfinEvent.Get_Elfin_Tree_Data_Event,self, function ( )
            self:updateRouseInfo()
        end)
    

    -- 古树升级成功
    EventManager:getInstance():Bind(ElfinEvent.Elfin_Tree_Lv_Up_Event,self, function ( )
...
function ElfinRousePanel:updateRouseInfo(  )
    ...
    if self.elfin_tree_data.lev >= Config.SpriteData.data_tree_up_lv_table_length then
