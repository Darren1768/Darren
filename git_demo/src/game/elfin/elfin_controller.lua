...
function ElfinController:handle26500( data )
	if data.sprite_hatchs then
		...
		EventManager:getInstance():Fire(ElfinEvent.Get_Elfin_Hatch_Data_Event)
	end
	if data.info then
...
function ElfinController:handle26504( data )
	...
	-- 使用道具加速成功
	if data.result == 1 then
		EventManager:getInstance():Fire(ElfinEvent.Use_Item_Hatch_Event, data.id)
	end
end
...
function ElfinController:handle26507( data )
	...
	if data.result == 1 and data.count and data.item_id then
		...
		EventManager:getInstance():Fire(ElfinEvent.Buy_Elfin_Item_Success_Event, data.item_id)
...
function ElfinController:handle26509( data )
	if data.awards then
		...
		EventManager:getInstance():Fire(ElfinEvent.Get_Activated_Elfin_Event)
	end
end
...
function ElfinController:handle26510( data )
	...
	EventManager:getInstance():Fire(ElfinEvent.Get_Elfin_Tree_Data_Event)
end
...
function ElfinController:handle26511( data )
	...
	if data.result == 1 then
		EventManager:getInstance():Fire(ElfinEvent.Elfin_Tree_Lv_Up_Event)
	end
end
...
function ElfinController:handle26550( data )
	if data then
		...
		EventManager:getInstance():Fire(ElfinEvent.Update_Elfin_Summon_Data_Event, data)
	end
end
...
function ElfinController:handle26553( data )
	...
	-- 	TimesummonController:getInstance():openActionTimeElfinSummonGainWindow(false)
	-- 	TimesummonController:getInstance():openActionTimeElfinSummonGainWindow(true,data,TRUE,2)
	-- 	EventManager:getInstance():Fire(ElfinEvent.Update_Elfin_Item_Event, data)
	-- else
	-- 	EventManager:getInstance():Fire(ElfinEvent.Update_Elfin_Summon_Rewards_Data_Event, data)
	-- end
...
function ElfinController:handle26555( data )
	EventManager:getInstance():Fire(ElfinEvent.Elfin_Plan_From_Info_Event, data)
end
...
function ElfinController:handle26556( data )
	...
	EventManager:getInstance():Fire(ElfinEvent.Elfin_Plan_Info_Event, data)
end
...
function ElfinController:handle26559( data )
	...
	EventManager:getInstance():Fire(ElfinEvent.Elfin_Plan_Update_Event, data)
end
...
function ElfinController:handle26560( data )
    ...
    EventManager:getInstance():Fire(ElfinEvent.Elfin_Plan_Save_From_Event, data)
end
...
function ElfinController:handle26562( data )
	...
	if data.flag == TRUE then
		EventManager:getInstance():Fire(ElfinEvent.Elfin_Plan_Buy_Event, data)
	end
end
...
function ElfinController:handle26563( data )
    EventManager:getInstance():Fire(ElfinEvent.Elfin_Plan_Must_Tips_Event, data)
end
...
function ElfinController:openElfinInfoWindow( status, data )
	if status == true then
		...
		EventManager:getInstance():Fire(ElfinEvent.Elfin_Check_Show_Tips_Event)--检测是否继续弹tips
	end
end
