...
function ActionTimeSummonPanel:updateImageBg(  )
	if self.config and self.config.res_id and (not self.cur_res_id or self.cur_res_id ~= self.config.res_id) then
		local bg_res = PathTool.getSingleImgForDownLoad("bigbg/timesummon", string.format("txt_cn_timesummon_bigbg_%d", self.config.res_id), true)
...
function ActionTimeSummonPanel:register_event(  )
	...
	-- 召唤数据更新
     EventManager:getInstance():Bind(TimesummonEvent.Update_Summon_Data_Event,self,function (data)
            self:setData(data)
        end)
