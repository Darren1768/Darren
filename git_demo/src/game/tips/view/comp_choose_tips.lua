...
function CompChooseTips:open_callback()
    ...
    self.text_name = main_panel:getChildByName("name")
    self.text_name:setString("")

    self.title_name = main_panel:getChildByName("title_container"):getChildByName("text_name")
    self.title_name:setString("碎片合成")
    
    self.closebtn = main_panel:getChildByName("closebtn")
    ...
    self.comp_number_edit = createEditBox(comp_node, PathTool.getResFrame("common2", "hl_common_0157"), cc.size(210, 40), cc.c4b(0x00,0x00,0x00,0xff), 26, cc.c4b(0x00,0x00,0x00,0xff), 26, "", nil, 6, LOADTEXT_TYPE_PLIST)
    ...
    self.goods_item:setPosition(cc.p(main_panel:getContentSize().width/2,300))
...
function CompChooseTips:register_event()
    ...
    registerButtonEventListener(self.closebtn, function()
        self:close()
        TipsManager:getInstance():showCompChooseTips(false)
    end,false, 2)
