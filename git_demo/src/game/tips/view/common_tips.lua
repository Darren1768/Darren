...
function CommonTips:LoadLayoutFinish()
    ...
    self.background = ccui.ImageView:create(PathTool.getResFrame("common2","hl_common_3033"), LOADTEXT_TYPE_PLIST)
    self.background:setScale9Enabled(true)
    self.background:setCapInsets(cc.rect(18,18,1,1))
...
function CommonTips:showTips(str, width, font_size)
    ...
    local size_width = math.max(40 + size.width, self.background:getContentSize().width)
    local size_height = math.max(40 + size.height, self.background:getContentSize().height)
    self.root_wnd:setContentSize(cc.size(size_width, size_height))
    self.background:setContentSize(cc.size(size_width, size_height))

    self.tips_label:setPosition(cc.p(20, size_height - 20))
    self.root_wnd:addChild(self.tips_label)
end

-- 从新计算文本的大小
function CommonTips:recoutTextFieldSize(str_label, width, font_size)
    local label = createRichLabel(font_size, cc.c4b(239,212,174,0xff), cc.p(0, 1), nil, 6, 0, width)
