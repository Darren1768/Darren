...
function BackpackCompTips:open_callback()
    ...
    self.goods_item:setPosition(cc.p(80,280))
...
function BackpackCompTips:showBtn(num)
    ...
    local btn = createTypeButton(self.main_panel,TI18N("合成"),pos,52, ButtonType.yellow)
    btn:setName("com_btn")
    btn:setBtnLabel('合成')
    ...
    if num == 2 then
        local btn1 = createTypeButton(self.main_panel,TI18N("详情"),self.main_panel:getContentSize().width/2 - 120,52, ButtonType.yellow)
        btn1:setBtnLabel(TI18N('详情'))
