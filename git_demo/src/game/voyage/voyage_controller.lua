...
function VoyageController:handle23800( data )
	...
	if data.order_times then
		self.model:setOrderTimes(data.order_times)
	end
	if data.max_order_times then
		self.model:setMaxOrderTimes(data.max_order_times)
	end
	EventManager:getInstance():Fire(VoyageEvent.UpdateVoyageDataEvent)
	EventManager:getInstance():Fire(VoyageEvent.UpdateVoyageRedEvent)
    
    self:checkVoyageRedStatus()
end

function VoyageController:checkVoyageRedStatus(  )
    local red_status = self:getModel():checkVoyageRedStatus()
    GhostdomController:getInstance():setBuildRedStatus(GhostdomConst.Build_Type.reward, {bid = 1, status = red_status})
end
...
function VoyageController:handle23802( data )
	...
	if data.flag == TRUE then
		local orderTime = self.model:getOrderTimes()
		orderTime = math.min(orderTime+1,3)
		self.model:setOrderTimes(orderTime)
		self:openVoyageDispatchWindow(false)
		EventManager:getInstance():Fire(VoyageEvent.UpdateOrderTimes)
	end
end
...
function VoyageController:handle23803( data )
	...
	if data.flag == TRUE and data.order_id then
		...
		EventManager:getInstance():Fire(VoyageEvent.DeleteOrderDataEvent)
		EventManager:getInstance():Fire(VoyageEvent.UpdateVoyageRedEvent)
        self:checkVoyageRedStatus()
	end
end
...
function VoyageController:handle23805( data )
	if data.flag then
		...
		EventManager:getInstance():Fire(VoyageEvent.UpdateActivityStatusEvent)
	end
end
