...
function CrossareanHonourItem:createRoorWnd(  )
    ...
    self.box = self.container:getChildByName("box")

    self.image_top = self.box:getChildByName("image_top")
    self.image_title = self.box:getChildByName("image_title")
    self.image_stage = self.box:getChildByName("image_stage")
    self.like_btn = self.container:getChildByName("like_btn")
    self.like_btn_label = self.like_btn:getChildByName("label")
    self.txt_role_name = self.box:getChildByName("txt_role_name")
    self.txt_no_role = self.box:getChildByName("txt_no_role")
...
function CrossareanHonourItem:registerEvent(  )
    registerButtonEventListener(self.box, handler(self, self.onClickRoleItem), false)
...
function CrossareanHonourItem:setData( data, index )
	...
	if data then
        ...
        if data.lookid then
            ...
            self.box:addChild(self.role_spine, 1)
        end
    ...
    -- 位置
    if index then
        local posX = {78,265,485}
        
        local star_x = 15
        local distance_x = 15
        local pos_x = posX[index]
        self.root_wnd:setPosition(cc.p(pos_x, 475))
        if self.role_spine then
            self.role_spine:setPosition(cc.p(95, 212))
        end
        self.root_wnd:setVisible(true)
        
        if index == 1 or index == 3 then
            self.root_wnd:setScaleX(157/188)
            self.root_wnd:setScaleY(438/442)
        end
