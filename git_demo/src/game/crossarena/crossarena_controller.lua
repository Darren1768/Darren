...
function CrossarenaController:sender25604( type, formations )
	...
	OrganizeController:getInstance():sender29704(type,pos_pid_info)
...
function CrossarenaController:handle25600( data )
	if data then
		...
		EventManager:getInstance():Fire(CrossarenaEvent.Update_MyBaseInfo_Event)
	end
end
...
function CrossarenaController:handle25601( data )
	if data then
		if data.type == 0 then -- 全部更新
			...
			EventManager:getInstance():Fire(CrossarenaEvent.Update_Challenge_Role_Event)
...
function CrossarenaController:handle25604( data )
	if data then
		...
		if data.code == 1 then -- 设置阵法成功，则通知布阵界面进入战斗
			EventManager:getInstance():Fire(CrossarenaEvent.Save_Crossarena_Form_Event, data)
...
function CrossarenaController:handle25605( data )
	if data.type == 2 then
        ...
        model:updateFormationRedPoint()
    end 
    -- for i,v in ipairs(data.formations) do
    --     v.old_order = v.order --记录旧的位置
    -- end
    
	EventManager:getInstance():Fire(CrossarenaEvent.Update_Form_Data_Event, data) 
end
...
function CrossarenaController:handle25607( data )
	if data then
		...
		EventManager:getInstance():Fire(CrossarenaEvent.Update_Challenge_Award_Event)
	end
end
...
function CrossarenaController:handle25609( data )
	if data then
		...
		if data.ref_time then
			...
			EventManager:getInstance():Fire(CrossarenaEvent.Update_Refresh_Time_Event)
		end
	end
end
...
function CrossarenaController:handle25610( data )
	if data then
		...
		EventManager:getInstance():Fire(CrossarenaEvent.Update_Open_Status_Event)
	end
end
...
function CrossarenaController:handle25612( data )
	if data and data.card_info and next(data.card_info) ~= nil then
		EventManager:getInstance():Fire(CrossarenaEvent.Update_Card_Info_Event, data.card_info)
	else
...
function CrossarenaController:handle25614( data )
	if data then
		...
		EventManager:getInstance():Fire(CrossarenaEvent.Update_Honour_Data_Event)
	end
end
...
function CrossarenaController:handle25615( data )
	if data then
		EventManager:getInstance():Fire(CrossarenaEvent.Update_Rank_Data_Event, data)
	end
end
...
function CrossarenaController:handle25616( data )
	if data then
		EventManager:getInstance():Fire(CrossarenaEvent.Update_Video_Data_Event, data)
	end
end
