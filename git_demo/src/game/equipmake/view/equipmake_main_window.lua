...
function EquipmakeMainWindow:__init()
	...
	self.cur_type = 0
	self.res_list = {
        {path = PathTool.getSingleImgForDownLoad("bigbg","bigbg_8"), type = ResourcesType.single},
...
function EquipmakeMainWindow:register_event()
    ...
    self.equipmake_btn:addTouchEventListener(function(sender, event_type) 
     ...
     EventManager:getInstance():Bind(EquipmakeEvent.UpdateEquipmakeEvent,self, function(id, type)
    ...
    if role_vo then
     ...
     EventManager:getInstance():Bind(BackpackEvent.Sell_Goods_Success,self, function() 
            -- 橙装碎片分解来源红点
