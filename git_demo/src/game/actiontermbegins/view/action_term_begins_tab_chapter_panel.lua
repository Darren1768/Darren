...
function ActiontermbeginsTabChapterPanel:registerEvents()
    ...
    for i,v in ipairs(self.chapter_item_list) do
     ...
     EventManager:getInstance():Bind(ActiontermbeginsEvent.TERM_BEGINS_BUY_COUNT_EVENT,self, function(data)
    ...
    for i,v in ipairs(self.chapter_item_list) do
        ...
        if config then
            ...
            if v.item_load == nil then
                local res = PathTool.getSingleImgForDownLoad("bigbg/termbegins", "term_begins_chapter_"..config.order_res)
