...
function ActiontermbeginsPanel:configUI()
    ...
    if not self.item_load then
        local res = PathTool.getSingleImgForDownLoad("bigbg/termbegins", "action_term_begins_bg", true)
...
function ActiontermbeginsPanel:register_event()
     EventManager:getInstance():Bind(ActiontermbeginsEvent.ACTION_TERM_BEGINS_EVENT,self,function (data)
            if not data then return end
            self:setData(data)
        end)

     EventManager:getInstance():Bind(ActionEvent.UPDATE_HOLIDAY_TAB_STATUS,self,function (function_id, vo)
            self:updateComfirmRedPoint(vo)
        end)
