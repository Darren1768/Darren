...
function ActiontermbeginsController:handle26700(data)
    EventManager:getInstance():Fire(ActiontermbeginsEvent.TERM_BEGINS_MAIN_EVENT, data)
end
--关卡购买挑战次数
...
function ActiontermbeginsController:handle26701(data)
    ...
    if data.code == TRUE then
        EventManager:getInstance():Fire(ActiontermbeginsEvent.TERM_BEGINS_BUY_COUNT_EVENT, data)
    end
end
...
function ActiontermbeginsController:handle26704(data)
    if data.code == TRUE then
        message(TI18N("提交成功"))
        EventManager:getInstance():Fire(ActiontermbeginsEvent.TERM_BEGINS_SUBIT_PAPER_EVENT, data)
    else
        message(data.msg)
    end
end
...
function ActiontermbeginsController:handle26705(data)
    EventManager:getInstance():Fire(ActiontermbeginsEvent.TERM_BEGINS_PAPER_REWARD_LIST_EVENT, data)
end
...
function ActiontermbeginsController:handle26706(data)
    message(data.msg)
    if data.code == TRUE then
        EventManager:getInstance():Fire(ActiontermbeginsEvent.TERM_BEGINS_RECEIVE_PAPER_EVENT, data)
    end
end
...
function ActiontermbeginsController:handle26710(data)
    EventManager:getInstance():Fire(ActiontermbeginsEvent.TERM_BEGINS_RANK_EVENT, data)
end
...
function ActiontermbeginsController:handle26711(data)
    EventManager:getInstance():Fire(ActiontermbeginsEvent.ACTION_TERM_BEGINS_EVENT, data)
end
...
function ActiontermbeginsController:handle26712(data)
    if data.code == TRUE then
        if data.flag == 0 then
            --布阵
            EventManager:getInstance():Fire(ActiontermbeginsEvent.TERM_BEGINS_BOSS_FORM_EVENT)
        else
            --发协议 不是0就是boss_id
