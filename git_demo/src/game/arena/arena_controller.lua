...
function ArenaController:openArenaLoopMathWindow(status, index)
    if status == false then
        ...
        if self.loop_match_window == nil then
            self.loop_match_window = ArenaLoopMatchWindow1.New()--ArenaLoopMatchWindow.New()
        end
...
function ArenaController:openArenaLoopMyLogWindow(status)
    if not status then
        ...
        if self.loop_log_window == nil then
            self.loop_log_window = ArenaLoopMyLogWindow1.New()
        end
        self.loop_log_window:open()
    end
end

function ArenaController:openArenaLoopActivityWindow(status)
    if not status then
        if self.loop_activity_window ~= nil then
            self.loop_activity_window:close()
            self.loop_activity_window = nil
            EventManager:getInstance():Fire(ArenaEvent.UpdateLoopTabStatus)
        end
    else
        if self.loop_activity_window == nil then
            self.loop_activity_window = ArenaLoopActivityPanel.New()
        end
        self.loop_activity_window:open()
    end
end

function ArenaController:openArenaLoopAwardWindow(status)
    if not status then
        if self.loop_award_window ~= nil then
            self.loop_award_window:close()
            self.loop_award_window = nil
            EventManager:getInstance():Fire(ArenaEvent.UpdateLoopTabStatus)
        end
    else
        if self.loop_award_window == nil then
            self.loop_award_window = ArenaLoopAwardsPanel.New()
        end
        self.loop_award_window:open()
    end
end 

function ArenaController:openArenaLoopRankWindow(status)
    if not status then
        if self.loop_rank_window ~= nil then
            self.loop_rank_window:close()
            self.loop_rank_window = nil
            EventManager:getInstance():Fire(ArenaEvent.UpdateLoopTabStatus)
        end
    else
        if self.loop_rank_window == nil then
            self.loop_rank_window = ArenaLoopRankPanel.New()
        end
        self.loop_rank_window:open()
    end
end
...
function ArenaController:handle20222(data)
	EventManager:getInstance():Fire(ArenaEvent.UpdateMylogListEvent, data.log_list)
end 
...
function ArenaController:handle20207(data)
    ...
    if data.code == 1 then
        ...
        EventManager:getInstance():Fire(ArenaEvent.UpdateArena_Number)
    end
end
...
function ArenaController:handle20210(data)
    ...
    -- end 
    EventManager:getInstance():Fire(ArenaEvent.ArenaFightResultEvent)
...
function ArenaController:handle20220(data)
    EventManager:getInstance():Fire(ArenaEvent.UpdateLoopChallengeStatueList, data.rank_list)
end
...
function ArenaController:handle20221(data)
    EventManager:getInstance():Fire(ArenaEvent.UpdateLoopChallengeRank, data)
end
...
function ArenaController:handle20280(data)
    EventManager:getInstance():Fire(ArenaEvent.UpdateChampionTop3Event, data.rank_list) 
end
...
function ArenaController:handle20252(data)
    EventManager:getInstance():Fire(ArenaEvent.UpdateMyMatchInfoEvent, data)
end
...
function ArenaController:handle20253(data)
    EventManager:getInstance():Fire(ArenaEvent.UpdateGuessMatchInfoEvent, data)
end
...
function ArenaController:handle20254(data)
    ...
    if data.code == TRUE then
        ...
        EventManager:getInstance():Fire(ArenaEvent.UpdateRoleInfoBetEvent, data.can_bet, data.bet_type)
...
function ArenaController:handle20257(data)
    EventManager:getInstance():Fire(ArenaEvent.UpdateBetMatchValueEvent, data)
end
...
function ArenaController:handle20255(data)
    EventManager:getInstance():Fire(ArenaEvent.UpdateMyGuessListEvent, data.list)
end
...
function ArenaController:handle20281(data)
    EventManager:getInstance():Fire(ArenaEvent.UpdateChampionRankEvent, data)
end
...
function ArenaController:handle20260(data)
    EventManager:getInstance():Fire(ArenaEvent.UpdateTop32InfoEvent, data.list)
end
...
function ArenaController:handle20261(data)
	EventManager:getInstance():Fire(ArenaEvent.UpdateTop4InfoEvent, data.pos_list)
end 
...
function ArenaController:handle20262(data)
	EventManager:getInstance():Fire(ArenaEvent.UpdateTop324GuessGroupEvent, data.group, data.pos)
end 
...
function ArenaController:handle20263(data) 
	EventManager:getInstance():Fire(ArenaEvent.UpdateTop324GroupPosEvent, data)
end
...
function ArenaController:handle20258(data)
	EventManager:getInstance():Fire(ArenaEvent.UpdateMylogListEvent, data.list)
end 
