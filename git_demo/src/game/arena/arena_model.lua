...
function ArenaModel:updateMyLoopData(data)
    ...
    EventManager:getInstance():Fire(ArenaEvent.UpdateMyLoopData)
...
function ArenaModel:updateChallengeTimesAwards(data)
    ...
    self:updateArenaRedStatus(ArenaConst.red_type.loop_challenge_box, need_red)

    EventManager:getInstance():Fire(ArenaEvent.UpdateLoopChallengeTimesList,data)
end
...
function ArenaModel:updateArenaRedStatus(type, status)
    ...
    GhostdomController:getInstance():setBuildRedStatus(GhostdomConst.Build_Type.battle, {bid = type, status = status})
    -- 事件用于同步更新红点
    EventManager:getInstance():Fire(ArenaEvent.UpdateArenaRedStatus, type, status)
end
...
function ArenaModel:updateLoopChallengeList(data)
    ...
    -- 只有是客户端请求的时候才做刷新
    if data.type == 0 then
        EventManager:getInstance():Fire(ArenaEvent.UpdateLoopChallengeList)
    end
end
