    ...
    self:registerScriptHandler(function(event)
            if "enter" == event then
                ...
                if USE_RES_LUA == true then
                    ui_action = require("luascript.arena.arena_champion_my_match_panel")['animation']
                else
...
function ArenaChampionMyMatchPanel:handleEvent(status)
    if status == true then
        EventManager:getInstance():Bind(ArenaEvent.UpdateMyMatchInfoEvent,self, function(data) 
                self:updateFightInfo(data)
            end)
        EventManager:getInstance():Bind(HeroEvent.Update_Save_Form,self, function(data)
        ...
        EventManager:getInstance():Bind(ElfinEvent.Elfin_Plan_Save_From_Event,self, function(data)
                -- if not data then return end
                -- if data.type and data.type == PartnerConst.Fun_Form.ArenaChampion then
