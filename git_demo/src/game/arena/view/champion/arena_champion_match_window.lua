...
function ArenaChampionMatchWindow:initConfig()
    ...
    self.background_path = string.format("resource/bigbg/hl_bigbg_76.jpg")
    self.background_up_path = string.format("resource/bigbg/hl_bigbg_77.png")

    table.insert(self.res_list, {path = self.background_path, type = ResourcesType.single} )
    table.insert(self.res_list, {path = self.background_up_path, type = ResourcesType.single} )
end 
...
function ArenaChampionMatchWindow:open_callback()
    ...
    self.my_guess_btn:setVisible(false)
...
function ArenaChampionMatchWindow:register_event()
    ...
    registerButtonEventListener(self.close_btn, function()
            ...
            if self.cur_selected and self.cur_selected.index == ArenaConst.champion_index.match then
                if self.cur_panel and self.cur_panel.is_in_check_info == true then
                    EventManager:getInstance():Fire(ArenaEvent.CheckFightInfoEvent, false) 
                else
    ...
    -- 更新个人信息
    EventManager:getInstance():Bind(ArenaEvent.UpdateChampionRoleInfoEvent,self, function(data)
            self:updateCurPanelInfo()
        end)

    EventManager:getInstance():Bind(ArenaEvent.ChangeTanFromTop324,self, function() 
...
function ArenaChampionMatchWindow:changeTabView( index )
    ...
    -- 排行榜的时候不需要显示弹幕
    EventManager:getInstance():Fire(BarrageEvent.SetVisibleStatus, (index ~= ArenaConst.champion_index.rank ))
end
...
function ArenaChampionMatchWindow:changeMyMatchStatus()
    local index = ArenaConst.champion_index.guess
    if self.model:getMyMatchStatus() == ArenaConst.champion_my_status.in_match then
        index = ArenaConst.champion_index.guess   
    end
    ...
    self.cur_panel = self:getPanel(ArenaConst.champion_index.guess)
    if self.cur_panel then
        self.cur_panel:addToParent(true)
    end
    return index
...
function ArenaChampionMatchWindow:openRootWnd(index)
    index = index or ArenaConst.champion_index.guess
    ...
    -- 打开弹幕
    if self.view_type == ArenaConst.champion_type.normal then
        EventManager:getInstance():Fire(BarrageEvent.HandleBarrageType, true, BarrageConst.type.champion) 
    else
        self.ctrl:sender26216()
        EventManager:getInstance():Fire(BarrageEvent.HandleBarrageType, true, BarrageConst.type.crosschampion)
    end
...
function ArenaChampionMatchWindow:close_callback()
    EventManager:getInstance():Fire(BarrageEvent.HandleBarrageType, false)
    self:clearTimeTicket()
