...
function MallController:registerEvents()
    EventManager:getInstance():Bind(Battle_dramaEvent.BattleDrama_Top_Update_Data,self, function()
...
function MallController:handle13401( data )
    ...
    print("------------handle13401----------------")
    printLuaTable(data)
    EventManager:getInstance():Fire(MallEvent.Open_View_Event,data)
end
...
function MallController:handle13402( data )
    ...
    if data.code == 1 then
        EventManager:getInstance():Fire(MallEvent.Buy_Success_Event,data)
    end
end
...
function MallController:handle13403( data )
    message(data.msg)
    EventManager:getInstance():Fire(MallEvent.Get_Buy_list,data)   
end

--服务端推送神秘商店可以刷新
function MallController:handle13404( data )
    -- Debug.info(data)
    -- EventManager:getInstance():Fire(MallEvent.Frash_tips_event)
end
--刷新列表
...
function MallController:handle13407( data )
    message(data.msg)
    if data.code == 1 then
        EventManager:getInstance():Fire(MallEvent.Buy_One_Success,data)
    end
end
...
function MallController:handle16660(data)
    message(data.msg)
    EventManager:getInstance():Fire(MallEvent.Update_Action_event,data)
end
--{uint32, bid, "子活动编号;"},
--{uint32, aim, "商品id"},
--{uint32, num, "购买数量"}
...
function MallController:handle16661(data)
    message(data.msg)
    if data.code == 1 then
        EventManager:getInstance():Fire(MallEvent.Buy_Action_Shop_Success_event,data)
    end
end
----------------------------------活动商城协议结束------------------------------------------
...
function MallController:handle27800( data )
    EventManager:getInstance():Fire(MallEvent.Get_Chose_Shop_Data_Event, data)
end
...
function MallController:handle21022( data )
    EventManager:getInstance():Fire(MallEvent.Get_Open_Charge_Shop_Event, data)
end
...
function MallController:handle13420( data )
    EventManager:getInstance():Fire(MallEvent.Free_Refresh_Data,data)
end
