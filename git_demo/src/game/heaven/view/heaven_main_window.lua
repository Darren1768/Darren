...
function HeavenMainWindow:__init(index)
	...
	self.res_list = {
		{path = PathTool.getSingleImgForDownLoad("bigbg","bigbg_85",true), type = ResourcesType.single },
		{path = PathTool.getSingleImgForDownLoad("bigbg","bigbg_86",true), type = ResourcesType.single},
	}
...
function HeavenMainWindow:register_event(  )
	...
	--红点
     EventManager:getInstance():Bind(HeavenEvent.Update_Heaven_Red_Status,self,function (bid)
...
function HeavenMainWindow:changeSelectedTab( index )
	...
	if index == HeavenConst.Tab_Index.Dungeon then
		self.background:loadTexture(PathTool.getSingleImgForDownLoad("bigbg","bigbg_85",true), LOADTEXT_TYPE)
	elseif index == HeavenConst.Tab_Index.DialRecord then
		self.background:loadTexture(PathTool.getSingleImgForDownLoad("bigbg","bigbg_86",true), LOADTEXT_TYPE)
	end
