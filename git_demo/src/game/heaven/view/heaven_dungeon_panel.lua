...
function HeavenDungeonPanel:createCellMapBg(  )
	local res = PathTool.getSingleImgForDownLoad("bigbg","bigbg_85", true)
...
function HeavenDungeonPanel:register_event(  )
	...
	--章节数据返回
     EventManager:getInstance():Bind(HeavenEvent.Get_Chapter_Data_Event,self,function ()
	...
	--挑战次数更新
     EventManager:getInstance():Bind(HeavenEvent.Update_Chapter_Count_Event,self,function ()
			self:refreshLeftCount()
		self:updateLeftBuyCount()
		end)

	--章节数据增加
     EventManager:getInstance():Bind(HeavenEvent.Add_Chapter_Data_Event,self,function ()
			self:updateChapterShowData()
		end)
end
