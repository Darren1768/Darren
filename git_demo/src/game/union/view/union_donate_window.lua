...
function UnionDonateWindow:open_callback()
    self.main_view = createCSBNote(PathTool.getTargetCSB("union/union_donate_window"))
    ...
    local config = Config.UnionData.data_const.day_exp_max
    if config then
        for i,v in ipairs(Config.UnionData.data_donate_box) do
            ...
            object.status = UnionConst.status.normal
            object.id = i
...
function UnionDonateWindow:register_event()
    registerButtonEventListener(self.explain_btn, function(param,sender, event_type)
        local config = Config.UnionData.data_const.game_rule1
    ...
    if self.my_union_info == nil then
        ...
        if self.my_union_info ~= nil and self.update_union_event == nil then
            self.update_union_event = self.my_union_info:Bind(UnionEvent.UpdateMyInfoEvent, function(key, value) 
                if key == "lev" then
                    self:updateMyUnionLev()
                end
            end)
        end
    end
     EventManager:getInstance():Bind(UnionEvent.UpdateDonateInfo,self, function() 
            self:updateDonateListStatus()
        end)

     EventManager:getInstance():Bind(UnionEvent.UpdateDonateBoxStatus,self, function(id) 
            if id ~= nil then
                local object = self.awards_list[id]
                if object then
                    object.is_show_tips = true
                    object.status = UnionConst.status.finish
...
function UnionDonateWindow:openRootWnd()
    local config_list = Config.UnionData.data_donate
...
--@return 
--==============================--
function UnionDonateWindow:updateDonateBoxList()
    ...
    for i, object in ipairs(self.awards_list) do
        ...
        local tmp_status = UnionConst.status.normal
        if box_status == true then                              -- 已经完成了
            tmp_status = UnionConst.status.finish
        else
            if activity_value >= object.config.box_val then     -- 可提交
                tmp_status = UnionConst.status.activity
            else
                tmp_status = UnionConst.status.un_activity
            end
        end
        local box_action = PlayerAction.action_1
        if tmp_status == UnionConst.status.finish then
            box_action = PlayerAction.action_3
        elseif tmp_status == UnionConst.status.activity then
            box_action = PlayerAction.action_2
        end
        if tmp_status == UnionConst.status.activity then
...
--@return 
--==============================--
function UnionDonateWindow:updateMyUnionLev()
    if self.my_union_info == nil then return end
    self.lev_title:setString(string.format(TI18N("公会等级：%s级"),self.my_union_info.lev))
    
    local next_lev = self.my_union_info.lev + 1
    local config = Config.UnionData.data_union_lev[next_lev]
...
function UnionDonateWindow:updateDonateListStatus()
...
function UnionDonateWindow:close_callback()
    ...
    controller:openUnionDonateWindow(false)
    if self.my_union_info ~= nil then
...
--
-- --------------------------------------------------------------------
UnionDonateItem = class("UnionDonateItem", function()
	return ccui.Layout:create()
end)

function UnionDonateItem:ctor()
    self.awards_list = {}           -- 富文本奖励列表
	self.root_wnd = createCSBNote(PathTool.getTargetCSB("union/union_donate_item"))
...
function UnionDonateItem:registerEvent()	
	self.donate_btn:addTouchEventListener(function(sender, event_type)
		customClickAction(sender, event_type)
		if event_type == ccui.TouchEventType.ended then
			playCommonButtonSound()
            if self.data ~= nil then
                controller:requestUnionDonate(self.data.id)
            end
		end
	end)
end

function UnionDonateItem:addCallBack(call_back)
	self.call_back = call_back
end

function UnionDonateItem:setData(data)
    self.data = data
    ...
    if data ~= nil then
        ...
        local res_id = PathTool.getResFrame("union",string.format("txt_cn_union_100%s", self.data.id))
...
--@return 
--==============================--
function UnionDonateItem:updateDonateStatus()
...
function UnionDonateItem:DeleteMe()
