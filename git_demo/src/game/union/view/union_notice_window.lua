...
-- @Description: 公会公告面板
-------------------------------------
UnionNoticeWindow = UnionNoticeWindow or BaseClass(BaseView)
...
function UnionNoticeWindow:__init()
	self.view_tag = ViewMgrTag.DIALOGUE_TAG
	
	self.win_type = WinType.Big
	-- self.title_str = TI18N("公告列表")
    self.title_sprite = PathTool.getResFrame("union", "hl_union_0001")
...
function UnionNoticeWindow:open_callback()
	self.main_view = createCSBNote(PathTool.getTargetCSB("union/union_notice_window"))
    self.main_view:setPositionY(self.main_view:getPositionY() + 24)
...
function UnionNoticeWindow:register_event()
     EventManager:getInstance():Bind(UnionEvent.UpdateUnionNoticeList,self, function()
            self:updateNoticeList()
        end)
    -- registerButtonEventListener(self.btn_check_detail, function()
    --     controller:openUnionMemberWindow(true, self.cur_index)
    -- end, true, 1, {}, 0.8)
...
function UnionNoticeWindow:openRootWnd()
    controller:send29277()

    self:updateBtnStatus() --初始化选项框状态
    --公会日志红点恢复
    local status = model:getRedStatus(UnionConst.red_index.notice)
    if status then
        model:updateUnionRedStatus(UnionConst.red_index.notice, false)
    end
    self:updateNoticeList()
...
--刷新单选框状态
function UnionNoticeWindow:updateBtnStatus(index, status)
	...
	-- if self.cur_index == UnionConst.show_type.all or not status then
...
--刷新公告列表
function UnionNoticeWindow:updateNoticeList(index)
	self.cur_index = index or UnionConst.show_type.all
    local list = model:getUnionNoticeList(self.cur_index)
    ...
    if is_empty then 
        commonShowEmptyIcon(self.background, true, {text = TI18N("暂无新的消息")})
    else 
...
function UnionNoticeWindow:adjustScrollViewSize(height)
...
function UnionNoticeWindow:close_callback()
    for k,v in pairs(self.notice_item_list) do
        v:DeleteMe()
        v = nil
    end
    EventManager:getInstance():UnBind(self)
    controller:openUnionNoticeWindow(false)
end

-- -------------------------------------------------------------------
-- @description: 公会公告子项
-- --------------------------------------------------------------------
UnionNoticeItem = class("UnionNoticeItem", function()
	return ccui.Layout:create()
end)

function UnionNoticeItem:ctor()
...
function UnionNoticeItem:layoutUI()
    self.container = ccui.Layout:create()
    self.container:setContentSize(self.size)
    self:addChild(self.container)

    self.img_time_bg = createImage(self.container, PathTool.getResFrame("common","hl_common_31"), 0, self.size.height, cc.p(0, 1), true, 1, true)
    self.img_time_bg:setContentSize(cc.size(self.size.width - 3, 44))

    self.day_label = createRichLabel(24, cc.c4b(98,53,43,0xff), cc.p(0, 0.5), cc.p(11, 22))
    self.day_label:setString("")
    self.img_time_bg:addChild(self.day_label)
end

function UnionNoticeItem:setData(data)
	self.data = data
    ...
    if data and next(data) ~= nil then
        ...
        for k,v in ipairs(data) do
            ...
            if list.item then
                ...
                list.content_label:setString(string_format("<div fontcolor=#47832d>%s</div> %s", v.role_name, v.msg))
...
function UnionNoticeItem:createOneNotice(time, name, content)
    ...
    local time_label = createRichLabel(22, cc.c3b(129,104,65), cc.p(0, 1), cc.p(10, size.height))
    time_label:setString(TimeTool.getHM(time))
    layout:addChild(time_label)

    local content_label = createRichLabel(24, cc.c3b(129,104,65), cc.p(0, 1), cc.p(15 + time_label:getContentSize().width, size.height), 5, 0, 500)
...
function UnionNoticeItem:DeleteMe()
