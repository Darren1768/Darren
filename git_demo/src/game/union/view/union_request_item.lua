...
--
-- --------------------------------------------------------------------
UnionRequestItem = class("UnionRequestItem", function()
...
function UnionRequestItem:ctor()
	self.root_wnd = createCSBNote(PathTool.getTargetCSB("union/union_list_item"))
	...
	self.union_value = container:getChildByName("union_value")

	local member_title = container:getChildByName("member_title")
	member_title:setString(TI18N("人数："))
	
	self.btn_res_id = PathTool.getResFrame("common", "hl_common_4")

	self.Image_1 = container:getChildByName("Image_1")

	self:registerEvent()
end

function UnionRequestItem:registerEvent()
		...
		if event_type == ccui.TouchEventType.ended then
			...
			if self.data ~= nil then
				if self.data.is_apply == TRUE then -- 这个时候点击就是取消掉状态
					controller:requestJoinUnion(self.data.lid , self.data.lsrv_id, 2) 
				else
					controller:requestJoinUnion(self.data.lid, self.data.lsrv_id, 1) 
				end
			end
		end
	end
...
function UnionRequestItem:setData(data)
	if self.data ~= nil then
	...
	if data ~= nil then
		self.data = data
		if self.update_self_event == nil then
			self.update_self_event = self.data:Bind(UnionEvent.UpdateUnionItemEvent, function(key, value) 
				print("-------------------UpdateUnionItemEvent-----------1-------------------")
		...
		self.leader_value:setString(string.format(TI18N("盟主：%s"), data.leader_name))
		

		self:setApplyStatus()
		self.member_value:setString(string.format("%s", data.members_num))

		self.union_value:setString(string.format("%s/%s", data.guild_num, data.members_max))
		...
		self.guild_lev:setPositionX(self.guild_name:getPositionX() + self.guild_name:getContentSize().width + 15)
	end
end

function UnionRequestItem:setApplyStatus()
	...
	if data.is_apply == TRUE then
		...
		if self.btn_res_id ~= PathTool.getResFrame("common","hl_common_4") then
			self.btn_res_id = PathTool.getResFrame("common","hl_common_4")
...
--更新条件信息
function UnionRequestItem:updateLimitInfo( data )
...
function UnionRequestItem:suspendAllActions()
	if self.data ~= nil then
		if self.update_self_event ~= nil then
			self.data:UnBind(self.update_self_event)
			self.update_self_event = nil
...
function UnionRequestItem:DeleteMe()
	if self.data ~= nil then
		if self.update_self_event ~= nil then
			self.data:UnBind(self.update_self_event)
			self.update_self_event = nil
