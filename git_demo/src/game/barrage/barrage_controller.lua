...
function BarrageController:registerEvents()
     EventManager:getInstance():Bind(BarrageEvent.HandleBarrageType,self, function(status, type, extend_data)
            if not status then
...
function BarrageController:on12733(data)
    EventManager:getInstance():Fire(BarrageEvent.UpdateBarrageData, data)
end
