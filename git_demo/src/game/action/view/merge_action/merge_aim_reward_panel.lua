...
function MergeAimRewardPanel:register_event(  )
    ...
    EventManager:getInstance():Bind(ActionEvent.Merge_Aim_Event,self,function(data)
...
function MergeAimRewardItem:configUI(  )
    ...
    self.root_wnd = createCSBNote(PathTool.getTargetCSB("seven_goal/seven_goal_adventure_lev_reward_item"))
...
function MergeAimRewardItem:register_event( )
	registerButtonEventListener(self.btn_get, handler(self, self.onClickGetBtn), true)
     EventManager:getInstance():Bind(ActionEvent.Merge_Box_Status_Event,self,function(data)
            if not data then return end
            if data.flag == 1 then
                self.status = false
            end
