...
function ActionLimitYuanZhenPanel:register_event(  )
    ...
    --元宵冒险的  走的不是同一个协议..无奈
     EventManager:getInstance():Bind(ActionEvent.YUAN_ZHEN_DATA_EVENT,self,function (data)
    ...
    --元宵冒险的  走的不是同一个协议..无奈
     EventManager:getInstance():Bind(ActionEvent.YUAN_ZHEN_UPDATA_EVENT,self,function (data)
            if not data then return end
            self:initYuanZhenData({data})
        end)
    --元宵冒险的  走的不是同一个协议..无奈
     EventManager:getInstance():Bind(ActionEvent.YUAN_ZHEN_TASK_EVENT,self,function (data)
            if not data then return end
            ...
            if config and self.dic_limit_list[config.f_id] then
                ...
                local config_list = Config.HolidayLantermAdventureData.data_lanterm_task[self.action_id][config.f_id]
                --如果有下一个任务档次
