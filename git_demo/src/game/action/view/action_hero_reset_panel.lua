...
function ActionHeroResetPanel:configUI( )
    ...
    -- self.title_name:setString(TI18N("凤凰涅"))
...
function ActionHeroResetPanel:register_event(  )
    ...
    --英雄重生返回
     EventManager:getInstance():Bind(ActionEvent.ACTION_HERO_RESET_EVENT,self,function (data)
    ...
    --选择英雄返回
     EventManager:getInstance():Bind(ActionEvent.HERO_RESET_SELECT_EVENT,self,function (select_hero_vo)
    ...
    --获取升星材料返回
     EventManager:getInstance():Bind(ActionEvent.ACTION_HERO_RESET_ITEM_EVENT,self, function(data)
            if not data then return end
