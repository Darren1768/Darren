...
function ActionHeroResetSelectPanel:_onClickBtnSelect()
    if self.is_hero_return == true then
        EventManager:getInstance():Fire(HeroEvent.HERO_RETURN_SELECT_EVENT, self.select_hero_vo)
    else
        EventManager:getInstance():Fire(ActionEvent.HERO_RESET_SELECT_EVENT, self.select_hero_vo)
    end
