...
function ActionPractiseTowerPanel:configUI()
    ...
    local bg_res = PathTool.getSingleImgForDownLoad("bigbg/action", "txt_cn_pt")
...
function ActionPractiseTowerPanel:register_event()
    if not self.Updata_Data_Event then
        self.Updata_Data_Event =EventManager:getInstance():Bind(PractisetowerEvent.Update_All_Data,self,function(data)
    ...
    if not self.Updata_My_Rank_Event then
        self.Updata_My_Rank_Event =EventManager:getInstance():Bind(PractisetowerEvent.Update_My_rank,self,function()
            self.pt_data = PractisetowerController:getInstance():getModel():getPractiseTowerData()
            self:updateMyRank()
