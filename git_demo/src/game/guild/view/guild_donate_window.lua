...
function GuildDonateWindow:register_event()
    ...
    if self.my_guild_info == nil then
     ...
     EventManager:getInstance():Bind(GuildEvent.UpdateDonateInfo,self, function() 
            self:updateDonateListStatus()
        end)

     EventManager:getInstance():Bind(GuildEvent.UpdateDonateBoxStatus,self, function(id) 
            if id ~= nil then
