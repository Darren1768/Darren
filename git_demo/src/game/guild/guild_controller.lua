...
--@return 
--==============================--
function GuildController:openGuildCreatWindow(status)
    if not status then
        ...
        if self.create_window == nil then
            self.create_window = GuildCreateWindow.New()
        end
...
function GuildController:requestExitGuild()
    ...
    if role_vo.position == GuildConst.post_type.leader then         -- 自己是帮主，则是解散公会
        ...
        if role_vo.guild_quit_time ~= 0 then
            extend_msg = TI18N("(解散公会后，12小时内无法加入其他公会，24小时内\n无法创建公会)")
        else
            extend_msg = TI18N("(首次解散公会后，可立即加入其他公会，24小时内\n无法创建公会。)") 
        end
...
function GuildController:handle16900(data)
    ...
    EventManager:getInstance():Fire(GuildEvent.UpdataGuildGoalBasicData, data)
end
...
function GuildController:handle16901(data)
    EventManager:getInstance():Fire(GuildEvent.UpdataGuildGoalTaskData, data)
end

--单条任务信息
function GuildController:handle16902(data)
    EventManager:getInstance():Fire(GuildEvent.UpdataGuildGoalSingleTaskData, data)
end
