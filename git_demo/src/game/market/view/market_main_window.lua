...
function MarketMainWindow:open_callback()
    ...
    self.refresh_btn = CustomButton.New(self.main_container, PathTool.getResFrame("common","hl_common_4.1"), nil, nil, LOADTEXT_TYPE_PLIST)
...
function MarketMainWindow:register_event()
    ...
    --获取金币市场指定分类的数据
     EventManager:getInstance():Bind(MarketEvent.Update_Gold_Category,self,function ( data )
    	...
    	--银币市场购买返回
     EventManager:getInstance():Bind(MarketEvent.Sliver_Market_Buy_Success,self,function ( data )
    ...
    --获取银币市场物品
     EventManager:getInstance():Bind(MarketEvent.Update_Sliver_Market,self,function ( data )
    ...
    --金币出售 成功返回
     EventManager:getInstance():Bind(MarketEvent.Gold_Sell_Success,self,function (  )
            if self.cur_son_index == nil then return end
    ...
    --银币摊位数据
     EventManager:getInstance():Bind(MarketEvent.Sliver_Shop_Data,self,function (  )
...
function MarketMainWindow:showEmpty( status)
    ...
    if self.cur_index == MarketTabConst.sliver_market and status then
        if self.go_to_btn == nil then
            self.go_to_btn = createTypeButton(self.scroll_con, TI18N("我要摆摊"), self.scroll_con:getContentSize().width/2, 190, ButtonType.yellow)
        end
