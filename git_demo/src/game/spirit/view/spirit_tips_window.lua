...
-- --------------------------------------------------------------------
SpiritTipsWindow = SpiritTipsWindow or BaseClass(BaseView)

function SpiritTipsWindow:__init(tower)
    self.ctrl = SpiritController:getInstance()
    
    self.layout_name = "spirit/spirit_tips_window"
    self.res_list = {
        { path = PathTool.getSingleImgForDownLoad("startower", "startower_bg_1"), type = ResourcesType.single },
        { path = PathTool.getSingleImgForDownLoad("startower", "startower_bg_2"), type = ResourcesType.single },
    }
...
function SpiritTipsWindow:open_callback()
...
function SpiritTipsWindow:register_event()
...
function SpiritTipsWindow:updateTimer()
    local time = 5
...
function SpiritTipsWindow:openRootWnd(data)
    self.battle_data = data
    self:updateTimer()
end

function SpiritTipsWindow:checkCloseBtn(  )
    self.ctrl:openTipsWindow(false)
    BattleController:getInstance():openFinishView(true, BattleConst.Fight_Type.Spirit, self.battle_data)
end

function SpiritTipsWindow:close_callback()
    GlobalTimeTicket:getInstance():remove("close_starTower_tips_view")
