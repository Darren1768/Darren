...
SpiritList = class("SpiritList", function()
...
function SpiritList:ctor(parent, pos, size)
    self.parent = parent
...
function SpiritList:createRootWnd()
...
--@return 
--==============================--
function SpiritList:registerEvent()
...
--创建一个新cell
function SpiritList:createNewCell()
...
function SpiritList:onCellTouched(cell, index)
...
--设置 scrollview 是否可点
function SpiritList:setClickEnabled(status)
    self.scroll_view:setTouchEnabled(status)
end
--设置 是否吞噬点击
function SpiritList:setSwallowTouches(status)
...
--@return 
--==============================--
function SpiritList:checkRectIntersectsRect()
...
--@return 
--==============================--
function SpiritList:checkOverShowByVertical()
...
--@return 
--==============================--
function SpiritList:updateMove(pos)
...
--@return 
--==============================--
function SpiritList:jumpToMove(pos, time, callback)
    local target_pos = self:checkPosition(pos.x, pos.y)
    time = time or 1
...
--@return 
--==============================--
function SpiritList:checkPosition(x, y)
...
--@return 
--==============================--
function SpiritList:setInnerContainer()
...
function SpiritList:resetCurrentItems()
...
function SpiritList:resetItemByIndex(index)
    -- body
...
--获取活跃中的cell对象
function SpiritList:getActiveCellList()
    local list = {}
    for i,v in pairs(self.activeCellIdx) do
...
function SpiritList:getCellByIndex(index)
...
function SpiritList:getCellXYByIndex(index)
    if  self.cellList[index] and self.cellList[index].cell then
...
--@setting: 如果有改变的话
--==============================--
function SpiritList:reloadData(select_index)
    self.cellList = {}
    self.activeCellIdx = {}
    ...
    delayRun(self,1 / display.DEFAULT_FPS,function ()
        if self.bottom_bg == nil then
            local res = PathTool.getSingleImgForDownLoad("bigbg","bigbg_28")
...
-----------------------中间背景的代码开始-----------------------------------------------------------
function SpiritList:updateBgList()
    if self.scroll_view_bg == nil then
        ...
        delayRun(self,3 / display.DEFAULT_FPS,function ()
            if self.scroll_view_bottom_bg == nil then
                local res = PathTool.getSingleImgForDownLoad("bigbg","bigbg_27",true)
                local container = self.scroll_view_bg:getContainer()
...
--创建cell 
function SpiritList:createNewCellBg()
    local res = PathTool.getSingleImgForDownLoad("bigbg","bigbg_36",true)
    local cell =  createImage(nil, res, nil, nil, cc.p(0.5,0.5), false, 0, false)
    cell:setScale(display.getMaxScale())
    return cell
end
--获取数据数量
function SpiritList:numberOfCellsBg()
    return self.bg_max_count or 0
end
-----------------------中间背景的代码结束-----------------------------------------------------------

--人物箭头
function SpiritList:showArrow()
...
--移动人物箭头到最新位置
function SpiritList:moveToArrowNewPosition()
...
function SpiritList:setNowArrowPos()
...
function SpiritList:updateArrowPos(top, bot)
...
function SpiritList:selectCellByIndex(index)
    --一屏幕显示的最大数量
    local maxRefreshNum = self.cacheMaxSize - self.col
    local number = self:numberOfCells()
...
function SpiritList:getCellPosition(index)
...
--获得格子下标对应的缓存itemCell
function SpiritList:getCacheCellByIndex(index)
...
--更新格子，并记为活跃
function SpiritList:updateCellAtIndex(index)
...
function SpiritList:getMaxSize()
    return self.container_size
end

function SpiritList:getContainer()
