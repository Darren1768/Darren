...
SpiritResultWindow = SpiritResultWindow or BaseClass(BaseView)


function SpiritResultWindow:__init(result, fight_type)
	self.result = result
	self.x = 100
...
function SpiritResultWindow:openRootWnd(data,fight_type,battle_extend_data)
	self:setData(data, fight_type, battle_extend_data)
end

--初始化
function SpiritResultWindow:open_callback()
	local res = ""
    ...
    --     local res = PathTool.getSingleImgForDownLoad("bigbg","bigbg_97")
    ...
    --     local res = PathTool.getSingleImgForDownLoad("bigbg","bigbg_98")
	...
	-- 通关时间
	local past_time_bg = createSprite(PathTool.getResFrame("battle", "battle_r1"),self.root_wnd:getContentSize().width / 2 +160,0, self.title_container, cc.p(0.5, 0.5))
	self.pass_time = createRichLabel(24, cc.c4b(239,212,174,0xff), cc.p(0, 0.5), cc.p(50, 16), nil, nil, 1000)
	self.pass_time:setString(TI18N("通关时间：00:00:00"))
	past_time_bg:addChild(self.pass_time) 

	-- 当前战斗
	local fight_text_bg = createSprite(PathTool.getResFrame("battle", "battle_r1"), self.root_wnd:getContentSize().width / 2-160,0, self.title_container, cc.p(0.5, 0.5))
	self.fight_text = createLabel(24, cc.c4b(239,212,174,0xff), nil, 50, 16, "",fight_text_bg, nil, cc.p(0,0.5))

	-- 冒险者实力过于强大，~敌人落荒而逃！
	 self.skip_text_bg = createSprite(PathTool.getResFrame("common2", "hl_common_0072"), self.root_wnd:getContentSize().width / 2,468, self.source_container, cc.p(0.5, 0.5))
	 local skip_text = createLabel(24, cc.c4b(239,212,174,0xff), nil, self.skip_text_bg:getContentSize().width / 2, 28, "冒险者实力过于强大，~敌人落荒而逃！",self.skip_text_bg, nil, cc.p(0.5,0.5),TITLE_FONT)
	 self.skip_text_bg:setVisible(false)

    --
	self.time_label = createRichLabel(20, cc.c3b(154,129,111), cc.p(0.5, 0.5), cc.p(self.root_wnd:getContentSize().width / 2 ,50), nil, nil, 1000)
	self.time_label:setString(TI18N("十秒后关闭"))
	...
	self.scroll_view = createScrollView(SCREEN_WIDTH, 230, 0, 180, self.source_container, ccui.ScrollViewDir.vertical) 

	self.comfirm_btn = createTypeButton(self.source_container,TI18N("确定"), self.root_wnd:getContentSize().width / 2 -170, 140, ButtonType.yellow)
	self.comfirm_btn:addTouchEventListener(function(sender, event_type)
		if event_type == ccui.TouchEventType.ended then
			SpiritController:getInstance():openResultWindow(false)
		end
	end)

	self.cancel_btn = createTypeButton(self.source_container,TI18N("挑战下卷"), self.root_wnd:getContentSize().width / 2 +170, 140, ButtonType.red)
	self.cancel_btn:addTouchEventListener(function(sender, event_type)
		if event_type == ccui.TouchEventType.ended then
            ...
            SpiritController:getInstance():openResultWindow(false)   -- 胜利之后可以直接进入下一层继续挑战
            if self.isCanNext  then
	            SpiritController:getInstance():sender11322(tower + 1)
	            if self.data.count > 0 and  tower < Config.StarTowerData.data_tower_base_length then
	            	message(TI18N("已进入下卷战斗"))   
	            end
	        else
	        	BattleResultReturnMgr:returnByFightType(self.fight_type) --先
	        end
...
function SpiritResultWindow:register_event()
...
function SpiritResultWindow:_onClickHarmBtn(  )
...
--剧情：{章节id,难度，副本id}
function SpiritResultWindow:setData(data, fight_type, dungeon_data)
	if data then
		self.data = data or {}
		...
		if data.is_skip and data.is_skip == 1 then
			...
			self.harm_btn:setVisible(false)
			self.skip_text_bg:setVisible(true)
			self.title_container:setPositionY(510)
			self.pass_time:setString(string.format(TI18N("通关时间：%s"),"00:00:00"))

		else
			self.skip_text_bg:setVisible(false)
			self.pass_time:setString(string.format( TI18N("通关时间：%s"),TimeTool.GetTimeFormat(pass_time)))
			self.harm_btn:setVisible(true)
			self.title_container:setPositionY(480)
			...
			if self.data.tower then
				...
				if config then 
					if self.data.tower <  config.val - 1 then
						...
						self.isCanNext = true
						self.cancel_btn:setBtnLabel(TI18N("挑战下卷"))
					end
				end
			end
		-- else
...
function SpiritResultWindow:handleEffect(status)
...
--奖励界面
function SpiritResultWindow:rewardViewUI()
	...
	for i, v in ipairs(self.reward_list) do
		...
		item:setGoodsName(name,nil,nil,cc.c3b(239,212,174))
...
function SpiritResultWindow:ItemAciton()
...
function SpiritResultWindow:updateTimer()
    local time = 10
...
--清理
function SpiritResultWindow:close_callback()
	...
	if not MainuiController:getInstance():checkIsInDramaUIFight() then
		GlobalAudio:getInstance():playLastMusic()
	end
	if BattleController:getInstance():getModel():getBattleScene() then
		local data = {result = self.result ,combat_type = BattleConst.Fight_Type.Spirit}
