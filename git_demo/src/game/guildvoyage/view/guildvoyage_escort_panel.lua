...
function GuildVoyageEscortPanel:createRootWnd()
	...
	self.resources_load = createResourcesLoad(PathTool.getSingleImgForDownLoad("bigbg", "bigbg_38"), ResourcesType.single, function() 
		self.bg_img = createImage(self, PathTool.getSingleImgForDownLoad("bigbg", "bigbg_38"), 0, 0, cc.p(0, 0), false)
	end)
...
function GuildVoyageEscortPanel:registerEvent(status)
	if status == true then
     EventManager:getInstance():Bind(GuildvoyageEvent.UpdateGuildvoyageOrderStatus,self, function(order_id, status)
