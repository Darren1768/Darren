...
function GuildVoyageOrderPanel:registerEvent(status)
    if status == true then
        EventManager:getInstance():Bind(GuildvoyageEvent.UpdateGuildvoyageOrderListEvent,self, function() 
                local list = model:acceptList()
                self:updateOrderList(list)
            end)
        EventManager:getInstance():Bind(GuildvoyageEvent.UpdateNearLogInfoEvent,self, function() 
                self:setLog()
            end)
        EventManager:getInstance():Bind(GuildvoyageEvent.UpdateDailyEscortTimes,self, function(times) 	
                self:escortDailyLeftTimes()
            end)
    else
