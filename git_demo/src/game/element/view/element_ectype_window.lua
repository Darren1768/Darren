...
function ElementEctypeWindow:setData( data )
	...
	local all_res_list = {}
	_table_insert(all_res_list, {path = PathTool.getSingleImgForDownLoad("bigbg/element", string.format("element_bg_%d", res_id), true), type = ResourcesType.single})
	_table_insert(all_res_list, {path = PathTool.getSingleImgForDownLoad("partner", self.monster_cfg.bust_id), type = ResourcesType.single})
	self.ele_res_load = ResourcesLoad.New()
	self.ele_res_load:addAllList(all_res_list, function()
        self.background:loadTexture(PathTool.getSingleImgForDownLoad("bigbg/element",string.format("element_bg_%d", res_id), true))
