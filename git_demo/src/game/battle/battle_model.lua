...
function BattleModel:resetStartInfo()
	self.main_vo 				 = RoleController:getInstance():getRoleVo() or {} --英雄人物信息
	self.finish_data 			 = {} 		--战斗结束数据
	self.select_skill 			 = nil 		--选择技能
	self.res_num 				 = 0 		--资源数量
	self.is_exit 				 = false 	--是否已退出
	self.is_speed 				 = 1     	--是否加速 因为需求 有三倍速了 所以变成 对应 x1 x2 x3  
...
function BattleModel:battleStart(data)
	...
	if not tolua.isnull(self.battle_scene) then
		...
		EventManager:getInstance():Fire(BattleEvent.BATTLE_DEBUG) --战斗调试
    else
...
function BattleModel:round()
	...
	if tableLen(self.order_list or {}) == 0 then
		...
		if self.all_object and next(self.all_object or {}) ~= nil then
			...
			for _, v in pairs(self.all_object or {}) do  --回合结束清掉缓存的buff_tips列表
				if v and v.spine_renderer and not v.spine_renderer.is_die then
					...
					if v and  v.spine_renderer.is_die then
						print("------------------------die---------------4-------------------------")
					end 
				end
		...
		print("--------------------通知服务器完成------------self.is_battle_type------------------------------",self.is_battle_type)
		...
		for _, round_one in pairs(round_data or {}) do
			...
			for _, round_one_temp in pairs(round_one or {}) do
				...
				if #round_one_temp.target_list > 0 then
					...
					print("---------------round_one_temp.target_list---------round_one_temp.target_list------------")
					printLuaTable(round_one_temp.target_list)
...
function BattleModel:initOrder(attacker, one)
	...
	-- printLuaTable(attacker )
	-- printLuaTable(one )
	print("-------初始化动作列表-------initOrder---------------end----------------")
...
function BattleModel:beginPlayEffectAction(attacker, one)
	...
	if effect_config then
		-- print("-------------------------------effect_config------`11111`-----------",effect_config)
		-- printLuaTable(effect_config)
		attacker.play_order = deepCopy(effect_config.action_list)											--动作列表
		attacker.closeup_id = effect_config.closeup or 0 		
...
function BattleModel:playOrder(attacker)
	print("=---playOrder-----------1-------------attacker.wait_act-----------",attacker.wait_act)
	...
	if attacker.play_order == nil or next(attacker.play_order) == nil then --判断是否还有动作进行
		...
		if not attacker.is_die then
			...
			if second_actor == 0 or (self.one and self.one.actor ~= second_actor) then
				print("=-----playOrder---------4--d---------------------",attacker.spine_renderer:getActionName())
		...
		printLuaTable(attacker.play_order)     
...
function BattleModel:addRoundData(data, is_round)
	...
	self.order_list = {}
	--重组技能播报数据
	self.skill_plays_order_list = {} -- 技能播放顺序
	self.act_playing = false
	if self.battle_controller:getCtrlBattleScene() then
		self.battle_controller:getCtrlBattleScene():updateRound(self.action_count)
		self.battle_controller:getCtrlBattleScene():updateHeavenStarInfo(self.star_sum_list) --天界副本星数
	end
	if next(self.all_object or {}) ~= nil and self.all_object then
		for _, v in pairs(self.all_object) do
			if v.is_round then
				v.is_round = false --没播放完成
			end
		end
	end

	-- printLuaTable(data.skill_plays)

	if (data.skill_plays or {}) ~= nil  then
		for j, v in ipairs(data.skill_plays) do  --动技能列表
			if next(v.effect_play or {}) ~= nil then
				for i, v1 in ipairs(v.effect_play) do
					...
					v1.skill_bid =v1.skill_bid_of_effect  ---v.skill_bid -- v1.skill_bid_of_effect
	...
	print("-------------skill_plays_order_list-------------------")
	printLuaTable(self.skill_plays_order_list)
	...
	if next(self.round_data_temp or {}) ~= nil then --判断是否存在播报
		...
		print("-------------------播放技能----------- skill_bid_of_effect----------addRoundData---------------------------")
		printLuaTable(self.order_list)
		print("-------------------播放技能----------- skill_bid_of_effect----------addRoundData---------------------------")
		printLuaTable(self.round_data)
...
function BattleModel:enterFightFunc()
	EventManager:getInstance():Fire(SceneEvent.ENTER_FIGHT, self.fight_type,self.battle_controller:getIsNoramalBattle())
end
...
function BattleModel:blackScreen2(attacker, delay_time, time, alpha, particle_id, effect_delay_time,is_loop)
	...
	local begin_fun = function()
		...
		self.battle_scene:setBlack(true)
	end
	local end_fun = function()
...
function BattleModel:createBattleRole(data)
	...
	for i, v in ipairs(data.objects or {}) do
		_render:doNextFrame(function() 
			self:addBattleRole(v, self.fight_type, data.current_wave, data.total_wave,nil,nil,#data.objects)
		end)
	end
...
--添加/复活角色
--param data:角色信息数据
function BattleModel:addBattleRole(data, combat_type, current_wave, total_wave,is_next,is_spec_add,count)
...
function BattleModel:actStart(attacker)
	...
	if self.battle_controller:getExtendFightType() == self.battle_controller:getCurFightType() or  self.battle_controller:getExtendFightType() == 0 then
		...
		print("--------------attacker.wait_act--------actStart---------",attacker.wait_act)
		-- printLuaTable(attacker.effect_config)
		print("--------------attacker.wait_act--------.action_name---------",attacker.spine_renderer.action_name )
		...
		end, 5, 1, "attackerActTimeout".. attacker.pos)
	end
end
...
function BattleModel:actFinish(attacker)
	print("-----------------actFinish---------------------------------",attacker.wait_act)
	if  self.battle_controller:getExtendFightType() == self.battle_controller:getCurFightType() or self.battle_controller:getExtendFightType() == 0 then
...
function BattleModel:moveBack(attacker, delay_time, move_time, grid_pos_x, action_name, is_jump, is_jump_delay, is_get_point, is_move, is_reverse)
	...
	grid_pos_x = grid_pos_x or 0
	move_time = move_time or dispaly.DEFAULT_FPS / 2
	local is_jump_delay = is_jump_delay or 0
	local is_move = is_move or 1
	local is_get_point = is_get_point or 0
	local target_pos
...
function BattleModel:attack(attacker, delay_time, hit_fun, start_fun, is_reverse)
	...
	local delay_time = delay_time or 0
	local start_callback =nil
	if start_fun then
		 start_callback = function()
			print("------------attack---------start_fun----------------",start_fun)
...
function BattleModel:item(attacker, delay_time, effect_name, move_time, is_back, funname, start_height, target_pos, scale, is_offset_angle, bid, x_fix, height, is_shadow, shadow_data)
	...
	if self.battle_controller:getIsNoramalBattle() then
		...
		print("_________---------远程子弹--------------effect_name--------------",effect_name)
...
function BattleModel:flyItem(attacker, delay_time, move_time, is_bcak, funname, start_height, pos, is_shadow, shadow_opacity, shadow_offset, shadow_num, is_reverse)
	...
	return SkillAct.attack(attacker, 0, anima_name, nil, nil, is_reverse, attack_func, anima_res)
end
...
function BattleModel:attackPoint(attacker)
	...
	if attacker.attack_sound and attacker.attack_sound ~= "" then
		if not self.battle_controller:getIsNoramalBattle() then
			GlobalAudio:getInstance():playEffect(GlobalAudio.AUDIO_TYPE.BATTLE, attacker.attack_sound, false)
		end
	end
	-- 播放攻击点特效
...
function BattleModel:attackReady(attacker)
	print("--------攻击准备动作---------attackReady---------------------------------")
    attacker.spine_renderer:showHallowsState(false)
	-- 播放发起点音效
	if attacker.ready_sound and attacker.ready_sound ~= "" then
		if not self.battle_controller:getIsNoramalBattle() then 
			GlobalAudio:getInstance():playEffect(GlobalAudio.AUDIO_TYPE.BATTLE, attacker.ready_sound, false)
		end
	end
	-- 播放特效
...
function BattleModel:effectSpineUser(attacker, delay_time, is_release, x_fix, height, effect_name, spine_renderer, scale, alpha, h_scale, callback, buff_bid, effect_bid, is_attack, object_type)	
	...
	if is_attack == true then
		print("---------------effectSpineUser---------------height-------------1--------",height)
		if height == 0 then
			height = 50
		end
		height = height * 0.01 * spine_renderer.role_height
	else
		print("---------------effectSpineUser---------------height-------------2--------",spine_renderer.role_height)
		if height == 0 then
...
function BattleModel:hurt(attacker,object)
	...
	local hurt_fun = function()
		...
		print("-----------hit_num,hit_time------------------",hit_num,hit_time,split_hurt)
...
function BattleModel:playMagic(attacker, effect_play, no_die, play_effect)
    ...
    if effect_play.play_num == nil then
        ...
        if effect_play.play_num > 1 then -- 需要拆分伤害效果
            effect_play.avg_hp_changed = math.ceil(effect_play.hp_changed / effect_play.play_num)
        end
    end
	print("---------------effect_play-------effect_play------------",effect_play.play_num)
	...
	if  next(self.all_object or {}) ~= nil and self.all_object then
		...
		if self.all_object and effect_play.target ~= nil and self.all_object[effect_play.target] then
			...
			if target and target.spine_renderer and not tolua.isnull(target.spine_renderer.root) then
				...
				local hp_changed = effect_play.avg_hp_changed or effect_play.hp_changed or 0
				...
				if target then 
					if is_dead == TRUE then
						if attacker.is_must_die == 0 then --
							_isHurt = false 
						end
					end 
		    	end 
...
function BattleModel:playBattleRoleHurt(attacker, target, dmg, play_effect)
	print("-----------------playBattleRoleHurt---------1------------")

	if attacker == nil or target == nil then return end
	if target.is_hurt_play == true then return end -- 正在受击就不处理了
	print("-----------------playBattleRoleHurt---------2------------",dmg,attacker.pos,target.pos,attacker.group,target.group)
			...
			if self.all_object[sub_effect_play.sub_target]  then
				...
				if sub_target.spine_renderer and sub_target then
					...
					if skill_config then
						...
						print("=======================skill_config.name===============================================",skill_config.name)
...
function BattleModel:playCloseup(closeup_id,group,attacker)
	if closeup_id == 0 or closeup_id == nil then
		return
	end
	local _config_data = Config.SkillData.data_get_closeup_data[closeup_id]
    if not _config_data then
        return
    end

	local target_pos
	target_pos = gridPosToScreenPos({x = attacker.grid_pos.x, y = attacker.grid_pos.y})
	local _center_pos = cc.p(SCREEN_WIDTH/2-target_pos.x,SCREEN_HEIGHT/2 - target_pos.y)
	 
	if _config_data.center==1 then 
		local _bound_box = attacker.spine_renderer.spine:getBoundingBox()
		_center_pos = cc.p(_center_pos.x,_center_pos.y-_bound_box.height/2)
	else
		_center_pos =cc.p(0,0)
	end

	if not tolua.isnull(self.battle_scene) then
		self.battle_scene:closeup(closeup_id,group,_center_pos)
	end
end
...
function BattleModel:playHitEffect(attacker, target, res_up, res_down, y_fix, x_fix, scale, alpha, h_scale, bid)
	...
	if res_up == nil  and res_down == nil  then return end
	...
	if buff.action_type == 1 or buff.action_type == 4 or buff.action_type == 6 then
		...
		if buff.action_type == 6 then
			...
			if buff_data.positive_or_negative == 1 then -- 增益
				text_color =cc.c3b(214,255,222) --221
				outline_color =cc.c3b(15,92,52) --222
			elseif buff_data.positive_or_negative == 2 then --减益 
				text_color =cc.c3b(190,231,255) -- 223
				outline_color =cc.c3b(15,51,92)  --224
			elseif buff_data.positive_or_negative == 3 then --控制
				text_color =cc.c3b(255,233,190)  -- 225
				outline_color =cc.c3b(92,49,15)  -- 226
			end
		end
		if client_desc ~= "" then
...
function BattleModel:showbuffName(target, msg, text_color, buff, is_effect, outline)
	print("=======================skill_config.name=================msg==============================",msg)
	...
	if target.spine_renderer and not tolua.isnull(target.spine_renderer.spine) then
		...
		if target.tips_list[id] == nil then
			local sp = createSprite(PathTool.getResFrame("battledrama", "hl_battledrama_1023"), 0, 0, tips, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST, -1)
			sp:setCascadeOpacityEnabled(true)
			tips = createLabel(20, text_color, outline, sp:getContentSize().width / 2, sp:getContentSize().height / 2-20, msg, sp, 2, cc.p(0.5, 0.5),TITLE_FONT)
			sp:setOpacity(100)
			sp:setScale(0.6)
			sp:setVisible(true)
...
function BattleModel:callfun(attacker, args)
	...
	if type(args) == "table" then
		funname = args[1]
		print("---------args[1]args[1]--------1--------",args[1],args[2], args[3], args[4])
		arg = {args[2], args[3], args[4]}
	else
		funname = args
		print("---------args[1]args[1]---------2-------",funname)
	end
	if type(self[funname]) == "function" then
...
function BattleModel:exitFightFunc()
	EventManager:getInstance():Fire(SceneEvent.EXIT_FIGHT, self.fight_type, self.result)
end
...
function BattleModel:getCurEffectList(list)
	...
	print("-----------------------------getCurEffectList--------------------effect_list------------")
	printLuaTable(effect_list)
	return effect_list
end
...
function BattleModel:addReadySum()
	print("-----------addReadySum---------BattleModelBattleModelBattleModel-----1--------")
	self.sum = self.sum + 1
	...
	if skill_data.effect_list and next(skill_data.effect_list) then
		for i,v in ipairs(skill_data.effect_list) do
			...
			if effect_data then
				...
				-- if effect_data.anime_user_atk ~= "" then
				-- 	local model_res_key = getNorKey(model_res, effect_data.anime_user_atk)
				-- 	print("--------------model_res_key-----anime_res----------",effect_data.anime_user_atk)

				-- 	print("--------------model_res_key---------------",model_res_key)
				-- 	if self.prepare_role_list[model_res_key] == nil then
				-- 		self.prepare_role_list[model_res_key] = {model_res, effect_data.anime_user_atk}
				-- 	end
				-- 	printLuaTable(self.prepare_role_list)
...
function BattleModel:addSpineRes()
	...
	if next(self.prepare_sound_list or {}) ~= nil then
		for k,v in pairs(self.prepare_sound_list) do
			GlobalAudio:getInstance():preLoadEffect(GlobalAudio.AUDIO_TYPE.BATTLE, v) 
		end
	end
	print("-----------------------------getCurEffectList--------------------self.effect_list------------")
	printLuaTable(self.effect_list)
...
function BattleModel:checkIsCanChangeBattleSpeed( battle_type, show_tips )
	...
	-- 再判断是否为某些不能开启变速的战斗  加了三倍速后 就去掉此功能判断了 
