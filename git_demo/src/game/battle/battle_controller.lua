...
function BattleController:registerEvents()
     EventManager:getInstance():Bind(EventId.ROLE_CREATE_SUCCESS,self, function()
...
function BattleController:scRoundNextFight(data)
    print("========下一波怪物更新接口=======scRoundNextFight====================")
    printLuaTable(data)
    if data then
...
function BattleController:scInitFightStart(data)
    ...
    if data then
        ...
        print("================fighting_type_list====11=================")

        printLuaTable(self.fighting_type_list)
    end
end
...
function BattleController:openFinishView(bool,fight_type,data)
    if bool == true then
        ...
        if not self.is_unlock_chapter then
            ...
            elseif fight_type == BattleConst.Fight_Type.Spirit then --星命塔
                SpiritController:getInstance():openResultWindow(true, data)
            ...
            elseif fight_type == BattleConst.Fight_Type.LeagueExplore then --探索历练
                print("------------------LeagueExplore-----------fight_type-------------",fight_type)
            else
...
-- 打开阵营克制面板
function BattleController:openBattleCampInfoView(status)
    if status == true then
        if self.battle_camp_info_view == nil then
            self.battle_camp_info_view = BattleCampInfoView.New()
        end
        if self.battle_camp_info_view:isOpen() == false then
            self.battle_camp_info_view:open()
        end
    else
        if self.battle_camp_info_view then
            self.battle_camp_info_view:close()
            self.battle_camp_info_view = nil
        end
    end
end
...
function BattleController:handle20060(data)
    ...
    print("-----COMBAT_TYPE_BACK----------------",data.combat_type)
    EventManager:getInstance():Fire(BattleEvent.COMBAT_TYPE_BACK, data.combat_type, data.type)
end
...
--播放剧情
function BattleController:playDrame()
    -- StoryController:getInstance():getModel():setCurStory({drama_bid=20000})
    -- delayRun(self,20 / display.DEFAULT_FPS, function ( )
    --     -- StoryController:getInstance():getModel():setCurStory({drama_bid=20000})
    --     -- StoryController:getInstance():handle_11100()
    --     StoryController:getInstance():getModel():setCurStory({drama_bid=20001})
    -- end)
    local map_id = BattleDramaController:getInstance():getModel():getDramaData().map_id 
    print("--------------playDrame-----------------------",map_id)
    StoryController:getInstance():send_11124(map_id)
end
--清理场景相关
