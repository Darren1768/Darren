...
function BattleResultShowInfoWindow:onClickReportBtn( )
	local role_vo = RoleController:getInstance():getRoleVo()
	print("-----------------csRecordBattle-----self.replay_id--------",self.replay_id)
	...
	if self.replay_id and role_vo then
		...
		elseif self.fight_type == BattleConst.Fight_Type.Spirit then
...
function BattleResultShowInfoWindow:onClickShareBtnByType( cross_type )
	print("---------------onClickShareBtnByType-------------",self.replay_id)
...
function BattleResultShowInfoWindow:changeTabIndex(index)
	...
	if self.select_tab then
		...
		-- self.select_tab.title:setTextColor(cc.c3b(247,221,200))
		-- self.select_tab.title:enableOutline(cc.c3b(247,221,200,1),1)
		self.select_tab =nil 
	end
	...
	self.select_tab.title:setTextColor(cc.c3b(245,241,234))
	self.select_tab.title:enableOutline(cc.c3b(65,52,45),2)
...
function BattleResultShowInfoWindow:openRootWnd(data, setting)
	...
	if self.fight_type and self.fight_type == BattleConst.Fight_Type.Arena 
		...
		or self.fight_type == BattleConst.Fight_Type.Spirit 
...
function BattleResultShowInfoWindow:setBaseInfo()
	...
	if self.success == 1 then
		...
		-- self.success_bg:setPositionY(self.top_bg_y)
		-- self.fail_bg:setPositionY(self.bottom_bg_y)
	elseif self.success == 2 then
		self.success_img:setPositionY(self.bottom_y)
		self.fail_img:setPositionY(self.top_y)

		self.success_bg:setPositionY(self.bottom_bg_y)
		self.fail_bg:setPositionY(self.top_bg_y)
	end
...
function BattleResultShowInfoWindow:setTopInfo(bast_hurt, bast_behurt, bast_cure)
	...
	if self.top_scroll_view == nil then
		...
		self.top_scroll_view = CommonScrollViewLayout.new(self.top_list_view, nil, ScrollViewDir.vertical, nil, size, setting)
	end
...
function BattleResultShowInfoWindow:setBottomInfo(bast_hurt, bast_behurt, bast_cure)
	...
	if self.bottom_scroll_view == nil then
		...
		self.bottom_scroll_view = CommonScrollViewLayout.new(self.bottom_list_view, nil, ScrollViewDir.vertical, nil, size, setting)
	end
...
function BattleResultReportItem:setExtendData(data)
	...
	if not tolua.isnull(node) and self.root_wnd == nil then
		...
		self.mvp:setPositionX(60)
		self.mvp:setPositionY(56)
		self.mvp:setLocalZOrder(999)
		self.partner_item = HeroExhibitionItem.new(0.45, true) 
		self.partner_item:addCallBack(handler(self, self._onClickHeroCallBack))
		self.partner_item:setPosition(40, 30)
...
function BattleResultReportItem:setData(data)
	if data and data.bid then
		...
		-- showHelpImg
		self.desc_1:setString(data.dps)
		self.desc_2:setString(data.be_hurt)
        self.desc_3:setString(data.cure)
