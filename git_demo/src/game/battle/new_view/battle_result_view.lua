...
function BattleResultView:open_callback()
	...
	local label  = createRichLabel(22,31, cc.p(0.5, 0.5), cc.p(self.root_wnd:getContentSize().width/2,775), nil, nil, 1000)
...
function BattleResultView:createComfirmBtn()
	...
	self.comfirm_btn = createTypeButton(self.root_wnd,TI18N("确 定"), ButtonType.yellow)
...
function BattleResultView:createComfirmAndCancelBtn()
	if not self.root_wnd then return end
	self.comfirm_btn = createTypeButton(self.root_wnd,TI18N("确 定"), 620, 580, ButtonType.yellow)
	...
	self.cancel_btn = createTypeButton(self.root_wnd,TI18N("返回玩法"), 620, 580, ButtonType.red)
