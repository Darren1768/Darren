...
function BattleRole:__init(data, combat_type,current_wave, total_wave)
	...
	self.enter_delay  		= data.enter_delay or 100		                    -- 进入时间
...
function BattleRole:initBattleRoleData(data)
	...
	if data.object_type == BattleObjectType.Pet then --伙伴
		...
		self.enter_effect = base_config.enter_effect
		self.die_effect = base_config.die_effect 
		if base_config then
	...
	elseif data.object_type == BattleObjectType.Unit then --怪物
		...
		if base_data then
			...
			if self.is_boss == TRUE then
				EventManager:getInstance():Fire(BattleEvent.Battle_Boss_Hp_Event, {show_type = 1, head_icon = base_data.head_icon})
			end
		end
	...
	if self.fashion ~= 0 then
		...
		if skin_config then
            ...
            --self.encircle_effect ~= "" 表示 该英雄有10星以上..由策划决定
...
function BattleRole:loadSpineRes()
	if self.type ~= BattleObjectType.Role then
		local js_path, atlas_path, png_path, prefix = PathTool.getSpineByName(self.res, PlayerAction.battle_stand)
...
function BattleRole:addToLayer()
	...
	local anima_name = PlayerAction.battle_stand
	print("=============anima_name========addToLayer======screen_pos======",screen_pos.x,screen_pos.y)
	self.spine_renderer = SpineRenderer.New(screen_pos, self.res, self.model_scale, true, PlayerAction.battle_stand, self.career, temp_group, self.type, self.is_boss, self.unit_is_boss, self.spine_scale)
...
function BattleRole:updataNextBattleRole(data, data_vo)
	...
	if self.is_boss == TRUE then
		--如果是boss 需要更新一下信息 
		EventManager:getInstance():Fire(BattleEvent.Battle_Boss_Hp_Event, {show_type = 2, battle_role = self})
	end
	...
	if not tolua.isnull(self.spine_renderer.root) and not BattleController:getInstance():getModel():getReconnectStatus() then
		if not self.spine_renderer.is_finish_action then
			if BattleConst.canDoBattle(self.combat_type) then
			   ...
			   local _time = self.enter_delay/60 or 2
			   print("---------_time------------------",_time)
			-- local _time = 1


				self.spine_renderer:runAction(cc.Sequence:create(cc.DelayTime:create(_time), cc.CallFunc:create(function()
					self.spine_renderer.root:setOpacity(255)


					self.spine_renderer:doEnter(self.enter_effect,self.die_effect)
					self.spine_renderer:reverse(self.obj_type)
					if not tolua.isnull(self.spine_renderer.root) then
				...
				-- self.spine_renderer:runAction(cc.Sequence:create(cc.DelayTime:create(1), cc.CallFunc:create(function()
...
function BattleRole:died(force)
	...
	self.is_die = true
	print("========角色死亡==========BattleRole:died=========",force)
	if not force then
		...
		local callFunc = cc.CallFunc:create(function()
			print("---------------------self.spine_renderer.is_die--------------",self.spine_renderer.is_die)				 

			if self.spine_renderer.is_die == true then
				-- SkillAct.setAnimation(self.spine_renderer, "die" ,false)
				self.spine_renderer:createDieRoleEffect(self.spine_renderer.die_spine_name)
				self.spine_renderer.spine:stopAllActions()
				self.spine_renderer.spine:setAnimation(0,"die", false)
				self.spine_renderer.spine:setToSetupPose()
				local function animationCompleteFunc(event)
	                print("---------------------event.animation----死亡移除---------------",event.animation)				 
					if event.animation == "die" then
					    self.spine_renderer:hideSpineModel()
					end 
				end
		...
		self.spine_renderer:runAction(cc.Sequence:create(delayTime, callFunc))
		-- self.spine_renderer:runAction(callFunc)

	else
		print("========角色死亡======11====BattleRole:died=========",force)
