...
function BattleFailView:open_callback()
	GlobalAudio:getInstance():playMusic(GlobalAudio.AUDIO_TYPE.BATTLE, "b_lose", false)
	...
	if self.fight_type == BattleConst.Fight_Type.Adventrue then
		...
		createLabel(24, cc.c4b(0x68,0xc7,0x4b,0xff), nil, 360, 60, str,self.source_container, nil, cc.p(0.5,0.5))

	end
    self.fight_textD= createImage(self.source_container, PathTool.getResFrame("battle", "battle_r1"),360, 550, cc.p(0.5, 0.5), true, 0, false)
	self.fight_text = createLabel(24, cc.c4b(239,212,234), nil, 360, 550, "",self.source_container, nil, cc.p(0.5,0.5))
    ...
    self.comfirm_btn = createTypeButton(self.source_container,TI18N("返回玩法"), 360, 200, ButtonType.red_big)
...
--           local image_bg = self.root_wnd:getChildByName("image_bg")
--    local bg_res=PathTool.getSingleImgForDownLoad("bigbg", "battleFailBg",false)
...
--    end
	self.help_btn = self.source_container:getChildByName("change_btn")
	...
	-- 	self.cancel_btn = createTypeButton(self.source_container,TI18N("返回玩法"), 620, 580, ButtonType.red)
	...
	if self.fight_type == BattleConst.Fight_Type.Adventrue then
		if self.data.adventure_end_hp and self.data.adventure_end_hp > 0 then
			...
			if not self.left_hp_txt then
				self.left_hp_txt = createRichLabel(22, cc.c4b(255,125,39,255), cc.p(0.5, 0.5), cc.p(self.source_container:getContentSize().width/2, 95))
				self.source_container:addChild(self.left_hp_txt)
			end
			self.left_hp_txt:setString(string.format(TI18N("出战英雄剩余血量为:<div fontColor=#68c74b>%d%%</div>"), self.data.adventure_end_hp/10))
			...
			if self.data.room_id and appear_cfg and appear_cfg.val <= (self.data.adventure_end_hp/10) then
				if not self.battle_again_btn then
					self.battle_again_btn = createTypeButton(self.source_container, TI18N("再次挑战"),self.source_container:getContentSize().width/2,-80, ButtonType.red)
					self.battle_again_btn:setRichText(TI18N("<div fontColor=#ffffff fontsize=24 outline=2,#6C2B00>再次挑战</div>"))
				end
			end
...
--	self.desc_fail_label = createRichLabel(20, 1, cc.p(0.5,0.5), cc.p(SCREEN_WIDTH/2,345), 15)
--	self.desc_fail_label:setString(string.format(TI18N("<div outline=2,#7b5132>     失败，不要气馁，\n可通过以下方式提升实力！</div>")))
--	self.source_container:addChild(self.desc_fail_label)
	...
	local start_x = 93
	local offset_x = 9
	local con_size = cc.size(169, 100)
		...
		if config then
			if not self.items_list[i] then
				...
				self.source_container:addChild(container)
                 local sprite1 = createImage(container, PathTool.getResFrame("battle", "battle_r8"),con_size.width/2, con_size.height/2, cc.p(0.5, 0.5), true, 0, false)
                ...
                local sprit2 = createImage(container, PathTool.getResFrame("battle", "battle_r9"),con_size.width/2, con_size.height/2-60, cc.p(0.5, 0.5), true, 0, false)
				local  label = createLabel(20, cc.c4b(36,46,63),nil,con_size.width/2, con_size.height/2-60,config.icon_name ,container,nil, cc.p(0.5,0.5), TITLE_FONT)

--                local label = createRichLabel(20, cc.c3b(255, 238, 209), cc.p(0.5, 0), cc.p(con_size.width/2, 0))
--				container:addChild(label)
--				label:setString(string.format("<div fontcolor=ffeed1 outline=2,#242e3f>%s</div>", config.icon_name))
				container.btn = btn
				container.label = label
				self.items_list[i] = container
			end
			local pos_x = start_x + (i-1)*(con_size.width+offset_x)
			self.items_list[i]:setPosition(pos_x, size.height/2+80)
