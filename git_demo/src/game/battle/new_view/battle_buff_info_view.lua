...
function BattleBuffInfoItem:configUI(  )
    ...
    self.hero_head = HeroExhibitionItem.new(0.8, true)
...
function BattleBuffInfoItem:setData( data )
	...
	if data.hp > 0 then
		self.hero_head:showStrTips1(false)
    else
        self.hero_head:showStrTips1(true)
    end
	...
	-- for i=1,8 do  
	-- 	table.insert(self.buff_list_data,{})
	-- end
	for i,bData in ipairs(self.buff_list_data) do
		if i <= 8 then -- 最多显示8个
			...
			local pos_y = start_y - (row -1)*(buff_icon_size.height+space_y) - buff_icon_size.height/2 
			buff_object.icon:setPosition(cc.p(pos_x, pos_y))

			local buff_icon_id =bData.res_id -- 3101 --
