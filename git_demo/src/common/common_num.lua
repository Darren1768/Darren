...
commom_battle_num[25] = true --冰冻伤害数字  吸收
...
-- 新幻灵
commom_battle_num[202] = true --加血 数字  --治疗
commom_battle_num[204] = true --伤害 数字  普通伤害 107
commom_battle_num[207] = true --暴击伤害数字
commom_battle_num[224] = true --诅咒伤害数字
commom_battle_num[225] = true --冰冻伤害数字
commom_battle_num[227] = true --克制伤害数字
commom_battle_num[228] = true --总伤害

commom_battle_num[219] = true --vip数字
commom_battle_num[220] = true --战力的 101
    ...
    if commom_battle_num[self.type] then
        ...
        if common_num_is_completed == true then
            ...
            if common_num_resources_load == nil then
                ...
                local res = PathTool.getPlistImgForDownLoad("num", "hl_battle_num") 
        ...
        if self.type>100 then
            self.plist_file = string.format("hl_type%s", self.type)
        else
        ...
        if self.is_completed == true then
            ...
            if self.resources_load == nil then
                ...
                local res = PathTool.getPlistImgForDownLoad("num", self.plist_file) 
...
function CommonNum:setOfficialNum(num, add)
    ...
    for i = 1, temp_len do
        ...
        local res 
        if self.type>100 then
            res = _getResFrame("num", _string_format("hl_type%s_%s", self.type, char), false, self.plist_file)
        else
            res = _getResFrame("num", _string_format("type%s_%s", self.type, char), false, self.plist_file)
        end
