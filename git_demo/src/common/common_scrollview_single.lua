...
function CommonScrollViewSingleLayout:analysisSetting(setting)
    ...
    self.row            = self.setting.row or 1                 -- 行数,作用于水平方向的滚动
    ...
    self.delta_y_list         = setting.delta_y_list or nil       --y偏移  在已经计算好位置的基础上微调
    --位置列表 
...
function CommonScrollViewSingleLayout:refreshSize(newSize)
    self.size = newSize
    self:setContentSize(self.size)    

    self.scroll_view:setContentSize(cc.size(newSize.width, newSize.height))
...
-- 横向
function CommonScrollViewSingleLayout:jumpToMoveByIndexHorizontal(index)
    
    if not self.cellList[index] then return end
    local x  = self.cellList[index].x or 0
    local pos = self.container_size.width - (x + self.size.width * 0.5 )
    if pos < 0 then
        pos = 0
    end
    local pos_per = pos * 100 / (self.container_size.width - self.size.width)
    if pos_per > 100 then
        pos_per = 100
    end
    pos_per = 100 -pos_per
 
    self.scroll_view:scrollToPercentHorizontal(pos_per, 0.8, true)
end
...
function CommonScrollViewSingleLayout:getCellPosition(index)
    ...
    if self.setting.delta_y_list and #self.setting.delta_y_list>=self.col and self.col > 0  then
        _y =_y + self.setting.delta_y_list[(index-1)%self.col + 1]
    end
    return _x, _y
end
