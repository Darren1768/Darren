...
function PlayerHead:ctor(type, offest_y, size, res, mask_res, is_plist)
	...
	elseif type == PlayerHead.type.circle then
		self.vSize = cc.size(98,98)
		bgRes = PathTool.getResFrame("common","hl_common_40")
		self.mask_res = PathTool.getResFrame("common", "hl_common_39") 
		self.is_plist = true
	elseif type == PlayerHead.type.other then
		self.vSize = size or cc.size(108, 108) 
		bgRes = res
		self.mask_res = mask_res or PathTool.getResFrame("common", "hl_common_39")
	end
...
function PlayerHead:setSex(sex, pos)
	...
	if self.sex_icon == nil then
		self.sex_icon = createSprite(res, 4, 4, LOADTEXT_TYPE_PLIST) 
		self.sex_icon:setAnchorPoint(cc.p(0, 0))
		self.sex_icon:setScale(0.8)
		self:addChild(self.sex_icon, 1)
	else
		loadSpriteTexture(self.sex_icon, res, LOADTEXT_TYPE_PLIST) 
	end
...
function PlayerHead:setLev(lev, pos)
    if not self.txtLev then
    	if not self.levBg then
        	self.levBg = createSprite(PathTool.getResFrame("common","hl_common_5"), 16, 22, nil)
			self.levBg:setCascadeOpacityEnabled(true)
        else
        	self.levBg:setVisible(true)
 		end
        self.levBg:setAnchorPoint(cc.p(0.5, 0.5))
        self:addChild(self.levBg,1)

		local scale = self.head_panel_scale or 1
		self.levBg:setScale(scale)

        self.txtLev = createLabel(20,cc.c3b(239,212,174),Config.ColorData.data_color4[152],self.levBg:getContentSize().width/2-2,self.levBg:getContentSize().height/2,"",self.levBg,0,cc.p(0.5,0.5))
...
---- VIP
function PlayerHead:setVIP(vip, pos)
	if not  vip then 
         return
	end 

	if not self.vip_sprite then 
		self.vip_sprite = createSprite(PathTool.getResFrame("common2", "hl_common_3000"), 76, 80, self, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST)
	end 

    if not self.txtVip then
		local scale = self.head_panel_scale or 1
		if vip > 10 then 
			self.txtVip = CommonNum.new(106, self.vip_sprite, 1, -7, cc.p(0.5, 0.5))
		else 
			self.txtVip = CommonNum.new(106, self.vip_sprite, 1, -5, cc.p(0.5, 0.5))


		end 
		self.txtVip:setPosition(15,28)
		self.txtVip:setScale(scale)
        self.txtVip:setLocalZOrder(1)
	end
    self.txtVip:setNum("v"..vip)
    if pos ~= nil and self.txtVip ~= nil then
    	self.txtVip:setPosition(pos)
    end
end

function PlayerHead:setVipScale(scale)
	if  self.txtVip then
		self.txtVip:setScale(scale)
	end
end
--设置VIP位置
function PlayerHead:setVipPositon(pos)
	if self.txtVip then 
		self.txtVip:setPosition(pos)
	else
		self.vip_pos = pos
...
--关闭Vip
function PlayerHead:closeVip()
    if self.txtVip then
    	self.txtVip:setString("")
	end
end
