...
function GmCmd:addButtonList(parent, name, x, y, scale, zorder, func, font_color)
    ...
    if zorder == 1 then 
         res = PathTool.getResFrame("common", "hl_common_4")
    end
...
function GmCmd:init_button()
    ...
    local base_button_info = {{"软重启", "restart"}, {"ui", "f1"},{"纹理信息", "dumpTextureInfo"},
         ...
         {"代金","testPerfer"},{"自定义头像","openCustomWindow"},
        }
...
function GmCmd:saveResourcesList()
    ...
    local first_list = { "spine/E88888", "spine/H99999", "spine/E51006", "spine/E65004", "spine/E65012", "spine/E31316", "spine/E20986", 
        ...
        "resource/functionicon/", "resource/honor", "resource/campbg/", "resource/starname/"
    }
...
function GmCmd:time_ticket()
    GlobalAudio:getInstance():showLoadMuiscPath()
...
function GmCmd:restart()
    -- local list = GlobalAudio:getInstance().effect_play_list
    -- Debug.info(list)
end
...
function GmCmd:autoTask()
    ...
    --     message("你已经开启自动任务状态")
    -- else
    --     GlobalTimeTicket:getInstance():setTimeScale(1)
    --     message("你已经开关闭动任务状态")
    -- end
end
...
function GmCmd:gm_callback(pSender)
    ...
    if msg ~= "" then
        ...
        elseif string.find( msg, "#" ) ~= nil then
            local str = string.sub(msg, 2, string.len(msg))
            GlobalTimeTicket:getInstance():setTimeScale(tonumber(str))
            return
        elseif string.find( msg, "~" ) ~= nil then
            local str = string.sub(msg, 2, string.len(msg))
        ...
        elseif string.find( msg, "goto" ) ~= nil then
            ...
            local arr_tmp = Split(str, ",")
            EventManager:getInstance():Fire(BigworldEvent.GotoPosByGrid, tonumber(arr_tmp[1]), tonumber(arr_tmp[2]) )
            return
        elseif string.find(msg, "play") ~= nil then
            local action_name = string.sub(msg, 6)
            GlobalTimeTicket:getInstance():setTimeScale(0.3)
            return
        ...
        if ok and type(data) == 'table' and type(data[1]) == "number" then
            ...
            if PROTO_MATE ==1 then 
                self.proto = self.proto or require("sys.net.proto_mate") or {}
            elseif PROTO_MATE == 2 then 
                self.proto = self.proto or require("sys.net.proto_mate2") or {}
            elseif PROTO_MATE ==3 then 
                self.proto = self.proto or require("sys.net.proto_mate3") or {}
            end 
...
function GmCmd:addButton(parent, name, x, y, scale, zorder, func, font_color)
    local button = ccui.Button:create()
    button:loadTextures(PathTool.getResFrame("common", "hl_common_4.1"), "", "", LOADTEXT_TYPE_PLIST)
    button:setTitleText(name)
    button:setTitleFontSize(18)
    button:setScale9Enabled(true)
    button:setTitleFontName(DEFAULT_FONT)
...
function GmCmd:setLowPerformance(bool, layer)
    ...
    if bool then
        ...
        GlobalAudio:getInstance():pauseMusic()             -- 暂停音乐
        -- 这里释放掉一些不需要用的资源
		display.removeUnusedTextures()
    	collectgarbage("collect")
    else
        ...
        GlobalAudio:getInstance():resumeMusic()            -- 音量恢复
    end
