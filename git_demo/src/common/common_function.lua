...
--@return 
--==============================--
function createSpineByName(spine_name, action_name, root, zorder, scale, pixelformal,isBig )
    ...
    local js_path, atlas_path, png_path = PathTool.getSpineByName(spine_name, action_name,isBig)
...
--@return 
--==============================--
function createRichLabel(fontsize, textcolor, ap, pos, lineSpace, charSpace, max_width,font)
...
function createCSBNote(csb_name)
    ...
    if USE_RES_LUA == true then
        csb_name = string.sub(csb_name,1,string.len(csb_name)-4)
        csb_name = string.gsub(csb_name,'/','.')
        local node = require(csb_name).create()
        -- cc.CSLoader:setCsbPlistFlag(false)
        return node['root']
    end
    local node = cc.CSLoader:createNode(csb_name)
    -- cc.CSLoader:setCsbPlistFlag(false)
    resetUiLayout(node) --针对于以layout_命名格式的节点下做适配（待测试）
    return node
end

function createCSBResult(csb_name)
    -- cc.CSLoader:setCsbPlistFlag(true)
    if USE_RES_LUA == true then
        csb_name = string.sub(csb_name,1,string.len(csb_name)-4)
        csb_name = string.gsub(csb_name,'/','.')
...
function createTitleLabel(font_size,text_color,line_color,x,y,text_content,parent_wnd,line_num, anchorpoint,font)
    font_size = font_size
    local label = createWithSystemFont(TITLE_FONT,   TITLE_FONT , font_size)
    if type(text_color) == "number" then
        text_color = Config.ColorData.data_color4[text_color]
    end
    label:setTextColor(text_color or Config.ColorData.data_color4[1])
...
--@label_color:
--==============================--
function createButton(parent, label, x, y, size, normal_res, fontsize, label_color)
    label = label or ""
    fontsize = fontsize or 30
    local btn = CustomButton.New(parent, normal_res, nil, nil, LOADTEXT_TYPE_PLIST)
    btn:setLabelSize(fontsize)
    label_color = label_color or cc.c4b(0xE7,0xD5,0xC4,0xFF)
    btn:setBtnLableColor(label_color)
    btn:setBtnLabel(label)
    x = x or 0
    y = y or 0
    ...
    --setOffsetPos 要放在setSize后面
    btn:setOffsetPos(0, 3)

    return btn
end

--==============================--
--desc:根据ButtonType类型创建自定义按钮
--@parent:
--@label:
--@x:
--@y:
...
function createTypeButton(parent, label, x, y, buttonType)
    label = label or ""
    buttonType = buttonType or ButtonType.red

    local normal_res
    if buttonType == ButtonType.red then
        normal_res = PathTool.getResFrame("common", "hl_common_4_6")
    elseif buttonType == ButtonType.red_big then
        normal_res = PathTool.getResFrame("common", "hl_common_4_8")
    elseif buttonType == ButtonType.yellow then
        normal_res = PathTool.getResFrame("common", "hl_common_4_5")
    elseif buttonType == ButtonType.yellow_mid then
        normal_res = PathTool.getResFrame("common", "hl_common_4.1")
    elseif buttonType == ButtonType.red_mid then
        normal_res = PathTool.getResFrame("common", "hl_common_4")
    else
        normal_res = PathTool.getResFrame("common", "hl_common_4_7")
    end
    ...
    -- local label = btn.btn_label or btn:getTitleRenderer()  
    -- btn:setTitleFontName(TITLE_FONT)

    -- btn.btn_label:setFont(TITLE_FONT)
    if buttonType == ButtonType.red then
        btn:setBtnLableColor(cc.c3b(0xEF,0xD3,0xD2))
        btn:enableOutline(cc.c3b(0x97,0x45,0x4c), 0)
        btn:setOffsetPos(0,3)

    elseif buttonType == ButtonType.red_big then
        btn:setBtnLableColor(cc.c3b(255,240,240))
        btn:enableOutline(cc.c3b(151,69,76), 0)
        btn:setOffsetPos(0,3)

    elseif buttonType == ButtonType.yellow then
        btn:setBtnLableColor(cc.c3b(0xff,0xf5,0xe3))
        btn:enableOutline(cc.c3b(0xba,0x8a,0x39), 0)
        btn:setOffsetPos(0,3)

    elseif buttonType == ButtonType.yellow_mid then
        btn:setBtnLableColor(cc.c3b(255,238,202))
        btn:enableOutline(cc.c3b(0xba,0x8a,0x39), 0)
        btn:setLabelSize(22)
        btn:setOffsetPos(0,1)

    elseif buttonType == ButtonType.red_mid then
        btn:setBtnLableColor(cc.c3b(247,213,196))
        btn:enableOutline(cc.c3b(0xba,0x8a,0x39), 0)
        btn:setLabelSize(22)
        btn:setOffsetPos(0,1)
        ...
        btn:setBtnLableColor(cc.c3b(0xff,0xf5,0xe3))
        btn:enableOutline(cc.c3b(0xba,0x8a,0x39), 2)
    end

    return btn
end
--==============================--
--desc:针对studio无法设置button自带字体描边
--@parent:
--@label:
--@x:
--@y:
...
function resetStudioButton(btn,btnType,str)
    if not btn then return end
    local label = btn.btn_label or btn:getTitleRenderer()  
    if label ~= nil then
        btn:setTitleFontName(TITLE_FONT)
        if btnType == ButtonType.red then
            btn:setTitleFontSize(22)
            btn:setTitleColor(cc.c3b(0xEF,0xD3,0xD2))
            label:enableOutline(cc.c3b(0x97,0x45,0x4c), 0)
        elseif btnType == ButtonType.yellow then
            btn:setTitleFontSize(22)
            btn:setTitleColor(cc.c3b(0xff,0xf5,0xe3))
            label:enableOutline(cc.c3b(0xba,0x8a,0x39), 0)
        elseif btnType == ButtonType.yellow_big then
            btn:setTitleFontSize(28)
            btn:setTitleColor(cc.c3b(0xe7,0xd5,0xc4))
            label:enableOutline(cc.c3b(0xb2,0x7f,0x3a), 0)
        elseif btnType == ButtonType.red_big then
            btn:setTitleFontSize(28)
            btn:setTitleColor(cc.c3b(0xef,0xd3,0xd2))
            label:enableOutline(cc.c3b(0x97,0x45,0x4c), 0)
        else
        ...
        label:setPosition(cc.p(btn:getContentSize().width/2,btn:getContentSize().height/2 + 3))
    end
end
...
--@type:
--@zorder:
--@force:不判断tolua.isnull父节点
--return 
--==============================--
function createSprite(res, x, y, container, anchorPoint, type, zorder,force)
    x = x or 0
    y = y or 0
    type = type or LOADTEXT_TYPE_PLIST
    ...
    elseif force == true and container then
        container:addChild(sprite, zorder or 0)
    end
    if anchorPoint ~= nil then
...
function createEditBox(parent, ui_res, size, font_color, font_size, placeholder_color, placeholder_size, placeholder_str, pos, max_len, load_type, input_model, input_flag, input_return)
    ...
    local editBox =nil 
    if not ui_res then 
        editBox = ccui.EditBox:create()
    else 
        editBox = ccui.EditBox:create(size, ui_res, load_type)
    end 
...
function createLoadingBar1(bg_res, progress_res, size, parent, ap, x, y, usePlist,is_Scale9)
    local load_type = usePlist and LOADTEXT_TYPE_PLIST or LOADTEXT_TYPE
    local is_Scale9 = is_Scale9 or true
    local bg = createScale9Sprite(bg_res, x, y, load_type, parent)
    bg:setContentSize(size)
    ...
    progress:loadTexture(progress_res,load_type)
    progress:setContentSize(cc.size(size.width-4, size.height-4))
    progress:setPosition(cc.p(2, size.height/2+1.5))
    bg:addChild(progress)
    return bg, progress
end
--==============================--
...
function delayOnce(func, time)
    GlobalTimeTicket:getInstance():add(function(dt)
            func()
        end,time,1)
end
...
function setChildBlackShader(bool, parent, color)
    if tolua.isnull(parent) then return end
    local shader_path = PathTool.getShaderRes("black_shader")
    setAllChildFilter(parent, bool, shader_path, color)
end
-- 设置模糊效果
...
--@return
--==============================--
function playCommonButtonSound()
    GlobalAudio:getInstance():playEffect(GlobalAudio.AUDIO_TYPE.COMMON, "c_button1")
end
...
function playQuitSound()
    GlobalAudio:getInstance():playEffect(GlobalAudio.AUDIO_TYPE.COMMON,"c_close")  
end
...
function playPartnerSound(sound_id)
    GlobalAudio:getInstance():playEffect(GlobalAudio.AUDIO_TYPE.DUBBING,sound_id,false)
end
...
function playOtherSound(sound_id, type)
    type = type or GlobalAudio.AUDIO_TYPE.COMMON 
    GlobalAudio:getInstance():playEffect(type, sound_id)
end
    ...
    if node and not tolua.isnull(node) then
        if status == true then
            if not node.red_point then
                ...
                zorder = zorder or 10
                local red_res = PathTool.getResFrame("common","hl_common_3")
                --红点从 common 图集 和 main_ui图集里面 通用用common 需要转大小
                local scale = 25/35  
                if red_type and red_type == 2 then
                    -- red_res = PathTool.getResFrame("common","hl_common_3")
                    scale = 1
                end
...
function addRedPointToNodeByStatusLeft( node, status, offset_x, offset_y, zorder, red_type )
    if node and not tolua.isnull(node) then
        if status == true then
            if not node.red_point then
                offset_x = offset_x or 0
                ...
                local red_point = createSprite(red_res,0,0,node,cc.p(1,1),LOADTEXT_TYPE_PLIST,zorder)
                local node_size = node:getContentSize()
                local pos_x = 10+ offset_x -- node_size.width + offset_x
                local pos_y = node_size.height + offset_y
                red_point:setPosition(cc.p(pos_x, pos_y))
                red_point:setScale(scale)
                node.red_point = red_point
...
--新的红点机制,通过字段判断是否需要显示
function addRedPointToNodeByStrs( node,str_list, offset_x, offset_y, zorder )
    if node and not tolua.isnull(node) then
        if not node.red_pointNew then
            offset_x = offset_x or 0
            offset_y = offset_y or 0
            zorder = zorder or 10
            local red_res = PathTool.getResFrame("common","hl_common_3")
            local red_point = createSprite(red_res,0,0,node,cc.p(0.7,0.7),LOADTEXT_TYPE_PLIST,zorder)
            local node_size = node:getContentSize()
            local pos_x = node_size.width + offset_x
            local pos_y = node_size.height + offset_y
            red_point:setPosition(cc.p(pos_x, pos_y))
            node.red_pointNew = red_point
            red_point:setScale(25/35)
        end
        node.red_pointNew:onNodeEvent("exit",function ()
                RedManager:getInstance():removeRedNode(node.red_pointNew)
            end)
        RedManager:getInstance():initRedNode(node.red_pointNew,str_list)
    end 
end
...
function registerButtonEventListener(button_node, clickCallBack, showClickAction, soundType, param, scale, clickDelay, clickScroll)
    ...
    button_node:setSwallowTouches(true)
...
function commonShowEmptyIcon(parent, bool, setting)
    ...
    if bool then
        ...
        if not parent.empty_con then
            local scale = setting.scale or 1
            parent.empty_con = createCSBNote(PathTool.getTargetCSB("common/empty_tip"))
            parent.empty_con:setAnchorPoint(cc.p(0.5, 0.5))

            parent:addChild(parent.empty_con, 10)
            parent.empty_con:setScale(scale)
            parent.empty_label = parent.empty_con:getChildByName('cat'):getChildByName('text_talk')
        else
            parent.empty_con:setVisible(true)
        end
        local parent_size = parent:getContentSize() 
        local pos = setting.pos or cc.p(parent_size.width * 0.5, parent_size.height * 0.5)
        parent.empty_con:setPosition(pos)
        parent.empty_label:setString(text)
    else
        if parent.empty_con then
...
--冒泡
function commonShowBuddle(bool,pos,parent,str,scaleX,scaleY)
    if not parent then return end
    if  bool == false then 
        if  parent.buddle_bg then 
            parent.buddle_bg:setVisible(false)
        end 
        if parent.buddle_label then 
            -- parent.buddle_label:removeFromParent()
            parent.buddle_label:setVisible(false)
        end 
        return
    end 
    if bool then 
        if not parent.buddle_bg then 
            parent.buddle_bg = createSprite(PathTool.getResFrame("common2","hl_common_0009"), pos.x, pos.y+50,parent, cc.p(0.5, 0.5),LOADTEXT_TYPE_PLIST,999)
        else
            parent.buddle_bg:setVisible(true)
        end 
        if not parent.buddle_label then 
            parent.buddle_label =  createLabel(22, cc.c3b(98,53,43), nil, pos.x,  pos.y +55, "", parent, nil, cc.p(0.5, 0.5))
            parent.buddle_label:setZOrder(1000)
        else
            parent.buddle_label:setVisible(true)
        end 
        parent.buddle_bg:setScaleY(0.8)
        parent.buddle_bg:setScaleX(scaleX or 1 )
        parent.buddle_bg:setScaleY(scaleY or 1 )

        parent.buddle_label:setString(str)
     end 
end

function commonShowRecordBtn(parent)
--好友界面留言按钮
    local btn=parent:getChildByName("btn_record")
    if not btn then
        btn=createButton(parent,"", 578,90, cc.size(50, 50), PathTool.getResFrame("common", "hl_common_25"))
        -- btn:setScale(1.5)
        local icon = createImage(btn:getRoot(), PathTool.getResFrame("common", "hl_common_81"), 25, 25, cc.p(0.5,0.5),true, false)
        btn:setName("btn_record")  
        btn:setZOrder(100)
        btn:addTouchEventListener(function(sender, event_type) 
            if event_type == ccui.TouchEventType.ended then
                RoleController:getInstance():openRolePersonalSpacePanel(true, {index = RoleConst.Tab_type.eMessageBoard},false)
            end
        end) 
    end
...
function updateHeroDrawPic(bid,hero_draw_sprite,scale)
    if not hero_draw_sprite then return end
    local scale = scale or 1
    local x = hero_draw_sprite:getPositionX()
    local y = hero_draw_sprite:getPositionY()
    ...
    local draw_res_id = partner_config.draw_res
    local bg_res = PathTool.getSingleImgForDownLoad("herodraw/herodrawres",draw_res_id, false)
    if hero_draw_sprite then
        loadSpriteTextureFromCDN(hero_draw_sprite, bg_res, ResourcesType.single) 
    end

    if library_config.scale == 0 then
        hero_draw_sprite:setScale(scale)
    else
        hero_draw_sprite:setScale(library_config.scale/100*scale)
    end
    if library_config.draw_offset and next(library_config.draw_offset) ~= nil then
        ...
        hero_draw_sprite:setPosition(x + offset_x,  y + offset_y) 
    end  
end
...
function resetUiLayout(root_wnd,self)
    ...
    for i,v in ipairs(children) do
        ...
        if name == "background" then
            v:setScale(display.getMaxScale())
        end
        
        local nodeTypeName = string.sub(name,1,6)
        if nodeTypeName == "layout" then autoUILayout(v) end

        traversingNode(v,self)
    end
end

function traversingNode(node,self)
    local children = node:getChildren()
    for i, v in ipairs(children) do
        local name = v:getName()
...
--自动UI布局靠边 (0,0为中心点)  当前坐标原点(0,0)位于屏幕左下角
function autoUILayout(node)
    local t = {node}
    for i,v in ipairs(t) do
        if not v._raw_x then
            local raw_x,raw_y = v:getPosition()
            v._raw_x = raw_x
            v._raw_y = raw_y
        end
        local x = v._raw_x
        ...
        --     if x < CC_DESIGN_RESOLUTION.width/4 then
        --         x = x - display.uiOffSetX
        --     elseif x > CC_DESIGN_RESOLUTION.width*3/4 then
        --         x = x + display.uiOffSetX
        --     end
        -- end
        
        if y < CC_DESIGN_RESOLUTION.height/4 then
            y = y - display.uiOffSetY
        elseif y > CC_DESIGN_RESOLUTION.height*3/4 then
            y = y + display.uiOffSetY
        end
        v:setPosition(x,y)
    end
