...
function SpineRenderer:hideSpineModel()
	-- self.spine_root:setVisible(visible)
...
function SpineRenderer:createElfinList(  )
    ...
    self.elfin_node:setVisible(false) -- 原来开着
					...
					if elfin_cfg and elfin_cfg.skill then
						...
						if skill_cfg then
							...
							if not elfin_object.clipNode then
								elfin_object.mask_bg = createSprite(PathTool.getResFrame("battle", "hl_battle_1002"), 29, 29.5, nil, cc.p(0.5, 0.5))
...
function SpineRenderer:setupUI(group, lev, camp_type, setting)
	...
	if self.shadow == nil and self.spine_type ~= BattleObjectType.Hallows and self.spine_type ~= BattleObjectType.Elfin then
		self.shadow = createSprite(PathTool.geHeroSpineShadow(camp_type), 0, 0, self.spine_root, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST, -2)
	...
	if not BattleController:getInstance():getIsNoramalBattle() or group == BattleGroupTypeConf.TYPE_GROUP_ENEMY then
		if self.ui_hp == nil and self.spine_type ~= BattleObjectType.Hallows and self.spine_type ~= BattleObjectType.Elfin then
			...
			self:updateNormalHpUI(self.hp_root,73,5,group)	
			self.hp_root:setPosition(20, height + 6)
...
function SpineRenderer:updateNormalHpUI(parent,width,height,group)
	if parent then
		...
		local res = PathTool.getResFrame("battle", "hl_battle_1017")

		if BattleController:getInstance():getModel():changeGroup(group) == BattleGroupTypeConf.TYPE_GROUP_ENEMY and not  BattleController:getInstance():getIsNoramalBattle() then
			-- res = PathTool.getResFrame("battle", "battle_enemy_bg")
			res = PathTool.getResFrame("battle", "hl_battle_1016")
		end
		-- local frame_res = PathTool.getResFrame("battle", "battle_hp_bg")
		local frame_res = PathTool.getResFrame("battle", "hl_battle_1019")

		local hp_frame = createScale9Sprite(frame_res, 0, 1, LOADTEXT_TYPE_PLIST, parent)
		-- hp_frame:setCapInsets(cc.rect(3, 4, 1, 1))
		-- hp_frame:setContentSize(cc.size(75, 7))
		...
		self.ui_hp:setPosition(-36,0.5)
		self.ui_hp:setContentSize(cc.size(73,height))
		-- self.ui_hp:setCapInsets(cc.rect(2, 3, 1, 1))
		self.ui_hp:loadTexture(res, LOADTEXT_TYPE_PLIST)
		parent:addChild(self.ui_hp,3)

		self.hp_white = createScale9Sprite( PathTool.getResFrame("battle", "hl_battle_1018"), 1, 0.5, LOADTEXT_TYPE_PLIST, self.ui_hp)
		self.hp_white:setPosition(self.ui_hp:getContentSize().width,1.4)
		self.hp_white:setZOrder(4)
...
function SpineRenderer:setHpPercent(percent, is_init)
	...
	if tolua.isnull(self.ui_hp)  then return end

	print("--------percent-------------------spine_name------------percent------------------",percent,spine_name)

	self.ui_hp:setPercent(percent)
    if self.is_boss == TRUE then
        EventManager:getInstance():Fire(BattleEvent.Battle_Boss_Hp_Event, {show_type = 3, percent = percent})
    end

	self.hp_white:setPositionX(self.ui_hp:getContentSize().width * (percent/100))

	if percent==100 then
		self.hp_white:setVisible(false)
	else
		self.hp_white:setVisible(true)
	end 
...
function SpineRenderer:doEnter(enter_effect ,die_effect)
	...
	if self.body_spine_name ~= nil and  self.body_spine_name ~= " " then
		...
		self.die_spine_name = die_effect
		self.enter_spine_name = enter_effect

		if action_name == "enter" and enter_effect ~=nil  then 
		  local	spine_enter_effect = createSpineByName(self.enter_spine_name) 
		  self.spine_root:addChild(spine_enter_effect, 6)

			if not tolua.isnull(spine_enter_effect) then
				spine_enter_effect:setAnimation(0, "animation", false)
				spine_enter_effect:setToSetupPose()
			end
			-- local function animationEffectCompleteFunc(event)
			-- 	if event.animation == action_name then 
			-- 		spine_enter_effect:removeFromParent()
			-- 	end 
			-- end
			-- spine_enter_effect:animationEffectCompleteFunc(animationCompleteFunc, sp.EventType.ANIMATION_COMPLETE)
		end
...
function SpineRenderer:createDieRoleEffect(spine_name)
	if spine_name == nil or spine_name == "" then return end
	if self.die_effect_path == spine_name then return end
	if tolua.isnull(self.spine_root) then return end

	self.die_effect_path = spine_name
	if self.roleDie_effect then
		self.roleDie_effect:removeFromParent()
		self.roleDie_effect = nil
	end
	self.roleDie_effect = createSpineByName(spine_name) 
	self.roleDie_effect:setAnimation(0, "animation", false)
	self.spine_root:addChild(self.roleDie_effect, 2)
...
function SpineRenderer:runAction(act)
	...
	print("-----------SpineRenderer:runAction------------------------",act )
	if not tolua.isnull(self.root) then
