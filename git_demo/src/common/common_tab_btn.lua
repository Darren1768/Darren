...
function CommonTabBtn:configUI(  )
    ...
    local csbPath = PathTool.getTargetCSB("common/common_tab_btn_ui")
    ...
    self.red_num = self.tab_tips:getChildByName("red_num")
    self.red_num:setVisible(false)

    self:setBtnSelectStatus(false)
    
    self.init_size = self.unselect_bg:getContentSize()
...
function CommonTabBtn:setExtendData( extend )
	if extend then
		...
		self.select_color = extend.select_color or cc.c4b(250,232,150,0xff)
		self.select_outline = extend.select_outline or self.select_outline
		self.normal_color = extend.normal_color or  cc.c4b(239,212,174,0xff)
		self.normal_outline = extend.normal_outline or self.normal_outline
		...
		if extend.tab_size then
		    ...
		    self.title:setPosition(cc.p(self.size.width/2+self.title_offset.x, self.size.height/2+self.title_offset.y + 3))
            ...
            local scale = self.size.width/self.init_size.width
            self.tab_tips:setScale(scale)
...
function CommonTabBtn:setData( data )
	...
	if data.isNotClick and data.isNotClick then
...
function CommonTabBtn:setBtnSelectStatus( status )
	if status then
        ...
        if self.select_outline then
            self.title:enableOutline(self.select_outline, 2)
        end
        ...
        if self.normal_outline then
            self.title:enableOutline(self.normal_outline, 2)
        end
	end
end
