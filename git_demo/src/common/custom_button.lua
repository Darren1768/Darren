...
-- ]]
function CustomButton:__init(parent, normal_res, press_res, disable_res, load_type)
...
--设置文本的按钮文字
function CustomButton:setBtnLabel(label)
   ...
   if self.btn_label == nil then
       ...
       self.btn_label = createTitleLabel(self.font_size,nil,nil,self:getSize().width/2, self:getSize().height/2+1,
          "",self.layout,nil, cc.p(0.5, 0.5),TITLE_FONT)
...
function CustomButton:setRichText(text,font_size,color,line_space,ap)
  ...
  if not self.rich_label then
    ...
    self.rich_label = createRichLabel(font_size, color,ap, cc.p(0,0), 5, 0,line_space or self:getSize().width,TITLE_FONT )
