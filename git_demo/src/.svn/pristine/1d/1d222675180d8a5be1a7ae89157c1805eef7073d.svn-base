...
function ActionFortuneBagDrawPanel:configUI()
    ...
    local bg_res = PathTool.getSingleImgForDownLoad("bigbg/action", str)
...
function ActionFortuneBagDrawPanel:register_event()
    ...
    EventManager:getInstance():Bind(
        ActionEvent.FORTUNE_BAG_DRAW_BASE_EVENT,self,
        function(data)
            if not data then
                return
            end
    ...
    EventManager:getInstance():Bind(
        BackpackEvent.MODIFY_GOODS_NUM,self,
        function(bag_code, data_list)
            self:updataItem(bag_code, data_list)
        end
    )
