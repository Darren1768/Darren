...
function TreasurePanel:loadResources()
    self.res_list = {
        ...
        {path = PathTool.getSingleImgForDownLoad("bigbg/festivalaction","txt_cn_festival_treasure"), type = ResourcesType.single},
    }
...
function TreasurePanel:configUI()
    ...
    local res = PathTool.getSingleImgForDownLoad("bigbg/festivalaction",str_bg)
...
function TreasurePanel:register_event()
    EventManager:getInstance():Bind(FestivalActionEvent.TreasureMessage,self,function(data)
    ...
    EventManager:getInstance():Bind(BackpackEvent.ADD_GOODS,self,function(bag_code,temp_list)
            self:changeGradeCount(temp_list)
        end)
    
    EventManager:getInstance():Bind(BackpackEvent.DELETE_GOODS,self,function(bag_code,temp_list)
            self:changeGradeCount(temp_list)
        end)
...
function TreasureItem:configUI()
	self.root_wnd = createCSBNote(PathTool.getTargetCSB("festivalaction/treasure_item"))
    self:addChild(self.root_wnd)
