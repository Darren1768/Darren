...
--      奖励一览的总界面
-- --------------------------------------------------------------------
UnionRewardWindow = UnionRewardWindow or BaseClass(BaseView)
local controller = UnionController:getInstance()
local string_format = string.format
function UnionRewardWindow:__init( ... )
   
    self.layout_name = "union/union_reward_window"
    self.win_type = WinType.Tips
    self.view_tag = ViewMgrTag.DIALOGUE_TAG
    self.item_scrollview = nil
end

function UnionRewardWindow:open_callback()
    ...
    local setting = {
        item_class = UnionRewardItem,      -- 单元类
        start_x = 4,                  -- 第一个单元的X起点
        space_x = 0,                    -- x方向的间隔
        start_y = 0,                    -- 第一个单元的Y起点
        space_y = 0,                   -- y方向的间隔
...
function UnionRewardWindow:register_event( ... )
    if self.background then
        self.background:addTouchEventListener(function(sender,event_type)
            if event_type == ccui.TouchEventType.ended then
                controller:openUnionRewardWindow(false)
            end
        end)
    end
...
function UnionRewardWindow:openRootWnd()
    local list = Config.UnionQuestData.data_lev_data
    self.item_scrollview:setData(list)
end

function UnionRewardWindow:close_callback()
...
--子项
UnionRewardItem = class("UnionRewardItem", function()
    return ccui.Widget:create()
end)

function UnionRewardItem:ctor()
    self.item_list = {}
    self:configUI()
end

function UnionRewardItem:configUI( )
    self.size = cc.size(599,159)
    self:setTouchEnabled(true)
    self:setContentSize(self.size)

    local csbPath = PathTool.getTargetCSB("union/union_reward_item")
    local root_wnd = createCSBNote(csbPath)
...
function UnionRewardItem:setData( data )
...
function UnionRewardItem:DeleteMe()
