...
function ArenaChampionCurGuessPanel:ctor(view_type)
    ...
    for i=1,2 do
        ...
        if tmp_container then
            ...
            object.Sprite_6 = tmp_container:getChildByName("Sprite_6")       -- 神器
            object.Sprite_6:setVisible(false)
            object.elfin_list = {}
            object.power = CommonNum.new(220, tmp_container, 99999, - 2, cc.p(0, 0.5))   -- 角色战力
            object.power:setScale(0.9)
            if i == 1 then
                object.power:setPosition(76, 376)
            else
                object.power:setPosition(73, 378)
            end
            object.role_head = PlayerHead.new(PlayerHead.type.circle)       -- 角色头像
            object.role_head:setScale(0.8)
            if i == 1 then
                object.role_head:setPosition(70, 450)
            else
                object.role_head:setPosition(70, 450)
            end
...
function ArenaChampionCurGuessPanel:handleEvent(status)
    if status == true then
     EventManager:getInstance():Bind(ArenaEvent.UpdateGuessMatchInfoEvent,self, function(data)
        ...
        --     end)
        -- end
     EventManager:getInstance():Bind(ArenaEvent.UpdateRoleInfoBetEvent,self, function(can_bet, bet_type) 
                can_bet = can_bet or 0
                self.guess_value:setString(can_bet)
                self:changeGuessBtnStatus(bet_type)
            end)
     EventManager:getInstance():Bind(ArenaEvent.UpdateBetMatchValueEvent,self, function(data) 
