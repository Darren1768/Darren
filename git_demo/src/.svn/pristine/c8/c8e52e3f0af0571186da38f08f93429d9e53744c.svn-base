...
function RoleController:registerEvents()
    ...
    --基础属性设置完毕
    EventManager:getInstance():Bind(RoleEvent.UPDATE_ROLE_BASE_ATTR,self,function ()
...
function RoleController:registerProtocals()
    ...
    -- 角色关系数据
    self:RegisterProtocal(29709, "handle29709")
    -- 赠送同心锁
    self:RegisterProtocal(29707, "handle29707")

    -- 角色关系buff数据初始化
    self:RegisterProtocal(29711, "handle29711")
    -- 角色关系buff数据领取
    self:RegisterProtocal(29712, "handle29712")
end
...
function RoleController:rgp_hander10301(data)
    ...
    if not self.init_role then
        ...
        -- 派发事件,角色登陆成功
        EventManager:getInstance():Fire(EventId.ROLE_CREATE_SUCCESS)
    ...
    if self.is_re_connect == true then
        ...
        EventManager:getInstance():Fire(LoginEvent.RE_LINK_GAME)
        self.is_re_connect = false
...
function RoleController:rgp_hander21500(data)
    EventManager:getInstance():Fire(RoleEvent.GetFaceList,data)
end
...
function RoleController:rgp_hander21501(data)
    -- EventManager:getInstance():Fire(RoleEvent.UseFaceItem,data)
end
...
function RoleController:rgp_hander12700(data)
    EventManager:getInstance():Fire(RoleEvent.GetBubbleList,data)
end
...
function RoleController:rgp_hander12701(data)
    EventManager:getInstance():Fire(RoleEvent.UseBubbleItem,data)
end
...
function RoleController:handle10328(data)
    EventManager:getInstance():Fire(RoleEvent.ROLE_BACKGROUND_LIST_EVENT,data)
end
...
--打开新的角色面板 个人空间
function RoleController:openRolePersonalSpacePanel( status, setting,notFriend)
    local setting = setting or {}
    local role_type = setting.role_type or RoleConst.role_type.eMySelf
    if notFriend==nil then
        notFriend=true
    end
    ...
    if role_type == RoleConst.role_type.eOther then
        ...
        if status == true then
            ...
            if self.role_personal_space_panel == nil then
                ...
                self.role_personal_space_panel.isNotFriend=notFriend
            end
...
-----------------------------------------------------------------新增关系数据(金兰/至交/情侣/夫妻)-start---------------------------------------
-----------------------------------------------角色关系数据回调--------------------------------
function RoleController:handle29709(data)
    print("===============================handle29709===============================")
    printLuaTable(data)
    self.model:updatePlayerRelationData(data)
    EventManager:getInstance():Fire(RoleEvent.UPDATE_PALYER_RELATION_EVENT)
end

--赠送同心锁
function RoleController:sender29707(rid,srv_id)
    local protocal = {}
    protocal.rid = rid
    protocal.srv_id = srv_id
    self:SendProtocal(29707, protocal)
end
--赠送同心锁回调
function RoleController:handle29707(data)
    print("=============================handle29707==================================")
    message(data.msg)
end
...
--===================关系buff====-----------------
function RoleController:sender29711()
    local protocal = {}

    self:SendProtocal(29711, protocal) 
end

function RoleController:handle29711(data)
    print("============handle29711==============")
    self.model:updateBuffData(data)
    
end

function RoleController:sender29712(buff_id)
    local protocal = {}
    protocal.buff_id = buff_id
    self:SendProtocal(29712, protocal) 
end

function RoleController:handle29712(data)
    print("============handle29712==============")
    if data.code ~= 0 then return end

    message(data.msg)
    self:sender29711()
    -- local buff_id = data.buff_id or 0
    -- local old_data = self.model:getBuffData()
    -- for i,v in ipairs(old_data)do
    --     if v.bid == buff_id then
    --         v.status = 2
    --     end
    -- end
    -- self.model:updateBuffData(old_data)
end
-----------------------------------------------------------------新增关系数据(金兰/至交/情侣/夫妻)-end---------------------------------------
-----------------------------------------------举报功能协议和打开方法-------------------------
...
function RoleController:handle12771(data)
    EventManager:getInstance():Fire(RoleEvent.ROLE_REPORTED_EVENT, data)
end
...
function RoleController:handle25800(data)
    message(data.msg)
    if data.code == TRUE then
        EventManager:getInstance():Fire(RoleEvent.ROLE_CITY_EVENT, data)
    end
end
...
function RoleController:handle25801(data)
    message(data.msg)
    if data.code == TRUE then
        EventManager:getInstance():Fire(RoleEvent.ROLE_FOLLOW_EVENT, data)
    end
end
...
function RoleController:handle25802(data)
    ...
    EventManager:getInstance():Fire(RoleEvent.ROLE_FANS_RANK_EVENT, data)
end
...
function RoleController:handle25805(data)
    EventManager:getInstance():Fire(RoleEvent.ROLE_UPDATE_HONOR_WALL_EVENT, data)
end
...
function RoleController:handle25806(data)
    EventManager:getInstance():Fire(RoleEvent.ROLE_GET_HONOR_WALL_EVENT, data)
end
...
function RoleController:handle25830(data)
    ...
    EventManager:getInstance():Fire(RoleEvent.ROLE_MYSELF_GROWTH_WAY_EVENT, data)
end
...
function RoleController:handle25832(data)
    ...
    EventManager:getInstance():Fire(RoleEvent.ROLE_OHTER_GROWTH_WAY_EVENT, data)
end
...
function RoleController:handle25836(data)
    message(data.msg)
    EventManager:getInstance():Fire(RoleEvent.ROLE_MESSAGE_BOARD_REPLY_EVENT, data)
end
...
function RoleController:handle25837(data)
    ...
    EventManager:getInstance():Fire(RoleEvent.ROLE_MESSAGE_BOARD_GET_INFO_EVENT, data)
end

--新增留言推送
function RoleController:handle25841(data)
    self.model:initMessageBoardData(data)
    EventManager:getInstance():Fire(RoleEvent.ROLE_MESSAGE_BOARD_NEW_INFO_EVENT, data)
end
...
function RoleController:handle25838(data)
    ...
    EventManager:getInstance():Fire(RoleEvent.ROLE_MESSAGE_BOARD_DELETE_INFO_EVENT, data)
end
...
function RoleController:handle25839(data)
    ...
    EventManager:getInstance():Fire(RoleEvent.ROLE_MESSAGE_BOARD_LIMMIT_EVENT, data)
end
...
function RoleController:rgp_hander10345( data )
    -- Debug.info(data)
    EventManager:getInstance():Fire(RoleEvent.GetModelList,data)
end
...
function RoleController:rgp_hander10346( data )
    ...
    message(data.msg)
    if data.code == 1 then
        EventManager:getInstance():Fire(RoleEvent.UpdateModel,data.id)
        self.model:setRoleAttribute("look_id", data.id)
    end
end
--激活形象
...
function RoleController:rgp_hander10347( data )
    ...
    message(data.msg)
    if data.code == 1 then
        EventManager:getInstance():Fire(RoleEvent.ActiveModel,data.id)
    end
end
...
function RoleController:rgp_hander10315( data )
    if self.setting_10315 then 
        ...
        EventManager:getInstance():Fire(RoleEvent.DISPATCH_PLAYER_VO_EVENT,data)
    end
end
...
function RoleController:rgp_hander10316(data)
    message(data.msg)
    if data.code == TRUE then
        EventManager:getInstance():Fire(RoleEvent.WorshipOtherRole, data.rid, data.srv_id, data.idx, data.type)
    end
end
...
function RoleController:hander23300( data )
    ...
    EventManager:getInstance():Fire(RoleEvent.GetTitleList,data)
end
...
function RoleController:hander23301( data )
    self.use_title_id = data.base_id
    EventManager:getInstance():Fire(RoleEvent.UseTitle,data.base_id)
end
...
function RoleController:hander23302( data )
    --self.use_title_id = data.base_id
    EventManager:getInstance():Fire(RoleEvent.UpdataTitleList,data)
end
...
function RoleController:rgp_hander10317(data)
    ...
    if role_vo ~= nil then
        ...
        EventManager:getInstance():Fire(RoleEvent.UpdateWorshipEvent,data.worship)
    end
end
...
function RoleController:rgp_hander10319( data )
    ...
    if role_vo ~= nil then
        ...
        EventManager:getInstance():Fire(RoleEvent.UpdateCrossChamWorshipEvent,data.worship)
    end
end
...
function RoleController:handle10994(data)
    if data.type == 0 then
        ...
        SpiritController:getInstance():sender11320()
    ...
    elseif data.type == 6 or data.type == 18 then   -- 这个时候需要切换一下主城资源了
        ...
        BattleDramaController:getInstance():changeBattleTimeType(data.type)
    end
end
...
function RoleController:handle10905(data)
    ...
    EventManager:getInstance():Fire(RoleEvent.WORLD_LEV, data.world_lev)
end
...
function RoleController:handle10906( data )
    ...
    EventManager:getInstance():Fire(RoleEvent.OPEN_SRV_DAY, data.open_day)
...
function RoleController:handle10960(data)
    EventManager:getInstance():Fire(RoleEvent.ROLE_NAME_AUTHENTIC, data)
end
...
function RoleController:handle10985(data)
    ...
    EventManager:getInstance():Fire(RoleEvent.UPDATE_RED_POINT)
...
--检查入口是否开启
function RoleController:checkEnterIsLock(bid)
    local role_vo = self:getRoleVo()
    local config = Config.CityData.data_base[bid]
    local is_lock = false
    local desc = ''
