...
--
-- --------------------------------------------------------------------
GuildCreateWindow = GuildCreateWindow or BaseClass(BaseView)

local controller = GuildController:getInstance()
local model = GuildController:getInstance():getModel()
local string_format = string.format

function GuildCreateWindow:__init()
	self.view_tag = ViewMgrTag.DIALOGUE_TAG
	
	self.win_type = WinType.Mini
	self.set_index = 1
	self.condition_index = 1
	self.layout_name = "guild/guild_create_window"
    self.limit_title_count=6

    self.limit_sad_count=32
    self.content_str=""
end 

function GuildCreateWindow:open_callback()
    ...
    self.title_label_content = createRichLabel(22, cc.c4b(129, 104,65,0xff), cc.p(0,1), cc.p(36, 270), 6, nil, 360,DEFAULT_FONT)
    self.title_label_content:setString( TI18N("请输入公会名字(2~6个字)"))
    container:addChild(self.title_label_content)



    local res = PathTool.getResFrame("common", "common_99998")
    self.edit_title = createEditBox(container, res, cc.size(490, 44), nil, 22,cc.c3b(255,255,255), 22, TI18N(""), nil, nil, LOADTEXT_TYPE_PLIST)
    ...
    self.edit_title:setPosition(cc.p(36, 256))
    ...
    local res = PathTool.getResFrame("common","common_99998")
    self.label_content = createRichLabel(22, cc.c4b(129, 104,65,0xff), cc.p(0,1), cc.p(36, 220), 6, nil, 480,DEFAULT_FONT)
    self.label_content:setString( TI18N("请输入公会宣言内容"))
    ...
    self.declaration_value:setPosition(cc.p(36, 220))
    ...
    if role_vo.vip_lev < lev.val then
        ...
        self.create_btn_label:setString(string.format("<img src=%s visible=true scale=0.3 /><div outline=2,#b27f3a>%s %s</div>", PathTool.getItemRes(item_icon), gold_num, TI18N("创建公会")))
...
function GuildCreateWindow:setLabelContent(str)
...
function GuildCreateWindow:setLabelTitleContent(str)
    if not str then return end
...
function GuildCreateWindow:register_event()
    self.create_btn:addTouchEventListener(function(sender, event_type)
        ...
        if event_type == ccui.TouchEventType.ended then
            ...
            local guild_name =  self.title_label_content:getString()
            ...
            elseif StringUtil.getStrLen(guild_name) > 12 then
                ...
                controller:requestCreateGuild(guild_name, sign, self.set_index, 1, 0) 
            end
        end
    end) 

	registerButtonEventListener(self.background, function()
        controller:openGuildCreatWindow(false)
    end, true, 2)
...
function GuildCreateWindow:openRootWnd()
end

function GuildCreateWindow:close_callback()
    controller:openGuildCreatWindow(false) 
end