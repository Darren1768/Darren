...
function Endless_trailController:checkNewEndLessIsOpen(show_desc)
    ...
    -- local open_lev = Config.EndlessData.data_const.endless_new_limit_lev_s
    -- if open_lev == nil then return false end
...
function Endless_trailController:handle23908(data)
    message(data.msg)
    if data.code == 1 then
        EventManager:getInstance():Fire(Endless_trailEvent.UPDATA_SENDPARTNER_SUCESS_DATA ,data)
    end
end
...
function Endless_trailController:openEndlessFirstRewardWindow(status,type)
    if status == true then
        if not self.endless_first_reward_window then
            self.endless_first_reward_window = EndlessFirstRewardWindow.New(type)
        end
        if self.endless_first_reward_window and self.endless_first_reward_window:isOpen() == false then
            self.endless_first_reward_window:open(type)
        end
    else
        if self.endless_first_reward_window then
            self.endless_first_reward_window:close()
            self.endless_first_reward_window = nil
        end
    end
end
