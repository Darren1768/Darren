...
function GuildMemberWindow:register_event()
    ...
    EventManager:getInstance():Bind(GuildEvent.UpdateMyMemberListEvent,self, function(type)
    ...
    EventManager:getInstance():Bind(GuildEvent.UpdateAssistantNumEvent,self, function() 
    ...
    EventManager:getInstance():Bind(GuildEvent.UpdateGuildRedStatus,self, function(red_type, status)
...
function GuildMemberWindow:updateMemberList(type)
    ...
    if self.show_menber_list ~= nil and next(self.show_menber_list) ~= nil then
        if self.scroll_view == nil then
            local list_size = cc.size(self.scroll_container:getContentSize().width,self.scroll_container:getContentSize().height)
            local list_setting = {
