...
function ItemExhibitionView:__init(extend, open_type, ref_class)
    ...
    self.res_list = {
        ...
        {path = PathTool.getPlistImgForDownLoad("bigbg", "hl_bigbg_31",false), type = ResourcesType.single},
    }
end
...
function ItemExhibitionView:open_callback()
    ...
    -- self.back_panel:setPosition(cc.p(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 -70))
    -- self.back_panel:setContentSize(cc.size(SCREEN_WIDTH,1513))
    -- self.back_panel:setCapInsets(cc.rect(0, 600, 0,900))

    self.image_top_bg = createScale9Sprite(PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_31"), SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 -70 , LOADTEXT_TYPE, main_container)
    ...
    self.title_container:setPositionY( SCREEN_HEIGHT / 2 +250)
    self.title_container:setZOrder(1)
    ...
    self.scroll_view:setPositionY(480)
...
function ItemExhibitionView:finalClose()
    ...
    self.render_list = {}
    EventManager:getInstance():Fire(MainuiEvent.CLOSE_ITEM_VIEW,self.extend)
