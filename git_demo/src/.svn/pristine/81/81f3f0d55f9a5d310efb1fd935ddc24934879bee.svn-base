...
function ReturnActionSummonPanel:updateImageBg(  )
	...
	if period and (not self.cur_res_id or self.cur_res_id ~= period) then
		local bg_res = PathTool.getSingleImgForDownLoad("bigbg/returnaction", string.format("txt_cn_return_summon_%d", period))
...
function ReturnActionSummonPanel:register_event(  )
	...
	-- 召唤数据更新
     EventManager:getInstance():Bind(ReturnActionEvent.Summon_Data_Event,self,function ()
            self:setData()
        end)
