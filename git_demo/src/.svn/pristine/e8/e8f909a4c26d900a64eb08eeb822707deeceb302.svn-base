...
function GuildvoyageOrderEscortWindow:register_event()
	...
	-- end)
     ...
     EventManager:getInstance():Bind(BackpackEvent.MODIFY_GOODS_NUM,self, function(bag_code, list)
            if bag_code ~= BackPackConst.Bag_Code.BACKPACK then return end
            self:updateTreasureList(list)
        end)

    -- 只有在准备护送的时候才监听物品更新时间,护送过程中不产生影响
     EventManager:getInstance():Bind(BackpackEvent.ADD_GOODS,self, function(bag_code, list)
            if bag_code ~= BackPackConst.Bag_Code.BACKPACK then return end
            self:updateTreasureList(list)
        end)

     EventManager:getInstance():Bind(GuildvoyageEvent.AddToOrderPartnerListEvent,self, function(data) 
