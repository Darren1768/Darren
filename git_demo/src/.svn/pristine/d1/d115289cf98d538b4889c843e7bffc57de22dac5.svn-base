...
function ActionCrossServerRankWindow:open_callback()
    ...
    if self.background ~= nil then
    	self.background:loadTexture(PathTool.getSingleImgForDownLoad("bigbg/welfare","welfare_bg"), LOADTEXT_TYPE)
...
function ActionCrossServerRankWindow:register_event(  )
	...
	--获取七天排行列表
     EventManager:getInstance():Bind(ActionEvent.UPDATE_CROSSSERVER_RANK_LIST,self,function ( data )
			self:createTabBtn(data.rank_list)
		end)

	--获取单个排行榜信息
     EventManager:getInstance():Bind(ActionEvent.UPDATE_CROSSSERVER_RANK_DATA,self,function ( data )
     ...
     EventManager:getInstance():Bind(ActionEvent.UPDATE_CROSSSERVER_QUEST,self, function() 
			self:fileSevenRankInfo()
		end)
end
