...
-- Date: 
--
BattleNationalSpineRenderer = BattleNationalSpineRenderer or BaseClass()
local size = cc.Director:getInstance():getVisibleSize()
local table_insert = table.insert
local table_remove = table.remove

--@ spine_scale2 仅模型的缩放比例 (替换大资源时间段用) 
function BattleNationalSpineRenderer:__init(screen_pos, spine_name,anima_name, scale, career, group, spine_type, spine_scale2)
...
function BattleNationalSpineRenderer:showSpineModel(visible)
...
function BattleNationalSpineRenderer:hideSpineModel()
	-- self.spine_root:setVisible(visible)
...
function BattleNationalSpineRenderer:setVisible(visible)
	self.root:setVisible(visible)
end


function BattleNationalSpineRenderer:setPosByGrid(pos)
	local pos = pos
...
function BattleNationalSpineRenderer:getPosition()

	return cc.p(self.root:getPosition())
end

function BattleNationalSpineRenderer:doRun()
...
--统一创建模型了,方便做管理
function BattleNationalSpineRenderer:createSpine(spine_name,action_name,is_loop)
...
-- 创建头顶修灵
function BattleNationalSpineRenderer:createXiuLing()
...
function BattleNationalSpineRenderer:createrSpineBtn(callback)
...
function BattleNationalSpineRenderer:removeBody()
...
function BattleNationalSpineRenderer:addToLayer(layer, zOrder)
...
function BattleNationalSpineRenderer:setScale(scale, time1, time2, time3)
	-- body
...
function BattleNationalSpineRenderer:release(bool, type)
...
function BattleNationalSpineRenderer:realrelease(bool)
	if bool then
...
-- 设灰度
function BattleNationalSpineRenderer:setBlack(enable)
...
function BattleNationalSpineRenderer:getSpineName()
	return self.spine_name
end

function BattleNationalSpineRenderer:addChild(...)
	if not tolua.isnull(self.spine_root) and self.spine_root then
		self.spine_root:addChild(...)
	end
end

function BattleNationalSpineRenderer:setActionName(action_name, action_res)
	self.action_name = action_name
	self.action_res = (action_res == nil) and action_name or action_res;
end

function BattleNationalSpineRenderer:getActionName()
	return self.action_name
end 

function BattleNationalSpineRenderer:getActionRes()
...
--@return 
--==============================--
function BattleNationalSpineRenderer:runAction(act)
	if act == nil then return end
	print("-----------BattleNationalSpineRenderer:runAction------------------------",act )
	if not tolua.isnull(self.root) then
		self.root:runAction(act) 
	end
end

function BattleNationalSpineRenderer:__delete()
