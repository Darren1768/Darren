...
function AdventureActivityWindow:__init()
	...
	self.res_list = {
		...
		{path = PathTool.getSingleImgForDownLoad("bigbg","bigbg_83", true), type = ResourcesType.single}
	}
...
function AdventureActivityWindow:open_callback()
	...
	if self.background then
		self.background:loadTexture(PathTool.getSingleImgForDownLoad("bigbg","bigbg_83",true), LOADTEXT_TYPE)
...
function AdventureActivityItem:configUI()
    self.root_wnd = createCSBNote(PathTool.getTargetCSB("adventureactivity/adventureactivity_item"))
...
function AdventureActivityItem:setData(data)
	...
	-- 背景
	local bg_res = PathTool.getSingleImgForDownLoad("bigbg/adventrueactivity", string.format("txt_cn_adventrueactivity_%d", data.res_id))
