...
function AuguryWindow:__init()
    ...
    self.res_list = {
        ...
        { path = PathTool.getSingleImgForDownLoad("bigbg","bigbg_30",true), type = ResourcesType.single },
        { path = PathTool.getSingleImgForDownLoad("bigbg","bigbg_31"), type = ResourcesType.single },
    }
    self.tab_list = {}
...
function AuguryWindow:open_callback()
    ...
    if self.background ~= nil then
        self.background:loadTexture(PathTool.getSingleImgForDownLoad("bigbg","bigbg_30",true), LOADTEXT_TYPE)
...
function AuguryWindow:register_event()
    ...
    self.flash_btn:addTouchEventListener(function(sender, event_type) 
        ...
        if event_type == ccui.TouchEventType.ended then
            playCommonButtonSound()
            local now_tower = SpiritController:getInstance():getModel():getNowTowerId() or 0
    ...
    for k, tab in pairs(self.tab_list) do
     ...
     EventManager:getInstance():Bind(AuguryEvent.Augury_Success_Event,self,function(data)
            self.success_data =data
            self:playActionEffect(data)
        end)

     EventManager:getInstance():Bind(AuguryEvent.Update_Event,self,function()
            --当前运势
