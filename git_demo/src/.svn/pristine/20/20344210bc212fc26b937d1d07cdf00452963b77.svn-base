...
function CrossarenaHonourPanel:createRoorWnd(  )
    ...
    if not self.check_award_txt then
        self.check_award_txt = createRichLabel(26, 1, cc.p(0.5, 0.5), cc.p(558, 308))
        self.check_award_txt:setString(string.format(TI18N("<img src='%s' scale=1 /><div href=xxx fontcolor=#F7E19C>奖励预览</div>"), PathTool.getResFrame("common", "common_1093")))
...
function CrossarenaHonourPanel:registerEvent(  )
    ...
    registerButtonEventListener(self.tab_btn, function (  )
            ...
            if crossarena_main_wnd then
                crossarena_main_wnd:changeSelectedTab(CrossarenaConst.Sub_Type.Challenge)
            end
        end, true)
    -- 个人信息
    ...
    -- 赛季荣耀数据
    EventManager:getInstance():Bind(CrossarenaEvent.Update_Honour_Data_Event,self, function ( )
