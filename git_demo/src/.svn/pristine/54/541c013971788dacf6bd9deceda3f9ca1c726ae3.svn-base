...
function ActionModel:setOpenFundIds( id_list )
    ...
    EventManager:getInstance():Fire(ActionEvent.UPDATA_WELFARE_FUND__STATUS_EVENT, self.welfare_fund_id_list)
...
--     --EventManager:getInstance():Fire(ActionEvent.UPDATA_FUND_RED_STATUS_EVENT, bid, status)
-- end
...
function ActionModel:setGiftRedStatus(data)
    ...
    if list and list.status ~= status then
        list.status = status
        EventManager:getInstance():Fire(ActionEvent.SHOW_ACTIVITY_RED_POINT, bid, status)
    end
end
------------------------------------------
--神装道具商店
...
function ActionModel:setPerferPrizeByJsonObj(jsonObj)
    ...
    if data.coupons then
        ...
        EventManager:getInstance():Fire(ActionEvent.ACTION_PERFER_ISOPEN, status)
    end
    EventManager:getInstance():Fire(ActionEvent.ACTION_PERFER_GET_DATA_EVENT)
end
