...
function VoiceMgr:setMsg(voice_name, voice_msg)
	...
	EventManager:getInstance():Fire(VoiceMgr.NewMsg, voice_name, voice_msg)
end
...
function VoiceMgr:startPlay(voice)
    ...
    delayOnce(function()
        ...
        GlobalAudio:getInstance():playChatSound(voice.file) 
    end, 0.2)
end
...
function VoiceMgr:loadFile(voice_url)
    ...
    if not PathTool.checkSound(file_name) then
        ...
        GlobalAudio:getInstance():playChatSound(file_name) 
    end
end
...
function VoiceMgr:setPlaying(bool)
	if self.playing ~= bool then
		self.playing = bool
		EventManager:getInstance():Fire(VoiceMgr.Played, bool)
		if bool then
			GlobalAudio:getInstance():pauseMusic()
		else
            delayOnce(function()
                if self.playing then return end
                GlobalAudio:getInstance():resumeMusic()
            end, 1)
		end
	end
end
