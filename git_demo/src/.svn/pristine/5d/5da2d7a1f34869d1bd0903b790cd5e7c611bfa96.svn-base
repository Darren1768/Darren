...
-- --------------------------------------------------------------------
SpiritVideoWindow = SpiritVideoWindow or BaseClass(BaseView)

function SpiritVideoWindow:__init(data,tower)
    self.ctrl = SpiritController:getInstance()
    
    self.layout_name = "spirit/spirit_video"
    self.res_list = {
...
function SpiritVideoWindow:open_callback()
...
function SpiritVideoWindow:register_event()
...
function SpiritVideoWindow:showSharePanel(bool)
    ...
    if not self.share_panel then 
        ...
        local btn1 =  createTypeButton(self.share_panel, TI18N("世界频道"), size.width/2, 120, ButtonType.yellow)
        btn1:enableOutline(Config.ColorData.data_color4[264],2)
        btn1:addTouchEventListener(function(sender, event_type) 
            if event_type == ccui.TouchEventType.ended then
                if not self.data then return end
                SpiritController:sender11333(self.data.m_replay_id,1,self.click_tower)
        ...
        btn2:addTouchEventListener(function(sender, event_type) 
            if event_type == ccui.TouchEventType.ended then
                if not self.data then return end
                SpiritController:sender11333(self.data.m_replay_id,4,self.click_tower)
                self:showSharePanel(false)
            end
        end)
    end
...
function SpiritVideoWindow:createDesc()
...
function SpiritVideoWindow:openRootWnd()
end
...
function SpiritVideoWindow:setPanelData()
end

function SpiritVideoWindow:close_callback()
    self.ctrl:openVideoWindow(false)
