...
function BattleDramaModel:setBuffData(data)
    self.buff_data = data
    EventManager:getInstance():Fire(Battle_dramaEvent.BattleDrama_Drama_Buff_View ,data)
end
...
function BattleDramaModel:setDramaData(data)
    if data then
        ...
        self:updateDunList(data,data.mode,data.chapter_id,data.map_id)

        if need_event == true then
            EventManager:getInstance():Fire(Battle_dramaEvent.BattleDrama_Update_Max_Id, data.max_dun_id) 
        end
        EventManager:getInstance():Fire(Battle_dramaEvent.BattleDrama_Top_Update_Data,data)
    end
end
...
function BattleDramaModel:updateDramaData(data)
    if data and self.drama_data then
        ...
        if self.drama_data and self.drama_data.map_id ~= data.map_id then
            EventManager:getInstance():Fire(Battle_dramaEvent.BattleDrama_Update_Dun_Id, data.map_id)
        end
        self.drama_data.mode = data.mode
        ...
        self.drama_data.map_id = data.map_id

        print("--------------updateDramaData-----------")
        printLuaTable(self.drama_data)
        self:updateModeListInfo(data)
        self:updateDunList(data,data.mode,data.chapter_id,data.map_id)
        EventManager:getInstance():Fire(Battle_dramaEvent.BattleDrama_Update_Data, data)
...
function BattleDramaModel:updateDunList(data,mode,chapter_id,map_id)

    self.cur_dun_list = {}
    if not data or not data.mode_list then return end
    for i, v in ipairs(data.mode_list) do
        if v.mode == mode then
            for k, v2 in ipairs(v.chapter_list) do
                if v2.chapter_id == chapter_id then
                    for j, v3 in ipairs(v2.map_list) do
                        if v3.map_id == map_id then
                            for l, v4 in ipairs(v3.dun_list) do
                                self.cur_dun_list[v4.dun_id] = v4
                            end
                        end
                    end
                end
...
--用于初始化当前显示关卡的信息展示
function BattleDramaModel:initDungeonList(mode,chapter_id,map_id)
    if Config.DungeonData.data_drama_info then
        ...
        self:updateHadModeListInfo(mode,chapter_id,map_id)
    end
end
...
--获取初始化当前章节和难道所有副本信息
function BattleDramaModel:updateHadModeListInfo(mode,chapter_id,map_id)
    if self.mode_list and next(self.mode_list or {}) ~= nil then
        local chapter_list = self:getChapterListByID(mode,chapter_id,map_id)
        if chapter_list then
...
function BattleDramaModel:getChapterListByID(mode,chapter_id,map_id)
        ...
        if chapter_list then
            for i, v in ipairs(chapter_list) do
                if v.chapter_id == chapter_id then
                    for j, v2 in ipairs(v.map_list) do 
                        if map_id~=nil and v2.map_id == map_id then
                            list = v2.dun_list
                            return list
                        end
                        if list == nil then
                            list = v2.dun_list
                        elseif #v2.dun_list > 0 then
                            for index,value in ipairs(v2.dun_list) do
...
function BattleDramaModel:updateModeListInfo(data)
    if self.mode_list and next(self.mode_list or {}) ~= nil then
        local chapter_list = self:getChapterListByID(data.mode,data.chapter_id,data.map_id)
        if chapter_list then
            local dun_list = chapter_list.dun_list
            if dun_list then
                local temp_has = true
...
function BattleDramaModel:getCurDunInfoByMapID(map_id)
    if self.cur_dun_list then

        local dun_id = Config.DungeonData.data_drama_dungeon_getmap_fun(map_id).checkpoint_id[1][1]
...
function BattleDramaModel:updateCurDunListInfo(data)
    if self.cur_dun_list and data then
        ...
        EventManager:getInstance():Fire(Battle_dramaEvent.BattleDrama_Boss_Update_Data,config)
    end
end

function BattleDramaModel:updateSingleCurInfo(data)
    if self.mode_list and next(self.mode_list or {}) ~= nil then
        local chapter_list = self:getChapterListByID(self.drama_data.mode,self.drama_data.chapter_id,self.drama_data.map_id)
...
--根据难度和章节id去获取已通过的关卡数
--map_id==nil，获取全章已通关卡
function BattleDramaModel:getHasCurChapterPassListNum(mode,chapter_id,map_id)
    if self.mode_list and next(self.mode_list or {}) ~= nil then
        local dun_list = self:getChapterListByID(mode,chapter_id,map_id)
        local sum = 0
        if dun_list then
            for i, v in ipairs(dun_list) do
                if v.status == 3 then
...
function BattleDramaModel:getHasCurChapterPassListBossNum(mode, chapter_id)
    if self.mode_list and next(self.mode_list or {}) ~= nil then
        local dun_list = self:getChapterListByID(mode, chapter_id)
        local sum = 1
        if dun_list then
            for i, v in ipairs(dun_list) do
...
function BattleDramaModel:setQuickData(data)
    ...
    self:checkRedPoint()
    EventManager:getInstance():Fire(Battle_dramaEvent.BattleDrama_Quick_Battle_Data, data)
end
...
function BattleDramaModel:setDramaReward(data)
    ...
    self:checkRedPoint()
    EventManager:getInstance():Fire(Battle_dramaEvent.BattleDrama_Drama_Reward_Data,data)
end
...
function BattleDramaModel:setPartnerUpgradeRedStatus(data)
    ...
    --     end
    -- end
    -- EventManager:getInstance():Fire(Battle_dramaEvent.UpdatePartnerRedStatus)
end
...
function BattleDramaModel:updateHookAccumulateTime(data)
    ...
    EventManager:getInstance():Fire(Battle_dramaEvent.UpdateHookAccumulateTime, self.hook_accumulate_data)
end
...
function BattleDramaModel:calcHookItemsReachTxt(data)
    local _lev =OrganizeController:getInstance():getModel():getMyTeamLev() or 1
    
    local str = ""
    if data and data.per_hook_items then
        local per_hook_items = self:calcHookItems(data)
        for i, v in ipairs(per_hook_items) do
            local config = Config.ItemData.data_get_data(v[1])
            str = str .. string.format(TI18N("<img src=%s visible=true scale=0.30 /><div>%d</div>      "), PathTool.getItemRes(config.icon),tonumber(v[2]) *tonumber(_lev))
