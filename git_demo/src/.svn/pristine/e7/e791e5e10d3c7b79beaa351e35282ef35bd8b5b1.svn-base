...
--      转盘活动
-- --------------------------------------------------------------------
BattleNationalWarModel = BattleNationalWarModel or BaseClass()
local _tolua_isnull = tolua.isnull
function BattleNationalWarModel:__init(ctrl)
    self.ctrl = ctrl
    self:config()
end

function BattleNationalWarModel:config()
	self.all_object = {}
	self.battle_national_scene = nil 
...
--创建战斗人物
--@param data:战斗人物列表
function BattleNationalWarModel:createBattleRole(data)
	data ={}
	data.objects ={}
	for i=1,4 do 
		data.objects[i] ={}
...
--添加/复活角色
--param data:角色信息数据
function BattleNationalWarModel:addBattleRole(data,count)
	    local  screen_pos =cc.p(100,100)
		local  spine_name = "H103030"
		local  anima_name = "stand"
		local role = BattleNationalSpineRenderer.New(screen_pos,spine_name,anima_name)

		self.battle_national_scene = self.ctrl:getCurrentScene()

		if self.battle_national_scene then 
			self.battle_national_scene:addRoleToLayer1(role)
		end 
		...
		if count and count ==self.roleCnt  then
			print("-------------------------addRoleToLayer-----------------",self.battle_national_scene._layer_index)
...
--定时器开启
function BattleNationalWarModel:spineMovescheduleUpdate()
    ...
    if self.battle_national_scene.map_layer and not _tolua_isnull(self.battle_national_scene.map_layer) then
        self.battle_national_scene.map_layer:scheduleUpdateWithPriorityLua(su, 0)
    end
end

--定时器关闭
function BattleNationalWarModel:unSpineMovescheduleUpdate()
    if not self.isStartUpdate then return end
    self.isStartUpdate = false
	print("-------------unscheduleUpdate----1--------")
    if  self.battle_national_scene.map_layer and not _tolua_isnull( self.battle_national_scene.map_layer) then
		print("-------------unscheduleUpdate------2------")
		self.battle_national_scene.map_layer:unscheduleUpdate()
    end
end

--每一帧更新
function BattleNationalWarModel:spineUpdate()
	if  not  self.isStartUpdate then return end 
...
--创建战斗人物
--@param data:战斗人物列表
function BattleNationalWarModel:createName(data)
	data ={}
	data.objects ={}
	for i=1,9 do 
		data[1]={"名字"}
...
function BattleNationalWarModel:__delete()
end