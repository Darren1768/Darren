...
-- --------------------------------------------------------------------
SpiritMainWindow = SpiritMainWindow or BaseClass(BaseView)
local table_insert = table.insert
function SpiritMainWindow:__init(data)
    self.ctrl = SpiritController:getInstance()
    
    self.layout_name = "spirit/spirit_main"
    self.cur_type = 0
    self.res_list = {
        { path = PathTool.getPlistImgForDownLoad("startower","startower"), type = ResourcesType.plist },
        -- { path = PathTool.getSingleImgForDownLoad("bigbg","bigbg_26"), type = ResourcesType.single },
    }
   self.data = data
...
function SpiritMainWindow:open_callback()
...
--改变扫荡按钮的状态
function SpiritMainWindow:changeClearTitleText()
    --已购买次数
...
function SpiritMainWindow:updateModel(data)
...
function SpiritMainWindow:register_event()
    self.close_btn:addTouchEventListener(function(sender, event_type) 
        if event_type == ccui.TouchEventType.ended then
            playQuitSound()
            self.ctrl:openSpiritMainView(false)
        end
    end)
    ...
    self.fight_btn:addTouchEventListener(function(sender, event_type) 
        if event_type == ccui.TouchEventType.ended then
            ...
            HeroController:getInstance():openFormGoFightPanel(true, PartnerConst.Fun_Form.Spirit, {tower_lev = tower})
        end
    end)
    ...
    self.video_btn:addTouchEventListener(function(sender, event_type) 
     ...
     EventManager:getInstance():Bind(SpiritEvent.Video_Data_Event,self,function(data)
            self.video_data = data
            self:updateVideoData(data)
        end)

    if not self.fight_success_event then 
        self.fight_success_event =  EventManager:getInstance():Bind(SpiritEvent.Fight_Success_Event,self,function()
            --挑战完成请求一下录像，可能自己破记录了
            if self.data then
                self.ctrl:sender11325(self.data.lev)
            end
    ...
    if not self.clear_title_change_event then 
        self.clear_title_change_event =  EventManager:getInstance():Bind(SpiritEvent.Count_Change_Event,self,function( )
            self:changeClearTitleText()
        end)
    end
end
--次数不足弹开购买次数界面
function SpiritMainWindow:openBuyCountPanel()
...
function SpiritMainWindow:updateVideoData(data)
    if not data then return end
...
function SpiritMainWindow:updateDesc()
    if not self.data then return end
    --boss描述
...
function SpiritMainWindow:openRootWnd()
end

--更新物品消耗
function SpiritMainWindow:updateGoodsList()
    if not self.data then return end
...
function SpiritMainWindow:setPanelData()
end

function SpiritMainWindow:close_callback()
    self.ctrl:openSpiritMainView(false)
