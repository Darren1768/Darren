...
function StoryModel:setCurStory(data_list)
	...
	if self.cur_story and next(self.cur_story.act) ~= nil then
        -- print("----------------------- self.cur_story--------------")
        -- printLuaTable(self.cur_story.act)
		self.cur_act_list = self.cur_story.act 
		EventManager:getInstance():Fire(StoryEvent.READ_CONFIG_COMPLETE)
    else        
...
function StoryModel:setStoryState(bool)
    ...
    --如果剧情已经结束，则判断下一个剧情
    if bool == false then
        EventManager:getInstance():Fire(StoryEvent.STORY_OVER)
