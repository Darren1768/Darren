...
function HonorYuekaPanel:loadResources()
    self.res_list = {
        { path = PathTool.getSingleImgForDownLoad("bigbg/welfare","txt_cn_welfare_yueka_bg2"), type = ResourcesType.single },
    } 
...
function HonorYuekaPanel:configUI()
	...
	local res_id = PathTool.getSingleImgForDownLoad("bigbg/welfare", "txt_cn_welfare_yueka_bg2")
...
function HonorYuekaPanel:register_event()
	EventManager:getInstance():Bind(WelfareEvent.Update_Yueka,self, function(data)
	...
	EventManager:getInstance():Bind(WelfareEvent.Update_Get_Yueka,self, function(_type)
		if _type == 2 then
			setChildUnEnabled(true, self.btn_get)
			self.btn_get_label:disableEffect(cc.LabelEffect.OUTLINE)
			self.btn_get_label:setString(TI18N("已领取"))
