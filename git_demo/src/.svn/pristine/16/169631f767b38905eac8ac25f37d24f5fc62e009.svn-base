...
function SkillTips:open_callback( ... )
    ...
    self.skill_item = SkillItem.new(false,false,false,nil,nil,false,true)
    self.skill_item:setPosition(72, 67)
    ...
    self.line_2= self.container:getChildByName("line_2")
end
...
function SkillTips:updateVo( skill_vo, is_lock, not_show_next, hide_flag, fire_cd, hallows_atk_val )
    ...
    if self.skill_vo.des ~= "" then
        if self.skill_desc == nil then
            self.skill_desc = createRichLabel(20,cc.c4b(239,212,174,0xff),cc.p(0,1),cc.p(22,10),4,nil,360)
    ...
    if self.extend_desc == nil then
        self.extend_desc = createRichLabel(20,cc.c4b(189,180,199,0xff),cc.p(0,1),cc.p(22,10),nil,nil,360)
    ...
    if not hide_flag then
        ...
        if self.skill_vo.cd == 0 then
            ...
            extend_str = string_format("<div fontcolor=#bdb4c7>%s</div><div fontcolor=#bdb4c7>%s</div><div fontcolor=#bdb4c7>%s</div>", TI18N("冷却"),self.skill_vo.cd, TI18N("回合") )
        end
        ...
        if fire_cd ~= 0 then
            extend_str = extend_str..string_format("<div fontcolor=#bdb4c7>%s</div><div fontcolor=#bdb4c7>%s</div><div fontcolor=#bdb4c7>%s</div>", TI18N("，第"), fire_cd, TI18N("回合释放")) 
        end
        ...
        for i, v in ipairs(self.skill_vo.buff_des[1]) do
            ...
            if config then
                if buff_desc_str ~= "" then
                    buff_desc_str = buff_desc_str.."<div fontcolor=#efd4ae>\n</div>"
                end
                local buff_desc = string_format("<div fontcolor=#efd4ae>[%s]</div><div fontcolor=#bdb4c7>\n%s</div>", config.name, config.desc)
    ...
    if self.skill_vo.open_desc ~= "" and not not_show_next and not hide_flag then
        ...
        --     -- self.line_2 = createSprite(PathTool.getResFrame("common","hl_common_586"), self.container_init_size.width/2, 0, self.container, cc.p(0.5, 1), LOADTEXT_TYPE_PLIST)
        ...
        if self.next_skill_txt == nil then
            self.next_skill_txt = createRichLabel(20,cc.c4b(211,112,52,0xff),cc.p(0,1),cc.p(22,10),4,nil,370)
...
function SkillTips:updatePosition()
    ...
    self.skill_name:setPositionY(self.max_height-55)
