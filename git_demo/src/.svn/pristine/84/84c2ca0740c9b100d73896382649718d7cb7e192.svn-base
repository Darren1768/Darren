...
function ElitematchMatchingWindow:__init()
    ...
    self.res_list = {
        ...
        -- {path = PathTool.getSingleImgForDownLoad("bigbg","bigbg_66",true), type = ResourcesType.single }
        {path = PathTool.getSingleImgForDownLoad("bigbg/elitematch", "elitematch_matching_bg", true), type = ResourcesType.single},
        {path = PathTool.getSingleImgForDownLoad("bigbg/elitematch", "elitematch_matching_head", false), type = ResourcesType.single}
    }
...
function ElitematchMatchingWindow:open_callback(  )
    ...
    self.background:loadTexture(PathTool.getSingleImgForDownLoad("bigbg/elitematch", "elitematch_matching_bg", true), LOADTEXT_TYPE)
    ...
    local function _getTeamItem(bg, panel)
        ...
        item.fight_label = CommonNum.new(220, fight_bg, 0, - 2, cc.p(0, 0.5))
...
function ElitematchMatchingWindow:reloadAudioEffect()
    self.audio_list = {
        [1] = PathTool.getSound(GlobalAudio.AUDIO_TYPE.COMMON, "c_vs") --vs 音效
        ,[2] = PathTool.getSound(GlobalAudio.AUDIO_TYPE.COMMON, "c_scroll") -- 滚动音效
    }

    for i,path in pairs(self.audio_list) do
        GlobalAudio:getInstance():preLoadEffectByPath(v)    
    end
    -- GlobalAudio:getInstance():playEffect(GlobalAudio.AUDIO_TYPE.COMMON,"c_scroll", true)
end
...
function ElitematchMatchingWindow:configShowUI()
    ...
    local head_res = PathTool.getSingleImgForDownLoad("bigbg/elitematch", "elitematch_matching_head", false)
...
function ElitematchMatchingWindow:setData(data)
    ...
    EventManager:getInstance():Fire(ElitematchEvent.Elite_Challenge_count_Event, data)
...
function ElitematchMatchingWindow:updateHeroInfo(team_info, pos_info, formation_type, is_other_team, other_info)
    ...
    if formation_config then
        ...
        for i,v in ipairs(formation_config.pos) do
            ...
            if hero_vo then
                --这里bugly说power是nil value 错的莫名其妙呀..先容错..
...
function ElitematchMatchingWindow:playMatchEffect(status)
    if status == false then
        ...
        if self.scroll_audio_effect then
            GlobalAudio:getInstance():removeEffectByData(self.scroll_audio_effect)
        ...
        if self.scroll_audio_effect == nil then
            self.scroll_audio_effect = GlobalAudio:getInstance():playEffect(GlobalAudio.AUDIO_TYPE.COMMON,"c_scroll", true)
        end
...
function ElitematchMatchingWindow:playVsEffect(status)
    if status == false then
        ...
        self.vs_audio_effect = GlobalAudio:getInstance():playEffect(GlobalAudio.AUDIO_TYPE.COMMON,"c_vs")
