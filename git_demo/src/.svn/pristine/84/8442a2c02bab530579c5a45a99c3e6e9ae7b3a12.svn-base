...
function RolePersonalSpaceTabMessageBoardPanel:setVisibleStatus(bool)
    ...
    if bool and self.is_load_completed then
        EventManager:getInstance():Fire(RoleEvent.ROLE_PS_CHANGE_PANEL_EVENT)
...
function RolePersonalSpaceTabMessageBoardPanel:config()
    ...
    self.default_content_msg = TI18N("输入留言内容......")
...
function RolePersonalSpaceTabMessageBoardPanel:layoutUI()
    ...
    --  -- 标题
    -- local res = PathTool.getSingleImgForDownLoad("bigbg/rolepersonalspace", "role_growth_way_bg", false)
    ...
    self.combobox_max_size = cc.size(self.combobox_bg_size.width, 80) --最大size 根据示意图得出来的
    ...
    self.message_btn:getChildByName("label"):setPositionX(self.message_btn:getChildByName("label"):getPositionX()-3)
...
--Config.ColorData.data_color3[81]
    self.input_edit = createEditBox(self.main_container, PathTool.getResFrame("mainui", "hl_chat_3"), cc.size(290, 48), cc.c3b(98,53,43), 26, nil, 20, self.default_content_msg, nil, self.limit_sad_count, LOADTEXT_TYPE_PLIST)
    self.input_edit:setAnchorPoint(cc.p(0, 1))
    -- cocos引擎对于文本编辑控件只接受系统字体
    -- self.input_edit:setPlaceholderFont(TITLE_FONT,26)
    -- self.input_edit:setFont(TITLE_FONT,26)
    
        -- self.input_edit:setText(desc)
        -- self.input_edit:getText()
    self.input_edit:setPosition(26, 72.6)
...
function RolePersonalSpaceTabMessageBoardPanel:initScrollview()
    ...
    self.item_width     = 543             -- 单元的宽度
    self.item_height    = 110              -- 单元的高度
...
function RolePersonalSpaceTabMessageBoardPanel:registerEvents()
    ...
    --获取留言信息
     EventManager:getInstance():Bind(RoleEvent.ROLE_MESSAGE_BOARD_GET_INFO_EVENT,self,function (data)
    ...
    --新增留言
     EventManager:getInstance():Bind(RoleEvent.ROLE_MESSAGE_BOARD_NEW_INFO_EVENT,self,function (data)
            self:initShowList()
    ...
    if self.parent.role_type == RoleConst.role_type.eMySelf then
        --个人特有
        --删除留言
     EventManager:getInstance():Bind(RoleEvent.ROLE_MESSAGE_BOARD_DELETE_INFO_EVENT,self,function (data)
                self:initShowList()
                if self.scroll_view then
                    --需要刷新当前
                    self.cur_container_x, self.cur_container_y = self.scroll_view_container:getPosition()
        ...
        --修改限制
     EventManager:getInstance():Bind(RoleEvent.ROLE_MESSAGE_BOARD_LIMMIT_EVENT,self,function (data)
                if not data then return end
    ...
    --共有有的
     EventManager:getInstance():Bind(EventId.CHAT_SELECT_FACE,self, function(face_id, from_name)
            ...
            if from_name == ChatConst.ChatInputType.eMessageBoardpanel then
                ...
                count = count + 1
                EventManager:getInstance():Fire(ChatEvent.FACE_COUNT_EVENT, count)
            end
        end)
end
...
function RolePersonalSpaceTabMessageBoardPanel:reloadData(is_keep_position, other_height)
    ...
    if self.test_content == nil then
        self.test_content = createRichLabel(22, cc.c4b(0x64,0x32,0x23,0xff), cc.p(0,1), cc.p(-10000, 48), 6, nil, 430)
    ...
    if number == 0 then
        ...
        commonShowEmptyIcon(self.lay_srollview, true, {text = TI18N("还没有留言信息哦"),pos = cc.p(size.width * 0.5, size.height * 0.5-40)})
    else
...
function RolePersonalSpaceTabMessageBoardPanel:showComboboxList(status)
    ...
    if status then
        ...
        if self.combox_item_list ==nil then
            ...
            local total_height = count * item_height + 20
...
function RolePersonalSpaceTabMessageBoardPanel:createNewComboxItem(height, callback)
    ...
    local res = PathTool.getResFrame("common2","hl_common_3045")
    ...
    cell.label = createTitleLabel(20, cc.c4b(0xff,0xf9,0xda,0xff), nil, 50, height * 0.5 , "", cell.layout, nil, cc.p(0,0.5))
    ...
    cell.line_bg = createImage(cell.layout, PathTool.getResFrame("common2","hl_common_0032"),30, 5, cc.p(0, 0), true, 0, true)
    cell.line_bg:setRotation(90)
    cell.line_bg:setScaleY(7)

    -- cell.line_bg1 = createImage(cell.layout, PathTool.getResFrame("common2","hl_common_0032"),60, 5, cc.p(0, 0), true, 0, true)
    -- cell.line_bg1:setRotation(90)
    -- cell.line_bg1:setScaleY(7)
...
function RoleMessageBoardItem:configUI(width, height)
    ...
    --self.desc = self.box_bg_04:getChildByName("message")
    self.desc = createRichLabel(22, cc.c4b(129,104,65), cc.p(0,1), cc.p(118, 38), 6, nil, 415,DEFAULT_FONT)
    self.main_container:addChild(self.desc)
    
    self.head = PlayerHead.new(PlayerHead.type.circle)
    self.head:setAnchorPoint(cc.p(0, 0.5))
    self.head:setPosition(cc.p(5, 56))
