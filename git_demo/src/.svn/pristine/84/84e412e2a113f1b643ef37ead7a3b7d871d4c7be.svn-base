...
function ElfinHatchPanel:initView(  )
    ...
    -- 灵窝背景
    local hatch_bg_res = PathTool.getSingleImgForDownLoad("elfin","elfin_hatch_bg")
...
function ElfinHatchPanel:registerEvent(  )
    ...
    EventManager:getInstance():Bind(VipEvent.PRIVILEGE_INFO,self, function()
            self:updateElfinPrivileg()
        end)
...
function ElfinHatchItem:register_event(  )
    ...
    -- 所有孵化器数据
    EventManager:getInstance():Bind(ElfinEvent.Get_Elfin_Hatch_Data_Event,self, function (  )
    ...
    -- 红点
    EventManager:getInstance():Bind(ElfinEvent.Update_Elfin_Red_Event,self, function ( bid, status )
...
function ElfinHatchItem:updateHatchInfo(  )
    ...
    local egg_path = PathTool.getSingleImgForDownLoad("elfin","elfin_egg_icon")
    local egg_scale = 0.6
    local egg_y = 38.8
    if self.cur_hatch_status == ElfinConst.Hatch_Status.Open and self.cur_hatch_vo.is_open == 0 then -- 空闲且未激活才显示为锁住
        loadSpriteTexture(self.hatch_lock_sp, PathTool.getResFrame("common","hl_common_92"), LOADTEXT_TYPE_PLIST)
