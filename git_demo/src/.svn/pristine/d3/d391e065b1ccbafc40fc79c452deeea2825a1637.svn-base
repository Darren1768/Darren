...
function RolePersonalSpacePanel:__init(ctrl)
    ...
    self.res_list = {
        ...
        {path = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_6", false), type = ResourcesType.single}
    }
    --显示的角色类型
...
function RolePersonalSpacePanel:open_callback(  )
    ...
    -- self.win_title = self:getChildByName("win_title")
    -- self.win_title:setString(TI18N("留言板"))
    ...
    -- 隐藏留言板按钮
    self.tab_list[4].btn:setVisible(false)
    self:checkTabUnlockInfo()
    self:showTabBtns(self.isNotFriend)
end

function RolePersonalSpacePanel:showTabBtns(notFriend)
  
    for i=1,3 do
       if  self.tab_list[i] then
        self.tab_list[i].btn:setVisible(notFriend)  
       end
...
function RolePersonalSpacePanel:openRootWnd(setting)
    ...
    self.image_2:setVisible(self.role_type ~= RoleConst.role_type.eMySelf)
