...
function GuildNewMainWindow:__init()
    ...
    self.res_list = {
        ...
        {path = PathTool.getSingleImgForDownLoad("bigbg/guild","guild_bg_1",false), type = ResourcesType.single },
    }
    self.icon_list = {} --图标
    self.effect_list = {} --特效
    self.npc_list = {} --特效
...
function GuildNewMainWindow:open_callback()
    ...
    self.info_panel = self.guild_panel:getChildByName("info_panel")
    ...
    self.guild_sign_value:setTextAreaSize(cc.size(225, 100))
    ...
    self.arrow_btn =  self.main_container:getChildByName("arrow_btn")
    self.arrow_img = self.arrow_btn:getChildByName("arrow_img")
   ...
   local  res = PathTool.getResFrame("common","hl_common_2032")
   self.union_btn = createButton( self.guild_bg, TI18N("传送联盟"),420,820, cc.size(131, 50), res, 24,Config.ColorData.data_color4[1])
...
function GuildNewMainWindow:register_event()
    ...
    registerButtonEventListener(self.arrow_btn, function()
            self:shrinkInfoContainer()
        end,false, 1)
 
    self.union_btn:addTouchEventListener(function(sender, event_type)
    ...
    EventManager:getInstance():Bind(GuildEvent.UpdateGuildRedStatus,self, function(type, status)
...
function GuildNewMainWindow:setGray( bool )
    if bool == true then
        setChildUnEnabled(true, self.icon_list[GuildConst.icon_type.guild_icon_type_10])
    else
        setChildUnEnabled(false, self.icon_list[GuildConst.icon_type.guild_icon_type_10])
    end
...
function GuildNewMainWindow:shrinkInfoContainer()
    ...
    if self.is_shrink == true then
        move_by_1 = cc.MoveTo:create(0.6, cc.p(-600, 1058))
        fade_1 = cc.FadeOut:create(0.6)
    else
        move_by_1 = cc.MoveTo:create(0.6, cc.p(20.84, 1058))
        fade_1 = cc.FadeIn:create(0.1)
        scaleY = -1
    end
    self.arrow_img:setScaleX(scaleY)
    ...
    self.guild_panel:runAction(cc.Sequence:create(cc.Spawn:create(ease_out, fade_1), call_fun_1)) 
end
...
function GuildNewMainWindow:openRootWnd()
    ...
    self.guild_bg:loadTexture(PathTool.getSingleImgForDownLoad("bigbg/guild","guild_bg_1",false), LOADTEXT_TYPE)
    ...
    for i,v in ipairs(tempArr) do
        if self.map_layer2 then
            if self.icon_list[v.type] == nil then
                ...
                self.icon_list[v.type]:setPosition(v.pos[1][1] + 74/2 ,v.pos[1][2] - 279/2)
                ...
                local bg = createImage(self.icon_list[v.type], PathTool.getResFrame("guild", "hl_guild_0001"),83/2,23,cc.p(0.5, 0.5),true,nil,true)
                bg:setContentSize(cc.size(74,279))

                createLabel(22, cc.c4b(250,232,150,0xff), cc.c4b(250,232,150,0xff), 83/2, 23,StringUtil.branchStr(v.name) , self.icon_list[v.type], 0, cc.p(0.5, 0.5))
...
function GuildNewMainWindow:updateGuildLeaderInfo()
    if self.my_guild_info == nil then return end 
    self.guild_leader_value:setString( "会长："..self.my_guild_info.leader_name) 
end
...
function GuildNewMainWindow:onCheckIcon(data )
    ...
    elseif data.type == GuildConst.icon_type.guild_icon_type_4 then  --新商店
        -- if self.icon_list[GuildConst.icon_type.guild_icon_type_4].is_unlock == false then
        --     local config = Config.GuildDunData.data_const.guild_lev
        --     if config then
        --         message(config.desc)
    ...
    elseif data.type == GuildConst.icon_type.guild_icon_type_5 then--公会站
        -- MallController:getInstance():openMallPanel(true, MallConst.MallType.UnionShop)
    ...
    elseif data.type == GuildConst.icon_type.guild_icon_type_6 then 
        -- if self.icon_list[GuildConst.icon_type.guild_icon_type_6].is_unlock == false then
        --     local config_lv = Config.GuildWarData.data_const.limit_lev -- 公会等级显示
        --     local config_day = Config.GuildWarData.data_const.limit_open_time -- 开服天数限制
        --     if config_lv and config_day then
    ...
    elseif data.type == GuildConst.icon_type.guild_icon_type_9 then --红包 公会名册
        -- RedbagController:getInstance():openMainView(true)
        controller:openGuildMemberWindow(true)
    elseif data.type == GuildConst.icon_type.guild_icon_type_10  then --公会排名
        RankController:getInstance():openRankView(true, RankConstant.RankType.union)
    elseif data.type == GuildConst.icon_type.guild_icon_type_11  then --命运树
       print("=------------命运树------------")
