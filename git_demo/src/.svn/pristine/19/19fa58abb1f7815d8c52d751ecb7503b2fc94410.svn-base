...
function OrderActionRewardPanel1:layoutUI()
    self.root_wnd = createCSBNote(PathTool.getTargetCSB("orderaction/reward_panel1"))
...
function OrderActionRewardPanel1:registerEvents()
	--等级变化
     EventManager:getInstance():Bind(OrderActionEvent.OrderAction_Updata_LevExp_Event,self,function(data)
            if data then
                self:setChangeLevelStatus(data.lev)
            end
        end)
     EventManager:getInstance():Bind(OrderActionEvent.OrderAction_LevReward_Event,self,function(lev)
            self:setChangeLevelStatus(lev)
        end)

    --进阶卡情况
     EventManager:getInstance():Bind(OrderActionEvent.OrderAction_BuyGiftCard_Event,self,function()
...
function OrderActiodRewardItem1:configUI()
    self.root_wnd = createCSBNote(PathTool.getTargetCSB("orderaction/reward_item1"))
