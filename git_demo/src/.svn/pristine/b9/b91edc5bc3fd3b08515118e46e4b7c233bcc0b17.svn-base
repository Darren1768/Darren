...
function PokedexController:registerEvents()
     ...
     EventManager:getInstance():Bind(PartnerEvent.Partner_Data_Update,self, function(_partner_vo, is_add)
            if is_add then 
                -- self:checkIsCanCall()
            end
        end)
    if not self.get_all_data then 
        self.get_all_data =  EventManager:getInstance():Bind(BackpackEvent.GET_ALL_DATA,self, function(bag_code)
            if bag_code == BackPackConst.Bag_Code.BACKPACK then 
                -- self:checkIsCanCall()
            end
        end)
...
function PokedexController:handle11040( data )
    ...
    EventManager:getInstance():Fire(PokedexEvent.Get_All_Event,data)
    self:checkIsCanCall()
end
...
function PokedexController:handle11041( data )
    print("---------------handle11041------------------",data)
    printLuaTable(data)
    EventManager:getInstance():Fire(PokedexEvent.Comment_List_Event,data)
...
function PokedexController:handle11042( data )
    ...
    if data.result == 1 then
        EventManager:getInstance():Fire(PokedexEvent.Comment_Like_Event,data)
    end
end
--伙伴评论
...
function PokedexController:handle11043( data )
    message(data.msg)
    if data.result ==1 then
        EventManager:getInstance():Fire(PokedexEvent.Comment_Say_Event,data)
...
function PokedexController:handle11044( data )
    message(data.msg)
    if data.result == 1 then
        EventManager:getInstance():Fire(PokedexEvent.Comment_Zan_Event,data)
    end
end
...
function PokedexController:handle11047(data)
    ...
    if data.result == 1 then
        ...
        EventManager:getInstance():Fire(PokedexEvent.Up_End_Event,self.star_data)
    end
end
