...
function BattleCampView:open_callback(  )
    ...
    local setting = {
        ...
        start_y = 0,                    -- 第一个单元的Y起点
        space_y = 0,                   -- y方向的间隔
        item_width = 467,               -- 单元的尺寸width
        item_height = 194,              -- 单元的尺寸height
        row = 0,                        -- 行数，作用于水平滚动类型
        col = 1,                         -- 列数，作用于垂直滚动类型
        need_dynamic = true
    }
...
function BattleCampView:updateAllAttrText(  )
	if not self.all_attr_txt then
		self.all_attr_txt = createRichLabel(20, cc.c4b(210,112,52,255), cc.p(0.5, 0.5), cc.p(self.main_size.width*0.5, 24), nil, nil, self.main_size.width)
	...
	if next(attr_list) == nil then
		...
		for i,v in ipairs(attr_list) do
    		...
    		if attr_name then
	            ...
	            if i == 1 then
	            	attr_str = attr_str .. _string_format("%s<div fontcolor=#5fb34f>+%s</div>", attr_name, attr_val)
	            else
	            	attr_str = attr_str .. _string_format("  %s<div fontcolor=#5fb34f>+%s</div>", attr_name, attr_val)
	            end
    		end
...
function BattleCampItem:configUI(width, height)
    ...
    self.attr_label = createRichLabel(20, cc.c3b(239,212,174), cc.p(0, 1), cc.p(80, 154), 5, nil, 364)
...
function BattleCampItem:setData( data, index )
	if data then
		...
		self.name:setString(data.name)
			self.name:setPositionY(170)

		-- if data.id == 4 or data.id == 5 then
		-- 	self.name:setPositionY(160)
		-- else
		-- 	self.name:setPositionY(150)
		-- end
		...
		if data.id == 6 then
			if self.tip_label == nil then
				self.tip_label = createRichLabel(20, cc.c3b(239,212,174), cc.p(0.5, 0.5), cc.p(self.size.width * 0.5, 35), 5, nil, self.size.width)
				self.container:addChild(self.tip_label)
				self.tip_label:setString(_string_format(TI18N("（本光环触发时，不同时触发<div fontcolor=#5fb34f>辉光之力</div>和<div fontcolor=#5fb34f>深黯之力</div>）")))
			else
		...
		for i,v in ipairs(data.attr_data) do
			...
			for _,attr in ipairs(v.attrs) do
        		...
        		if attr_name then
		            ...
		            if i == index_flag then
		            	str = str .. _string_format("  %s<div fontcolor=#5fb34f>+%s</div>", attr_name, attr_val)
		            else
			...
			if i == index_flag then
				str = _string_format("<div fontcolor=#efd4ae>%s</div>", str)
			else
				str = _string_format("<div fontcolor=#bdb4ca>%s</div>", str)
			end
