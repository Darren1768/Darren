...
function Endless_trailModel:setEndlessData(data)
    if data then
        ...
        self:checkRedPoint()
        EventManager:getInstance():Fire(Endless_trailEvent.UPDATA_BASE_DATA)
    end
end
...
function Endless_trailModel:updateRankInfo(data)
    if data and self.endless_data and next(self.endless_data or {}) ~= nil then
        ...
        EventManager:getInstance():Fire(Endless_trailEvent.UPDATA_RANK_DATA)
    end
end
...
function Endless_trailModel:setFirstData(data)
    if data then
        ...
        self:checkRedPoint()
        EventManager:getInstance():Fire(Endless_trailEvent.UPDATA_FIRST_DATA)
    end
end
...
function Endless_trailModel:setSendPartnerData(data)
   ...
   EventManager:getInstance():Fire(Endless_trailEvent.UPDATA_SENDPARTNER_DATA, data)
   self:checkRedPoint()
end
...
function Endless_trailModel:setHirePartnerData(data)
    ...
    EventManager:getInstance():Fire(Endless_trailEvent.UPDATA_HIREPARNER_DATA, data)
end
...
function Endless_trailModel:setHasHirePartnerData(data)
    ...
    EventManager:getInstance():Fire(Endless_trailEvent.UPDATA_HASHIREPARNER_DATA, data)
end
...
function Endless_trailModel:setEndlessBattleData(data)
    ...
    EventManager:getInstance():Fire(Endless_trailEvent.UPDATA_ENDLESSBATTLE_DATA, data)
end
...
function Endless_trailModel:getRaknRoleTopThreeList(type)
    ...
    if type == Endless_trailEvent.endless_type.singlemode then
        rank_list = self.endless_data.rank_list_individual
    end
...
function Endless_trailModel:checkRedPoint()
    ...
    if self.first_data and next(self.first_data or {}) ~= nil then
       ...
       EventManager:getInstance():Fire(Endless_trailEvent.UPDATA_REDPOINT_FIRST_DATA)
    end
    ...
    if self.endless_data and self.endless_data.day_pass_round then
        ...
        EventManager:getInstance():Fire(Endless_trailEvent.UPDATA_REDPOINT_REWARD_DATA)
    end
    ...
    --设置入口红点
    EventManager:getInstance():Fire(Endless_trailEvent.UPDATA_REDPOINT_SENDPARTNER_DATA)
    EventManager:getInstance():Fire(Endless_trailEvent.UPDATA_ESECSICE_ENDLESS_REDPOINT)

    MainuiController:getInstance():setBtnRedPoint(MainuiConst.btn_index.esecsice, {bid=RedPointType.endless, status=is_show_red}) 
    GhostdomController:getInstance():setBuildRedStatus(GhostdomConst.Build_Type.temper, {bid = 1, status = is_show_red})
end
