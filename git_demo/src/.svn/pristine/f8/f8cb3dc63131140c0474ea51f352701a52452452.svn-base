...
function RoleMessageBoardReplyPanel:open_callback()
    ...
    self.title = self.main_container:getChildByName("title_con"):getChildByName("title_label")
    self.title:setString(TI18N("留言回复"))

    self.close_btn = self.main_container:getChildByName("close_btn")

    local res = PathTool.getResFrame("common","common_99998")
    self.label_content = createRichLabel(22, cc.c4b(0x83,0x52,0x4B,0xff), cc.p(0,1), cc.p(52,234), 6, nil, 480)
    ...
    self.edit_content:setPosition(cc.p(52,334))
    ...
    resetStudioButton(self.comfirm_btn,ButtonType.red,TI18N("回 复"))
    
    self.face_btn = self.main_container:getChildByName("face_btn")
    resetStudioButton(self.face_btn,ButtonType.red,TI18N("添加表情"))
end
...
function RoleMessageBoardReplyPanel:onClickBtnFace()
    local world_pos = self.main_container:convertToWorldSpace(cc.p(281, -240))
    local setting = {}
