...
function LimitExercisePanel:configUI()
    ...
    local bg_res = PathTool.getSingleImgForDownLoad("bigbg/limitexercise", "txt_cn_limit_exercise")
...
function LimitExercisePanel:register_event()
     EventManager:getInstance():Bind(LimitExerciseEvent.LimitExercise_Message_Event,self,function(data)
	...
	registerButtonEventListener(self.btn_change, function()
     ...
     EventManager:getInstance():Bind(LimitExerciseEvent.LimitExercise_BuyCount_Event,self,function(data)
            if not data then return end
