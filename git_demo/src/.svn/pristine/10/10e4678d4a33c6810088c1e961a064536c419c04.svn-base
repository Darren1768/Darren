...
function LimitExerciseChangeWindow:open_callback()
	...
	local bg_res = PathTool.getSingleImgForDownLoad("bigbg/limitexercise", "limit_exercise_bg1")
	...
	if self.item_scrollview then
		if change_boss_list[round] and change_boss_list[round][diff] then
			...
			for i,v in pairs(change_boss_list[round][diff]) do
				table_insert(self.area_list,v) --这里可能出问题，因为表里去掉了type by spp
			end
...
function LimitExerciseChangeWindow:getLevelMessage(ord_type,ord_id)
	...
	if change_boss_list[round] and change_boss_list[round][diff] then
		...
		if lev_data[ord_id]  then
			local lev_count = lev_data[ord_id].order_id
			...
			local str = ""
			local desc = lev_data[ord_id].add_skill_decs or {}
			for i=1, #desc do
				str = str..desc[i].."\n"
			end
			self.level_effect_desc:setString(str)
			local power = lev_data[ord_id].power or 0
...
function LimitExerciseChangeItem:configUI()
	self.root_wnd = createCSBNote(PathTool.getTargetCSB("limitexercise/limitexercise_change_item"))
