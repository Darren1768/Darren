...
function HeroChipsBreakWindow:__init()
    ...
    -- self.res_list = {
    --     { path = PathTool.getSingleImgForDownLoad("bigbg/hero","hero_reset_bg", true), type = ResourcesType.single },
    --     { path = PathTool.getSingleImgForDownLoad("bigbg/hero","hero_return_bg", true), type = ResourcesType.single },
    --     { path = PathTool.getSingleImgForDownLoad("bigbg/action","txt_cn_hero_convert_bg", true), type = ResourcesType.single }
    -- }
...
function HeroChipsBreakWindow:open_callback()
    ...
    self.title_img:getChildByName("title_label"):setString(TI18N("碎片分解"))
    ...
    self.close_btn =  self.disband_container:getChildByName("Button_1")
    ...
    self.lab_have_count:setFontSize(20)
    self.lab_have_count:setTextColor(cc.c3b(98,53,43))

    self.resolve_count:setFontSize(22)
    self.resolve_count:setTextColor(cc.c3b(98,53,43))

    -- self.max_btn_label:setFontSize(24)
    -- self.max_btn_label:setTextColor(cc.c3b(231,213,196))
end
...
function HeroChipsBreakWindow:updateHeroList(select_camp, reset)
    ...
    if not self.list_view then
        local scroll_view_size = cc.size(480,180)
        self.hero_setting = {
    ...
    if cur_select_chip_data == nil then
        self.select_chip_data = nil     
        self.list_view:reloadData(nil, {item_height = 120})
    else
...
function HeroChipsBreakWindow:showEmptyIcon(bool)
    ...
    if not self.empty_con then
        ...
        self.empty_con:setPosition(x, y+20)
...
function HeroChipsBreakWindow:createNewCell(width, height)
    local height = 100 --高度写死
    local cell = ccui.Widget:create()
    local hero_item = BackPackItem.new(0.7, true)
...
function HeroChipsBreakWindow:updateCellByIndex(cell, index)
    ...
    if hero_vo then
        ...
        cell.hero_item:showCompNumber(total_count,need_count)
