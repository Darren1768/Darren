...
function MainChatUiMsg:ctor(width, height, max_width, channel)
    print("---------------MainChatUiMsg----dddddddddddddddd---data.channel-------------",data.channel)
...
function MainChatUiMsg:setData(data)
    print("-------------------dddddddddddddddd---data.channel-------------",data.channel)
    ...
    print("----------------------data.channel-------------",data.channel)
    ...
    if data.channel and self.channel ~= ChatConst.Channel.Friend then
        local channel_res = PathTool.getResFrame("mainui","hl_txt_cn_chat_icon_"..ChatConst.ChannelRes[data.channel])
...
function MainChatUiMsg:onTranslateEnd(voice_name, voice_msg)
    if self.dataObj and self.dataObj.len==1 and self.dataObj.msg==voice_name then
        ...
        EventManager:getInstance():Fire(ChatConst.Voice_Translate_Main)
    end
end
