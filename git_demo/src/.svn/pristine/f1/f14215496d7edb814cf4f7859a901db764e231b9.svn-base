...
function ArenapeakchampionGuessingTabGuessing:loadResources()
    ...
    --竞猜信息 必须优先注册
    EventManager:getInstance():Bind(ArenapeakchampionEvent.ARENAPEAKCHAMPION_GUESSING_INFO_EVENT,self,function ( data )
    ...
    self.res_list = {
        {path = PathTool.getSingleImgForDownLoad("bigbg/arenapeakchampion", "arenapeakchampion_guessing_centre", false), type = ResourcesType.single},
        {path = PathTool.getSingleImgForDownLoad("bigbg/arenapeakchampion", "txt_cn_arenapeakchampion_guessing_top", false), type = ResourcesType.single},
    } 
...
function ArenapeakchampionGuessingTabGuessing:layoutUI()
    ...
    --缺 bg29
    loadSpriteTexture(self.top_img, PathTool.getSingleImgForDownLoad("bigbg/arenapeakchampion", "txt_cn_arenapeakchampion_guessing_top", false), LOADTEXT_TYPE)
    loadSpriteTexture(self.centre_img, PathTool.getSingleImgForDownLoad("bigbg/arenapeakchampion", "arenapeakchampion_guessing_centre", false), LOADTEXT_TYPE)
    ...
    self.left_fight_label = CommonNum.new(220, left_power_click, 0, - 2, cc.p(0.5, 0.5))
    ...
    self.right_fight_label = CommonNum.new(220, right_power_click, 0, - 2, cc.p(0.5, 0.5))
...
function ArenapeakchampionGuessingTabGuessing:registerEvents()
    ...
    --主面板信息 更新时间状态用
    EventManager:getInstance():Bind(ArenapeakchampionEvent.ARENAPEAKCHAMPION_MAIN_EVENT,self,function ( data )
            self:updateTime()
    ...
    --通知客户端观看录像
    EventManager:getInstance():Bind(ArenapeakchampionEvent.ARENAPEAKCHAMPION_SHOW_VEDIO_EVENT,self,function ( data )
    ...
    --竞猜信息
     EventManager:getInstance():Bind(ArenapeakchampionEvent.ARENAPEAKCHAMPION_UPDATE_GUESSING_INFO_EVENT,self,function ( data )
            if not data then return end
            self:updateGuessPanel(data)
        end)
    -- --竞猜押注返回
     EventManager:getInstance():Bind(ArenapeakchampionEvent.ARENAPEAKCHAMPION_GUESSING_STAKE_EVENT,self,function ( data )
            if not data then return end
            if self.scdata then
    ...
    -- --刷新竞猜币
     EventManager:getInstance():Bind(ArenapeakchampionEvent.ARENAPEAKCHAMPION_SINGLE_INFO_EVENT,self,function ( data )
            self:updateItemCount()
        end)
    -- 红点
     EventManager:getInstance():Bind(ArenapeakchampionEvent.ARENAPEAKCHAMPION_ALL_RED_POINT_EVENT,self,function ( data )
            self:updateRedPoint()
        end)
    -- self.role_vo = RoleController:getInstance():getRoleVo()
...
function ArenapeakchampionGuessingTabGuessing:showEmptyInfo(is_show, main_data)
    ...
    if is_show then
        ...
        if self.gril_img_load == nil then
            ...
            local res  = PathTool.getSingleImgForDownLoad("bigbg","bigbg_50", false)
        ...
        if self.talk_img_load == nil then
            ...
            local res  = PathTool.getSingleImgForDownLoad("bigbg","bigbg_96", false)
