...
--
-- --------------------------------------------------------------------
-- ArenaLoopActivityPanel = ArenaLoopActivityPanel or BaseClass(BaseView) -- ==使用此行表示 是layer模式==
...
-- ==layer使用使用==
-- function ArenaLoopActivityPanel:__init()
--     self.view_tag = ViewMgrTag.DIALOGUE_TAG
-- 	self.win_type = WinType.Big
	
-- 	self.layout_name = "arena/arena_loop_activity_panel"
-- 	self.res_list = {
...
function ArenaLoopActivityPanel:register_event()
    self.background:addTouchEventListener(function(sender, event_type)
		if event_type == ccui.TouchEventType.ended then
			playQuitSound()
			-- controller:openArenaLoopActivityWindow(false)
...
--  ============================作为layer时候使用===start================-------------------------
function ArenaLoopActivityPanel:open_callback()
    self.temp_list = {}
    self.item_list = {}
    self.background = self.root_wnd:getChildByName("background")
	self.background:setScale(display.getMaxScale())
    ...
    self:updateScrollData() --此处调用 如果是layer则注释掉
end

function ArenaLoopActivityPanel:openRootWnd()
    self:updateScrollData()
    self:updatePanelInfo()
end

function ArenaLoopActivityPanel:close_callback()
...
--  ============================作为layer时候使用===end================-------------------------
function ArenaLoopActivityPanel:updateScrollData()
    local tmp_list = deepCopy(Config.ArenaData.data_activity)
    for i,v in ipairs(tmp_list) do
        v.index = i
    end
		...
		if data.index ~= nil then
			if data.index <= 3 then
				...
				if data.rank == 0 then
					...
					local res_id = PathTool.getResFrame("common", string.format("hl_common_%s", data.index+2000-1))  
