...
function ArenateamHallTapInvitationPanel:registerEvents()
    registerButtonEventListener(self.key_btn, function() self:onKeyBtn()  end ,true, REGISTER_BUTTON_SOUND_BUTTON_TYPY, nil, nil, 1)
     ...
     EventManager:getInstance():Bind(ArenateamEvent.ARENATEAM_INVITATION_LIST_EVENT,self,function (data)
            if not data then return end
            self.is_init = true
            self:setData(data)
        end)
     EventManager:getInstance():Bind(ArenateamEvent.ARENATEAM_KEY_CLEAR_EVENT,self,function (data)
            self.show_list = {}
            self:updateTeamlist()
        end)
end
