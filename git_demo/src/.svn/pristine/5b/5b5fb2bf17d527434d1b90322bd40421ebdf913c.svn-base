...
function LoginController:registerEvents()
    if self.close_usr_panel == nil then
        self.close_usr_panel = self.dispather:Bind(LoginEvent.CLOSE_CHANGE_PANEL, self, function ()
    ...
    if self.request_defualt_server_success == nil then
        self.request_defualt_server_success = self.dispather:Bind(LoginEvent.DEFAULE_SERVER_SUCCESS, self, function(sign)
            --不是断线重连的时候才打开面板--
    ...
    if self.first_load_finish == nil then
        self.first_load_finish = self.dispather:Bind(SceneEvent.FIRST_TIME_LOAD_FINISH, self, function()
    ...
    if self.connet_success_event == nil then
        self.connet_success_event = self.dispather:Bind(EventId.CONNECTED, self, function()
    ...
    if self.disconnect_event == nil then
        self.disconnect_event = self.dispather:Bind(EventId.DISCONNECT, self, function()
...
function LoginController:openReconnect(status)
    ...
    if BattleController:getInstance():isInFight() then
        EventManager:getInstance():Fire(BattleEvent.DISCONNECTVIEW,status)
    end
end
