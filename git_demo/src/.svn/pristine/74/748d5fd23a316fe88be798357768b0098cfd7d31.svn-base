...
function ActionSevenRankWindow:register_event(  )
	...
	--获取七天排行列表
     EventManager:getInstance():Bind(ActionEvent.UPDATE_SEVEN_RANK_LIST,self,function ( data )
			self:createTabBtn(data.rank_list)
			-- dump(data)
		end)
	--获取单个排行榜信息
     EventManager:getInstance():Bind(ActionEvent.UPDATE_SEVEN_RANK_DATA,self,function ( data )
     ...
     EventManager:getInstance():Bind(ActionEvent.UPDATE_SEVENT_QUEST,self, function() 
			self:fileSevenRankInfo()
		end)
end
