...
function StoryView:actLonelyPlayer(obj, is_async)
    ...
    for k,v in pairs(obj) do
        ...
        elseif k == "play_drama" then
            -- 播放剧情
            self:showDrameTalk(v[3],v[4],v[5],v[6],v[7],v[8],v[9],v[10],v[11],v[12],v[13],v[14])
            self:playActSound(v[8])
        end
...
--action_name  资源id 
--left_right 左右  0 左 
--quilty_type 全身的品质 0 低  
function StoryView:showDrameTalk(type, bid, actiontype, name, msg, res_type, res_rid, action_name ,left_right,quilty_type,offset_x,offest_y)

    self.is_next_action = true
    if self.drame_talk == nil then
        if not self.cur_drama then
            self.drame_talk = StoryDrameTalk.New(0)
        else
            self.drame_talk = StoryDrameTalk.New(self.cur_drama.is_skip)
        end 
        self.drame_talk:open(type, bid, actiontype, name, msg,res_type, res_rid, action_name ,left_right,quilty_type,offset_x,offest_y)
    else
        self.drame_talk:setData(type, bid, actiontype, name, msg, res_type, res_rid, action_name ,left_right,quilty_type,offset_x,offest_y)
    end
end
...
function StoryView:hideTalk()
    ...
    if self.drame_talk then
        self.drame_talk:close()
        self.drame_talk = nil
    end
end
