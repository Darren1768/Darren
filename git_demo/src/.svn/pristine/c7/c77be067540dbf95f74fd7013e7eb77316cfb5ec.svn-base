...
-- Time: 
-- 文件功能：用于提示的功能
CommonAlertYellow = CommonAlertYellow or BaseClass(BaseView)

CommonAlertYellow.type =
{
    common  = 1,
    rich    = 2,
}
CommonAlertYellow.WIDTH = 573
CommonAlertYellow.HEIGHT = 278

AlertMapList =  {}

function CommonAlertYellow:__init(type, title, is_auto_close, is_show_title, view_tag, win_type)
    self.type = type or CommonAlertYellow.type.common
    ...
    self.layout_name = "common/common_ui_alert_yellow"
    self.win_type = win_type or WinType.Tips
end

function CommonAlertYellow:initParam(confirm_label, confirm_callback, cancel_label, cancel_callback, close_callback)
...
function CommonAlertYellow:open()
    BaseView.open(self)
    self:openCallBack()
end

function CommonAlertYellow.closeAllWin()
    for i,v in ipairs(AlertMapList) do
        v:close()
    end
    AlertMapList = {}
end

function CommonAlertYellow:close()
    for i,v in ipairs(AlertMapList) do
        if v == self then
            table.remove(AlertMapList, i)
        end
    end
    BaseView.close(self)
end

function CommonAlertYellow:close_callback()
...
function CommonAlertYellow:setTimerBtn(btn, btn_label, call_back)
...
function CommonAlertYellow:open_callback()
    ...
    resetStudioButton(self.ok_btn,ButtonType.yellow)
    ...
    resetStudioButton(self.cancel_btn,ButtonType.red)

end

--打开调用
function CommonAlertYellow:openCallBack()
...
--dis_y:是与中心的Y偏移量
function CommonAlertYellow:setTextString(val, font_size, dis_y)
	dis_y = dis_y or 0
    ...
    if self.type == CommonAlertYellow.type.common then
        self.alert_txt = self:recoutTextFieldSize(val, self.alert_panel:getContentSize().width, font_size, cc.c4b(0x83,0x52,0x4b,0xff))
    else
        self.alert_txt = self:recoutRichTextField(val, self.alert_panel:getContentSize().width, font_size, cc.c4b(0x83,0x52,0x4b,0xff))
...
--@return 
--==============================--
function CommonAlertYellow:setExtendTextString(val, font_size, dis_y, type, aligment, index) 
    local font_size = font_size or 22
    local dis_y = dis_y or 0
    local type = type  or CommonAlertYellow.type.common
    ...
    if type == CommonAlertYellow.type.common then
...
-- 从新计算文本的大小
function CommonAlertYellow:recoutTextFieldSize(str_label, width, font_size, color, aligment)
...
--
function CommonAlertYellow:recoutRichTextField(str_label,maxWidth,font_size, color)
    color = color or cc.c4b(0x83,0x52,0x4b,0xff)
...
function CommonAlertYellow:register_event()
    ...
    self.background_panel:setTouchEnabled(true)
...
-- ]]
function CommonAlertYellow.show(str, confirm_label, confirm_callback, cancel_label, cancel_callback, type, close_callback, other_args, font_size,close)
    other_args = other_args or {}
    local view_tag = other_args.view_tag or ViewMgrTag.DIALOGUE_TAG
    local win_type = other_args.win_type or WinType.Tips
    local alert = CommonAlertYellow.New(type, other_args.title, nil, nil, view_tag, win_type)
    ...
    -- 引导需要这个,所以暂时这样打开
    table.insert( AlertMapList, alert )
    return alert
end
...
--@return 
--==============================--
function CommonAlertYellow.showInputApply(desc_str, placeholder_str, confirm_label, confirm_callback, cancel_label, cancel_callback, close, close_callback, font_size, type, is_auto_close,size,max_len, other_args,bool)
    other_args = other_args or {}
    local alert = CommonAlertYellow.New(type, title_str, is_auto_close)
    alert.timer = other_args.timer
...
--单选框选中状态
function CommonAlertYellow:isSelected()
...
--保存下时间的数据吧
CommonAlertYellow.SaveLocalTime = {}
function CommonAlertYellow:showCheckBox(tishi_id)
    ...
    self.check_box:addTouchEventListener(function(sender, event_type)
        if event_type == ccui.TouchEventType.ended then
            ...
            if self.check_box:isSelected() then
                CommonAlertYellow.SaveLocalTime[tishi_id] = GameNet:getInstance():getTime()
            else
                CommonAlertYellow.SaveLocalTime[tishi_id] = 0
            end
        end
    end)
end

function CommonAlertYellow:ActionshowCheckBox(tishi_id)
...
function CommonAlertYellow.showItemApply(str, list, confirm_callback, confirm_label, cancel_callback, cancel_label, title_str, font_size, type, close,close_callback,desc_label,item_info,view_tag,margin)
    type = type or CommonAlertYellow.type.common
    ...
    local alert = CommonAlertYellow.New(type, title_str,nil,nil,view_tag, win_type)
...
function CommonAlertYellow:showItemList(list,show_need,margin)
...
--计算判断是否属于今日之内的
function CommonAlertYellow.countIsToday(tishi_id)
    local per_time = CommonAlertYellow.SaveLocalTime[tishi_id]
...
function CommonAlertYellow:createInputContainer(placeholder,size,max_len, desc, off_y, touch_enabled,off_x,bool)
    if self.input_edit == nil then
        if bool == true then
        self.input_edit = createEditBox(self.alert_panel, PathTool.getResFrame("common", "hl_chat_3"), size or cc.size(300, 60), Config.ColorData.data_color3[81], 22, nil, 25, placeholder, nil, max_len, LOADTEXT_TYPE_PLIST)
        else
        self.input_edit = createEditBox(self.alert_panel, PathTool.getResFrame("common", "hl_chat_3"), size or cc.size(300, 60), Config.ColorData.data_color3[81], 25, nil, 25, placeholder, nil, max_len, LOADTEXT_TYPE_PLIST)
        end
    end
