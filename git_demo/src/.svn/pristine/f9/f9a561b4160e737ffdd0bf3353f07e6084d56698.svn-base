...
function GuildSearchPanel:ctor(ctrl)
    ...
    self.guild_value = createEditBox(self.search_container, res, cc.size(370, 50), nil, 18, Config.ColorData.data_color4[1], 22, TI18N("请输入需要查找的内容"), nil, nil, LOADTEXT_TYPE_PLIST)
    self.guild_value:setAnchorPoint(cc.p(0.5, 0.5))
    local color=cc.c3b(116,88,60)
    self.guild_value:setPlaceholderFontColor(color)
    self.guild_value:setFontColor(color)
    self.guild_value:setPosition(cc.p(280, 122))
