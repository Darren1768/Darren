...
function EquipTips:open_callback()
    ...
    self.container = self.root_wnd:getChildByName("container")            -- 背景,需要动态设置尺寸
    self.container_init_size = self.container:getContentSize()

    -- 基础属性,名字,类型和评分等
    self.base_panel = self.container:getChildByName("base_panel")
    self.base_panel_bg = self.base_panel:getChildByName("bg")
    ...
    self.scrollview_attr = self.container:getChildByName("scrollview")
    self.container_attr = self.scrollview_attr:getInnerContainer() 
    self.scrollview_attr_height = self.scrollview_attr:getContentSize().height

    self.all_container = self.scrollview_attr:getChildByName("all_container")
    -- 基础属性
    self.baseattr_title  = self.all_container:getChildByName("baseattr_title")
    -- 觉醒属性
    self.awakeattr_title = self.all_container:getChildByName("awakeattr_title")
    --套装属性
    self.suit_title = self.all_container:getChildByName("suit_title")
    ...
    self.desc_label = createRichLabel(20, cc.c4b(239,212,174,0xff), cc.p(0, 1), cc.p(20, 64), 8, nil, 380) 
    self.scroll_view:addChild(self.desc_label)
  --  self.container:setLayoutComponentEnabled(true)
    -- self:checkIsShowBtn()
end
function EquipTips:setBGHeight()
--    local childrens=self.container:getChildren()
--    local totalH=0
...
--    end
--    local cWidth=self.container:getContentSize().width
-- self.container:setContentSize(cc.size(cWidth,totalH))
...
function EquipTips:onTabBtn3()
    ...
    -- 跳转合成
    ForgeHouseController:getInstance():openForgeHouseView(true, ForgeHouseConst.Tab_Index.Equip)
...
function EquipTips:openRootWnd(object)
    ...
    print("-----------object.data.awake-----self.cloth_type------------",self.cloth_type)
...
function EquipTips:initData()
      self:setBaseInfo()
    --   self.scrollview_attr 
    --  self.scrollview_attr_height
     -- 基础属性
    --  print("------------self.item_config-----------------",self.item_config) 
    --  printLuaTable(self.item_config)
    --  print("------------self.data----enchant-------------",self.data.enchant) 
    --  printLuaTable(self.data)

     self.baseattr_title:setPositionY(0)
     self:setBaseAttrInfo()
     -- 基础属性最后高度
     local _label = self.base_list[#self.base_list]
     local y = -_label:getContentSize().height + _label:getPositionY()

     -- 觉醒属性
     local _awake = self.data.awake
     if _awake and _awake > 0 then 
        self.awakeattr_title:setPositionY(y-10)
        y = -self.awakeattr_title:getContentSize().height + self.awakeattr_title:getPositionY()
        self:setAwakeAttrInfo(y)
        -- 觉醒属性最后高度
         local _label_awake = self.awake_list[#self.awake_list]
         y = -_label_awake:getContentSize().height + _label_awake:getPositionY() 
     else
        self.awakeattr_title:setVisible(false)
     end 
    ...
    local suit_list = {}
    if self.suit_config then 
        self.suit_title:setVisible(true)
        self.suit_title:setPositionY(y-10)
        y = -self.suit_title:getContentSize().height + self.suit_title:getPositionY()-10
        y =  self:setSuitAttrInfo(y)
        -- 套装属性最后高度
        --_label, y = self.suit_item_list[#self.suit_item_list].node
    else
        self.suit_title:setVisible(false)
    end 
  
    -- --   套装技能
    -- if #suit_list>0 then

    --     self.skill_list_item = {}
    --     local index = 1 
    --     for i,v in pairs(suit_list) do 
    --         local config = Config.SkillData.data_get_skill(v.skill_id)
    --         if config then

    --             local  skill_item = self.skill_list_item[index]
    --             if not skill_item then
    --                 skill_item = self:createSkillItem(self.all_container,y)
    --             end
    ...
    --             if v.is_active then 
    --                 setChildUnEnabled(false, skill_item.skill)
    --                 skill_item.name:setTextColor(cc.c3b(239,212,174))
    --                 skill_item.desc:setColor(cc.c3b(189,180,199))
    --             else
    --                 setChildUnEnabled(true, skill_item.skill)
    --                 skill_item.name:setTextColor(cc.c3b(168,168,168))
    --                 skill_item.desc:setColor(cc.c3b(168,168,168))
    --             end 


    --             local desc_height =  skill_item.desc:getContentSize().height
    --             if desc_height <60 then 
    --                 desc_height = 60
    --             end 

    --             skill_item.skill:setPositionY(-(desc_height+skill_item.name:getContentSize().height)/2 + skill_item.name:getPositionY())
    --             index = index +1 
    --             y = -desc_height +  skill_item.desc:getPositionY()
    ...
    -- end
       ...
       local all_height = 0
      if -y <300 then 
        self.scrollview_attr:setContentSize(cc.size(426,-y))
      end 
      all_height=-y

      self.scrollview_attr:setInnerContainerSize(cc.size(426,all_height))
      self.all_container:setPositionY(all_height)
    ...
    if need_show_btn then
        ...
        self.tab_panel:setContentSize(cc.size(0,0))
    end

      local totol_height =self.base_panel:getContentSize().height + self.scrollview_attr:getContentSize().height + self.desc_panel:getContentSize().height + self.tab_panel:getContentSize().height +25

      self.container:setContentSize(cc.size(self.container:getContentSize().width,totol_height))
    
      self.base_panel:setPositionY(totol_height-10)
      self.scrollview_attr:setPositionY(self.base_panel:getPositionY() -self.base_panel:getContentSize().height)
      self.desc_panel:setPositionY(self.scrollview_attr:getPositionY() -self.scrollview_attr:getContentSize().height)
    ...
    --     local y = target_height - 40
    --     self.base_panel:setPositionY(y) --基本信息
    --     y = y - self.base_panel:getContentSize().height-20 
...
function EquipTips:updateBtnList()
    ...
    if self.cloth_type == PartnerConst.EqmTips.backpack then  --穿戴
        -- self.tab_btn_3:setTitleText(TI18N("穿戴"))
        self.tab_btn_3:setTitleText(TI18N("合成"))
...
function EquipTips:setBaseInfo()
    ...
    if self.record_background_res == nil or self.record_background_res ~= background_res then
        ...
        loadSpriteTexture(self.base_panel_bg, background_res, LOADTEXT_TYPE_PLIST) 
        self.base_panel_bg:setVisible(true)
    else
        self.base_panel_bg:setVisible(false)
    end
    ...
    if self.item_config.sub_type == BackPackConst.item_tab_type.HOLYEQUIPMENT then --神装
        ...
        self.suit_name:setString(all_score)
    end
...
function EquipTips:setBaseAttrInfo()
    ...
    local curKey = getNorKey(self.data.base_id, self.data.enchant)
    local curConfig = Config.PartnerEqmData.data_partner_eqm(curKey)
    print("----------curKey--------------",curKey)

    self.enchant_attr ={}
    if curConfig then 
        for i,v in pairs(curConfig.add) do 
            local attr_key = v[1]
            local attr_val = v[2]
            self.enchant_attr[attr_key] =attr_val 
        end 
    end 

    for i,v in pairs(base_attr) do
        -- printLuaTable(self.item_config)
        local attr_key = v[1]
        local attr_val = v[2]
        ...
        if attr_name then
            if not self.base_list[index] then 
                self.base_list[index] = createRichLabel(20, cc.c4b(0xc1,0xb7,0xab,0xff), cc.p(0, 1), cc.p(0, 24), nil, nil, 200)
                self.all_container:addChild(self.base_list[index])
            end
            ...
            local _xx = (index-1) %2 
            local _x = _xx* 200 
            local _y= -24*_yy - self.baseattr_title:getContentSize().height -10
            ...
            if  self.enchant_attr[attr_key] then  
                attr_val = attr_val + self.enchant_attr[attr_key]
            end 
            if is_per == true then
                attr_val = (attr_val*0.1).."%"
            end
            --, PathTool.getResFrame("common", icon)
...
--@return 
--==============================--
function EquipTips:setAwakeAttrInfo(pos_y)
    if not self.item_config or not self.item_config.ext or not self.item_config.ext[1] then return end
    local base_attr = self.item_config.ext[1][2] or {}

    local curKey = getNorKey(self.data.base_id, self.data.awake)
    local curConfig = Config.PartnerEqmData.data_get_eqm_awake_info[curKey][1]

    print("------------curKey-------------",pos_y)
    print("------------curKey---------curConfig[1]----",curConfig)
    local index = 1 
    for i,v in pairs(curConfig) do
        local attr_key = i
        local attr_val = v
        print("------------attr_name-------------",attr_key)
        local attr_name = Config.AttrData.data_key_to_name[attr_key]
        if attr_name then
            if not self.awake_list[index] then 
                self.awake_list[index] = createRichLabel(20, cc.c4b(0xc1,0xb7,0xab,0xff), cc.p(0, 1), cc.p(0, 24), nil, nil, 200)
                self.all_container:addChild(self.awake_list[index])
            end
            local label = self.awake_list[index]
        
            local _yy =math.floor((index-1)/2 ) 
            local _xx = (index-1) %2 
   
            local _x = _xx* 210 
            local _y= -32 * _yy + pos_y  -10
    ...
    if lustr_status == true then
        ...
        for i=1,2 do
            ...
            if dic_holy_eqm_attr[i] then
                ...
                local icon = PathTool.getAttrIconByStr(attr_key)
                local attr_color = "BDB4C7"
                local val_color = "6FAF49"
                local ext_str = ""
                if self.item_config.sub_type == BackPackConst.item_tab_type.HOLYEQUIPMENT then --神装
...
--==============================--
function EquipTips:setSuitAttrInfo(pos_y)
            ...
            if cur_suit_config and cur_suit_config[v.num] then
                for _,cur_suit in ipairs(cur_suit_config[v.num]) do
                    if cur_suit.num == v.num then
                        ...
                        suit.is_suit =  true
                        suit.skill_id  =  v.skill_id       
                        suit.suit_name =v.name   
    ...
    if self.item_config.sub_type == BackPackConst.item_tab_type.HOLYEQUIPMENT then --神装 
        ...
        if count > 0 then
            ...
            if suit_list[1] and suit_list[1].is_suit then
                ...
                self.suit_title:setString(tips)
            else
                local str = string_format("%s (%s/%s)", name, act_count, max_count)
                -- self.suitattr_name:setString(str)
                self.suit_title:setString(str)
            end
        end
    end
    self.skill_list_item = {}
    ...
    for i,suit in ipairs(suit_list) do
        if self.suit_item_list[i] == nil then
            self.suit_item_list[i] = self:createattrItem(i, self.all_container )
        end
        local _x = 0
        self.suit_item_list[i].node:setPosition(cc.p(_x, _y))
        _y= -self.suit_item_list[i].node:getContentSize().height + _y  

        local type_str = type(suit.attr)
        local attr_color = "a8a8a8"
        local val_color = "a8a8a8"
        --是否激活
        ...
        elseif type_str == "number" then
            ...
            --套装
            local attr_str = string_format("<div fontcolor=#%s> %s</div>", attr_color, attr_name)
            self.suit_item_list[i].attr:setString(attr_str)    
        end
      
        if suit.name then
            local act_str = string_format("<div fontcolor=#%s>%s（%s%s%s）</div>",attr_color,suit.suit_name, suit.name, suit.count,TI18N("件激活"))
            self.suit_item_list[i].act_info:setString(act_str)
        else
            local act_str = string_format("<div fontcolor=#%s>%s（%s%s）</div>", attr_color,suit.suit_name, suit.count, TI18N("件激活"))
            self.suit_item_list[i].act_info:setString(act_str)
        end
      
    
        local config = Config.SkillData.data_get_skill(suit.skill_id)
        local  skill_item = self.skill_list_item[i]
        if not skill_item then
            skill_item = self:createSkillItem(self.all_container,_y)
        end
        skill_item.skill:showLockIcon(false)
        skill_item.skill:setData(config)
        skill_item.name:setString(config.name)
        ...
        if suit.is_active then 
            setChildUnEnabled(false, skill_item.skill)
            skill_item.name:setTextColor(cc.c3b(189,180,199))
            skill_item.desc:setColor(cc.c3b(189,180,199))
        else
            setChildUnEnabled(true, skill_item.skill)
            skill_item.name:setTextColor(cc.c3b(168,168,168))
        ...
        skill_item.skill:setPositionY(-(desc_height+skill_item.name:getContentSize().height)/2 + skill_item.name:getPositionY())
        _y = -desc_height +  skill_item.desc:getPositionY() -20
...
function EquipTips:createattrItem(index, parent)
    ...
    object.node:setAnchorPoint(cc.p(0, 1))
    object.node:setContentSize(cc.size(360, 60))
    parent:addChild(object.node)

    object.act_info = createRichLabel(20, cc.c4b(255,255,255,0xff), cc.p(0, 1), cc.p(0, 60), nil, nil, 380)
    object.node:addChild(object.act_info)

    object.attr = createRichLabel(20, cc.c4b(255,255,255,0xff), cc.p(0, 1), cc.p(0, 30), nil, nil, 380)
...
--desc:创建技能显示单例
--==============================--
function EquipTips:createSkillItem(parent,pos_y)
    print("---------createSkillItem--------------",_y)
    local item = {}
    local skill = SkillItem.new(true,true,true,0.9,nil,nil,true)
    parent:addChild(skill)
    skill:setPosition(52,pos_y-70)
    local name = createLabel(20,cc.c4b(239,212,174,0xff),nil,105, pos_y-10,"",parent,0,cc.p(0,1))
    local desc = createRichLabel(20,cc.c4b(189,180,199,0xff),cc.p(0,1),cc.p(105, pos_y-40),4,nil,300)
