...
function FriendListItem:configUI( ... )
    ...
    self.play_head:setPosition(cc.p(75,73))
    --名字
    ...
    self.role_power:setPosition(196,65)
    --在线
...
function FriendListItem:updateMyFriend()
    ...
    if not self.my_friend then 
        ...
        -- self.chat_btn = createTypeButton(self.my_friend, str, 525, 55, ButtonType.red)
...
function FriendListItem:updateApplyPanel()
    if not self.apply_panel then 
        ...
        local str = TI18N("拒绝")
        self.cancel_btn = createTypeButton(self.apply_panel, str, 435, 55, ButtonType.red)
        ...
        local str = TI18N("接受")
        self.ok_btn = createTypeButton(self.apply_panel, str, 545, 55, ButtonType.red)
...
function FriendListItem:updateBlackPanel()
    if not self.black_panel then 
        ...
        local str = TI18N("移除")
        self.del_btn = createTypeButton(self.black_panel, str, 545, 57, ButtonType.red)
...
function FriendListItem:updateRecommendPanel()
    if not self.recommend_panel then 
        ...
        self.add_btn = createTypeButton(self.recommend_panel, str, 525, 55, ButtonType.red)
...
function FriendListItem:setDelStatus(bool)
    ...
    if not self.del_friend_btn then 
        ...
        self.del_friend_btn = createTypeButton(self.main_panel, str, 525, 55, ButtonType.yellow)
