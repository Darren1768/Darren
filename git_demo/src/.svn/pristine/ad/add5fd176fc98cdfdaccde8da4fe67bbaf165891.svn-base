...
function ArenaManyPeopleHallTeamPanel:registerEvents()
    ...
    registerButtonEventListener(self.refresh_btn, function() self:onRefreshBtn()  end ,true, REGISTER_BUTTON_SOUND_BUTTON_TYPY,nil,nil, 1)
     ...
     EventManager:getInstance():Bind(ArenaManyPeopleEvent.ARENAMANYPOEPLE_GET_RECOMMEND_INFO_EVENT,self,function (data)
    ...
    --搜索返回
     EventManager:getInstance():Bind(ArenaManyPeopleEvent.ARENAMANYPOEPLE_SEARCH_TEAM_EVENT,self,function (data)
            if not data then return end
            self:setSearchBack(data)
        end)
   
    --申请单个
     EventManager:getInstance():Bind(ArenaManyPeopleEvent.ARENAMANYPOEPLE_INVITATION_PLAYER_EVENT,self,function (data)
            if not data then return end
    ...
    --申请多个返回
     EventManager:getInstance():Bind(ArenaManyPeopleEvent.ARENAMANYPOEPLE_UPDATE_MENBER_EVENT,self,function (data)
            if not data then return end
