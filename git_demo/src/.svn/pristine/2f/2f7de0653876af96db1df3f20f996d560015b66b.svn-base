...
function JumpController:jumpViewByEvtData( evt_data )
    ...
    print("-------------------jumpViewByEvtData------------------------",evt_id)
	if evt_id == 1 then 		-- 召唤
	...
	elseif evt_id == 8 then 	-- 背包
		...
		MainuiController:getInstance():changeMainUIStatus(MainuiConst.icon.backpack, sub_type)
	...
	elseif evt_id == 13 then 	-- 公会捐献
        ...
        if role_vo:isHasGuild() then
            ...
            GhostdomController:getInstance():onClickGhostdomBuildById(GhostdomConst.Ghostdom_Type,GhostdomConst.Build_Type.guild)
        end
