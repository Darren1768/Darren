...
function ArenaChampionRankWindow:open_callback()
    ...
    main_panel:getChildByName("win_title"):setString(TI18N("将灵排行榜"))
    ...
    self.Image_no_rank = self.my_container:getChildByName("Image_no_rank") 
    self.role_head = PlayerHead.new(PlayerHead.type.circle)
    self.role_head:setHeadLayerScale(0.85)
    self.role_head:setPosition(157, 65)
    self.role_head:setLev(99)
    self.my_container:addChild(self.role_head)

    local imgheadPos = self.my_container:getChildByName("Image_4")
    if imgheadPos then
        imgheadPos:setVisible(false)
        self.role_head:setPosition(imgheadPos:getPositionX(),imgheadPos:getPositionY())
    end
...
function ArenaChampionRankWindow:register_event()
	...
	self.close_btn:addTouchEventListener(function(sender, event_type)
     ...
     EventManager:getInstance():Bind(ArenaEvent.UpdateChampionRankEvent,self, function(data) 
...
function ArenaChampionRankWindow:updateMyInfo(data)
    ...
    if rank <= 3 then
        ...
        if rank == 0 then
            ...
            self.Image_no_rank:setVisible(true)
        else
            ...
            self.Image_no_rank:setVisible(false)
        end
    else
        if self.rank_num == nil then
    ...
    if not self.my_score_label then
        self.my_score_label = createRichLabel(18, cc.c3b(239,212,174), cc.p(0.5, 0.5), cc.p(520, 34))
        self.my_container:addChild(self.my_score_label)
    end
    local my_score = data.score or 0

    self.my_score_label:setString(string_format("%s<div fontColor=#5fb34f>%s</div>", TI18N("选拔赛分数："), my_score))
