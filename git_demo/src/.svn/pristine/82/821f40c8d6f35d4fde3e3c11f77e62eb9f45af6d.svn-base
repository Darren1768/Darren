...
--红点状态 改为箭头
function BackPackItem:createSpriteRedPoint1()
	if not self.redpoint1 then
		local res = PathTool.getResFrame("common2","hl_common_3023")
		-- self.redpoint1 = createSprite(res,101,95,self.main_container,cc.p(0.5, 0.5),LOADTEXT_TYPE_PLIST,10)
		self.redpoint1 = createSprite(res,95,89,self.main_container,cc.p(0.5, 0.5),LOADTEXT_TYPE_PLIST,10)

		self.redpoint1:setVisible(false)
		-- self.redpoint1:setScale(1.5)
		-- self.redpoint1:setRotation(-90)
	end
end
function BackPackItem:setVisibleResPoint1(visible)
	if self.redpoint1 then
		self.redpoint1:setVisible(visible)
	end
end
--选中
...
-- 设置灰色遮罩
function BackPackItem:setMaskVisible2( status,scale  )
	if status == false then
		if self._mask_icon then
			self._mask_icon:setVisible(false)
		end
		-- self:setItemIconUnEnabled(false)
	else
		if self._mask_icon == nil then
			self._mask_icon = createImage(self.main_container,PathTool.getResFrame("common2","hl_common_0007"),BackPackItem.Width/2-1,BackPackItem.Height/2-1,
			cc.p(0.5,0.5),true,10,true)
		end
		self._mask_icon:setContentSize(cc.size(BackPackItem.Width-6,BackPackItem.Height-6))
		self._mask_icon:setVisible(true)
		-- self:setItemIconUnEnabled(true)
		setChildUnEnabled(false, self._mask_icon)
		self._mask_icon:setScale(scale or self.scale)
				...
				if is_click == true then
					...
					if self.btn_call_fun then
						...
						if self.is_show_tips and self.data ~= nil then
							...
							print("--------------self.is_tips_source-----type-------bid-----",bid)
							print("--------------self.is_tips_source-----type-------type-----",type)
						...
						-- 	HeroController:getInstance():openArtifactTipsWindow(true, vo, PartnerConst.ArtifactTips.backpack)
							...
							elseif BackPackConst.checkIsArtifact(type) then 
								-- 符文
								local goodvo = GoodsVo.New(self.item_config.id)
								HeroController:getInstance():openArtifactTipsWindow(true, goodvo)
							elseif BackPackConst.checkIsElfin(type) then -- 精灵
								...
								elseif self.is_tips_source then
									print("--------------self.is_tips_source-----------------",self.is_tips_source)
	...
	if not self.is_hide_effect == true then
		if config and config.is_effect and config.is_effect >= 1 then
			...
			if effect_id >= 1671 and effect_id <= 1674 then
				--这个几个是背景框的资源 因为 背景框是 138 这里是 105 为了不输出两份资源 通用的只有按比例缩放 暂时不改item表情况下写死  
				--后面如果有比较多类似的特有特效 建议在item_data表加字段支持
				scale = 105/138
			elseif effect_id == 354 then--猫咪物语 --记得加字段支持了  
				scale = 0.7
				self.item_icon:setVisible(false)
			end 
...
function BackPackItem:setData(data,is_hide_effect)
	...
	if self.data == nil or id == nil then
		...
		if self.data.is_show_finish then 
		   self:setReceivedIcon2(self.data.is_show_finish)
		else 
			self:setReceivedIcon2(false)

	    end 

		if self.data.is_show_mask then 
			self:setMaskVisible2(self.data.is_show_finish)
		 else 
			 self:setMaskVisible2(false)
		 end 
		-- 设置碎片
...
function BackPackItem:createCompLayer()
	if not self.compLayer then
	    ...
	    local res = PathTool.getResFrame("common2","hl_common_2203")
	    local res1 = PathTool.getResFrame("common2","hl_common_2202")
	    local bg,comp_bar = createLoadingBar1(res, res1, cc.size(107, 19), self.compLayer, cc.p(0,0.5), 0, self.compLayer:getContentSize().height/2, true, true)
	    self.camp_progress_bar = comp_bar

	    local text_bar = createLabel(20,cc.c3b(255,255,255),cc.c3b(80,71,80),self.compLayer:getContentSize().width/2,self.compLayer:getContentSize().height/2,"0/0",self.compLayer,1, cc.p(0.5,0.5))
...
function BackPackItem:setLockStatus( item_type )
	...
	if lock_status == true then
		if not self.lock_icon then
			self.lock_icon = createSprite(PathTool.getResFrame("common","hl_common_92"), self.size.width-5, self.size.height-5, self.main_container, cc.p(1, 1), LOADTEXT_TYPE_PLIST)
		end
...
function BackPackItem:showArtifactLock(bool)
	...
	if not self.artifact_lock then 
		local res = PathTool.getResFrame("common","hl_common_92")
...
function BackPackItem:showOrderWarLock(bool,pos)
	...
	if not self.order_war_lock then 
		...
		self.order_war_lock = createSprite(PathTool.getResFrame("common","hl_common_92"), temp_pos.x,temp_pos.y, self.root_wnd, cc.p(0.5,0.5))
	end
...
function BackPackItem:IsGetStatus(bool,opacity,res)
	...
	if not self.is_get_select then
        ...
        self.is_get_select:setContentSize(cc.size(self.size.width/4,self.size.height/4))
        self.is_get_select:setPosition(self.size.width/8*7, self.size.height/8*7) 
        self.is_get_select:setTouchEnabled(false)
        showLayoutRect(self.is_get_select, opacity)
		local temp_res = res or PathTool.getResFrame("common","hl_common_591")
		createSprite(temp_res, self.size.width/8,self.size.height/8, self.is_get_select, cc.p(0.5,0.5))
...
function BackPackItem:showAddIcon(bool)
	...
	if not self.add_btn then 
		local res = PathTool.getResFrame("common","hl_common_597")
...
function BackPackItem:showRedPoint(status )
    ...
    if not self.red_point then 
        local res = PathTool.getResFrame("common","hl_common_3")
        self.red_point =createImage(self,res,86,86,cc.p(0,0),true,10,false)
    end
...
function BackPackItem:setFirstIcon(status,scale)
	if status == false then
		...
		if self.first_icon == nil then
			self.first_icon = createSprite(PathTool.getResFrame("common2","hl_common_3020"),22,120,self,cc.p(1,1),LOADTEXT_TYPE_PLIST,20)
			local _label = createLabel(20,cc.c3b(239,212,174), nil,10,36,"首\n通", self.first_icon, nil, cc.p(0.5,0.5))
...
-- 设置已领取标识2 已完成 
function BackPackItem:setReceivedIcon2( status,scale  )
	if status == false then
		if self.had_finish_icon then
			self.had_finish_icon:setVisible(false)
		end
		-- self:setItemIconUnEnabled(false)
	else
		if self.had_finish_icon == nil then
			self.had_finish_icon = createSprite(PathTool.getResFrame("common2","hl_common_0026"),BackPackItem.Width/2,BackPackItem.Height/2,self.main_container,cc.p(0.5, 0.5),LOADTEXT_TYPE_PLIST,30)
		end
		self.had_finish_icon:setVisible(true)
		-- self:setItemIconUnEnabled(true)
		setChildUnEnabled(false, self.had_finish_icon)
		self.had_finish_icon:setScale(scale or 1.36)
...
-- 设置达到未领取 代替特效
function BackPackItem:setReceivedIcon3( status,scale)
	if status == false then
		if self.will_finish_icon then
			self.will_finish_icon:setVisible(false)
		end
		-- self:setItemIconUnEnabled(false)
	else
		if self.will_finish_icon == nil then
			self.will_finish_icon = createSprite(PathTool.getResFrame("common2","hl_common_0025"),BackPackItem.Width/2-1,BackPackItem.Height/2,self.main_container,cc.p(0.5, 0.5),LOADTEXT_TYPE_PLIST,20)
		end
		self.will_finish_icon:setVisible(true)
		-- self:setItemIconUnEnabled(true)
		setChildUnEnabled(false, self.will_finish_icon)
		self.will_finish_icon:setScale(scale or 1.4)
...
function BackPackItem:showUseTag(status,show_type,scale)
	if status == true then
		scale = scale or 1
		local res = PathTool.getResFrame("common","hl_common_2201")
		-- if show_type == 2 then
		...
		if not self.use_ponit then
			self.use_ponit = createImage(self.main_container, res,0, 124, cc.p(0, 1), true, 10, false)
			self.use_ponit:setScale(0.7)
			local _label = createLabel(20,cc.c3b(250,232,150), nil,40,30,"使用", self.use_ponit, nil, cc.p(0.5,0.5),TITLE_FONT)
		end
...
--vip界面道具
function BackPackItem:showVIPstatus()
	if not self.vipBackPic then
		local res = PathTool.getResFrame("vip","hl_vip_1003")
		self.vipBackPic = createSprite(res,59.5,50,self.main_container,cc.p(0.5, 0.5),LOADTEXT_TYPE_PLIST,-1)
	end
    self.vipBackPic:setScale(1.1)
    self.vipBackPic:setVisible(true)
    -- self.num_bg:setVisible(false)
    self.num_label:setAnchorPoint(cc.p(0.5,0.5))
    self.num_label:setPosition(cc.p(59.5,-30))
    self.num_label:setTextColor(cc.c4b(0xe7,0xd5,0xc4,0xff))
	self.num_label:enableOutline(cc.c4b(0x53,0x24,0x17,0xff), 1)
    self.num_label:setFontSize(20)
end
...
function BackPackItem:playItemSound()
	if self.item_sound ~= nil then
		GlobalAudio:getInstance():removeEffectByData(self.item_sound)
	end
	self.item_sound = GlobalAudio:getInstance():playEffect(GlobalAudio.AUDIO_TYPE.COMMON, 'c_get02', false)
end
...
function BackPackItem:setSummonNumber(label)
	if not self.summon_text then
		self.summon_text = createLabel(20,cc.c4b(0x8e,0x59,0x38,0xff),nil,62,-15,"",self.main_container,nil, cc.p(0.5,0.5))
	end
...
function BackPackItem:showStrTipsV(status, str, setting)
    if status then
        if self.tips_img1 == nil then
			local size = cc.size(80, 33)
            local res = PathTool.getResFrame("common2", "hl_common_0008")
			
            self.tips_img1 = createScale9Sprite(res, 0, 1, LOADTEXT_TYPE_PLIST, self.main_container)
            self.tips_img1:setContentSize(cc.size(36,  96))
            self.tips_img1:setPosition(60,60)

			local setting = setting or {}
			local color = setting.color or  cc.c3b(250,232,150)
            local font_size =  24
            self.tips_text1 = createLabel(font_size, color,nil, 16, 48, str, self.tips_img1, nil, cc.p(0.5, 0.5))

        else
            self.tips_img1:setVisible(true)
            if self.tips_text1 and str then
            	self.tips_text1:setString(str)
            end
        end
    else
        if self.tips_img1 then
            self.tips_img1:setVisible(false)
        end
    end
end
