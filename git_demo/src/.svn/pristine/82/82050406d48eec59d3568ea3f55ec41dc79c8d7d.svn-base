...
function HomeworldRole:createRoorWnd(  )
    ...
    -- 预加载音效
    GlobalAudio:getInstance():preLoadEffect(GlobalAudio.AUDIO_TYPE.COMMON, 'c_home_character01')
    GlobalAudio:getInstance():preLoadEffect(GlobalAudio.AUDIO_TYPE.COMMON, 'c_home_character02')
end
...
function HomeworldRole:playRoleAction( action )
    ...
    if action == PlayerAction.caught_1 then -- 提起来
        GlobalAudio:getInstance():playEffect(GlobalAudio.AUDIO_TYPE.COMMON, "c_home_character01")
    elseif action == PlayerAction.caught_3 then -- 放下去
        GlobalAudio:getInstance():playEffect(GlobalAudio.AUDIO_TYPE.COMMON, "c_home_character02")
    end
