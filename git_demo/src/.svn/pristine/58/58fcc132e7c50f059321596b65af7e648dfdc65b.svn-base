...
function CommonOptionsListBtn:ctor(parent, func_name, label_str, target, width, height, call_back)
	...
	self.button = createTypeButton(self, label_str, width*0.5, height*0.5, ButtonType.red)
...
function CommonOptionsListBtn:add_friend()
	if self.target ~= nil then
		 EventManager:getInstance():Fire(FriendEvent.FRIEND_ADD, self.target.srv_id, self.target.rid)
	end
end
