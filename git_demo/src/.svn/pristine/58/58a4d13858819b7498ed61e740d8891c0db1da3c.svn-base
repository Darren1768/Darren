...
function HalidomUpStepWindow:__init()
    ...
    self.res_list = {
       {path = PathTool.getSingleImgForDownLoad("bigbg","txt_cn_bigbg_28"), type = ResourcesType.single },
    }
...
function HalidomUpStepWindow:open_callback(  )
    ...
    local res = PathTool.getSingleImgForDownLoad("bigbg","txt_cn_bigbg_28")
    loadSpriteTexture(title, res, LOADTEXT_TYPE)
    
    local size = self.main_panel:getContentSize()

    self.icon_kuang = createSprite(PathTool.getResFrame("common", "hl_common_91"), size.width/2, 530, self.main_panel, cc.p(0.5, 0.5))
