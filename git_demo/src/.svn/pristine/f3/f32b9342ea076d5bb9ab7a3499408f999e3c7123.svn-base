...
function HeavenModel:updateChapterData( data )
	...
	self:checkHeavenRedStatus()
	if is_add then
		EventManager:getInstance():Fire(HeavenEvent.Add_Chapter_Data_Event)
	end
end
...
function HeavenModel:updateHeavenRedStatus( bid, status )
    ...
    EventManager:getInstance():Fire(HeavenEvent.Update_Heaven_Red_Status, bid, status)
end
...
function HeavenModel:setAllScore( score )
	...
	EventManager:getInstance():Fire(HeavenEvent.Update_All_Score)
end
