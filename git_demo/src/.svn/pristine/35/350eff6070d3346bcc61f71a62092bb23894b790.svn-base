...
function GuideController:registerEvents()
    -- 剧情结束,判断是否需要播放引导
     EventManager:getInstance():Bind(StoryEvent.STORY_OVER,self, function() 
    ...
    -- 升级界面关闭之后,判断是否需要播放引导
     EventManager:getInstance():Bind(StoryEvent.PREPARE_PLAY_PLOT,self, function() 
            if self.guide_list ~= nil and next(self.guide_list) ~= nil then
                local config = table.remove(self.guide_list, 1)
                self:checkGuideToPlay(config)
            end
...
function GuideController:startPlayGuide(status, id, is_skip)
    if status == false then
        ...
        -- 结束引导
        EventManager:getInstance():Fire(GuideEvent.Update_Guide_Status_Event, false)
    else
        ...
        -- 播放引导
        EventManager:getInstance():Fire(GuideEvent.Update_Guide_Status_Event, true)
    end
end
...
function GuideController:handle_11120(data)
    ...
    --引导前给需要特殊处理界面抛事件
    EventManager:getInstance():Fire(GuideEvent.Update_Guide_Open_Event)
