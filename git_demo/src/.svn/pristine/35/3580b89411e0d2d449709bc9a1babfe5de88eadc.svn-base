...
function RoleScenePlayer:step( dt )
	...
	if self.move_pass_distance >= self.move_total_distance then
		...
		if self.is_hero then
			...
			EventManager:getInstance():Fire(SceneEvent.SCENE_WALKEND, pos)
			self.stepPost = 0
		end
	else
		...
		if self.is_hero then
			...
			if MapUtil.isOnRange( self.move_end_pos, now_pos, self.move_speed ) then --拐点
				-- EventManager:getInstance():Fire(SceneEvent.SCENE_WALKING, now_pos)
				EventManager:getInstance():Fire(SceneEvent.SCENE_WALKNEXT, now_pos)
...
function RoleScenePlayer:clickHandler()
	...
	EventManager:getInstance():Fire(SceneEvent.SCENE_PLAYER_CLICK, self.vo)
end
