...
function VoyageMainWindow:__init()
	...
	self.res_list = {
        ...
        {path = PathTool.getSingleImgForDownLoad("bigbg","hl_bigbg_12", false), type = ResourcesType.single },
	}
...
function VoyageMainWindow:open_callback(  )
	...
	win_title:setString(TI18N("金鱼悬赏"))
	...
	self.refresh_btn_label = createRichLabel(26, 1, cc.p(0.5, 0.5), cc.p(refresh_btn_size.width/2, refresh_btn_size.height/2+3),nil,nil,nil,TITLE_FONT)
	self.refresh_btn:addChild(self.refresh_btn_label)

	local node_mask = main_container:getChildByName("node_mask")
	local x,y = 0,0
	local res = PathTool.getResFrame("common","hl_common_44")
	self.hl_common_44 = createScale9Sprite(res, x,y, LOADTEXT_TYPE_PLIST, node_mask)
	self.hl_common_44:setContentSize(cc.size(600,144))

	self.lbl_challange_num = main_container:getChildByName("lbl_challange_num")
    ...
    local setting = {
        ...
        start_y = 0,                    -- 第一个单元的Y起点
        space_y = 8,                   -- y方向的间隔
        item_width = 187,               -- 单元的尺寸width
        item_height = 283,              -- 单元的尺寸height
        row = 0,                        -- 行数，作用于水平滚动类型
        col = 3,                         -- 列数，作用于垂直滚动类型
        need_dynamic = true
    }
    self.order_scrollview = CommonScrollViewSingleLayout.new(order_list, cc.p(0,0) , ScrollViewDir.vertical, ScrollViewStartPos.top, scroll_view_size, setting)
...
function VoyageMainWindow:register_event(  )
	...
	-- 刷新次数
	self:addGlobalEvent(VoyageEvent.UpdateOrderTimes, function ( )
		self:refreshOrderTimes()
	end)
...
function VoyageMainWindow:refreshOrderList( keep_pos )
	...
	if next(self.order_data) == nil then
		commonShowEmptyIcon(self.main_container,true,{text = '刷新获取新的订单'})
...
function VoyageMainWindow:refreshBtnStatus(  )
	...
	if free_times < free_count then
		self.refresh_btn_label:setString(TI18N("<div fontcolor=#e5dde2>免费刷新</div>"))
	...
	if count >= self.cost_item_num then
		...
		self.refresh_btn_label:setString(string.format(TI18N("<img src='%s' scale=0.4 /><div fontcolor=#e5dde2>%d 刷新</div>"), res, self.cost_item_num))
		setChildUnEnabled(false, self.refresh_btn)
		return
	end
	-- 钻石
	...
	if coin_times < coin_count then
		...
		local res = PathTool.getItemRes(item_config.icon)
		self.refresh_btn_label:setString(string.format(TI18N("<img src='%s' scale=0.3 /><div fontcolor=#e5dde2>%d 刷新</div>"), res, num))
		setChildUnEnabled(false, self.refresh_btn)
		return
	end

	self.refresh_btn_label:setString(TI18N("<div fontcolor=#e5dde2>刷新</div>"))
...
function VoyageMainWindow:refreshOrderTimes()
	
	local order_times = model:getOrderTimes()
	local max_order_times = model:getMaxOrderTimes()
	self.orderTimes = order_times
	self.maxOrderTimes = max_order_times

	print("===========order_times======",order_times)
	local left_times = math.max(self.maxOrderTimes - self.orderTimes,0)
	self.lbl_challange_num:setString(string.format("今日剩余次数：%d/%d",left_times,max_order_times))
end
