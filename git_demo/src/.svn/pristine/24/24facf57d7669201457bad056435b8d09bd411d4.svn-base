...
function TaskPanel:ctor(ctrl)
    ...
    self.item_scrollview = CommonScrollViewLayout.new(self.quest_container,cc.p(0, 0),
        ScrollViewDir.vertical,ScrollViewStartPos.top,cc.size(self.size.width, self.size.height),setting)
                ...
                if event_type == ccui.TouchEventType.ended then
                    if sender.config ~= nil then
                        if sender.is_show_tips == true then
                            CommonRewardAlert.showItemApply(TI18N("当前活跃度奖励"),sender.config.rewards,nil,TI18N("确定"),nil,nil,nil,nil,nil,true,nil, nil,40)
                        else
...
function TaskPanel:handleDynamicEvent(status)
    ...
    EventManager:getInstance():UnBind(self)
     ...
     EventManager:getInstance():Bind(TaskEvent.UpdateActivityInfo,self,function(data)
                --self:updateActivityData(data)
            end)
     EventManager:getInstance():Bind(TaskEvent.UpdateTaskList,self,function(is_new)
