...
function ActionHeroConvertPanel:configUI( )
    ...
    if self.left_img_load == nil then
        ...
        local res = PathTool.getSingleImgForDownLoad("bigbg/action", title_str, false)
    ...
    if self.item_load == nil then
        ...
        local res = PathTool.getSingleImgForDownLoad("bigbg/action", title_str, true)
...
function ActionHeroConvertPanel:register_event(  )
    ...
    for select_camp, v in pairs(self.camp_btn_list) do
     ...
     EventManager:getInstance():Bind(HeroEvent.Upgrade_Star_Select_Event,self,function (data)
            --刷新数据就星了
            if self.select_data then
                self:updateHeroItem(self.select_data)
            end
        end)
     EventManager:getInstance():Bind(HeroEvent.Del_Hero_Event,self,function (data)
            --刷新数据就星了
    ...
    --获取升星材料返回
     EventManager:getInstance():Bind(HeroEvent.Hero_Reset_Star_Event,self, function(data)
