...
function ActionTimeElfinSummonGainWindow:createRootWnd()
    ...
    self.image_top_bg = createScale9Sprite(PathTool.getSingleImgForDownLoad("bigbg", "bigbg_12"), SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 + 55, LOADTEXT_TYPE, self.source_container)
    ...
    self.image_bottom_bg = createScale9Sprite(PathTool.getSingleImgForDownLoad("bigbg", "bigbg_12"),SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 + 55, LOADTEXT_TYPE, self.source_container)
...
function ActionTimeElfinSummonGainWindow:updateItemData(data)
    ...
    if is_show_effect == true then
        ...
        self.bg_res_id = PathTool.getSingleImgForDownLoad("bigbg/partnersummon", "partnersummon_call_bg_400", true)
    else
...
function ActionTimeElfinSummonGainWindow:resgiter_event()
    ...
    -- 召唤数据更新
     EventManager:getInstance():Bind(ElfinEvent.Elfin_Check_Show_Tips_Event,self,function ()
            delayOnce(function ()
                self:checkElfinTips()
            end,0.1)
        end)
