...
function EndlessTrailBattleView:registerEvent( ... )
    ...
    if self.backpack_btn then
     ...
     EventManager:getInstance():Bind(Endless_trailEvent.UPDATA_ENDLESSBATTLE_DATA,self,function(data)
    ...
    if self.battle_data and data and Config.EndlessData.data_first_data[self.battle_data.type] and Config.EndlessData.data_first_data[self.battle_data.type][data.id] then
        ...
        if first_data then
            ...
            if data.status == 1 then
                ...
                str = string_format(TI18N('<div fontcolor=#fff22a outline=2,#581818>%s</div><div fontcolor=#ffffff, outline=2,#581818>位领取</div>'), first_data.limit_id) -- - self.battle_data.max_round
