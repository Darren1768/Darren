...
function VoyageDispatchWindow:__init()
    ...
    self.res_list = {
        ...
        {path = PathTool.getSingleImgForDownLoad("bigbg","hl_bigbg_13", false), type = ResourcesType.single },
    }
...
function VoyageDispatchWindow:open_callback(  )
    ...
    self.hero_box = main_container:getChildByName("Image_2")

    self.node_item = main_container:getChildByName("item_node")
    ...
    self.quick_btn:loadTexture('resource/voyage/hl_form_1015.png')
    self.quick_btn:setScale9Enabled(true)
    self.quick_btn:setContentSize(cc.size(160,45))
    local label=self.quick_btn:getChildByName("label")
    label:setString(TI18N("一键添加"))
    -- label:setTextColor(cc.c3b(98,53,43))
    self.dispatch_btn = main_container:getChildByName("dispatch_btn")
    self.dispatch_btn:getChildByName("label"):setString(TI18N("派遣"))
    ...
    self.status_label:setString(TI18N("派遣武灵"))
    ...
    local scroll_view_size = cc.size(bg_size.width, bg_size.height)
    local setting = {
    ...
    self.hero_scrollview = CommonScrollViewLayout.new(lay_scrollview, cc.p(0,0 ) , ScrollViewDir.vertical, ScrollViewStartPos.top, scroll_view_size, setting)
...
function VoyageDispatchWindow:openRootWnd( data )
    ...
    self:refreshOrderInfo(data)
end

--添加支援将
function VoyageDispatchWindow:addHirePartnerData(allHeroList,herolist)
    for i,v in pairs(herolist) do
        -- print("==========输出支援队伍数据======i,v=== :",i,"    v  :",v)
        v.partner_id = v.pid
        v.id = v.pid
...
function VoyageDispatchWindow:refreshCampHeroList(  )
    ...
    if hero_num <= 0 then
        ...
        commonShowEmptyIcon(self.hero_box,true,{text = '暂无该阵营英雄'})
        self.hero_scrollview:setData({})
        return
    else
        commonShowEmptyIcon(self.hero_box,false)
    end
    local hero_list = hero_array.items
    local new_hero_list = clone(hero_list)
    ...
    table_sort(new_hero_list, sortFunc)

    --**************************** 添加队伍支援 *********************************
    local _team_data = organize_model:getHirePartnerData()

    print("==========添加队伍支援=== :")
    self:addHirePartnerData(new_hero_list,_team_data.list)
    ...
    --     v.name =  partner_config[v.bid].name
    --     --支援的人物使用过之后就放到最后
    --     table_insert(new_hero_list,v)
    -- end
    --**************************** 添加队伍支援 *********************************
        
    local extendData = {scale = 0.9, can_click = true, click_delay = 0.5, from_type = HeroConst.ExhibitionItemType.eVoyage}
    self.hero_scrollview:setData(new_hero_list, function(item, hero_vo) self:_onClickHero(item ,hero_vo) end, nil, extendData)
end
...
function VoyageDispatchWindow:showBaseInfo( data )
    ...
    if not self.des_label then
        self.des_label = createRichLabel(22, cc.c4b(0x78,0x72,0x69,0xff), cc.p(0, 0.5), cc.p(103, 422), 3, 0, 500)
        self.main_container:addChild(self.des_label)
        self.des_label:setString(TI18N( '最近生珲春老是有小偷，帮助老杰克村长抓住小偷吧。大不列颠大人，我家小孩说你有一些你有一些五菱。很可爱！'))
    end
    ...
    for i=1,limit_num do
        ...
        if hero_box == nil then
            hero_box = createImage(self.hero_layout, PathTool.getResFrame("common", "hl_common_180"), 0, 0, cc.p(0.5, 0.5), true)
            local add = createImage(hero_box, PathTool.getResFrame("common", "hl_common_181"), 0, 0, cc.p(0.5, 0.5), true)
            add:setPosition(cc.p(44,44))
            hero_box:setScale9Enabled(true)
    ...
    if need_star > 0 then
        -- self.star_icon = createImage(self.condition_layout, PathTool.getResFrame("voyage", "voyage_1009"), 0, -2, cc.p(0.5, 0.5), true)
        self.star_icon = createImage(self.condition_layout, PathTool.getResFrame("common", "hl_common_58"), 0, -2, cc.p(0.5, 0.5), true)
        local star = createImage(self.star_icon, PathTool.getResFrame("common", "hl_common_1007"), 26, 27, cc.p(0.5, 0.5), true)
        star:setScale(1.3)
        self.star_text = createLabel(22,1,cc.c3b(132,65,0),24.4,23.5,star_num,self.star_icon,nil,cc.p(0.5, 0.5))
        -- self.star_text_unenable = createLabel(22,cc.c3b(98,53,43),cc.c3b(132,65,0),x1,y1,star_num,self.main_container,nil,cc.p(0.5, 0.5))
        self.star_icon:setTouchEnabled(true)
        self.star_icon:setScale(1.3)
        self.star_icon:setScale9Enabled(true)
        ...
        -- self.star_text:setVisible(false)
        -- self.star_text_unenable:setVisible(true)
    ...
    for i,camp_type in ipairs(need_camps) do
        ...
        if camp_icon == nil then
            camp_icon = createImage(self.condition_layout, PathTool.getHeroCampTypeIcon(camp_type), 0, -4, cc.p(0.5, 0.5), true)
            camp_icon:setScale(0.8)
            camp_icon:setTouchEnabled(true)
            camp_icon:setScale9Enabled(true)
...
function VoyageDispatchWindow:refreshConditionStatus(  )
    ...
    for k,v in pairs(self.conditions or {}) do
        ...
        if v.star_num then
            ...
            -- self.star_text:setVisible(true)
            -- self.star_text_unenable:setVisible(false)
        elseif v.camp_type then
    ...
    if self.con_status then
        self.status_label:setTextColor(cc.c3b(37,18,28))
    else
        self.status_label:setTextColor(cc.c3b(201,38,6))
    end
end

function VoyageDispatchWindow:refreshOrderInfo( data )
...
function VoyageDispatchWindow:createChoseHeroIcon( hero_vo, item )
    ...
    if hero_box then
        ...
        if item then
            local world_pos = item:convertToWorldSpace(cc.p(0, 0))
            local node_pos = hero_box:convertToNodeSpace(world_pos)
            hero_icon:setPosition(cc.p(node_pos.x+44, node_pos.y+44))
            hero_icon:runAction(cc.MoveTo:create(0.07, cc.p(44, 44)))
        else
            hero_icon:setPosition(cc.p(44, 44))
        end
        hero_icon:setScale(0.9)
    end
end
...
function VoyageDispatchWindow:_onClickQuickChoseBtn(  )
    ...
    local new_all_hero = clone(all_hero)

    --**************************** 添加队伍支援 *********************************
    local _team_data = organize_model:getHirePartnerData()
    self:addHirePartnerData(new_all_hero,_team_data.list)
    -- for i,v in pairs(_team_data.list) do
    --     v.partner_id = v.pid
    --     v.id = v.pid
    --     v.camp_type = partner_config[v.bid].camp_type
    ...
    for k,hero_vo in pairs(new_all_hero or {}) do
...
function VoyageDispatchWindow:_onClickDispatchBtn(  )
    local order_times = model:getOrderTimes()
	local max_order_times = model:getMaxOrderTimes()
    print("=====order_times===max_order_times======",order_times,max_order_times)
    if order_times>=max_order_times then
        message(TI18N("今日派遣次数已用完"))
        return
    end
