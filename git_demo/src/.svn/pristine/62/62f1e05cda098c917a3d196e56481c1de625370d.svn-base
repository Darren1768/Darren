...
function HomeworldMyUnit:registerEvent(  )
    ...
    registerButtonEventListener(self.save_btn, function (  )
        EventManager:getInstance():Fire(HomeworldEvent.Save_My_Home_Data_Event)
    end, true)

    -- 从场景中移除一个家具(进入家园仓库)
     EventManager:getInstance():Bind(HomeworldEvent.Discharge_Furniture_Event,self, function(unit_bid) 
    ...
    -- 场景中增加一个道具
     EventManager:getInstance():Bind(HomeworldEvent.Add_Furniture_Event,self, function(unit_bid)
            if not self.cur_show_status then return end
            self:updateCurSoft()
    ...
    -- 红点
     EventManager:getInstance():Bind(HomeworldEvent.Update_Red_Status_Data,self,function(bid, status)
...
function HomeworldMyUnit:_onClickClearBtn(  )
    EventManager:getInstance():Fire(HomeworldEvent.Clear_All_Furniture_Event)
    self.my_all_unit_data = {}
    -- 背包中的家具
...
function HomeworldMyUnit:_onClickMyUnitCallback( unit_bid )
    ...
    -- 通知家园场景添加一个家具
    EventManager:getInstance():Fire(HomeworldEvent.Add_One_Furniture_Event, unit_bid)
end
