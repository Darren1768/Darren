...
function ServerPanel:layoutUI()
	...
	if PLATFORM_NAME == "release2" then
		self.editbox =  createEditBox(self.root_wnd, PathTool.getResFrame("common", "hl_chat_3"), cc.size(320,50), nil, 24, Config.ColorData.data_color3[151],
...
function ServerPanel:registerEvent()
	if self.btn_close then
     ...
     EventManager:getInstance():Bind(LoginEvent.SERVER_ROLELIST_CHANGE,self, function()
			self:updateCell()
    	end)
end
