...
function HomepetController:registerEvents()
    ...
    --事件触发
    EventManager:getInstance():Bind(HomepetEvent.HOME_PET_CHECK_TRIGGER_EVENT,self,function()
    ...
    --监听窗口关闭事件 检测事件触发
    EventManager:getInstance():Bind(EventId.CLOSE_BASE_VIEW,self,function(close_win)
...
function HomepetController:handle26103( data )
    ...
    if data.flag == 1 then
        ...
        EventManager:getInstance():Fire(HomepetEvent.HOME_PET_TALK_EVENT, data)
    end
end
--宠物交互支付道具
...
function HomepetController:handle26108( data )
    ...
    EventManager:getInstance():Fire(HomepetEvent.HOME_PET_GO_OUT_NEW_EVENT, data)
end
...
function HomepetController:handle26107( data )
    ...
    if data.code == TRUE then
        ...
        EventManager:getInstance():Fire(HomepetEvent.HOME_PET_TRAVELLING_BAG_UPDATE_EVENT)
    end
end  
...
function HomepetController:handle26109( data )
    EventManager:getInstance():Fire(HomepetEvent.HOME_PET_THIS_TIME_ALL_EVENT, data)
end 
