...
function GuildBossBoxRewardWindow:__init()
	...
	self.res_list = {
		...
		{path = PathTool.getSingleImgForDownLoad("bigbg", "guildboss/guildboss_1"), type = ResourcesType.single}, 
    }
end 
...
function GuildBossBoxRewardWindow:register_event()
	self.close_btn:addTouchEventListener(function(sender, event_type)
     ...
     EventManager:getInstance():Bind(GuildbossEvent.UpdateBoxRewardsStatus,self, function(status)
	...
	self.model:addTouchEventListener(function(sender, event_type)
     ...
     EventManager:getInstance():Bind(EventId.CAN_OPEN_LEVUPGRADE,self, function()
