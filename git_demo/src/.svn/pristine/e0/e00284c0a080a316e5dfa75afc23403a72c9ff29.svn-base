...
function RoleHonorItem:ctor(scale, can_click, click_delay, can_effect, not_swallow)
    ...
    local csbPath = PathTool.getTargetCSB("common/personal_badge")
    self.root_wnd = createCSBNote(csbPath)
    self:addChild(self.root_wnd)
    
    local has = self.root_wnd:getChildByName('has')
    self.has = has
    self.icon = has:getChildByName('pic')
    self.name = has:getChildByName('back_label'):getChildByName('Text_1')
...
function RoleHonorItem:setIcon(icon_res)
    ...
    if self.record_icon_res == nil or self.record_icon_res ~= icon_res then
        ...
        local res = PathTool.getSingleImgForDownLoad("rolehonorwall/honorwarllicon", icon_res, false)
...
function RoleHonorItem:setIconUnEnabled(bool)
    if self.has then
        setChildUnEnabled(bool, self.has)
    end
end
