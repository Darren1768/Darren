...
function LadderLogWindow:open_callback(  )
    ...
    self.image_cat = container:getChildByName("image_cat")
    self.lbl_tips = self.image_cat:getChildByName("Text_2")
    self.image_cat:setVisible(false)
    ...
    for i=1,2 do
        ...
        if tab_btn then
            ...
            local title_select = tab_btn:getChildByName("title_0")
            title:setVisible(i~=1)
            title_select:setVisible(i==1)
            if i == 1 then
                title:setString(TI18N("我的记录"))
                title_select:setString(TI18N("我的记录"))
            elseif i == 2 then
                title:setString(TI18N("大神风采"))
                title_select:setString(TI18N("大神风采"))
            end
            object.tab_btn = tab_btn
            object.label = title
            object.title_select = title_select
            object.index = i
    ...
    self.my_log_scrollview = CommonScrollViewLayout.new(self.my_log_panel, cc.p(0,0) , ScrollViewDir.vertical, ScrollViewStartPos.top, scroll_view_size, setting)
    ...
    self.god_log_scrollview = CommonScrollViewLayout.new(self.god_log_panel, cc.p(0,0) , ScrollViewDir.vertical, ScrollViewStartPos.top, scroll_view_size, setting)
...
function LadderLogWindow:register_event(  )
    ...
    EventManager:getInstance():Bind(LadderEvent.UpdateLadderMyLogData,self, function ( data )
    ...
    EventManager:getInstance():Bind(LadderEvent.UpdateLadderGodLogData,self, function ( data )
...
function LadderLogWindow:changeSelectedTab( index )
    ...
    if self.tab_object then
        self.tab_object.tab_btn:loadTextures(PathTool.getResFrame("ladderrank","hl_ladder_rank_1003"), "", "", LOADTEXT_TYPE_PLIST)
        self.tab_object.label:setVisible(true)
        self.tab_object.title_select:setVisible(false)
        self.tab_object = nil
    end
    self.tab_object = self.tab_list[index]
    if self.tab_object then
        self.tab_object.tab_btn:loadTextures(PathTool.getResFrame("ladderrank","hl_ladder_rank_1013"), "", "", LOADTEXT_TYPE_PLIST)
        self.tab_object.label:setVisible(false)
        self.tab_object.title_select:setVisible(true)
    end
    ...
    local foo = (index == 1 and self.myLogData and next(self.myLogData) ~= nil) or (index == 2 and self.godLogData and next(self.godLogData) ~= nil)
    self.image_cat:setVisible(not foo)
    self.lbl_tips:setString(TI18N("暂无战报"))
    ...
    --     commonShowEmptyIcon(self.god_log_panel,false)

    -- else

    --     commonShowEmptyIcon(self.god_log_panel,true, {text = TI18N("暂无战报")})
    -- end
...
function LadderLogWindow:setMyLogData( myLogData )
    ...
    self.image_cat:setVisible(not (myLogData and next(myLogData) ~= nil))
    self.lbl_tips:setString(TI18N("暂无战报"))
end
...
function LadderLogWindow:setGodLogData( godLogData )
    ...
    self.image_cat:setVisible(not (godLogData and next(godLogData) ~= nil))
    self.lbl_tips:setString(TI18N("暂无战报"))
end
