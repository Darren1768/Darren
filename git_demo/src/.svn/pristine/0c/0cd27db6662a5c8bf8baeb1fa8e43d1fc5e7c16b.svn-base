...
function GuildskillMainWindow:__init()
    ...
    self.res_list = {
        ...
        -- {path = PathTool.getSingleImgForDownLoad("guildskill", "guild_skill_bg", true), type = ResourcesType.single},
        {path = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_106",false), type = ResourcesType.single},
        {path = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_107",false), type = ResourcesType.single},

        {path = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_108",false), type = ResourcesType.single},
        {path = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_109",false), type = ResourcesType.single},
        {path = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_110",false), type = ResourcesType.single},
        {path = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_111",false), type = ResourcesType.single},


        {path = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_14",false), type = ResourcesType.single},
        -- {path = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_15",false), type = ResourcesType.single},
        -- {path = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_16",false), type = ResourcesType.single},
    ...
    -- self.pvp_box_pos_height = 165
       --box_bg 原来高度
       self.box_pos_height = 100
       --box_bg 原来pvp高度
       self.pvp_box_pos_height = 100
...
function GuildskillMainWindow:open_callback()
    ...
    self.background:loadTexture( PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_106",false), LOADTEXT_TYPE)
    self.background:setScale(display.getMaxScale())

    self.bg_up= self.root_wnd:getChildByName("bg_up")
    if  not  self.bg_up_load then
        self.bg_up_load = loadSpriteTextureFromCDN(self.bg_up,  PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_107",false), ResourcesType.single, self.bg_up_load) 
    end
    -- self.bg_up:setPositionY(750)
    ...
    for i=1,4 do
        ...
        if tab_btn then
            ...
            tab_btn.label = title
            tab_btn.label:setTextColor(cc.c4b(231, 213, 196, 0xff))
            tab_btn.label:enableOutline(cc.c4b(83, 59, 59, 0xff),1)
    ...
    self.fight_label = CommonNum.new(220, self.power_click, 99999, - 2, cc.p(0.5, 0.5))
    ...
    for i=1,6 do
        ...
        local item_bg  = self.main_panel:getChildByName("item_bg_"..i)
        if item then
            object.node = item 
            object.lev = item_lev 
            object.index = i
    ...
    if config and config.val then
        ...
        for i,v in ipairs(config.val) do
            ...
            self.attr_pos_x[i] = _x + 12
            self.attr_list[i] = createRichLabel(22,cc.c3b(0x64,0x32,0x23),cc.p(0,0.5),cc.p(self.attr_pos_x[i], _y),nil ,nil ,nil )
            self.scroll_view:addChild(self.attr_list[i])
            self.attr_list_1[i] = createLabel(22,cc.c3b(100,50,35),nil,100,-5,"",self.scroll_view,nil,cc.p(0, 0.5))
        end
    end
...
function GuildskillMainWindow:adaptationScreen()
    ...
    self.main_container:setPositionY(top_y - (self.container_size.height - tab_y))


    self.bg_up:setPositionY(top_y - (self.container_size.height - self.bg_up:getPositionY()))
...
function GuildskillMainWindow:register_event()
    ...
    --更新技能
     EventManager:getInstance():Bind(GuildskillEvent.UpdateSkillStatusEvent,self, function(career, skill_id)
            if self.selected_tab and self.selected_tab.career == career then
     ...
     EventManager:getInstance():Bind(GuildskillEvent.UpdateSkilUpgradeEvent,self, function(career, group_id)
            if self.selected_tab and self.selected_tab.career == career then
                self:updateSkillList(career, true)
            end
        end)

     EventManager:getInstance():Bind(GuildEvent.UpdateGuildRedStatus,self, function(bid, status) 
    ...
    -- 删除一个物品更新,也需要判断当前标签页类型
     EventManager:getInstance():Bind(BackpackEvent.ADD_GOODS,self, function(bag_code, del_list)
            if bag_code ~= BackPackConst.Bag_Code.BACKPACK then return end
            -- self:changeNeedItemInfo(del_list)
            self:updateCostInfo()
        end)

     EventManager:getInstance():Bind(BackpackEvent.MODIFY_GOODS_NUM,self, function(bag_code, change_list)
            if bag_code ~= BackPackConst.Bag_Code.BACKPACK then return end
...
function GuildskillMainWindow:updatePowerInfo(career, show_type, id)
    ...
    GlobalMessageMgr:getInstance():showPowerMove( chang_power,PathTool.getResFrame("common", "txt_cn_hl_common_134_1"), career_power - chang_power, setting)
end
...
function GuildskillMainWindow:updateChangeBtnImg()
    local res 
    if self.is_pvp_show then
        res = PathTool.getResFrame("guildskill","hl_guildskill_243")
    else
        res = PathTool.getResFrame("guildskill","hl_guildskill_242")
    end
...
function GuildskillMainWindow:changeSelectedTab(career, index)
    ...
    if self.selected_tab then
        self.selected_tab.label:setTextColor(cc.c4b(231, 213, 196, 0xff))
        self.selected_tab.label:enableOutline(cc.c4b(83, 59, 59, 0xff),1)
        self.selected_tab.select_bg:setVisible(false)
        self.selected_tab = nil
    end
    self.selected_tab = self.tab_list[career]
    if self.selected_tab then
        self.selected_tab.label:setTextColor(cc.c4b(255, 238, 202, 0xff))
        self.selected_tab.label:enableOutline(cc.c4b(98, 53, 43, 0xff),2)
...
function GuildskillMainWindow:changeShowSkillInfo(career, is_chang_btn)
    ...
    if self.is_pvp_show then
        ...
        self.box_bg:setContentSize(cc.size(660, self.pvp_box_pos_height))
        ...
        self.box_bg:setContentSize(cc.size(660, self.box_pos_height))
        for i,v in ipairs(self.cost_bg_list) do
...
function GuildskillMainWindow:updateSkillItemIcon(career, is_chang_btn)
    ...
    for i,v in ipairs(self.item_list) do
        if self.is_pvp_show then --pvp的
                    ...
                    if pos then
                        ...
                        if is_chang_btn and act_pos then
                            if act_pos then
                                ...
                                self:attrIconRunAction(v.lev, act_pos[1], act_pos[2], pos[1], pos[2])
                            else
            ...
            --原来的
            local _attr_skill = {}
            _attr_skill[2] ={"218","225","224","222","223","226"}
            _attr_skill[3] ={"218","219","224","222","223","220"}
            _attr_skill[4] ={"218","219","227","218","223","221"}
            _attr_skill[5] ={"218","225","228","218","223","221"}


            print("-------self.selected_item.indexself.selected_item.index-------",self.selected_item.index)
            if not tolua.isnull(v.node) then

                if i >= self.selected_item.index then 
                    item_res_id = string_format("hl_guildskill_%s",_attr_skill[career][i])
                    loadSpriteTexture( v.item_bg, PathTool.getResFrame("common","hl_common_195"), LOADTEXT_TYPE_PLIST)
                    v.lev:setTextColor(cc.c3b(168,150,121))
                else
                    item_res_id = string_format("hl_guildskill_select_%s",_attr_skill[career][i])
                    loadSpriteTexture( v.item_bg, PathTool.getResFrame("common","hl_common_196"), LOADTEXT_TYPE_PLIST)
                    v.lev:setTextColor(cc.c3b(239,212,174))
                end 
                ...
                if is_chang_btn and act_pos then
                    ...
                    -- self:attrIconRunAction(v.lev, act_pos[1] , act_pos[2], v.x, v.y)
                else
                    -- v.node:setPosition(v.x, v.y)
                    -- v.lev:setPosition(v.x, v.y )
                end
...
function GuildskillMainWindow:updateSkillAttrNameAndVal(career)
    if self.is_pvp_show then
        ...
        local _updateitem = function(index, index2)
            ...
            local attr_item_1 = self.attr_list_1[index2]
...
function GuildskillMainWindow:updateAttrItem(attr_item, data, attr_key, attr_str)
    ...
    local str_res = string.format("<img src='%s' scale=1.0 /><div fontcolor=#68452A> %s</div><div fontsize=22 fontcolor=#4F7C1A> %s</div>",PathTool.getResFrame("common", attr_res),attr_str,value_str)
...
function GuildskillMainWindow:updateMiddleBg(career, is_chang_btn)
    ...
    if self.Sprite_5 then
        if self.is_pvp_show then
            ...
            loadSpriteTexture(self.Sprite_5, PathTool.getResFrame("common", "hl_common_200"), LOADTEXT_TYPE_PLIST)     
            self.attr_desc:setString(TI18N("加\n成\n总\n览"))
    ...
    --战士 坦克 辅助 法师 
    local camp_res= PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_108",false)
    if self.is_pvp_show then
        ...
        if career  ==5 then 
            camp_res = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_109",false) -- 辅助

        end
        
        if career  ==2 then 
            camp_res = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_108",false) -- 法师

        end
        
        if career  ==3 then 
            camp_res = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_111",false) -- 战士

        end
        
        if career  ==4 then 

            camp_res = PathTool.getSingleImgForDownLoad("bigbg", "hl_bigbg_110",false) -- 坦克
    ...
    -- self.middle_bg:setPosition(cc.p(350,210))

    -- if career then
    --     local camp_res = PathTool.getSingleImgForDownLoad("guildskill","guildskill_career_"..career, false)
            ...
            if cost_data then
                ...
                if item_config then
                    ...
                    if have_num >= num then
                        cost_txt:setTextColor(cc.c3b(239,212,174))

                    else
                        condition_status = false
                        cost_txt:setTextColor(cc.c3b(212, 112, 52))
...
function GuildskillMainWindow:checkUpgradeCostStatus(condition_status)
    ...
    if condition_type == 2 then
        if self.is_pvp_show then
            ...
            self.handle_btn_label:setString(TI18N("点亮"))
            if self.show_pvp_tips then
                self.show_pvp_tips:setVisible(false)
            end
        end
