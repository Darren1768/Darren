...
function ActionTimeElfinSummonPanel:updateImageBg(  )
	if self.config and self.config.res_id and (not self.cur_res_id or self.cur_res_id ~= self.config.res_id) and self.config.call_bg then
		local bg_res = PathTool.getSingleImgForDownLoad("bigbg/timesummon", string.format("txt_cn_elfinsummon_bigbg_%d", self.config.call_bg), true)
		self.bg_load = loadImageTextureFromCDN(self.image_bg, bg_res, ResourcesType.single, self.bg_load)

		local bg_res_2 = PathTool.getSingleImgForDownLoad("bigbg/timesummon", string.format("txt_cn_elfinsummon_%d", self.config.res_id), false)
...
function ActionTimeElfinSummonPanel:register_event(  )
	...
	-- 召唤数据更新
     EventManager:getInstance():Bind(TimesummonEvent.Update_Elfin_Summon_Data_Event,self,function (data)
            self:setData(data)
        end)

	-- 精灵抽奖结果更新
     EventManager:getInstance():Bind(TimesummonEvent.Update_Elfin_Summon_Rewards_Data_Event,self,function (data)
			self.rewards_info = data
            self:startEffectAwardShow()
        end)

	-- 精灵抽奖结果item更新
     EventManager:getInstance():Bind(TimesummonEvent.Update_Elfin_Item_Event,self,function (data)
			self.rewards_info = data
	...
	if item_id then
		for i=1,3 do
			...
			if config and config.type and BackPackConst.checkIsElfin(config.type) then
				res_id = PathTool.getSingleImgForDownLoad("bigbg/elfin", "elfin_"..item_id)	
