...
function CrossarenaChallengePanel:createRoorWnd()
    ...
    self.tab_btn = bottom_panel:getChildByName("tab_btn")
...
function CrossarenaChallengePanel:registerEvent(  )
    ...
    -- 切换
    registerButtonEventListener(self.tab_btn, function (  )
            local crossarena_main_wnd = CrossarenaController:getInstance().crossarena_main_wnd
            if crossarena_main_wnd then
                crossarena_main_wnd:changeSelectedTab(CrossarenaConst.Sub_Type.Honour)
            end
        end, true)
    -- 刷新
    ...
    -- 个人信息
    EventManager:getInstance():Bind(CrossarenaEvent.Update_MyBaseInfo_Event,self, function ( )
            self:updateMyselfInfo()
        end)


    -- 活动开启状态
    EventManager:getInstance():Bind(CrossarenaEvent.Update_Open_Status_Event,self, function ( )
            self:updateMyselfInfo()
        end)


    -- 刷新时间更新
    EventManager:getInstance():Bind(CrossarenaEvent.Update_Refresh_Time_Event,self, function ( )
            self:updateRefreshBtnStatus()
        end)


    -- 挑战玩家数据更新
    EventManager:getInstance():Bind(CrossarenaEvent.Update_Challenge_Role_Event,self, function ( )
            self:updateRoleItemList()
        end)


    -- 每日挑战奖励更新
    EventManager:getInstance():Bind(CrossarenaEvent.Update_Challenge_Award_Event,self, function ( )
    ...
    EventManager:getInstance():Bind(BackpackEvent.DELETE_GOODS,self,function(bag_code, data_list)
            self:updateTicketNum(bag_code,data_list)
        end)
...
function CrossarenaChallengePanel:updateRoleItemList(  )
    ...
    for i,data in ipairs(role_datas) do
        delayRun(self.back, i / display.DEFAULT_FPS, function ()
                local role_item = self.role_item_list[i]
                if not role_item then
                    role_item = CrossareanRoleItem.new(self.back)
...
-- 位置
function CrossarenaChallengePanel:setItemPos( index )
    if self.role_item_list[index] then
        local posX = {40,230,450}
        local pos_x = posX[index]
        self.role_item_list[index]:setPosition(cc.p(pos_x, 102))
        self.role_item_list[index]:setVisible(true)
        self.cur_index = index
        if index == 1 or index == 3 then
            self.role_item_list[index]:setScaleX(157/188)
            self.role_item_list[index]:setScaleY(438/442)
        end
    end
end
...
function CrossarenaChallengePanel:updateRefreshBtnStatus(  )
    ...
    self.btn_refresh:stopAllActions()
    ...
    if less_time <= 0 then
        ...
        self.btn_refresh:runAction(cc.RepeatForever:create(cc.Sequence:create(
                    cc.DelayTime:create(1), cc.CallFunc:create(function()
                            less_time = less_time - 1
                            if less_time < 0 then
                                setChildUnEnabled(false, self.btn_refresh)
...
function CrossarenaChallengePanel:updateChallengeAwardInfo(  )
    ...
    for i,aData in ipairs(self.award_cfg) do
        ...
        local pos_x = 180 + (aData.num/max_count)*321
        box_node:setPosition(cc.p(pos_x, 240))
