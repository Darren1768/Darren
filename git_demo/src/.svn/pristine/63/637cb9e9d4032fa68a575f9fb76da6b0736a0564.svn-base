...
function CrossgroundMainWindow:__init()
    ...
    self.res_list = {
        ...
        {path = PathTool.getSingleImgForDownLoad("bigbg","bigbg_82", true), type = ResourcesType.single}
    }
...
function CrossgroundMainWindow:open_callback(  )
    ...
    if self.background then
        self.background:loadTexture(PathTool.getSingleImgForDownLoad("bigbg","bigbg_82",true), LOADTEXT_TYPE)
    ...
    self.single_btn = main_panel:getChildByName("single_btn")
...
function CrossgroundMainWindow:register_event(  )
    ...
    registerButtonEventListener(self.single_btn, function (  )
            ArenaController:getInstance():requestOpenArenWindow()
        end, false, 1)

    -- 竞技场红点
    self:addGlobalEvent(ArenaEvent.UpdateArenaRedStatus, function(type, status)
            self:checkActivityStstus()
        end)
    -- 天梯红点
...
function CrossgroundMainWindow:checkActivityStstus()
    local red_status = ArenaController:getInstance():getModel():checkLoopMatchRedStatus()
    addRedPointToNodeByStatus(self.single_btn, red_status, 0, 0)
end
