...
function MainUiView:initConfig()
    ...
    self.top_left_off = 20
    self.left_off = 20
    self.right_off = 20
    self.top_left_max_sum = 5
    self.left_max_sum = 7
...
function MainUiView:initRightButtons()
    self.top_right_container = self.root_wnd:getChildByName("top_right_container")
    self.top_right_container:setPositionY(display.getTop()-68)
    self.top_right_container:setPositionX(display.getRight()-49)
    for i = 1, 5 do
        local btn = self.top_right_container:getChildByName("Button_"..i)
        local tips_point = btn:getChildByName("tips_point")
        tips_point:setVisible(false)
        local id = MainuiConst.btn_right_id[i]
        btn:addTouchEventListener(function(sender, event_type)
                if event_type == ccui.TouchEventType.ended then
                    playMainButtonSound()
                    MainuiController:getInstance():iconClickHandle(id)
                end
            end)

        EventManager:getInstance():Bind(MainuiController.UPDATE_ICON_EVENT,tips_point, function(key)
                if key ~= id then
                    return
                end
                local status = MainuiController:getInstance().cache_tips_list[id]
                if status then
                    --tips_point:setVisible(false)
                    if type(status) ~= "table" then
                        tips_point:setVisible(status)
                        return
                    end
                    ...
                    for k,v in pairs(status) do
                        if type(v) == "table" then
                            local num = v.num
                            local show = v.status
                            if  num ~= nil and type(num) == "number" and num > 0 then
                                tips_point:setVisible(true)
                                break
                            end
...
function MainUiView:createRootWnd()
    ...
    for i = 1, 5 do
        ...
        -- 用于飘物品的位置使用
        if i == MainuiConst.icon.backpack then
        ...
        object.selected:setVisible(false)
    ...
    local head_container = self.top_container:getChildByName("head"):getChildByName("head_container")
    ...
    self.top_container:getChildByName("head"):getChildByName("Sprite_13_0"):setVisible(false)
    self.lev_label = self.top_container:getChildByName("head"):getChildByName("Sprite_13_0"):getChildByName("lev_label")
    ...
    self.hide_container = self.top_right_container:getChildByName("hide_container")
    ...
    addRedPointToNodeByStrs(self.effect_container,{MainuiConst.red_str.vedio,MainuiConst.red_str.formation},-10,-5)
    ...
    local handle_y = display.getBottom() + self.bottom_container:getContentSize().height + 110
    local handle_x = display.getRight() - 45
    self.handle_btn:setPosition(handle_x, handle_y)
    self.prompt_container:setPositionY(handle_y-90 + 300)
    ...
    for k, v in pairs(FunctionIconVo.type) do
        ...
        if icon_container then
            ...
            elseif v == FunctionIconVo.type.right_top_2 then -- 右上纵向
                _x = 30
                _y = display.getTop() - self.top_container:getContentSize().height - 68
    ...
    --头顶的
    local Image_2 = self.top_container:getChildByName("Image_2")
    -- self.vip_label = CommonNum.new(106, Image_2, 1, -2, cc.p(0, 0.5))
    -- self.vip_label:setPosition(0 , Image_2:getContentSize().height)
    ...
    if self.chat_panel == nil then
        self.chat_panel = ChatMainuiPanel:new()
        self.bottom_container:addChild(self.chat_panel) 
        self.chat_panel:setPosition(cc.p(360,240))
    end


    self:updateWorldLevelIcon()
    -- local armature = spSkeletonAnimationCreate("spine/H10000/zixiaxianzi.skel", "spine/H10000/zixiaxianzi.atlas", 2)
    -- if not tolua.isnull(self.bottom_container) then
    --     self.bottom_container:addChild(armature, 100)
    --     armature:setPosition(cc.p(100,200))
    -- end
end

function MainUiView:showTeacherPanel()
    if self.tip_buttons_panel == nil then
        self.tip_buttons_panel = TipButtonsPanel:new()
        self.bottom_container:addChild(self.tip_buttons_panel) 
        self.tip_buttons_panel:setPosition(cc.p(220,440))
    else
        self.tip_buttons_panel:setVisible(not self.tip_buttons_panel:isVisible())
    end
end
...
function MainUiView:checkFightUi(index)
    return index == MainuiConst.btn_index.main_scene or index == MainuiConst.btn_index.drama_scene or index == MainuiConst.btn_index.esecsice  or index == MainuiConst.btn_index.partner
end
...
function MainUiView:changeMainUiStatus(index, sub_type, extend_data, force)
    print("-------------------------changeMainUiStatus-------------------------------------------------",index)
    if index <= #self.bottom_btn_list then
        local btn = self.bottom_btn_list[index]
        if btn == nil or btn.is_unlock == false then
    ...
    elseif index == MainuiConst.btn_index.ghostdom then
        GhostdomController:getInstance():openGhostdomScene(true, Config.city_data[4])
        --Area_sceneController:getInstance():openAreaScene(true, Config.city_data[3])
    elseif index == MainuiConst.icon.backpack then
    ...
    elseif index == MainuiConst.btn_index.esecsice then
        ...
        BattleFieldController:getInstance():openMainScene(true)
    ...
    delayRun(self.root_wnd, 0.2, function()
            ...
            if main_type == MainuiConst.btn_index.main_scene then
                ...
                elseif sub_type == MainuiConst.sub_type.startower then --试练塔
                    controler:requestOpenBattleRelevanceWindow(BattleConst.Fight_Type.Spirit)
                ...
                elseif sub_type == MainuiConst.sub_type.leagueexplore then --探索历练
                    MainuiController:getInstance():requestOpenBattleRelevanceWindow(BattleConst.Fight_Type.LeagueExplore)
                end
...
function MainUiView:changeBackgroundMusic()
    ...
    GlobalAudio:getInstance():playMusic(GlobalAudio.AUDIO_TYPE.SCENE, music_name, true)
end
    ...
    if self.wait_update == nil then
        self.wait_update = GlobalTimeTicket:getInstance():add(function() 
                if self.hide_container_status == true then
                    ...
                    self.top_right_container:setVisible(self.is_open)
                    self.prompt_container:setVisible(self.is_open)
                    self.chat_panel:setVisible(self.is_open)
                    self.bottom_container:setVisible(self.is_open)
                end
...
function MainUiView:close()
    if not tolua_isnull(self.hide_container) then
        ...
        self.top_right_container:setVisible(false)
        self.icon_container:setVisible(false)
        self.prompt_container:setVisible(false)
        self.chat_panel:setVisible(false)
        if self.tip_buttons_panel then
            self.tip_buttons_panel:setVisible(false)
        end
...
function MainUiView:showFuncIconList( status )
    if not tolua_isnull(self.icon_container) then
        ...
        self.top_right_container:setVisible(status)
    end
end
...
function MainUiView:updateIconLayout(type)
    ...
    for i = 1, len do
        ...
        if data and data.config and data.pos ~= 0 then
...
-- 世界等级
function MainUiView:updateWorldLevelIcon(  )
    if role_vo and role_vo.lev < 60 then return end
    local need_update_pos = false
    if not self.world_lv_btn then
        local top_size = self.icon_container:getContentSize()
        local pos_x = 22
        local pos_y = self.bottom_container:getPositionY() + self:getBottomHeight() + 156
        self.world_lv_btn = createImage(self.icon_container, PathTool.getResFrame("common1","hl_common_594"), pos_x, pos_y, cc.p(0.5, 0.5), true)
        self.world_lv_btn:setTouchEnabled(true)
        -- self.world_lv_btn.index = 6
        createLabel(22,cc.c3b(254,247,187), nil,self.world_lv_btn:getContentSize().width/2,self.world_lv_btn:getContentSize().height/2,"界",self.world_lv_btn, nil, cc.p(0.5,0.5))
...
-- 显示世界等级tips
function MainUiView:showWorldLevelTips( status )
    if status == true then
        if not self.world_lv_layout then
            ...
            self.root_wnd:addChild(self.world_lv_layout,1)
            ...
            local world_lv_bg = createImage(self.world_lv_layout, PathTool.getResFrame("common2","hl_common_3033"), local_pos.x+28, local_pos.y+20  , cc.p(0, 1), true, nil, true)
            world_lv_bg:setTouchEnabled(true)
            world_lv_bg:setCapInsets(cc.rect(16,16,1,1))
            local world_bg_size = cc.size(320, 110)

            -- 世界等级描述
            local world_lev_cfg = Config.WorldLevData.data_const["worldlev_des"]
            if world_lev_cfg then
                local world_lv_desc = createLabel(20,cc.c3b(239,212,174),nil,20,180,world_lev_cfg.desc,world_lv_bg,nil,cc.p(0, 1))
                world_lv_desc:setMaxLineWidth(300)
                local desc_size = world_lv_desc:getContentSize()
                world_bg_size.height = world_bg_size.height + desc_size.height
                world_lv_desc:setPosition(cc.p(15, world_bg_size.height-58))
            end
            world_lv_bg:setContentSize(world_bg_size)
            world_lv_bg:setPositionY(world_lv_bg:getPositionY()+world_bg_size.height+12)
            -- 图标
            ...
            local world_lv_title = createLabel(22,cc.c3b(239,212,174),nil,15,world_bg_size.height-25,TI18N("世界等级"),world_lv_bg,nil,cc.p(0, 0.5))
     
            local world_lv_line = createImage(world_lv_bg, PathTool.getResFrame("common1","hl_common_586"), 160, world_bg_size.height-45 , cc.p(0.5, 0.5), true, nil, true)
            world_lv_line:setCapInsets(cc.rect(15,0,1,1))
            world_lv_line:setContentSize(cc.size(290,9))
            -- 世界等级:setContentSize(world_bg_size)
            self.world_lv_txt = createRichLabel(20, cc.c3b(239,212,174), cc.p(0, 0.5), cc.p(15, 30))
            world_lv_bg:addChild(self.world_lv_txt)
            local world_lev = RoleController:getInstance():getModel():getWorldLev()
            self.world_lv_txt:setString(string.format(TI18N("当前世界等级：<div fontcolor=#6faf49>%d级</div>"), world_lev))
        end
...
function MainUiView:updateRoleLev()
    ...
    self.role_head:setLev(self.cur_lev)
    self.role_head:setVIP(role_vo.vip_lev)
    -- 判断等级是否需要显示气泡
    -- self:createChatBubble()
end
...
function MainUiView:updateRoleVip()
    ...
    -- self.vip_label:setNum("v"..role_vo.vip_lev)
...
function MainUiView:getBottomHeight()
    if self.bottom_height == nil then
        self.bottom_height = self.bottom_container:getContentSize().height
    end
...
function MainUiView:registerEvent()
    if role_vo then
        if self.role_update_event == nil then
            self.role_update_event = role_vo:Bind(RoleEvent.UPDATE_ROLE_ATTRIBUTE, function(key, value)
                    ...
                    elseif key == "look_id"  then
                        MainSceneController:getInstance():changeSceneSpine()
                    end
                end)
        end
    end
    ...
    --点金红点
    EventManager:getInstance():Bind(ExchangeEvent.Extra_Reward,self, function(data)
            self.exchange_data = data
            self:updataExchangeRedPoint()
        end)


    EventManager:getInstance():Bind(EventId.CHAT_NEWMSG_FLAG,self, function()
            self:updateChatMsgNum()
        end)


    --红包推送
    EventManager:getInstance():Bind(RedbagEvent.Can_Get_Red_Bag,self,function(vo)
    ...
    EventManager:getInstance():Bind(PetardActionEvent.Update_Petard_Main_Redbag_Event,self, function(code)
    ...
    EventManager:getInstance():Bind(ActionyearmonsterEvent.Update_Year_Main_Redbag_Event,self, function(code)
    ...
    EventManager:getInstance():Bind(ReturnActionEvent.Update_Return_Main_Redbag_Event,self, function(code)
            if code and code ~= 0 then 
    ...
    EventManager:getInstance():Bind(ActionEvent.Update_Time_Collect_Main_Icon_Event,self, function(code)
    ...
    --合服调查问卷事件
    EventManager:getInstance():Bind(MergeserverEvent.Update_Main_Mergeserver_Event,self, function(data)
    ...
    EventManager:getInstance():Bind(MergeserverEvent.Update_Merge_Success_Event,self, function(data)
    ...
    EventManager:getInstance():Bind(MergeserverEvent.vote_mergeAction_event,self, function(status)
    ...
    --头部资产更新事件
    EventManager:getInstance():Bind(MainuiEvent.HEAD_UPDATE_WEALTH_EVENT,self, function(index, id)
    ...
    --系统提示增加
    EventManager:getInstance():Bind(PromptEvent.ADD_PROMPT_DATA,self,function ( data )
            -- 主城正在显示且不在聊天界面才显示气泡
    ...
    --系统提示移除
    EventManager:getInstance():Bind(PromptEvent.REMOVE_PROMPT_DATA,self,function (  )
            local model = PromptController:getInstance():getModel()
            local list = model:getPromptList()
            if tableLen(list) > 0 then 
                self:showPromptTips(true,list)
    ...
    --新手训练营数据刷新
    EventManager:getInstance():Bind(TrainingcampEvent.Update_Trainingcamp_Data_Event,self,function (  )
            self:showPromptEffect(true)
            self:updateTipsBtnStatus()
...
function MainUiView:updataExchangeRedPoint()
    ...
    for i,v in ipairs(self.exchange_data.list) do
        if v.id == 1 and self.coin_redpoint then
            ...
            addRedPointToNodeByStatus(self.coin_redpoint,status,7,0)
        end
    end
end
...
function MainUiView:createChatBubble()
    ...
    self.chat_bubble:setPosition(self.handle_x, self.handle_y + 412)
...
-- 隐藏/显示mianui
function MainUiView:setShowTopUI( status )
    self.top_container:stopAllActions()
    if status then
        self.top_container:runAction(cc.Spawn:create(cc.MoveTo:create(0.2, cc.p(360, display.getTop())), cc.FadeIn:create(0.2)))
    else
