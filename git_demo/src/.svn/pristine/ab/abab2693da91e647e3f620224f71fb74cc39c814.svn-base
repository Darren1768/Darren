...
function ItemSellPanel:open_callback()
    ...
    self.text_name = self.main_panel:getChildByName("itemName")
    self.text_name:setString("")
    self.center_panel = self.main_panel:getChildByName("scrollCon")
    self.btn_redu = self.center_panel:getChildByName("btn_redu")
    self.btn_add = self.center_panel:getChildByName("btn_add")
    self.btn_max = self.center_panel:getChildByName("btn_max")
    self.comp_num = createRichLabel(20,175, cc.p(0,0.5),cc.p(125 , 120))
    self.center_panel:addChild(self.comp_num)
    self.item_name = self.center_panel:getChildByName("itemName_0")
    self.total_label = createRichLabel(20,175, cc.p(0,0.5),cc.p(30 ,22))
    self.center_panel:addChild(self.total_label)

    self.btn_comp = self.main_panel:getChildByName("btn_comp")
    self.btn_comp_label = self.btn_comp:getChildByName("Text_1")
    self.btn_comp_label:setString(TI18N("一键出售"))

 
    self.clse_btn=self.main_panel:getChildByName("close_btn")
  
    self.cost_bg = self.main_panel:getChildByName("Image_2")
    self.cost_lable =  createRichLabel(26,175, cc.p(0.5,0.5),cc.p(40 , 13))
    self.cost_bg:addChild(self.cost_lable)
    self.goods_item =  BackPackItem.new(true,true,nil,1,false)
    self.goods_item:setPosition(cc.p(100,320))
   ...
   self.main_panel:addChild(self.goods_item)
	...
	self.slider = self.center_panel:getChildByName("Slider_1")
...
function ItemSellPanel:register_event()
    ...
    registerButtonEventListener(self.clse_btn, function() controller:openItemSellPanel(false) end,false, 2)
    ...
    registerButtonEventListener(self.btn_comp, function()
     ...
     EventManager:getInstance():Bind(ElfinEvent.Elfin_Plan_Must_Tips_Event,self, function(data)
...
function ItemSellPanel:onSliderScaleBuild( percent )
	-- 最小只能缩到 SCREEN_WIDTH/HomeworldConst.Build_Width(屏幕宽度除以建筑宽度)
	self.cur_number=math.round(percent*self.init_number/100)
    if self.cur_number<1 then
        self.cur_number=1
    end
    if self.cur_number>self.init_number then
...
function ItemSellPanel:setBtnStatus()
    ...
    if self.init_number>self.cur_number then
...
function ItemSellPanel:setTouchEnable_Max(bool)
...
function ItemSellPanel:setBaseInfo()
    ...
    self.item_name:setString(self.item_config.name)
    ...
    self.comp_num:setString(self.cur_number)
     self.slider:setPercent(math.round(100* self.cur_number/self.init_number))
    self:setBtnStatus()
...
function ItemSellPanel:setCostInfo(count)
    ...
    if not self.price  then
        if self.item_config.value and next(self.item_config.value) ~= nil then
            ...
            self.price = self.item_config.value[1][2]
       
            local item_cfg = Config.ItemData.data_get_data(item_id)
            if item_cfg then
            local res = PathTool.getItemRes(item_cfg.icon)
                local str = string.format("<img src=%s scale=0.35 visible=true />%s",res, self.price)
                self.cost_lable:setString(str)
            end
        end  
    end
    local cur=math.round(100*self.cur_number/self.init_number)
    self.slider:setPercent(cur)
    if self.price then
        local total_price = self.cur_number *self. price
        local str = string.format("总价：<div fontColor=#5fb34f>%s</div>",total_price)
        self.total_label:setString(TI18N(str))
