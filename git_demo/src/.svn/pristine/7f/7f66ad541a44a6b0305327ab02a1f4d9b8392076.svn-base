...
local organize_model = OrganizeController:getInstance():getModel() --队伍

local string_format = string.format
local table_sort = table.sort
local table_insert = table.insert
local table_remove = table.remove
FormGoFightPanel.RESOURCE_BINDING = 
{
    -- ["fight_label"]     = {["varname"] = "fight_label" },
    -- ["wealth_label_1"]       = {["varname"] = "wealth_label_1"},
    -- ["wealth_label_2"]      = {["varname"] = "wealth_label_2"},
    -- ["lev_label"]      = {["varname"] = "lev_label"},
    -- ["name_label"]      = {["varname"] = "name_label"},
    -- ["head_container"]      = {["varname"] = "head_container"},
    -- ["wealth_icon_1"]      = {["varname"] = "wealth_icon_1"},
    -- ["wealth_icon_2"]      = {["varname"] = "wealth_icon_2"},
    -- ["Sprite_8"]      = {["varname"] = "coin_redpoint"},
    
    -- ["Button_22"]      = {["varname"] = "_btn1",events = {{event="touch",method="onBtnTouch"}} },
    ["power_click"]    = {["varname"] = "power_click",onClick="onClick_power_click"} ,
} 
function FormGoFightPanel:onClick_power_click()
    print("FormGoFightPanel:onClick_power_click:",self.power_click:getName())
end
function FormGoFightPanel:__init()
    self.view_tag = ViewMgrTag.WIN_TAG

    self.win_type = WinType.Full
    self.layout_name = "hero/form_go_fight_new_panel"
    self.res_list = {
        ...
        -- { path = PathTool.getSingleImgForDownLoad("bigbg","bigbg_56"), type = ResourcesType.single }
    }
    ...
    self.cur_power = 0 
    self.cell_width = 134
    self.cell_height = 134

    --9个位置
    self.nine_position = {}

    for i=1,9 do
        local grid_pos = SkillAct.newPos2Gird(0, i, true, 1)
        self.nine_position[i] = gridPosToScreenPos(grid_pos)

        self.nine_position[i].y = self.nine_position[i].y +80
        local _yu =  i / 3 
        if _yu == 0 then 
             _yu = 3 
        end
        self.nine_position[i].x  = self.nine_position[i].x  - _yu*60 +200
    end
...
function FormGoFightPanel:open_callback()
    ...
    self.top_panel:setPositionY(display.height  - SCREEN_HEIGHT)
    
    self.main_container:setPositionY(display.getBottom())
    -- self.main_container:getChildByName("exit_tips_label"):setString(TI18N("点击空白区域关闭窗口"))

    --阵营  --神，人，妖，佛，魔  -- 人 妖 神  魔 佛
    ...
    for index=1,5 do
        self.hero_item_list[index] = ccui.Layout:create()
        self.hero_item_list[index]:setEnabled(true)
        self.hero_item_list[index]:setContentSize(cc.size(109,109))
        ...
        self.hero_item_list[index]:setTouchEnabled(true)
        self.hero_item_list[index]:setZOrder(1000 - pos[index])

        local back = createImage(self.hero_item_list[index], PathTool.getResFrame("form", "hl_form_1017"), 54.5, 0, cc.p(0.5, 0), true)
        self.hero_item_list[index].back = back
    end

    self.pos_tips = self.main_container:getChildByName("pos_tips")
    self.pos_tips:setString(TI18N("长按英雄可查看英雄详细信息"))

    --穿戴
    self.equip_btn = self.form_panel:getChildByName("equip_btn")

    -- local size = self.equip_btn:getContentSize()
    -- self.hallows_item = BackPackItem.new(false, false, false, 0.4)
    ...
    self.equip_btn_label:setString(TI18N("神器"))
    --精灵
    ...
    self.form_label_count = self.form_change_btn:getChildByName("label_count")
    self.form_change_btn:setEnabled(true)
    --出战
    ...
    self.key_up_btn:getChildByName("label"):setString(TI18N("上阵"))
    --阵法icon
    self.form_icon = self.form_change_btn:getChildByName("form_icon")
    --光环icon
    self.halo_info_btn = self.top_panel:getChildByName("halo_info_btn")
    self.halo_info_label = self.halo_info_btn:getChildByName("label")
    self.halo_info_label:setString(TI18N("克制"))

    self.halo_btn_up =  self.top_panel:getChildByName("halo_btn")
    self.halo_btn = self.halo_btn_up:getChildByName("halo_btn")
    self.halo_label = self.halo_btn:getChildByName("label")
    --关系buff
    self.buff_btn = self.top_panel:getChildByName("buff_btn")
    self.sp_icon = self.buff_btn:getChildByName("sp_icon") --关系buff icon
    self.sp_name = self.buff_btn:getChildByName("sp_name") --关系buff 名字
    self.sp_name:setVisible(false) --暂时不显示（不知道显示啥）
    ...
    self.tab_btn:setPosition(cc.p(560,630))
    ...
    local up_back = self.main_container:getChildByName('up_back')
    local battle_res_id = BattleController:getInstance():curBattleResId(BattleConst.Fight_Type.Darma) or 10001
    local res = PathTool.getBattleSceneRes(string_format("%s/b_bg", battle_res_id), false)
    loadImageTextureFromCDN(up_back, res, ResourcesType.single)
    up_back:setContentSize(cc.size(720,1280))
end
...
function FormGoFightPanel:register_event()
    ...
    registerButtonEventListener(self.halo_btn_up, handler(self, self.onClickHaloBtn) ,true, 2)

    registerButtonEventListener(self.halo_info_btn, handler(self, self.onClickHaloInfoBtn) ,true, 2)
        ...
        item:addTouchEventListener(function(sender, event_type)
                ...
                elseif event_type == ccui.TouchEventType.canceled then
                    ...
                    elseif self.long_touch_type == LONG_TOUCH_END_TYPE then
                        ...
                        self:addHeroSpine(item,self.five_hero_vo[self.select_team_index][i])
                        return
                    end
    ...
    self:addGlobalEvent(ElitematchEvent.Update_Elite_Fun_Form, function(data)
            ...
            if self.fun_form_type == PartnerConst.Fun_Form.Drama then
                print("---------------setHeroIsSelect-------Update_Elite_Fun_Form------2--------------")
                --说明是在剧情布阵的
                ...
                print("---------------setHeroIsSelect-------Update_Elite_Fun_Form------1--------------")
                --常规赛
                if #data.formations > 0 then
    ...
    self:addGlobalEvent(AdventureEvent.ADVENTURE_MINE_SAVE_FORM_EVENT, function (  )
            EventManager:getInstance():Fire(AdventureEvent.ADVENTURE_MINE_SAVE_BACK_EVENT, self.mine_pos_info, self.formation_type, self.hallows_id)
...
function FormGoFightPanel:onClickHaloInfoBtn()
...
function FormGoFightPanel:onClickKeyUpBtn()
    ...
    if  self.can_once_all_down then 
        self:downFormAllHero()
        return
    end 
    ...
    for index, hero_vo in pairs(key_pos_info_list) do
        ...
        if self.hero_item_list[index] then
            ...
            self:addHeroSpine(self.hero_item_list[index],self.five_hero_vo[self.select_team_index][index])
        end
    end
    for index, hero_vo in pairs(self.five_hero_vo[self.select_team_index]) do
        if dic_change[index] == nil then
            ...
            if self.hero_item_list[index] then
                ...
                self:addHeroSpine(self.hero_item_list[index],nil)
            end    
    ...
    self.can_once_all_down = true
    self.key_up_btn:getChildByName("label"):setString("下阵")

end

function FormGoFightPanel:downFormAllHero()
    self.key_up_btn:getChildByName("label"):setString("上阵")
    self.can_once_all_down = false
...
function FormGoFightPanel:onClickFightBtn()
    ...
    elseif self.fun_form_type == PartnerConst.Fun_Form.EndLess or self.fun_form_type == PartnerConst.Fun_Form.EndLessWater or 
    self.fun_form_type == PartnerConst.Fun_Form.EndLessFire or self.fun_form_type == PartnerConst.Fun_Form.EndLessWind or 
    self.fun_form_type == PartnerConst.Fun_Form.EndLessLightDark or 
    self.fun_form_type == PartnerConst.Fun_Form.EndLessSingleMode then
        --无尽试炼
        self:onFightEndLess()
...
function FormGoFightPanel:onFightPractiseTower()
    ...
    if self.setting and self.setting.power and self.cur_power and self.setting.power> self.cur_power then
        ...
        local str = string_format(TI18N('当前队伍战力未达推荐战力(<div fontColor=#d95014 >%s/%s</div>)，挑战难度较大，是否确认挑战？'), self.cur_power,self.setting.power)
...
function FormGoFightPanel:onFightEndLess()
    ...
    elseif self.fun_form_type == PartnerConst.Fun_Form.EndLessSingleMode then--光暗系
        type = Endless_trailEvent.endless_type.singlemode
    end
...
--发送试练塔
function FormGoFightPanel:onFightSpirit()
    local tower = self.setting.tower_lev or 0
    SpiritController:getInstance():sender11322(tower)
end
--发送远征
        ...
        if not is_drama then
            for i,v in ipairs(team_data_list) do
                if i > 1 then
                    ...
                    if hero_datas then
                        print("----------------five_hero_vo-------1------",hero_datas )
                        for pos,hero_vo in pairs(hero_datas) do
                            local temp_data = {}
...
function FormGoFightPanel:onFightDrama()
    ...
    for i,v in pairs(self.five_hero_vo[self.select_team_index]) do
        ...
        table_insert(pos_pid_info, {pid = v.partner_id})
    end
    ...
    elseif tab_data.fun_form_type == PartnerConst.Fun_Form.Planes then -- 位面
        ...
        -- 更新守卫界面数据
        EventManager:getInstance():Fire(PlanesafkEvent.Update_Form_Data_Event, self.formation_type, planes_pos_info)
        ...
        print("-------------------------tab_data.fun_form_type------------------------",self.formation_type)
        OrganizeController:getInstance():sender29704(tab_data.fun_form_type,pos_pid_info)
...
function FormGoFightPanel:gotoFight()
    ...
    if self.fun_form_type == PartnerConst.Fun_Form.Drama then
        BattleDramaController:getInstance():send13003(self.setting.checkpoint_id)
    elseif self.fun_form_type == PartnerConst.Fun_Form.Spirit then
        --试练塔 --星命塔
        self:onFightSpirit()
    elseif self.fun_form_type == PartnerConst.Fun_Form.Sandybeach_boss then 
...
function FormGoFightPanel:addHeroSpine(node,hero_data)
    if node.hero_data == hero_data then return end
    node.hero_data = hero_data
    if node.spine_item then
        node.spine_item:removeFromParent()
        node.spine_item = nil
    end
    if hero_data then 
        local base_data = Config.PartnerData.data_partner_star(getNorKey(hero_data.bid, hero_data.star))

        local spine_name
        local encircle_effect
        if base_data then
            spine_name = base_data.res_id
            encircle_effect = base_data.fight_effect 		-- 10星特效
        end

        local spine_item = createArmature(spine_name,'stand') --PlayerAction.battle_stand)
        spine_item:setAnchorPoint(0.5, 0)
        spine_item:setAnimation(0,'stand', true)
        node:addChild(spine_item)
        spine_item:setPosition(cc.p(54.5,30))
        node.spine_item = spine_item
        
        -- if node.back then
        --     node.back:loadTexture(PathTool.geHeroSpineShadow(hero_data.camp_type), LOADTEXT_TYPE_PLIST)
        -- end
    else
        if node.back then
            node.back:loadTexture(PathTool.getResFrame("form", "hl_form_1017"), LOADTEXT_TYPE_PLIST)
        end
    end
...
function FormGoFightPanel:updateMoveItem(hero_data)
    if self.move_hero_item and self.move_hero_item.hero_data == hero_data then return end

    if self.move_hero_item ~= nil then
        self.move_hero_item:removeFromParent()
    ...
    local base_data = Config.PartnerData.data_partner_star(getNorKey(data.bid, data.star))
    ...
    self.move_hero_item = createArmature(spine_name,'stand') --PlayerAction.battle_stand)
    self.move_hero_item:setAnchorPoint(0.5, 0)
    self.move_hero_item:setAnimation(0,'stand', true)
    self.fight_hero_node:addChild(self.move_hero_item, 1000)
    self.move_hero_item.hero_data = hero_data
end

--点击5个英雄Item move
function FormGoFightPanel:onClickHeroItemMove(index, sender)
    print("----------onClickHeroItemMove---------------")
    if not self.five_hero_vo[self.select_team_index] then return end
    --判断是否有移动
    self.is_move_hero = true

    self:updateMoveItem(self.five_hero_vo[self.select_team_index][index])
    ...
    for i,rect in ipairs(self.pos_rect_list) do
        if cc.rectContainsPoint( rect, target_pos ) then
            if i ~= index then
                ...
                self:addHeroSpine(self.hero_item_list[i],self.five_hero_vo[self.select_team_index][index])
                ...
                if temp_hero_vo ~= nil then
                    ...
                    self:addHeroSpine(self.hero_item_list[index],temp_hero_vo)
...
function FormGoFightPanel:onClickBtnShowByIndex(select_camp)
    for k,v in pairs(self.camp_btn_list) do
        v:setEnabled(true)
        local _res = PathTool.getResFrame("form", "race_name_"..k)
        v:loadTextures(_res,"","", LOADTEXT_TYPE_PLIST)
        v:setContentSize(cc.size(49,25))

    end
    if self.img_select and self.camp_btn_list[select_camp] then
        local x, y = self.camp_btn_list[select_camp]:getPosition()
        self.img_select:setPosition(x - 0.5, y + 1)
        self.camp_btn_list[select_camp]:setEnabled(false)

        local _res_p  = PathTool.getResFrame("form",  "race_name_select_"..select_camp)
        self.camp_btn_list[select_camp]:loadTextures(_res_p,"","", LOADTEXT_TYPE_PLIST)
        self.camp_btn_list[select_camp]:setContentSize(cc.size(58,35))
    end

    print("--------select_camp-------------",select_camp)
...
function FormGoFightPanel:initFormSetInfo()
    ...
    elseif self.fun_form_type == PartnerConst.Fun_Form.EndLessSingleMode then
        self.pos_tips:setString(string_format(TI18N("只可上阵1个英雄")))
        self.pos_tips:setTextColor(cc.c3b(36,144,3))
    end
end
--发送获取队伍信息 根据队伍类型 
function FormGoFightPanel:sendByFormType(fun_form_type)
   print("-----------sendByFormType-----------------",fun_form_type)
    ...
    print("-----------sendByFormType-------12----------",fun_form_type)
    ...
    elseif fun_form_type == PartnerConst.Fun_Form.EndLessSingleMode then
        --无尽试炼单人模式
        self:initSingleModeFormInfo()
    else 
        --其他的都根据协议获取阵容
        print("--------------sender11211-----------------------")
...
function FormGoFightPanel:initTabBtnList()
    ...
    local config_info = HeroController:getInstance():getIndexByFormType(self.fun_form_type)
    if self.fun_form_type == PartnerConst.Fun_Form.Drama then
        if self.form_show_type == HeroConst.FormShowType.eFormFight then --出战 
    ...
    elseif self.fun_form_type == PartnerConst.Fun_Form.Ladder or self.fun_form_type == PartnerConst.Fun_Form.Arena then --天梯赛（天梯赛防守） 竞技场防御类型
        self:setTabData(self.fun_form_type, config_info.name, config_info.index, 1, false, true)
    else
        self:setTabData(self.fun_form_type, TI18N("队伍"), 1)
    end
...
function FormGoFightPanel:updateTabBtnList()
    ...
    if not self.tab_btn_list_view then
        ...
        self.tab_btn_list_view = CommonScrollViewSingleLayout.new(self.tab_btn, cc.p(size.width * 0.5, size.height * 0.5) , ScrollViewDir.vertical, ScrollViewStartPos.top, size , setting, cc.p(0.5,0.5))
    ...
    self.tab_btn_list_view:setSwallowTouches(false)
...
function FormGoFightPanel:createNewCellTabBtn(width, height)
    ...
    cell.label:setTextColor(cc.c4b(239, 212, 174, 0xff))
...
function FormGoFightPanel:updateCellByIndexTabBtn(cell, index)
    ...
    if tab_data then
        cell.label:setPositionY(50)

        if StringUtil.getStrLen(tab_data.name)  > 8 then 
            cell.label:setPositionY(50)
        end 
        cell.label:setString(StringUtil.branchStr(tab_data.name))
        ...
        if tab_data.is_lock ~= nil then
            if tab_data.is_lock then
                ...
                -- setChildUnEnabled(true, cell.select_img)
                -- setChildUnEnabled(true, cell.normal_img)

            else 
                setChildUnEnabled(false, cell.tab_btn)
                cell.label:setVisible(true)
                -- setChildUnEnabled(false, cell.select_img)
                -- setChildUnEnabled(false, cell.normal_img)

                -- if self.select_team_index and self.select_team_index == tab_data.index then
                --     cell.label:setTextColor(cc.c4b(0x6d, 0x43, 0x1b, 0xff))
                -- else
                --     cell.label:setTextColor(cc.c4b(0xde, 0xcd, 0xcd, 0xff))    
                -- end
...
function FormGoFightPanel:onCellTouchedTabBtn(cell)
    ...
    cell.label:setTextColor(cc.c4b(255, 255, 255, 0xff))
...
function FormGoFightPanel:changeTabType(index, check_repeat_click, is_move_tab)
    ...
    if self.cur_tab ~= nil then
        ...
        self.cur_tab.label:setTextColor(cc.c4b(239, 212, 174, 0xff))
    ...
    if self.cur_tab ~= nil then
        ...
        self.cur_tab.label:setTextColor(cc.c4b(250, 232, 150, 0xff))
    end
...
function FormGoFightPanel:onClickTeamTabBtn(index, is_check)
    ...
    if self.team_select_team_index ~= nil and self.team_select_team_index > 1 and self.team_data_list[self.select_team_index] then
        --说明切换页签了
    ...
    self.cur_tab_data and (self.cur_tab_data.fun_form_type ==  PartnerConst.Fun_Form.CrossArenaDef or
        ...
        if hero_datas and formation then
            ...
            for pos,hero_vo in pairs(hero_datas) do
                local temp_data = {}
                temp_data.id = hero_vo.id
                print("========================updateMoreTeamHeroInfo============================",hero_vo.id)
                temp_data.pos = pos
    ...
    if tab_data then
        ...
        for i,v in pairs(dic_team_data) do
            if self.five_hero_vo[i] == nil then
                ...
                for k,info in pairs(pos_info) do
                    ...
                    elseif  is_can_employ == 1 and self.show_list then 
                        for i,s_data in ipairs(self.show_list) do
                            if info.id == s_data.partner_id  then
                                hero_vo = s_data
                                break
                            end
                        end 
                ...
                for k,_hero_vo in pairs(self.five_hero_vo[i]) do
                    ...
                    elseif  is_can_employ == 1 and self.show_list then 
                        for i,s_data in ipairs(self.show_list) do
                            if _hero_vo.id == s_data.partner_id  then
                                hero_vo = s_data
                                break
                            end
                        end 
        ...
        if more_team_data then
            for k,v in pairs(more_team_data) do
                ...
                for _,info in pairs(pos_info) do
                    local hero_vo = model:getHeroById(info.id)
                    if is_can_employ == 1 and self.show_list then 
                        for i,s_data in ipairs(self.show_list) do
                            if info.id == s_data.partner_id  then
                                hero_vo = s_data
                                break
...
--剧情布阵信息
function FormGoFightPanel:InitEliteMatch()
    --是否布阵网络信息返回
    self.is_form_back = true
    local pos_list = {}
    local data = {}
...
--单人模式的布阵信息
function FormGoFightPanel:initSingleModeFormInfo()
    ...
    for i =1,5 do 
        self.hero_item_list[i]:setVisible(false)
        self.hero_item_list[1]:setVisible(true)
    end

    -- self.form_change_btn:setEnabled(false)
    self:initFormInfo(data)
...
function FormGoFightPanel:initFormInfo(data)
    ...
    self:initFormationData(formation_config) --阵法
    self:updateFormationIcon()
    self:updateFiveHeroItem()
    self:updateHallowsIcon()
...
function FormGoFightPanel:teamInitFormInfo(index, is_change_data)
    ...
    local is_can_employ = model:getShowTeamZhiYuanByFormType(cur_fun_form_type)
    ...
    if index == 1 then
        ...
        if self.crossarena_form_list_panel then
            ...
            if self.elfin_team_data_list[self.select_team_index]  and self.team_data_list[self.select_team_index] then
                -- sprites
                local old_order = self.team_data_list[self.select_team_index].old_order
...
function FormGoFightPanel:initFormationData(formation_config)
    ...
    for i,v in ipairs(formation_config.pos) do
        ...
        if self.hero_item_list[index] then
            ...
            self.hero_item_list[index]:setZOrder(1000 - pos)
        end
        if pos <= 3 then
        ...
        local rect = cc.rect( x - width*0.5 ,y - self.cell_height*0.5 , width, width)
        self.pos_rect_list[index] = rect
         if self.fun_form_type == PartnerConst.Fun_Form.EndLessSingleMode then 
             break
         end 
...
function FormGoFightPanel:updateFormationIcon()
    ...
    local res = PathTool.getResFrame("form", "hl_form_icon_"..self.formation_type)
    loadSpriteTexture(self.form_icon, res, LOADTEXT_TYPE_PLIST)
    -- self.form_icon:setContentSize(cc.size(40,37))
...
function FormGoFightPanel:updateFightPower()
    ...
    self.fight_label:setPosition(103, 28)
    delayRun(self.arrow_bg, 0.1, function (  )
        self.arrow_bg:setPositionX(103 - self.fight_label:getContentSize().width/2 -20)
    end)
    if self.cur_power == 0 then 
    ...
    local num = 0
    for i,item in ipairs(self.hero_item_list) do
        if item.spine_item then
            num = num + 1
        end
    end

    if self.fun_form_type == PartnerConst.Fun_Form.EndLessSingleMode  then 
        self.form_label_count:setString(TI18N('已上阵:')..num..'/1')
    else
        self.form_label_count:setString(TI18N('已上阵:')..num..'/5')
    end 


end

function FormGoFightPanel:updateRelationBuff()
    local relation_buff_data = role_model:getActivateBuffData()
    self.buff_btn:setVisible(relation_buff_data~=nil)
    if not relation_buff_data then return end

    local res = PathTool.getBuffRes(relation_buff_data.icon)
    self.sp_icon:loadTexture(res,LOADTEXT_TYPE)
end
...
function FormGoFightPanel:updateEffect(status, halo_res)
    if status then
        if self.select == nil then
            ...
            self.select = createImage(self.halo_btn_up, PathTool.getResFrame("common", "common_1101"), x+2, y, cc.p(0.5, 0.5), true)
            self.select:setScale(0.78)
...
function FormGoFightPanel:updateHeroList(select_camp, must_reset)
    ...
    if not self.list_view then
        ...
        local scroll_view_size = cc.size(size.width,size.height)
        local setting = {
            -- item_class = HeroExhibitionItem,      -- 单元类
            start_x = 40,                  -- 第一个单元的X起点
            space_x = 44,                    -- x方向的间隔
            start_y = 18,                    -- 第一个单元的Y起点
            space_y = 15,                   -- y方向的间隔
            item_width = 105,               -- 单元的尺寸width
            item_height = 133,              -- 单元的尺寸height
            row = 2,                        -- 行数，作用于水平滚动类型
            col = 5,                         -- 列数，作用于垂直滚动类型
            need_dynamic = true
        }
        self.list_view = CommonScrollViewSingleLayout.new(self.lay_scrollview, cc.p(size.width * 0.5, size.height * 0.5) , ScrollViewDir.horizontal, ScrollViewStartPos.top, scroll_view_size, setting, cc.p(0.5,0.5))
    ...
    elseif self.fun_form_type == PartnerConst.Fun_Form.Planes then -- 位面冒险
     ...
     --**************************** 添加队伍支援 *********************************
    local _team_data = organize_model:getHirePartnerData()
    local is_can_employ = model:getShowTeamZhiYuanByFormType(self.fun_form_type)
    print("--------------is_can_employ----------------------",is_can_employ)
    local partner_config = Config.PartnerData.data_partner_base

    if self.fun_form_type == PartnerConst.Fun_Form.Drama and is_can_employ and _team_data and _team_data.list then
        for i,v in pairs(_team_data.list) do
            v.partner_id = v.pid
            v.id = v.pid
            v.camp_type = partner_config[v.bid].camp_type
            v.is_show_help = 1
            v.name =  partner_config[v.bid].name
            --支援的人物使用过之后就放到最后
            table_insert(show_list,1,v)
        end
    elseif self.fun_form_type == PartnerConst.Fun_Form.Arena and is_can_employ and _team_data and _team_data.list then
        for i,v in pairs(_team_data.list) do
            v.partner_id = v.pid
            v.id = v.pid
            v.camp_type = partner_config[v.bid].camp_type
    ...
    elseif self.fun_form_type == PartnerConst.Fun_Form.ArenaChampion and is_can_employ and _team_data and _team_data.list then
        for i,v in pairs(_team_data.list) do
            v.partner_id = v.pid
            v.id = v.pid
            v.camp_type = partner_config[v.bid].camp_type
    ...
    elseif self.fun_form_type == PartnerConst.Fun_Form.Ladder and is_can_employ and _team_data and _team_data.list then
        for i,v in pairs(_team_data.list) do
            v.partner_id = v.pid
            v.id = v.pid
            v.camp_type = partner_config[v.bid].camp_type
    ...
    elseif self.fun_form_type == PartnerConst.Fun_Form.ArenaTeam and is_can_employ and _team_data and _team_data.list then
        for i,v in pairs(_team_data.list) do
            v.partner_id = v.pid
            v.id = v.pid
            v.camp_type = partner_config[v.bid].camp_type
    ...
    elseif self.fun_form_type == PartnerConst.Fun_Form.EliteMatch and is_can_employ and _team_data and _team_data.list then
        for i,v in pairs(_team_data.list) do
            v.partner_id = v.pid
            v.id = v.pid
            v.camp_type = partner_config[v.bid].camp_type
    ...
    elseif self.fun_form_type == PartnerConst.Fun_Form.CrossArenaDef and is_can_employ and _team_data and _team_data.list then
        for i,v in pairs(_team_data.list) do
            v.partner_id = v.pid
            v.id = v.pid
            v.camp_type = partner_config[v.bid].camp_type
    ...
    elseif self.fun_form_type == PartnerConst.Fun_Form.ArenapeakchampionDef and is_can_employ and _team_data and _team_data.list then
        for i,v in pairs(_team_data.list) do
            v.partner_id = v.pid
            v.id = v.pid
            v.camp_type = partner_config[v.bid].camp_type
    ...
    elseif self.fun_form_type == PartnerConst.Fun_Form.LeagueExploreMode and is_can_employ and _team_data and _team_data.list then
        for i,v in pairs(_team_data.list) do
            v.partner_id = v.pid
            v.id = v.pid
            v.camp_type = partner_config[v.bid].camp_type
 ...
 -- ======================重新根据战力排序 start==临时先放着不知道是不是这个意思===========-------------------
    local sort_func = SortTools.tableUpperSorter({"power","star","lev","flag"})
    table_sort(show_list, sort_func)
...
function FormGoFightPanel:createNewCell(width, height)
    ...
    elseif  self.fun_form_type == PartnerConst.Fun_Form.EndLess or self.fun_form_type == PartnerConst.Fun_Form.EndLessWater or 
    self.fun_form_type == PartnerConst.Fun_Form.EndLessFire or self.fun_form_type == PartnerConst.Fun_Form.EndLessWind or 
    self.fun_form_type == PartnerConst.Fun_Form.EndLessLightDark or self.fun_form_type==PartnerConst.Fun_Form.EndLessSingleMode then -- 无尽试炼
...
function FormGoFightPanel:selectHero(item, hero_vo)
    ...
    print("---------------selectHero---5----------------------",#self.five_hero_vo)
    ...
    print("---------------selectHero---6----------------------",self.select_team_index)
    ...
    -- -- 支援判断是否是已经上了同一个人的了
    if hero_vo.is_show_help ==1 then 
        for i,v in pairs(self.five_hero_vo[self.select_team_index]) do
            if v.srv_id == hero_vo.srv_id and v.rid == hero_vo.rid and v.partner_id ~= hero_vo.partner_id then 
                message(TI18N("同支队伍只能使用同队友的唯一支援武灵"))
                -- local function fun()
                    
                -- end
                -- local str = string.format(TI18N("同支队伍只能使用同队友的唯一支援武灵，确定要替换为该武灵吗？"))
    ...
    if index ~= -1 then
        ...
        --                 -- item:setSelected(v.is_ui_select )
        --                 --换新
        --                 local world_pos = item:convertToWorldSpace(cc.p(HeroExhibitionItem.Width * 0.5, HeroExhibitionItem.Height * 0.5))    
        --                 local start_pos = self.fight_hero_node:convertToNodeSpace(world_pos) 
        --                 local x, y =  self.hero_item_list[i]:getPosition()
        --                 self:showMoveEffect(start_pos, cc.p(x,y), hero_vo, function()
        --                         if self.hero_item_list and self.hero_item_list[i] then
        --                             self.five_hero_vo[self.select_team_index][i] = hero_vo
        --                             self:addHeroSpine(self.hero_item_list[i],self.five_hero_vo[self.select_team_index][i])
        --                             self:updateMoreTeamHeroInfo()
        --                             self:updateFightPower()
        --                         end
        --                     end)
        ...
        self:showMoveEffect(start_pos, cc.p(x,y), hero_vo, function()
                if self.hero_item_list and self.hero_item_list[new_index] then
                    ...
                    self:addHeroSpine(self.hero_item_list[new_index],self.five_hero_vo[self.select_team_index][new_index])
                    self:updateMoreTeamHeroInfo()
                    self:updateFightPower()
                    -- printLuaTable(self.five_hero_vo[self.select_team_index][new_index])
                end
            end)
        hero_vo.is_ui_select = true
        item:setSelected(hero_vo.is_ui_select )
...
function FormGoFightPanel:checkSameHeroTips(hero_vo, dic_team_data, more_team_data, fun_form_type)
    local is_can_employ = model:getShowTeamZhiYuanByFormType(fun_form_type)
    if more_team_data then
        --剧情多队伍
...
function FormGoFightPanel:findHeroIndex(hero_vo, dic_team_data, more_team_data)
    ...
    if more_team_data then
        ...
        print("----findHeroIndex---------index--------------",index )
...
function FormGoFightPanel:setTimeFormatString(time)
    if time <= 0 then
        if self.fun_form_type == PartnerConst.Fun_Form.EliteMatch then --段位赛
            --标志意外关闭的 ps: 进入战斗后,再关闭此界面. 如果没有此标志, baseview 会把上一级的界面显示出来 
