...
function RolePersonalSpaceTabHonorWallPanel:registerEvents()
    ...
    registerButtonEventListener(self.honor_btn, function() self:onHonorBtn()  end ,true, 1)
     ...
     EventManager:getInstance():Bind(RoleEvent.ROLE_GET_HONOR_WALL_EVENT,self,function (data)
            if not data then return end
            self.is_init = true
            self:setData(data)
        end)

     EventManager:getInstance():Bind(RoleEvent.ROLE_UPDATE_HONOR_WALL_EVENT,self,function (data)
            if not data then return end
...
function RolePersonalSpaceTabHonorWallPanel:setData(scdata)
    ...
    --荣誉icon res_id
    res_id = res_id or 1
    local res = PathTool.getSingleImgForDownLoad("rolehonorwall/honorwarllicon", "honor_level_"..res_id, false)
