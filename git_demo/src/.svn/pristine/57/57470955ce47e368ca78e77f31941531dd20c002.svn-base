...
function BattlDramaBossInfoWindow:__init(data)
    ...
    self.res_list = {
            { path = PathTool.getSingleImgForDownLoad("bigbg", "bigbg_5"), type = ResourcesType.single },
        }
...
function BattlDramaBossInfoWindow:open_callback()
    ...
    self.window_title_label:setString(TI18N("武灵信息"))
    ...
    -- 战力
    self.fight_label = CommonNum.new(220, self.main_container, 99999, - 2, cc.p(0.5, 0.5))
    self.fight_label:setPosition(360, 570)
    self.arrow_bg = self.main_container:getChildByName("arrow_bg")
    ...
    self.title_label:setString(TI18N("掉落奖励"))
    ...
    --     -- loadSpriteTexture(self.role_bg,PathTool.getSingleImgForDownLoad("bigbg", "bigbg_5"), LOADTEXT_TYPE)
    -- end
...
function BattlDramaBossInfoWindow:updateData()
    if self.dungeon_data then
        ...
        if config then
            self.boss_name:setString(StringUtil.branchStr(config.name) )

        end
        self.diffcult_label:setString(self.dungeon_data.info_data.lev_limit.."级以上可以挑战")

        self.fight_label:setNum(self.dungeon_data.info_data.power)
        self.arrow_bg:setPositionX(360 - self.fight_label:getContentSize().width/2 -20)
...
function BattlDramaBossInfoWindow:updateRole(body_id)
    if not self.spine_model and body_id ~= "" then
        self.spine_model = createSpineByName(body_id, PlayerAction.stand)
        self.spine_model:setPosition(self.role_bg:getContentSize().width / 2, self.role_bg:getContentSize().height/2 + 10)
        self.spine_model:setScale(1.7)
        self.role_bg:addChild(self.spine_model)
        self.spine_model:setAnimation(0, PlayerAction.stand, true)
    end
end
...
function BattlDramaBossInfoWindow:updateItemReward(data)
    ...
    for i, v in ipairs(data) do
        ...
        if item then
            item:setPosition(BackPackItem.Width/2 + (i-1) * (BackPackItem.Width ),self.item_scroolview:getContentSize().height/2-16)
...
function BattlDramaBossInfoWindow:register_event()
    ...
    --     end)
    -- end
    EventManager:getInstance():Bind(Battle_dramaEvent.BattleDrama_Boss_Update_Data,self, function(data)
            if data then
