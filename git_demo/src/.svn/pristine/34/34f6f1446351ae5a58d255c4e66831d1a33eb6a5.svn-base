...
ChatWindow = ChatWindow or BaseClass(BaseView)
...
function ChatWindow:__init(ctrl)
    ...
    self.layout_name = "mainui/chat_window"
...
function ChatWindow:updateTabBar()
    self.stack_tag =
    {
        [1] = ChatConst.Channel.Notice,       
        [2] = ChatConst.Channel.World, 
        [3] = ChatConst.Channel.Friend,
        [4] = ChatConst.Channel.Cross,
        [5] = ChatConst.Channel.Gang,
        [6] = ChatConst.Channel.Province,
    }

    self.stack_pos =
    {
        [ChatConst.Channel.Notice]   = 1,
        [ChatConst.Channel.World]  = 2,
        [ChatConst.Channel.Friend] = 3,
        [ChatConst.Channel.Cross] = 4,
        [ChatConst.Channel.Gang] = 5,
...
function ChatWindow:initView()
    ...
    self.tab_container =left_container:getChildByName('tab_container')
...
function ChatWindow:createTabBtnList(  )
    self.tabArray = {
        {title = TI18N("系统"), index = 1, status = true},
        {title = TI18N("世界"), index = 2, status = true},
        {title = TI18N("私聊"), index = 3, status = true},
        {title = TI18N("跨服"), notice = TI18N("等级不足50级"), index = 4, status = true},
        {title = TI18N("公会"), notice = TI18N("您暂时没有加入公会"), index = 5, status = true},
    ...
    self.tab_btn_list = {}
    for i=1,6 do
        local node = self.tab_container:getChildByName('tab_btn_'..i)
        node.lightNode = node:getChildByName('light')
        node.tipNode = node:getChildByName('tab_tips')
        node.tipNum = node.tipNode:getChildByName('red_num')
        table_insert(self.tab_btn_list,node)
        node.index = i
        node.is_open = true
        node.tipNode:setVisible(false)
        node:addTouchEventListener(function(sender, event)
                if event == ccui.TouchEventType.ended then
                    if node.is_open == true then
                        ...
                        message(self.tabArray[i].notice)
                    end
                end
            end)
    end
...
function ChatWindow:_onClickTabBtn( tab_btn )
    if tab_btn then
        ...
        self.cur_selected.lightNode:setVisible(true)

        self:openTagBtn(self.cur_selected.index)

        if self.cur_selected.index == ChatConst.Channel.World then
...
function ChatWindow:checkBtnIsOpen( index )
    if index == 5 and RoleController:getInstance():getRoleVo():isHasGuild()==false then
        return false
    elseif index == 4 then
...
function ChatWindow:setTabBtnTouchStatus(status, index)
    local tab_btn = self.tab_btn_list[index]
    if tab_btn then
        tab_btn.is_open = status
        tab_btn.lightNode:setVisible(false)
        setChildUnEnabled(not status, tab_btn)
        --tab_btn:setTouchEnabled(status)
    end
end
...
function ChatWindow:setRedStatus( status, num ,tab)
...
-- 打开界面
function ChatWindow:open_callback()
    self:initHandle()
...
--清理
function ChatWindow:close_callback()
