...
function ElfinSummonPanel:configUI(  )
	...
	local bg_res = PathTool.getSingleImgForDownLoad("elfin", "txt_cn_elfin_summ_bg", false)
...
function ElfinSummonPanel:register_event(  )
	...
	-- 召唤数据更新
	EventManager:getInstance():Bind(ElfinEvent.Update_Elfin_Summon_Data_Event,self,function (data)
            self:setData(data)
        end)
	

	-- 精灵抽奖结果更新
	EventManager:getInstance():Bind(ElfinEvent.Update_Elfin_Summon_Rewards_Data_Event,self,function (data)
	...
	-- 精灵抽奖结果item更新
	EventManager:getInstance():Bind(ElfinEvent.Update_Elfin_Item_Event,self,function (data)
			self.rewards_info = data
...
-- 			local res_id = PathTool.getSingleImgForDownLoad("bigbg/elfin", "elfin_1")	
-- 			local scale = 0.8
-- 			if item_id == 1 then
-- 				scale = 1
...
-- 					res_id = PathTool.getSingleImgForDownLoad("bigbg/elfin", "elfin_"..config.icon)	
-- 					item:setItemIcon(res_id)
-- 					scale = 1
-- 				end
-- 			end
