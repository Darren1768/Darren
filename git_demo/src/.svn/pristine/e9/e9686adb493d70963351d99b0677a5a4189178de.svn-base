...
function ActionSandybeachBossFightPanel:configUI()
    ...
    if not self.item_load then
        local res = PathTool.getSingleImgForDownLoad("bigbg/sandybeachbossfight", "txt_cn_sandybeach_1")
...
function ActionSandybeachBossFightPanel:register_event()
     EventManager:getInstance():Bind(ActionEvent.Aandybeach_Boss_Fight_Action_Event,self,function (data)
            if not data then return end
            self:setData(data)
        end)
     EventManager:getInstance():Bind(ActionEvent.Aandybeach_Boss_Fight_Buy_Count_Event,self,function (data)
            if not data then return end
