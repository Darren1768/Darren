...
function DialActionMainPanel:ctor(bid)
	...
	self.res_list = {
        ...
        {path = PathTool.getSingleImgForDownLoad("bigbg/action","txt_cn_action_dial_bg", true), type = ResourcesType.single},
	}
...
function DialActionMainPanel:configUI(  )
    ...
    image_bg:loadTexture(PathTool.getSingleImgForDownLoad("bigbg/action","txt_cn_action_dial_bg", true), LOADTEXT_TYPE)
...
function DialActionMainPanel:register_event(  )
    ...
    -- 抽奖结果
    EventManager:getInstance():Bind(DialActionEvent.Update_Dial_Result_Event,self,function (id)
            if id then
    ...
    -- 转盘数据
    EventManager:getInstance():Bind(DialActionEvent.Update_Dial_Data_Event,self,function (data)
            if data then
    ...
    -- 转盘全服记录
    EventManager:getInstance():Bind(DialActionEvent.Update_Dial_Record_Event,self, function ( r_type )
            if r_type == 2 then
    ...
    -- 奖池数据变化
    EventManager:getInstance():Bind(DialActionEvent.Update_Dial_Gold_Event,self, function (  )
            self:updateGialGoldNum()
        end)
    

    -- 活动数据
    EventManager:getInstance():Bind(ActionEvent.UPDATE_HOLIDAY_SIGNLE,self,function (data)
