...
function BackpackTips:open_callback()
    ...
    self.goods_item:setPosition(cc.p(80,66))
    ...
    self.desc_label = createRichLabel(20, cc.c3b(189,180,199), cc.p(0, 1), cc.p(-2, 100), 8, nil, 400) 
    ...
    for i=1, 3 do
        ...
        if btn then
            local object = {}
            object.btn = btn
            object.label = btn:getChildByName("text")
...
function BackpackTips:getLimitBuytLabel()
    if self.limit_buy_label == nil then
        ...
        self.limit_buy_label = createRichLabel(20, cc.c4b(189,180,199,0xff), cc.p(0, 0.5), cc.p(16, y) ,nil,nil,1000)
...
function BackpackTips:adjustContentSizeAndPos(  )
    ...
    if self.show_button_num == 3 then 
        self.tab_panel_height  = 140
        target_height = target_height + 60
    else
        self.tab_panel_height  = 80
    end 
    ...
    self.bg:setPositionY(target_height)
    self.bg:setContentSize(cc.size(self.container_init_size.width, target_height))
    ...
    self.scroll_view:setPositionY(self.desc_panel:getContentSize().height-5)

    self.tab_panel:setPositionY(self.desc_panel:getPositionY()-self.desc_panel:getContentSize().height+8)
...
function BackpackTips:updateBtnList()
    ...
    if btn_sum == 1 then        -- 如果只有1个按钮,按钮1移到按钮3的位置 移动到中间
        ...
        -- end
        if object_1.btn then
            object_1.btn:setPositionX(233.5)
        end
    end

    self.show_button_num = btn_sum
	...
	for i, v in ipairs(tips_btn) do
        ...
        if object and object.btn then
            ...
            if v ==BackPackConst.tips_btn_type.source then 
                object.label:setColor(cc.c3b(231,213,196))
                object.btn:loadTextures(PathTool.getResFrame("common", "hl_common_4_6"),"","", LOADTEXT_TYPE_PLIST)
            end 
