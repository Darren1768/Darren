...
function PresageSummonPanel:tabChangeView(index)
    ...
    local bg_res = PathTool.getSingleImgForDownLoad("bigbg/timesummon", "txt_cn_presage_summon"..index)
...
function PresageSummonPanel:register_event()
	EventManager:getInstance():Bind(EliteSummonEvent.PresageSummon_Message,self,function(data)
            if not data then return end
    ...
    --刷新
    EventManager:getInstance():Bind(EliteSummonEvent.PresageSummon_ReFresh,self,function(data)
        	self:setTouchChangeView()
	        self:openPineEffect()
        end)
    
    --打开松果
    EventManager:getInstance():Bind(EliteSummonEvent.PresageSummon_Open_Pine,self,function()
        	self:setTouchChangeView()
        	self:openPineEffect()
        end)
    
    --购买物品返回
    EventManager:getInstance():Bind(EliteSummonEvent.PresageSummon_Buy_Return,self,function()
        	self:setLimitCount()
        end)
