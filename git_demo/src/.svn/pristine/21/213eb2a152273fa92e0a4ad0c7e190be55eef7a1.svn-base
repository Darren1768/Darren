...
function ReturnActionTaskPanel:register_event()
     EventManager:getInstance():Bind(ReturnActionEvent.Task_Event,self,function(data)
     ...
     EventManager:getInstance():Bind(ReturnActionEvent.Task_Updata_Event,self,function()
            local quest_list = model:getActionTaskQuestList()
            self:setConfigData(quest_list)
        end)

     EventManager:getInstance():Bind(ReturnActionEvent.Limin_Task_Event,self,function (data)
            ...
            if config and self.dic_limit_list[config.f_id] then
                ...
                local config_list = Config.HolidayReturnNewData.data_lanterm_task[period][config.f_id]
                --如果有下一个任务档次
...
function ReturnActionTaskItem:configUI()
    self.root_wnd = createCSBNote(PathTool.getTargetCSB("returnaction/returnaction_task_item"))
