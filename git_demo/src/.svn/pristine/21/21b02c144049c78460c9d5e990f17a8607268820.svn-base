...
function MonopolyController:handle27400(data)
	if data.flag == 1 then
		...
		EventManager:getInstance():Fire(MonopolyEvent.Update_Monopoly_Base_Data_Event)
	else
		-- 活动未开启，关闭相关所有功能
...
function MonopolyController:handle27401(data)
	...
	if data.flag == TRUE then -- 进入地图成功
		...
		EventManager:getInstance():Fire(MonopolyEvent.Update_Monopoly_Map_Data_Event)
	end
end
...
function MonopolyController:handle27403(data)
	...
	if data.flag == TRUE then -- 扔骰子成功
		...
		if self.model:checkIsInCurMap(data.id) then
			...
			EventManager:getInstance():Fire(MonopolyEvent.Get_Dice_Result_Event, data.num, is_move)
		end
	end
end
...
function MonopolyController:handle27405(data)
	EventManager:getInstance():Fire(MonopolyEvent.Get_Morra_Result_Event, data)
end
...
function MonopolyController:handle27408(data)
	EventManager:getInstance():Fire(MonopolyEvent.Update_Monopoly_Buff_Event, data)
end
...
function MonopolyController:handle27410(data)
	EventManager:getInstance():Fire(MonopolyEvent.Get_Master_Data_Event, data)
end
...
function MonopolyController:handle27500(data)
	if data.flag == 1 then
		EventManager:getInstance():Fire(MonopolyEvent.Get_Boss_Data_Event, data)
	end
end
...
function MonopolyController:handle27502( data )
	EventManager:getInstance():Fire(MonopolyEvent.Get_Personal_Rank_Data_Event, data)
end
...
function MonopolyController:handle27503(data)
	EventManager:getInstance():Fire(MonopolyEvent.Get_Guild_Rank_Data_Event, data)
end
...
function MonopolyController:handle27504(data)
	if data then
		...
		EventManager:getInstance():Fire(MonopolyEvent.Update_Buff_Data_Event)
	end
end
...
function MonopolyController:handle27505( data )
	...
	EventManager:getInstance():Fire(MonopolyEvent.Update_Boss_Num_Event)
end
...
function MonopolyController:checkShowWaitTips()
	...
	if not self.monopoly_dialog_wnd and not self.chose_step_wnd and not self.monopoly_pumpkin_wnd and not self.monopoly_morra_wnd then
		if self.monopoly_main_scene then -- 当前没在大富翁场景，则无需显示tips，直接显示奖励
			EventManager:getInstance():Fire(MonopolyEvent.Get_Show_Tips_Event)
		else
