...
function BattleBuffListItem:configUI(  )
	...
	self.buff_icon:setScale(1.8)
	self.buff_icon:setPositionY(100)
...
function BattleBuffListItem:setData( data )
	...
	self.name_label:setPositionX(90)
	...
	if not self.buff_desc then
		self.buff_desc = createRichLabel(20, cc.c3b(98, 53, 43), cc.p(0, 1), cc.p(100, 75), 4, nil, 430)
		self.container:addChild(self.buff_desc)
	end
	self.buff_desc:setString(data.desc or "")
	-- self.buff_desc:setString("回合后失效回合后失效回合后失效回合后失效回合后失效回合后失效回合后失效回合后失效回合后失效回合后失效回合后失效回合后失效回合后失效回合后失效回合后失效回合后失效回合后失效回合后失效")
	-- self.bg:setVisible(true )
end
