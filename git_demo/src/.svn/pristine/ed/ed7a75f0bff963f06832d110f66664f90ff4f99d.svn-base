...
function LadderTopThreeWindow:__init()
	...
	self.res_list = {
        ...
        {path = PathTool.getSingleImgForDownLoad("bigbg","bigbg_59"), type = ResourcesType.single },
        {path = PathTool.getSingleImgForDownLoad("bigbg","bigbg_60"), type = ResourcesType.single },
	}
...
function LadderTopThreeWindow:open_callback(  )
    ...
    for i=1,3 do
    	...
    	state_data.label = statue:getChildByName("label")
		if state_data.label then
			state_data.label:setString("")
		end
	...
	-- ======================================不知道这个类是干啥的 没有这个lua文件报错 所以注释掉===============--------------------------
	-- local layer = ViewManager:createPanelByPath("mainui.view.topBarPlayerInfo")
	-- if layer then
	--  	self:addChild(layer)
...
function LadderTopThreeWindow:register_event(  )
	...
	for i=1,3 do
     ...
     EventManager:getInstance():Bind(LadderEvent.UpdateLadderTopThreeRoleData,self, function ( data )
            self:setData(data)
        end)
