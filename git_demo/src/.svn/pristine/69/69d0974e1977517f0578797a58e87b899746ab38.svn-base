...
function MailModel:addMailItem(data_list)
    ...
    self:checkMailRedSum()
    -- 新增一个邮件的时候刷新邮件列表
    EventManager:getInstance():Fire(MailEvent.UPDATE_ITEM)
end
...
function MailModel:readMailItem(data)
    ...
    self:checkMailRedSum()
    -- 读取单封邮件的处理
    EventManager:getInstance():Fire(MailEvent.READ_MAIL_INFO, key)
end
...
function MailModel:getMailGood(data)
    ...
    self:checkMailRedSum()
    -- 提取一个邮件的物品
    EventManager:getInstance():Fire(MailEvent.GET_ITEM_ASSETS, key)
end
...
function MailModel:updateRedStatus(bid, num)
    ...
    -- 更新红点, 1为邮件 2位公告
    EventManager:getInstance():Fire(MailEvent.UPDATEREDSTATUS, bid, num)
end
...
function MailModel:initNoticeList(data_list, data_type)
    ...
    -- 这里包含了更新删除之类
    EventManager:getInstance():Fire(MailEvent.UPDATE_NOTICE)
end
...
function MailModel:readNoticeItem(id)
    ...
    self:checkNoticeRedSum()
    -- 读取一封公告
    EventManager:getInstance():Fire(MailEvent.READ_INFO_NOTICE,id)
end
