...
function RedbagController:registerEvents()
       --角色数据创建完毕后，监听资产数据变化情况
        ...
        self.role_create_success = EventManager:getInstance():Bind(EventId.ROLE_CREATE_SUCCESS,self,function()
  
            EventManager:getInstance():UnBindEvent(self.role_create_success)
...
function RedbagController:handle13540( data )
    EventManager:getInstance():Fire(RedbagEvent.Get_List_Event,data)
end
--发红包排名
...
function RedbagController:handle13545( data )
    EventManager:getInstance():Fire(RedbagEvent.Rank_List_Event,data)
end
...
function RedbagController:handle13546(data)
    EventManager:getInstance():Fire(RedbagEvent.Update_Red_Bag_Event, data.list) 
end
