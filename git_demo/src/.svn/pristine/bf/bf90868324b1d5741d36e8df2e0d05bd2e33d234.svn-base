...
function HeavenChapterWindow:__init()
    ...
    self.res_list = {
        ...
        { path = PathTool.getSingleImgForDownLoad("bigbg","bigbg_84", true), type = ResourcesType.single },
    }
...
function HeavenChapterWindow:open_callback(  )
    ...
    --self.background:loadTexture(PathTool.getSingleImgForDownLoad("bigbg","bigbg_84",true), LOADTEXT_TYPE)
...
function HeavenChapterWindow:setData(  )
    ...
    if self.chapter_cfg.map_id and self.chapter_cfg.map_id ~= 0 and self.background then
        local map_res = PathTool.getSingleImgForDownLoad("bigbg", _string_format("bigbg_" .. self.chapter_cfg.map_id), true)
