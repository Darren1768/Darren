...
function HalidomMainPanel:ctor(callback)
    ...
    _table_insert(res_list, {path = PathTool.getSingleImgForDownLoad("bigbg/haildom", "haildom_bg_1"), type = ResourcesType.single})
...
function HalidomMainPanel:createHalidomSkillItem(  )
    ...
    local icon_kuang = createSprite(PathTool.getResFrame("common", "hl_common_91"), layer_size.width/2, layer_size.height/2, skill_layer, cc.p(0.5, 0.5))
...
function HalidomMainPanel:registerEvent(  )
    ...
    -- 圣物数据返回
     EventManager:getInstance():Bind(HalidomEvent.Get_Halidom_Data_Event,self, function (  )
            self:setData()
        end)

    -- 圣物数据更新
     EventManager:getInstance():Bind(HalidomEvent.Update_Halidom_Data_Event,self, function ( id )
    ...
    -- 圣物红点
     EventManager:getInstance():Bind(HalidomEvent.Update_Halidom_Red_Event,self, function()
            self:updateHalidomData()
            self:updateHalidomRedStatus()
            self:updateTabRedStatus()
        end)
