...
function LadderModel:updateLadderRedStatus(bid, status)
    ...
    GhostdomController:getInstance():setBuildRedStatus(GhostdomConst.Build_Type.battle, {bid = CrossgroundConst.Red_Type.ladder, status = ladder_status})
	-- 更新天梯界面红点
	EventManager:getInstance():Fire(LadderEvent.UpdateLadderRedStatus, bid, status)
end
