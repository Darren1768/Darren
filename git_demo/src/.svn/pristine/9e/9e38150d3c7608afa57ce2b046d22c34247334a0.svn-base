...
function MallItem:configUI( ... )
    ...
    local x,y = self.sold:getPosition()
    self.goods_item = BackPackItem.new(true,true)
    self.goods_item:setPosition(x,y)
    ...
    self.discount_label = createRichLabel(20,cc.c3b(255, 252, 220),cc.p(0.5,0),cc.p(90,212))
    self.discount_label:setString(TI18N(""))
    self.main_container:addChild(self.discount_label)
    self.discount_label:setVisible(false)

    self.back = self.main_container:getChildByName('base')
    self.table_desk = self.main_container:getChildByName('table')
...
function MallItem:setData( data )
    ...
    local x = 85.06
    local btn_x = 84.06
    if data.order%3 == 1 then
        res = PathTool.getResFrame('shop', 'hl_shop_11')
        x = 85.06
        btn_x = 83.06
    elseif data.order%3 == 2 then
        res = PathTool.getResFrame('shop', 'hl_shop_12')
        x = 86.06
        btn_x = 85.56
    else
        res = PathTool.getResFrame('shop', 'hl_shop_13')
        x = 86.56
        btn_x = 88.56
    end
    loadSpriteTexture(self.back, res, LOADTEXT_TYPE_PLIST)
    self.back:setPositionX(x)
    self.count_bg:setPositionX(btn_x)

    self.table_desk:setVisible(data.order%3 == 1)
...
function MallItem:registerEvent()
    ...
    --除神秘商城以外的购买成功
    EventManager:getInstance():Bind(MallEvent.Buy_Success_Event,self,function ( data )
    ...
    --神秘/神格商城购买成功
    EventManager:getInstance():Bind(MallEvent.Buy_One_Success,self,function ( data )
            if not data or not self.data then return end
