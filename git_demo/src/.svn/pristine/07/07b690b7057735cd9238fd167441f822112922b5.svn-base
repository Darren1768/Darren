...
--
-- --------------------------------------------------------------------
UnionCreatePanel = class("UnionCreatePanel", function()
	return ccui.Layout:create()
end)

local controller = UnionController:getInstance() 

function UnionCreatePanel:ctor(ctrl) 
    ...
    self.root_wnd = createCSBNote(PathTool.getTargetCSB("union/union_create_panel"))
    ...
    name_title:setString(TI18N("联盟大名："))
    ...
    declaration_title:setString(TI18N("联盟宣言：")) 
...
set_title:setString(TI18N("验证设置：")) 
    ...
    self.edit_title = createEditBox(container, res, cc.size(403, 50), nil, 22, Config.ColorData.data_color4[151], 22, TI18N("请输入联盟名字"), nil, nil, LOADTEXT_TYPE_PLIST)
    ...
    local res = PathTool.getResFrame("common", "common_99998")
    self.declaration_value = createEditBox(container, res, cc.size(403, 146), nil, 22, Config.ColorData.data_color4[151], 22, TI18N("请输入联盟宣言内容"), nil, nil, LOADTEXT_TYPE_PLIST)
    ...
    if role_vo.vip_lev < lev.val then
        ...
        self.create_btn_label:setString(string.format("<img src=%s visible=true scale=0.3 /><div outline=2,#764519>%s %s</div>", PathTool.getItemRes(item_icon), gold_num, TI18N("创建联盟")))
    end
...
function UnionCreatePanel:registerEvent()
    ...
    self.create_btn:addTouchEventListener(function(sender, event_type)
        ...
        if event_type == ccui.TouchEventType.ended then
            ...
            local union_name = self.edit_title:getText()
            if union_name == "" then
            ...
            elseif StringUtil.getStrLen(union_name) > 12 then
                ...
                if config and config_2 then
                    controller:requestCreateUnion(union_name, sign, self.set_index, config.lev, config_2) 
                end
            end
        end
    end) 
...
function UnionCreatePanel:addToParent(status)
...
function UnionCreatePanel:setUnionConditionInfo(index)
...
function UnionCreatePanel:setUnionConditionInfo2(index)
...
function UnionCreatePanel:initConditionList()
    self.condition_list = {
        {index = 1, lev = 1,  desc = TI18N("1级")},
        {index = 2, lev = 2, desc = TI18N("2级")}, 
        {index = 3, lev = 3, desc = TI18N("3级")},
    }
...
function UnionCreatePanel:setUnionSetInfo(index)
	if index == 0 then
...
function UnionCreatePanel:DeleteMe()
end