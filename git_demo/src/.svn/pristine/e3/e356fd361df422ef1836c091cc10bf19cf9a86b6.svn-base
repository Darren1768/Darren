...
function ArenaChampionTop321View:handleEvent(status)
    ...
    EventManager:getInstance():UnBind(self)
     ...
     EventManager:getInstance():Bind(ArenaEvent.UpdateTop32InfoEvent,self, function(list) 
                self:updateTop32InfoList(list)
            end)

     EventManager:getInstance():Bind(ArenaEvent.UpdateTop324GuessGroupEvent,self, function(group,pos)
...
function ArenaChampionTop32Item:registerEvent()
    for k,check_btn in pairs(self.check_btn_list) do
        check_btn:addTouchEventListener(function(sender, event_type)
            if event_type == ccui.TouchEventType.ended then
                ...
                if self.data then
                    EventManager:getInstance():Fire(ArenaEvent.CheckFightInfoEvent, true, self.data.group, k) 
                end
            end
        end)
    end
    self.guess_btn:addTouchEventListener(function(sender, event_type)
        if event_type == ccui.TouchEventType.ended then
            playCommonButtonSound()
            EventManager:getInstance():Fire(ArenaEvent.ChangeTanFromTop324)
        end
    end)
end
