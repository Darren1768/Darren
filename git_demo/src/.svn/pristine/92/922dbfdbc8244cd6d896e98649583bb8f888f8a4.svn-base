...
function PrimusMainWindow:__init()
    ...
    self.res_list = {
        ...
        {path = PathTool.getSingleImgForDownLoad("bigbg/primus", "primus_bigbg_1", false), type = ResourcesType.single},
        {path = PathTool.getSingleImgForDownLoad("bigbg/primus", "primus_bigbg_2", false), type = ResourcesType.single}
    }
    --站台数据
    self.station_list = {}
...
function PrimusMainWindow:open_callback()
    ...
    self.background:loadTexture(PathTool.getSingleImgForDownLoad("bigbg/primus", "primus_bigbg_1", false), LOADTEXT_TYPE)
...
function PrimusMainWindow:updateStationInfoByPos(pos_index)
    ...
    if station_item.local_data then
        ...
        if honor_data then
            local res = PathTool.getSingleImgForDownLoad("honor","txt_cn_honor_"..honor_data.res_id,false,false)
