...
function VedioMainWindow:open_callback(  )
    ...
    if self.background ~= nil then
        self.background:loadTexture(PathTool.getSingleImgForDownLoad("bigbg","hl_guild",false), LOADTEXT_TYPE)
    ...
    resetStudioButton(self.collect_btn,ButtonType.red,TI18N("个人收藏"))
    self.myself_btn = self.main_panel:getChildByName("myself_btn")
    resetStudioButton(self.myself_btn,ButtonType.red,TI18N("个人记录"))


    commonShowEmptyIcon(self.main_panel,false,{text = TI18N("暂无录像信息")})
...
function VedioMainWindow:openRootWnd( sub_index )
    ...
    RedManager:getInstance():changeStrStatus(MainuiConst.red_str.vedio, false)
...
function VedioMainWindow:setData(  )
    ...
    if next(self.vedio_show_data) ~= nil then
        ...
        commonShowEmptyIcon(self.main_panel,true,{text = TI18N("暂无录像信息")})
    end
end
...
function VedioMainWindow:updateEliteVedioData(  )
    ...
    commonShowEmptyIcon(self.main_panel,not next(self.elite_vedio_data) ~= nil,{text = TI18N("暂无录像信息")})
end
