...
function PetardActionMainPanel:ctor(bid)
	...
	self.res_list = {
        ...
        {path = PathTool.getSingleImgForDownLoad("bigbg/action","txt_cn_action_petard", true), type = ResourcesType.single},
	}
...
function PetardActionMainPanel:configUI(  )
    ...
    image_bg:loadTexture(PathTool.getSingleImgForDownLoad("bigbg/action","txt_cn_action_petard", true), LOADTEXT_TYPE)
...
function PetardActionMainPanel:registerEvent(  )
    ...
    -- 花火大会数据
     EventManager:getInstance():Bind(PetardActionEvent.Get_Base_Info_Event,self, function ()
            self:setData()
        end)
