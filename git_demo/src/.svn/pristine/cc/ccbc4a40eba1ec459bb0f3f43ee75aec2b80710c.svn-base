...
function MailInfoWindow:__init()
    ...
    self.title_str = TI18N("邮件")
	self.show_background = false
	self.show_close_btn = false
end

function MailInfoWindow:open_callback()
    local csbPath = PathTool.getTargetCSB("mail/mail_info_window")
    local root = createCSBNote(csbPath)
    self.container:addChild(root)
	self.container:setPosition(260,-50)
    
	local main_container = root:getChildByName("main_container")
    ...
    self.scrollView_half_height = self.info_container_size.height - self.scrollView_half_y
    ...
    self.content = createRichLabel(22,cc.c3b(0x83,0x52,0x4b),cc.p(0,1),cc.p(20,height-50),8,0,480)
    self.scrollView:addChild(self.content)
    self.youxiao = createLabel(20,cc.c3b(87,149,32),nil,width - 20 ,height - 25,TI18N("有效期"),self.scrollView,nil, cc.p(1, 0.5))
    self.goods_container = info_container:getChildByName("goods_container")
    self.goods_scroll = createScrollView(width-10,120,5,35,self.goods_container,ccui.ScrollViewDir.horizontal)
    ...
    resetStudioButton(self.btn_del,ButtonType.red_big,TI18N("删除"))
    
    self.btn_submit = self.goods_reple:getChildByName("btn_submit")
    resetStudioButton(self.btn_submit,ButtonType.yellow_big,TI18N("提交评分"))
    
    self.btn = main_panel:getChildByName("btn")
    resetStudioButton(self.btn,ButtonType.yellow_big)
...
function MailInfoWindow:changeButtonStatus( status )
	if status then
		local res = PathTool.getResFrame("common","hl_common_4_7")
		self.btn:loadTextures(res, res, "", LOADTEXT_TYPE_PLIST)
        resetStudioButton(self.btn,ButtonType.yellow_big,TI18N("领取"))
	else
		local res = PathTool.getResFrame("common","hl_common_4_8")
		self.btn:loadTextures(res, res, "", LOADTEXT_TYPE_PLIST)
        resetStudioButton(self.btn,ButtonType.red_big,TI18N("删除"))
	end
end
	...
	if status then
		if status == 1 then --已读
			if (self.data.assets and self.data.items) and (#self.data.assets>0 or #self.data.items>0) then --读了没领
				...
				loadSpriteTexture(self.icon, PathTool.getResFrame("common", "hl_common_132"), LOADTEXT_TYPE_PLIST)
			else --读了领了
		...
		elseif status == 0 then --未读
			if (self.data.assets and self.data.items) and (#self.data.assets>0 or #self.data.items>0) then --有物品
				...
				loadSpriteTexture(self.icon, PathTool.getResFrame("common", "hl_common_131"), LOADTEXT_TYPE_PLIST)
			else
