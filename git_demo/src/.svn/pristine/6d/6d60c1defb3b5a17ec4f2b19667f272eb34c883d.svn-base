...
function ActionSmasheggPanel:ctor(bid)
	...
	self.res_list = {
        ...
        {path = PathTool.getSingleImgForDownLoad("bigbg/action","txt_cn_action_smash_egg", true), type = ResourcesType.single},
	}
...
function ActionSmasheggPanel:configUI(  )
    ...
    image_bg:loadTexture(PathTool.getSingleImgForDownLoad("bigbg/action","txt_cn_action_smash_egg", true), LOADTEXT_TYPE)
...
function ActionSmasheggPanel:register_event(  )
	...
	-- 砸蛋活动数据
     EventManager:getInstance():Bind(SmasheggEvent.Update_Smashegg_Data_Event,self, function (data)
            if data then
                self:setData(data)
            end
        end)
    ...
    -- 日志数据
     EventManager:getInstance():Bind(SmasheggEvent.Update_Smashegg_Record_Event,self, function (data)
