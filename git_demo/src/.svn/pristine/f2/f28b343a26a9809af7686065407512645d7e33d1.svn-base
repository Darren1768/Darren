...
function ArenaEnterChampionView:handleEvent(status)
    if status == false then
        ...
        EventManager:getInstance():Bind(ArenaEvent.UpdateChampionBaseInfoEvent,self, function(data) 
                self:setBaseInfo()
            end)

        EventManager:getInstance():Bind(ArenaEvent.UpdateChampionRoleInfoEvent,self, function() 
                self:setRoleInfo()
            end)

        EventManager:getInstance():Bind(ArenaEvent.UpdateChampionTop3Event,self, function(list)
