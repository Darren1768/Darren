...
function SkinShopWindow:__init()
    ...
    self.res_list = {
		...
		{path = PathTool.getSingleImgForDownLoad("bigbg/mall","skin_bg_2"), type = ResourcesType.single},
		{path = PathTool.getSingleImgForDownLoad("bigbg/mall","skin_bg_3"), type = ResourcesType.single},
		{path = PathTool.getSingleImgForDownLoad("bigbg/mall","txt_cn_skin_banner"), type = ResourcesType.single},
	}
...
function SkinShopItem:setData(data)
	...
	-- 卡牌
    local card_res = PathTool.getSingleImgForDownLoad("bigbg/partnercard", "partnercard_" .. self.skin_cfg.head_card_id)
...
function SkinShopItem:createItemBgSprite()
	if not self.item_grid_bg then
		self.item_grid_bg = createSprite(PathTool.getSingleImgForDownLoad("bigbg/mall","skin_bg_3"), -14, 20, self, cc.p(0,1), LOADTEXT_TYPE, -1)
	end
