...
function ActionSweetPanel:ctor(bid)
	...
	self.res_list = {
        ...
        {path = PathTool.getSingleImgForDownLoad("actionsweet","txt_cn_actionsweet_bg", true), type = ResourcesType.single},
	}
...
function ActionSweetPanel:registerEvent(  )
    ...
    -- 甜蜜大作战数据
     EventManager:getInstance():Bind(ActionEvent.Update_Sweet_Data_Event,self, function ()
            self:setData()
        end)

    -- 捐献成功
    --[[ if not self.put_success_event then
        self.put_success_event = EventManager:getInstance():Bind(ActionEvent.Sweet_Put_Success_Event,self, function (id)
