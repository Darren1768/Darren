...
function OnlineGiftWindow:__init()
    ...
    self.res_list = {
        {path = PathTool.getSingleImgForDownLoad("bigbg","hl_bigbg_39"), type = ResourcesType.single},
    }
    self.item_list = {}
end
...
function OnlineGiftWindow:open_callback()
    ...
    local res = PathTool.getSingleImgForDownLoad("bigbg","hl_bigbg_39")
...
function OnlineGiftWindow:getOnlineItems()
    self.goods_con:setInnerContainerSize(cc.size(511,0.9*119*(math.floor(length/4)) + 20))
    local pos_y = self.goods_con:getInnerContainerSize().height - 50
    for i=1, length do
        ...
        if self.item_list[i] and get_time_items[i] then
            ...
            self.item_list[i]:setPosition(cc.p(33+width+tvl*4,pos_y - height-2 ))
...
function OnlineGiftWindow:register_event()
    ...
    self:addGlobalEvent(OnlineGiftEvent.Updata_Data, function(data)
        ...
        setChildUnEnabled(false, self.item_list[num])
        self.item_list[num]:setReceivedIcon2(true)

        -- self.item_list[num]:showItemEffect(false, 263, PlayerAction.action_1, true, 1.1)
        self.item_list[num]:setReceivedIcon3(false)

        self.item_list[num]:setDefaultTip(false)
        self.item_list[num]:showStrTipsV(false)
    end)
    ...
    for i,v in ipairs(get_time_items) do
        ...
        if self.item_list[i] then
            if self.dic_time[v.time] then
                ...
                self.item_list[i]:setReceivedIcon2(true) --self.main_container
                self.item_list[i]:setDefaultTip(false)
                self.item_list[i]:setReceivedIcon3(false)

            else
                if i <= num then 
                    --可领取
                    -- self.item_list[i]:showItemEffect(true, 263, PlayerAction.action_1, true, 1.1)
                    self.item_list[i]:setReceivedIcon3(true)

                    if not is_show then
                        is_show = true
                        self.item_list[i]:showStrTipsV(true, TI18N("可\n领\n取"))
                    end
                    self.item_list[i]:setDefaultTip(false)
                else
                    --不可领取
