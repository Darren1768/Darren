    ...
    self.handle_btn:addTouchEventListener(function(sender, event_type)
        ...
        if event_type == ccui.TouchEventType.ended then 
            ...
            if self.data ~= nil then
                ...
                EventManager:getInstance():Fire(GuildvoyageEvent.AddToOrderPartnerListEvent, self.data)
            end
        end
    end)
end
...
function GuildvoyageChoosePartnerItem:changeInFormStatus()
    ...
    if self.data.voyage_in_form == true then
        ...
        self.handle_btn:loadTexture(PathTool.getResFrame("common","hl_common_4.1"), LOADTEXT_TYPE_PLIST) 
        ...
        self.handle_btn:loadTexture(PathTool.getResFrame("common","hl_common_4"), LOADTEXT_TYPE_PLIST) 
