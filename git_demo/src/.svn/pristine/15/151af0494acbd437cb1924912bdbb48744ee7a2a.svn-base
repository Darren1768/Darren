...
function DialActionController:handle16670( data )
	if data then
		...
		EventManager:getInstance():Fire(DialActionEvent.Update_Dial_Data_Event, data)
	end
end
...
function DialActionController:handle16671( data )
	if data then
		...
		if data.result == TRUE and data.flag == 0 then -- 设置为播放动画时才分发事件
			EventManager:getInstance():Fire(DialActionEvent.Update_Dial_Result_Event, data.id)
		end
	end
end
...
function DialActionController:handle16674( data )
	if data then
		...
		EventManager:getInstance():Fire(DialActionEvent.Update_Dial_Record_Event, data.type)
	end
end
...
function DialActionController:handle16676( data )
	if data then
		...
		EventManager:getInstance():Fire(DialActionEvent.Update_Dial_Gold_Event)
	end
end
