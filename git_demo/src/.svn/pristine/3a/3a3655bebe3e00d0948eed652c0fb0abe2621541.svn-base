...
function SingleRankPanel:registerEvent()
    ...
    --
     EventManager:getInstance():Bind(GuildsecretareaEvent.GUILD_SECRET_AREA_RANK_COUNT_EVENT,self, function(data)
            if not data then return end
    ...
    --巅峰冠军赛
     EventManager:getInstance():Bind(ArenapeakchampionEvent.ARENAPEAKCHAMPION_CURRENT_RANK_EVENT,self, function(data)
            if not data then return end
    ...
    --年兽排行榜
     EventManager:getInstance():Bind(ActionyearmonsterEvent.Year_Rank_Info_Event,self, function(data)
            if not data then return end
    ...
    -- 点赞事件监听，
     EventManager:getInstance():Bind(RoleEvent.WorshipOtherRole,self, function(rid, srv_id, rank, _type) 
...
function SingleRankPanel:updateOtherInfo(data, role_vo)
    ...
    elseif self.rank_type == RankConstant.RankType.arena_peak_champion then --巅峰冠军赛
        ...
        if self.good_btn == nil then
            local res = PathTool.getResFrame("common","hl_common_4")
