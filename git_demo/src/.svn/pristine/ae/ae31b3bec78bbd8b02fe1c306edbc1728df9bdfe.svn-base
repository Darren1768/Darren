...
function ArenapeakchampionGuessingTabPromotion:registerEvents()
    ...
    --64强 晋级赛
     EventManager:getInstance():Bind(ArenapeakchampionEvent.ARENAPEAKCHAMPION_TOP_64_EVENT,self,function ( )
            self:initData()
        end)
    --8强
     EventManager:getInstance():Bind(ArenapeakchampionEvent.ARENAPEAKCHAMPION_TOP_8_EVENT,self,function ( data )
            self:initData()
        end)
