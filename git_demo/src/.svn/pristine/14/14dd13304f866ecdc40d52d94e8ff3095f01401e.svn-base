...
function GmAdjustConfigPanel:showChangeLayer( status, cfg_data, father_key )
    if status == true then
        ...
        if not self.change_layer then
            ...
            self.key_input = ccui.EditBox:create(cc.size(220, 60), PathTool.getResFrame("common", "hl_chat_3"), LOADTEXT_TYPE_PLIST)
...
function InputItem:configUI()
    ...
    self.input_box = ccui.EditBox:create(cc.size(200, 30), PathTool.getResFrame("common", "hl_chat_3"), LOADTEXT_TYPE_PLIST)
