...
function FriendWindow:__init()
    ...
    self.scroll_width = 650
    self.scroll_height = 885
...
function FriendWindow:register_event()
    ...
    if self.role_vo ~= nil then
       ...
       --更新红点用
       self:addGlobalEvent(FriendEvent.Update_Friend_List_Red_Point, function(data)
            if self.cur_type == FriendConst.Type.MyFriend  then 
                if self.pre_panel  then 
                    self.pre_panel:setDelRedBtn(data)
                end
            end
       end)
...
function FriendWindow:openRootWnd(type)
    type = FriendConst.Type.MyFriend
    self:selectedTabCallBack(type)
    self:showRedPoint()
...
function FriendWindow:updateFriendList(change_index, is_del)
    ...
    commonShowRecordBtn(self.container)
...
function FriendWindow:showEmptyIcon(bool)
    ...
    commonShowEmptyIcon(self.container,bool,{text = str})
end
