...
function ArenapeakchampionModel:checkPeakChampionTotalRedPoint(need_check)
    ...
    if need_check then
        EventManager:getInstance():Fire(ArenapeakchampionEvent.ARENAPEAKCHAMPION_ALL_RED_POINT_EVENT)
        ...
        GhostdomController:getInstance():setBuildRedStatus(GhostdomConst.Build_Type.battle, {bid = CrossgroundConst.Red_Type.peakChampion, status = redpoint})
    end
    return redpoint
end
