...
function ChargeShopWindow:__init()
    ...
    self.res_list = {
        ...
        {path = PathTool.getSingleImgForDownLoad("bigbg/mall","charge_bg_1", true), type = ResourcesType.single},
    }
...
function ChargeShopWindow:open_callback( )
    ...
    if self.background ~= nil then
    	self.background:loadTexture(PathTool.getSingleImgForDownLoad("bigbg/mall","charge_bg_1", true), LOADTEXT_TYPE)
...
function ChargeShopWindow:updateShopBgById(  )
    ...
    if shop_cfg and shop_cfg.bg_id ~= "" then
        local res_path = PathTool.getSingleImgForDownLoad("bigbg/mall", shop_cfg.bg_id)
