...
function PokedexWindow:__init()
    ...
    self.res_list = {
        ...
        { path = PathTool.getSingleImgForDownLoad("bigbg","bigbg_35",true), type = ResourcesType.single },
    }
...
function PokedexWindow:open_callback()
    ...
    if self.background ~= nil then
        self.background:loadTexture(PathTool.getSingleImgForDownLoad("bigbg","bigbg_35",true), LOADTEXT_TYPE)
...
function PokedexWindow:register_event()
    ...
    for i,tab in pairs(self.tab_list) do
     ...
     EventManager:getInstance():Bind(PokedexEvent.Get_All_Event,self,function(data)
     ...
     EventManager:getInstance():Bind(PokedexEvent.Up_End_Event,self,function(data)
            if not data then return end
            self:updateAttrList(data.old_star,data.cur_lev)
        end)
     EventManager:getInstance():Bind(PartnerEvent.Partner_Call_Event,self,function()
            self.is_call = true 
            self.ctrl:sender11040()
        end)
end
...
function PokedexWindow:createBaseMessage()
    ...
    self.up_btn = createTypeButton(self.pokedex_panel, TI18N(''), 590,-12, ButtonType.red)
