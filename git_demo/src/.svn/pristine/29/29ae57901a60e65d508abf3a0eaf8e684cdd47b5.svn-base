...
function VedioController:handle19901( data )
	if data then
		if data.type == VedioConst.MyVedio_Type.Myself then
			EventManager:getInstance():Fire(VedioEvent.UpdateMyselfVedioEvent, data.replay_list)
		elseif data.type == VedioConst.MyVedio_Type.Collect then
			EventManager:getInstance():Fire(VedioEvent.UpdateCollectVedioEvent, data.replay_list)
		end
	end
end
...
function VedioController:handle19902( data )
	if data then
		...
		EventManager:getInstance():Fire(VedioEvent.UpdatePublicVedioEvent, data.type)
	end
end
...
function VedioController:handle19904( data )
	...
	if data.code == 1 and data.type == 1 then -- 收藏成功
		...
		EventManager:getInstance():Fire(VedioEvent.CollectSuccessVedioEvent, data.id)
	end
	if data.type == 0 then
		EventManager:getInstance():Fire(VedioEvent.CancelCollectVedioEvent, data.id)
	end
end
...
function VedioController:handle19906( data )
	if data.like then
		...
		EventManager:getInstance():Fire(VedioEvent.UpdateTodayLikeNum)
	end
end
...
function VedioController:handle19908(data)
	...
	-- EventManager:getInstance():Fire(VedioEvent.LOOK_VEDIO_EVENT, data)
end
