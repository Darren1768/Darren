...
function GlobalMessageMgr:showMoveVertical(msg, color)
	print("============showMoveVertical=====================msg===========",msg )
	...
	--背景
	local image = createScale9Sprite(PathTool.getResFrame("common2", "hl_common_0125"))
	image:setScale9Enabled(true)
	local image_height = 47
	container.image = image
	container:addChild(image)
	
	--文本
	local temp_msg = string.format("<div fontcolor=#fee896 fontsize=24 >%s</div>", msg)
	...
	local max_height = math.max(label_size.height + 10, image_height)
	local size = cc.size(max_width, max_height)
	local image_max_width = math.max(label_size.width + 500, 500)
...
function GlobalMessageMgr:showPermanentMsg( is_show, msg )
	if is_show == true then
		if not self.permanentMsg then
			...
			--背景
			local image = createScale9Sprite(PathTool.getResFrame("common", "common_90056"))   --common_90056
			image:setScale9Enabled(true)
...
function GlobalMessageMgr:PowerMove(num, res, old_num, setting)
	-- local title_res = res or PathTool.getResFrame("common", "txt_cn_hl_common_134")
	...
	if not self.power_container then
		...
		self.power = CommonNum.new(108, self.power_container, 0, -3, cc.p(0, 0.5))
		self.power:setPosition(cc.p(310, 127))
		
		self.add_num = CommonNum.new(109, self.power_container, 0, -3, cc.p(0, 0.5))
		self.add_num:setPosition(cc.p(310, 125))
	...
	self.add_num:setPositionX(count * 26 + 320)
	local add_count = 0
