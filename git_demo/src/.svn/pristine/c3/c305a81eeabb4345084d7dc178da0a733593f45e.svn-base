...
function RedbagModel:updateData(data)
    ...
    EventManager:getInstance():Fire(RedbagEvent.Get_Data_Event,data)
...
function RedbagModel:checkRedBagRedPoint( )
    ...
    --抛去主界面播特效
    EventManager:getInstance():Fire(RedbagEvent.Can_Get_Red_Bag,vo)
    --抛去公会界面红点
    EventManager:getInstance():Fire(GuildEvent.UpdateGuildRedStatus,GuildConst.red_index.red_bag, self.all_red_show) 
    --更新场景红点
    local bool, desc = RoleController:getInstance():checkEnterIsLock(GhostdomConst.Build_Type.guild)
    if bool == false then

        GhostdomController:getInstance():setBuildRedStatus(GhostdomConst.Build_Type.guild,{bid = GuildConst.red_index.red_bag, status = self.all_red_show}) 
    end
end
