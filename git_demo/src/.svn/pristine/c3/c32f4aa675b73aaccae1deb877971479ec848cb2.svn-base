...
function BarrageMainView:open_callback()
    ...
    self.record_btn:getChildByName("label"):setString(TI18N("竞猜记录11"))
...
function BarrageMainView:register_event()
    ...
    self.close_btn:addTouchEventListener(function(sender, event_type)
     ...
     EventManager:getInstance():Bind(BarrageEvent.UpdateBarrageData,self, function(data)
            if data == nil or data.type == nil then return end
            self:updateCacheBarrageList(data)
        end)

     EventManager:getInstance():Bind(BarrageEvent.SetVisibleStatus,self, function(status)
