...
function HomeWorldScene:__init()
	...
	self.res_list = {
		{path = PathTool.getSingleImgForDownLoad("homeworld","homeworld_big_bg_1"), type = ResourcesType.single },
        {path = PathTool.getSingleImgForDownLoad("homeworld","homeworld_big_bg_4"), type = ResourcesType.single },
...
function HomeWorldScene:openRootWnd( other_home_data )
    ...
    delayRun(self.root_wnd, 1.2, function() 
        --检测一下是否有萌宠事件
        EventManager:getInstance():Fire(HomepetEvent.HOME_PET_CHECK_TRIGGER_EVENT)
    end)

    self:updateRedBtnStatus()

    --测试音效 --"lwc"
    -- GlobalAudio:getInstance():playMusic(GlobalAudio.AUDIO_TYPE.SCENE, "s_006", true) --
end
...
function HomeWorldScene:updateHomeFloorById( floor_id )
    ...
    EventManager:getInstance():Fire(HomeworldEvent.Discharge_Furniture_Event, self.cur_floor_id)
    ...
    EventManager:getInstance():Fire(HomeworldEvent.Add_Furniture_Event, floor_id)
end
...
function HomeWorldScene:updateHomeWallById( wall_id )
    ...
    EventManager:getInstance():Fire(HomeworldEvent.Discharge_Furniture_Event, self.cur_wall_id)
    ...
    EventManager:getInstance():Fire(HomeworldEvent.Add_Furniture_Event, wall_id)
end
...
function HomeWorldScene:createOneUnit( unit_data, is_add )
	...
	if unit_vo then
        ...
        EventManager:getInstance():Fire(HomeworldEvent.Add_Furniture_Event, unit_vo.bid)
	end
end
...
function HomeWorldScene:close_callback( )
    ...
    GlobalAudio:getInstance():playMusic(GlobalAudio.AUDIO_TYPE.SCENE, "s_002", true) -- 恢复主城背景音乐播放
	MainuiController:getInstance():setMainUIShowStatus(true)
