...
function HeroUpgradeStarFusePanel:register_event()
    ...
    --添加英雄选择返回事件
     EventManager:getInstance():Bind(HeroEvent.Upgrade_Star_Select_Event,self, function() self:updateHeroItemInfo() end)
    -- --添加英雄升星成功返回
    ...
    --升星成功后 会更新主卡英雄信息    
     EventManager:getInstance():Bind(HeroEvent.Hero_Data_Update,self, function()
        if not self.fuse_data then return end
...
--         local bg_res = PathTool.getSingleImgForDownLoad("bigbg/hero",HeroConst.CampBgRes[camp_type], true)
...
function HeroUpgradeStarFusePanel:updateHeadHeroInfo(fuse_data)
    ...
    --背景门
    local camp_res = PathTool.getSingleImgForDownLoad("bigbg/hero",HeroConst.CampBottomBgRes[camp_type], false)
