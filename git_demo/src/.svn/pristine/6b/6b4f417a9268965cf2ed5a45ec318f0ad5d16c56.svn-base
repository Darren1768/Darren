...
function SeerpalacePreviewWindow:setData(  )
	...
	if award_config then
		...
		local max_height = math.max(content_h+ 80, self.scroll_view_size.height)
		self.scroll_view:setInnerContainerSize(cc.size(self.scroll_view_size.width, max_height))

		-- 5星
		local desc_txt_1 = createLabel(20,cc.c3b(0x8e,0x59,0x38),nil,start_x,max_height-desc_height/2,five_star_config.desc,self.scroll_view,nil,cc.p(0, 0.5))
		...
		local desc_txt_2 = createLabel(20,cc.c3b(0x8e,0x59,0x38),nil,start_x,start_y-desc_height/2,four_star_config.desc,self.scroll_view,nil,cc.p(0, 0.5))
	...
	if summon_data and summon_data[SeerpalaceConst.Summon_Index[index]] then
		...
		for i,v in pairs(tab) do
			delayRun(self.list_panel, i/60, function()
		        ...
		        item_node:setDefaultTip()
		        local pos_x = 12 + (BackPackItem.Width*scale + 12) * ((i-1)%4)
