...
function ActiontermbeginsTabBossPanel:layoutUI()
    ...
    local bg_res = PathTool.getSingleImgForDownLoad("bigbg/termbegins", "term_begins_panel_bg", false)
...
function ActiontermbeginsTabBossPanel:registerEvents()
    ...
    --打开布阵事件
     EventManager:getInstance():Bind(ActiontermbeginsEvent.TERM_BEGINS_BOSS_FORM_EVENT,self, function()
            HeroController:getInstance():openFormGoFightPanel(true, PartnerConst.Fun_Form.TermBeginsBoss) 
        end)
    --提交试卷数量
     EventManager:getInstance():Bind(ActiontermbeginsEvent.TERM_BEGINS_SUBIT_PAPER_EVENT,self, function(data)
