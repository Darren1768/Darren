...
function GuildvoyageChooseConfirmWindow:open_callback()
    ...
    container:getChildByName('win_title'):setString(TI18N("½±ÀøÑ¡ÔñÈ·ÈÏ"))
    container:getChildByName('title_1'):setString(TI18N("µ¥±¶½±Àø"))
    container:getChildByName('title_2'):setString(TI18N("Ë«±¶½±Àø"))
    ...
    self.cancel_btn:getChildByName("label"):setString(TI18N("µ¥±¶½±Àø"))
...
function GuildvoyageChooseConfirmWindow:openRootWnd(order)
    ...
    if order.config then
        local str = string.format("<img src=%s visible=true scale=0.35 />,<div fontcolor=#ffffff fontsize=24 outline=1,#c45a14>%s %s</div>", PathTool.getItemRes(15), order.config.double_cost, TI18N("Ë«±¶½±Àø"))
		self.confirm_btn_label:setString(str)
    end
end

--[[
    @desc:´´½¨»ù´¡½±Àø,°üº¬ÁË¹Ì¶¨ºÍ¸ÅÂÊ 
    author:{author}
...
function GuildvoyageChooseConfirmWindow:createRewardsList()
    ...
    local list = {}
    local is_gain = order.is_gain -- ÊÇ·ñÓÐË«±¶½±Àø
