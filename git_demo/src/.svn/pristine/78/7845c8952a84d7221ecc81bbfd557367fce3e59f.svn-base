...
function GuildBossMainWindow:open_callback()
	...
	self.comfirm_btn = createTypeButton(main_panel,TI18N("激活"), self.buff_container_pos_x + 400, self.buff_container_pos_y + 22 , ButtonType.red)
...
function GuildBossMainWindow:register_event()
	...
	if self.muster_btn then
     ...
     EventManager:getInstance():Bind(GuildbossEvent.MusterCoolTime,self, function(data)
			if self.role_vo.position == 1 or self.role_vo.position == 2 then
     ...
     EventManager:getInstance():Bind(GuildbossEvent.BossActivityDoubleTime,self, function(data)
	...
	-- 更新面板基础信息
     EventManager:getInstance():Bind(GuildbossEvent.UpdateGuildDunBaseInfo,self, function()
     ...
     EventManager:getInstance():Bind(GuildbossEvent.UpdateChangeStatus,self,function (data)
			self:updateChangeStatus(data)
		end)

     EventManager:getInstance():Bind(GuildbossEvent.UpdateGuildDunRank,self,function ()
	...
	-- 更新当前剩余挑战次数
     EventManager:getInstance():Bind(GuildbossEvent.UpdateGuildBossChallengeTimes,self, function(buy_type)
...
function GuildBossMainWindow:updateBg(image)
	local res_id = PathTool.getSingleImgForDownLoad("bigbg/guildboss",image)
