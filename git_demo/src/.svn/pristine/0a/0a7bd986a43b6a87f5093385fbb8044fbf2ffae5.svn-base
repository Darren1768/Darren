...
--ui层
BATTLE_VIEW_UI_LAYER_Z = 9999
BATTLE_VIEW_UI_LAYER_TAG = 9999
--人物层
...
function BattleSceneNewView:regitsterEvent()
    EventManager:getInstance():Bind(Battle_dramaEvent.BattleDrama_Drama_Buff_View,self, function(data)
           print("---------------createBuffIcon------·-0---0------------")

            self:createBuffIcon(data)
        end)		


    EventManager:getInstance():Bind(EventId.CHAT_HEIGHT_CHANGE,self, function()
...
function BattleSceneNewView:configUI()
    ...
    if not self.main_layer then
        ...
        self.map_sLayer:setScale(display:getMaxScale())
...
function BattleSceneNewView:createMap()
    ...
    self.music_info = GlobalAudio:getInstance():getMusicInfo()
    self:playMusic()
...
function BattleSceneNewView:playMusic()
    ...
    if music_config ~= nil then
        ...
        if music_config.bg_music ~= "" then
            GlobalAudio:getInstance():playMusic(GlobalAudio.AUDIO_TYPE.BATTLE, music_config.bg_music, true)
        end
    end
end
...
function BattleSceneNewView:createHorizontalMap(battle_res_id)
    print("------------------self.is_single_bg-------------------------",self.is_single_bg,self.battle_type)
    if self.is_single_bg == FALSE then
        local res = PathTool.getBattleSceneRes(_string_format("%s/map_bg", self.battle_res_id), false)
        if self.map_bg_res_id ~= res and not self.map_scroll_view then		-- 创建前景层,前景层创建完成之后开始加载假战斗
        ...
        -- local res = PathTool.getBattleSceneRes(_string_format("%s/m_bg", self.battle_res_id), true)
        ...
        -- local res = PathTool.getBattleSceneRes(_string_format("%s/b_bg", self.battle_res_id), false)
        ...
        -- 剧情副本特有的一些东西
        if self.battle_type == BattleConst.Fight_Type.Darma then
            print("------------------ BattleConst.Fight_Type.Darma-----------------------1-----------------------------")
                local res = PathTool.getBattleSceneRes(_string_format("%s/b_bg", self.battle_res_id), false)
                if self.map_bg_res_id ~= res and not self.map_scroll_view then		-- 创建前景层,前景层创建完成之后开始加载假战斗
                    self.map_bg_res_id = res
                    self.load_res_map_bg = createResourcesLoad(res, ResourcesType.single, function()
            ...
            print("------------------ BattleConst.Fight_Type.Darma-----------------------2-----------------------------")
...
function BattleSceneNewView:createNewBg(res_path)
    ...
    if not self.map_bg_2 and not _tolua_isnull(self.map_sLayer) and not _tolua_isnull(self.map_bg) then
        if not _tolua_isnull(self.map_sLayer) and not _tolua_isnull(self.map_bg) then
            ...
            self.map_bg_2 = createSprite(res_path, map_width, self.slayer_init_y, self.map_sLayer, cc.p(0, 0), LOADTEXT_TYPE, BATTLE_VIEW_BACK_LAYER_Z)
        end
    end
end
...
function BattleSceneNewView:createUiLayer(data, main_vo, combat_type)
    ...
    BattleDramaController:getInstance():setMaplayerShow(false)
    ...
    if not _controller:getIsNoramalBattle() then
        ...
        if not self.double_speed_btn then
            self.double_speed_btn = createImage(self.ui_layer, PathTool.getResFrame("common", "hl_common_2050"), 672, display:getTop(self) - 130, cc.p(0.5, 0.5), true, nil, true)
            self.double_speed_btn:setTouchEnabled(true)
            self.double_speed_Label = createLabel(26,cc.c3b(69,44,30),nil,36,36,TI18N("X1"),self.double_speed_btn,2,cc.p(0.5, 0.5))
        ...
        if not BattleController:getInstance():getWatchReplayStatus() and _config and _config.is_skip == "true" then
            if not self.skip_btn then
                self.skip_btn = createImage(self.ui_layer, PathTool.getResFrame("common", "hl_common_2050"), 672, display:getTop(self) - 220, cc.p(0.5, 0.5), true, nil, true)
                self.skip_btn_label = createLabel(26,cc.c3b(69,44,30),nil,36,36,TI18N("跳过"),self.skip_btn,2,cc.p(0.5, 0.5))
    ...
    if not _controller:getWatchReplayStatus() then --正常战斗下
        ...
        elseif self.battle_type == BattleConst.Fight_Type.PractiseTower then  --演武场试练塔活动
            self:addBoosHpUI()
        elseif self.battle_type == BattleConst.Fight_Type.LeagueExplore then  --探索历练
            -- self:addExitBtnUI()
        end
    else
        self:addExitBtnUI()
...
function BattleSceneNewView:updataZhenfaInfo(data)
    ...
    MainuiController:getInstance():setIsShowMainUITop(false)
    ...
    if self.ui_layer and not _tolua_isnull(self.ui_layer) then
        ...
        if _tolua_isnull(self.form_view) then
            ...
            self.form_view:setPosition(self.main_size.width/2, display.getTop(self) - 62)
...
function BattleSceneNewView:getStarPanelUI()
    ...
    if self.ui_layer and not _tolua_isnull(self.ui_layer) then
        if _tolua_isnull(self.star_panel) then
            ...
            self.star_panel:setPosition(200, display.getTop(self) - 215)
            local star_list = {}
            local desc_txt_list = {}
            for i=1,3 do
...
function BattleSceneNewView:addBoosHpUI()
    ...
    self.resources_hp_load:addAllList(res_list, function()
            if self.battle_boss_hp_progress == nil then
                ...
                self.battle_boss_hp_progress:setPosition(360, display.getTop(self) - 215)
            ...
            local mask_res = PathTool.getResFrame("common", "hl_common_39") 
            ...
            self.boss_hp_max = 0
            EventManager:getInstance():Bind(BattleEvent.Battle_Boss_Hp_Event,self, function(setting)
...
function BattleSceneNewView:setMonsterYearHead(head_panel, head_id)
    ...
    head_panel.mask_res = PathTool.getResFrame("common", "hl_common_39") 
...
function BattleSceneNewView:addLimitExerciseBossFightUI()
    ...
    if desc_txt_list and star_list then
        ...
        if self.dunge_data[1] and self.dunge_data[2] and self.dunge_data[3] and self.dunge_data[4] then
            ...
            if boss_list[round] and boss_list[round][diff] and boss_list[round][diff][order_id]  then
                local config = boss_list[round][diff][order_id]
                if config then
                    for i=1,3 do
                        if desc_txt_list[i] then
                            desc_txt_list[i]:setPositionX(12)
            ...
            for i,object in ipairs(self.session_list) do
                ...
                if status then
                    ...
                    if status ~= 0 and cur_session > i then
                        local status_res
                        if status == 1 then -- 胜利
                            status_res = PathTool.getResFrame("battle", "txt_cn_battle_win")
                        elseif status == 2 then -- 失败
                            status_res = PathTool.getResFrame("battle", "txt_cn_battle_lose")
                        elseif status == 3 then -- 平局
                            status_res = PathTool.getResFrame("battle", "txt_cn_battle_dogfall")
                        end
...
function BattleSceneNewView:createLevTips(suppress)
    ...
    -- 		if event_type == ccui.TouchEventType.ended then
    -- 			playMainButtonSound()
    -- 			local str = TI18N("等级压制：怪物<div fontcolor=#3dbf5f>平均等级<div>超过己方英雄平均等级时，怪物<div fontcolor=#3dbf5f>伤害和免伤</div>提升，高出等级越多提升越大")--<div>怪物等级超过玩家英雄等级<div fontcolor=#289b14>5<div>级后，超过的每一级，怪物伤害提升<div fontcolor=#289b14>3%<div>，免伤提升<div fontcolor=#289b14>1%<div><div>")
...
function BattleSceneNewView:addRoleEnterSpine(objects)
    local 	_num_tiem ={}
 
    for k,v in pairs(objects) do
        -- delayRun(self.main_layer,v.enter_delay/60 or 1,function()
        --     if v.object_type ==2 then 
        --         local _config = Config.PartnerData.data_partner_base[v.object_bid]
        --         local spineStr="E10000"
        --         if _config.quality and  _config.quality >=1 and _config.quality <=2 then
        --             spineStr="E10000"
        --         elseif  _config.quality   and _config.quality ==3 then 
        --             spineStr="E10001"
        --         elseif  _config.quality   and _config.quality >3 then 
        --             spineStr="E10002"
        --         end
        --         local screen_pos = gridPosToScreenPos(v.grid_pos_back)
        --         --  playEffectOnce(spineStr, screen_pos.x, screen_pos.y+300, self.main_layer, nill, nil, nil, nil, PlayerAction.action, 1)
        --         local spineNode= createEffectSpine(spineStr, cc.p(screen_pos.x,screen_pos.y+250), cc.p(1,0.5), false, PlayerAction.action)
        --         spineNode:setScale(1)
        --         self.main_layer:addChild(spineNode, 10)
        --     end 
        -- end)
...
function BattleSceneNewView:showSpecStart(call_back, data)
    ...
    if self.spec_tips then
        ...
        if form_info and form_info[1] and form_info[1] [1] then
            local left_form_icon = createSprite(PathTool.getResFrame("form", "hl_form_icon_" .. form_info[1] [1]), 120, 27, self.left_bg, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST)
        end
        ...
        if form_info and form_info[2] and form_info[2] [1] then
            local right_form_icon = createSprite(PathTool.getResFrame("form", "hl_form_icon_" .. form_info[2] [1]), 120, 27, self.right_bg, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST)
        end
...
function BattleSceneNewView:addExitBtnUI( )
    if self.leave_btn == nil then
        local btn_res = PathTool.getResFrame("common", "hl_common_2050")
        self.leave_btn = createButton(self.ui_layer, "", 0, 0, nil, btn_res, 24)
        createLabel(26,cc.c3b(69,44,30),nil,36,36,TI18N("返回"),self.leave_btn,2,cc.p(0.5, 0.5))
        self.leave_btn:setPosition(672, display:getTop(self) - 610)
...
function BattleSceneNewView:updateRound(round)
    print("_-----------------updateRound------------------")
    -- 用于通知buff总览界面更新数据
    EventManager:getInstance():Fire(BattleEvent.UPDATE_ROUND_NUM)
    -- 检测是否进入天平模式
...
function BattleSceneNewView:showSkillName(attacker, call_back, is_act)
    print("============战斗播报==========showSkillName=================",attacker.skill_data.name)
    ...
    if not is_act and attacker.temp_skill_bid ~= attacker.attacker_info.skill_bid then
        ...
        local _num = math.floor(attacker.temp_skill_bid/100)  
        local _type = math.floor(_num%10) 
        print("---------------attacker.temp_skill_bid----------------",attacker.temp_skill_bid,_num,_type)

        if not (_type==2 or _type==4 or _type==6) then 
            if call_back then
                call_back()
            end
            return
        ...
        if self.skill_container == nil then
            ...
            self.skill_container:setContentSize(cc.size(SCREEN_WIDTH, 190))
            self.skill_container:setAnchorPoint(cc.p(0.5, 0.5))
            self.skill_container:setPosition(SCREEN_WIDTH * 0.5, 550)
            self.effect_layer_1:addChild(self.skill_container)


            -- 星技背景
            self.star_skill_bg = createSprite(nil, SCREEN_WIDTH * 0.5, 0, self.skill_container, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST,0)
            self.star_skill_bg:setVisible(false)

            self.player_head = createSprite(nil, 173, -60, self.skill_container, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST,0)
            

            -- 星际名字
            self.star_skill_name = createSprite(nil, 200, -300, self.skill_container, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST,0)
            self.star_skill_name:setVisible(false)

            self.skill_name_label = createLabel(26, cc.c3b(213,213,196), cc.c3b(155,51,50), 247, 182, "", self.skill_container, 0, cc.p(0.5, 0.5),TITLE_FONT)
        ...
        if object_type == BattleObjectType.Elfin then --精灵喊招特效
            ...
            if self.skill_effect_2 then
                self.skill_effect_2:setVisible(false)
            end
            ...
            if self.skill_effect == nil then
                self.skill_effect = createEffectSpine(PathTool.getEffectRes(276), cc.p(360, 100), cc.p(0.5, 0.5), false, action) --E31321
                self.skill_container:addChild(self.skill_effect, 1)
            else
                self.skill_effect:setVisible(true)
            end

            if self.skill_effect_2 == nil then
                self.skill_effect_2 = createEffectSpine("E31357", cc.p(360, 100), cc.p(0.5, 0.5), false, action)
                self.skill_container:addChild(self.skill_effect_2, 1)
            else
                self.skill_effect_2:setVisible(true)
            end
            ...
            if _type ==4 then 
                ...
                self.skill_effect:setPosition(560,130)
                self.skill_effect:setRotation(90)


                self.skill_name_label:setString( StringUtil.branchStr(attacker.skill_data.name) )
                self.skill_name_label:setAnchorPoint(cc.p(0.5,0.5))
            end
            if _type ==2 then 
                self.skill_effect_2:setVisible(true)
                self.skill_effect_2:setAnimation(0, action, false)
                if is_left then 
                    self.skill_effect_2:setPosition(100,100)
                    self.skill_effect_2:setScaleX(1)
                    self.skill_name_label:setAnchorPoint(cc.p(0,0.5))

                else
                    self.skill_effect_2:setPosition(620,100)
                    self.skill_effect_2:setScaleX(-1)
                    self.skill_name_label:setAnchorPoint(cc.p(1,0.5))

                end 
                self.skill_effect_2:setRotation(0)
        ...
        if object_type == nil then
            ...
            if head_icon == nil or head_icon == 0 or head_icon == "" then
                ...
                local head_move_to = cc.MoveTo:create(_controller:getActTime("skill_bg_move_time"), cc.p(180, 200))
                local label_move_to = cc.MoveTo:create(_controller:getActTime("skill_bg_move_time"), cc.p(name_pos_x-30, 120))
                ...
                if object_type == BattleObjectType.Elfin then -- 精灵的喊招头像特殊处理
                     ...
                     self.player_head:setPosition(114, 200)
                     self.skill_name_label:setPosition(280, 120)
                else
                    -- res = PathTool.getHeadIcon(head_icon)
                    if _type ==2 then 
                        res = PathTool.getHeadIcon(head_icon)
                        self.player_head:setScale(0.45)
                        self.player_head:setZOrder(2)

                        if is_left then 
                            self.player_head:setPosition(0, 130)
                            self.skill_name_label:setPosition(50, 120)
                            head_move_to = cc.MoveTo:create(_controller:getActTime("skill_bg_move_time"), cc.p(30, 120))
                            label_move_to = cc.MoveTo:create(_controller:getActTime("skill_bg_move_time"), cc.p(70, 120))
                        else
                            self.player_head:setPosition(720, 130)
                            self.skill_name_label:setPosition(670, 120)
                            head_move_to = cc.MoveTo:create(_controller:getActTime("skill_bg_move_time"), cc.p(690, 120))
                            label_move_to = cc.MoveTo:create(_controller:getActTime("skill_bg_move_time"), cc.p(650, 120))

                        end 
                       
                    elseif _type==4 then  --半身
                        res = PathTool.getPartnerBustRes(head_icon)
                        self.player_head:setScale(1)
                        self.player_head:setZOrder(0)
                        self.player_head:setPosition(200, 200)
                        self.skill_name_label:setPosition(400, 100)

                        head_move_to = cc.MoveTo:create(_controller:getActTime("skill_bg_move_time"), cc.p(360, 200))
                        label_move_to = cc.MoveTo:create(_controller:getActTime("skill_bg_move_time"), cc.p(580, 100))

                        
                        head_delay = cc.DelayTime:create(_controller:getActTime("skill_name_delay_time10"))
                        head_delay_2 = cc.DelayTime:create(_controller:getActTime("skill_name_delay_time12"))

                    elseif _type==6 then 
                        -- res = PathTool.getPartnerBustRes(head_icon) --head_icon
                        local _hero_cfg = Config.PartnerData.data_partner_base[head_icon]
                        res = PathTool.getSingleImgForDownLoad("herodraw/herodrawres", _hero_cfg.draw_res, false)
                        self.player_head:setScale(1)
                     
                        self.player_head:setPosition(360, 0)
                        head_move_to = cc.MoveTo:create(_controller:getActTime("skill_bg_move_time"), cc.p(360, 0))
                        self.star_skill_bg:setVisible(true)
                        self.star_skill_name:setVisible(true )
                        local _star_bg_res = PathTool.getSingleImgForDownLoad("campbg", "1", false) --_hero_cfg.
                        local _star_name_res = PathTool.getSingleImgForDownLoad("starname","name", false)

                        loadSpriteTexture(self.star_skill_bg,_star_bg_res , LOADTEXT_TYPE)
                        loadSpriteTexture(self.star_skill_name,_star_name_res , LOADTEXT_TYPE)

                        head_delay = cc.DelayTime:create(_controller:getActTime("skill_name_delay_time20"))
                        head_delay_2 = cc.DelayTime:create(_controller:getActTime("skill_name_delay_time22"))
                    end 
                end
...
function BattleSceneNewView:showElfinSkillAni(attacker, call_back, is_act)
    ...
    if not is_act and attacker.temp_skill_bid ~= attacker.attacker_info.skill_bid then
        ...
        if self.elfin_head == nil then
            self.elfin_head = createSprite(nil, 140, 148, self.elfin_skill_container, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST, 2)
        ...
        if object_type == nil then
            ...
            if head_icon == nil or head_icon == 0 or head_icon == "" then
                ...
                self.elfin_head:setPosition(80, 148)
                ...
                local call_back_fun = cc.CallFunc:create(call_back)
                local head_move_to = cc.MoveTo:create(_controller:getActTime("skill_bg_move_time"), cc.p(140, 148))
                local head_fade_in = cc.FadeIn:create(_controller:getActTime("skill_bg_fadeIn"))
                local head_delay = cc.DelayTime:create(_controller:getActTime("skill_name_delay_time"))
                local head_delay_2 = cc.DelayTime:create(_controller:getActTime("skill_name_delay_time2"))
                local head_over = cc.CallFunc:create(function()
...
-- 		GlobalAudio:getInstance():playEffect(GlobalAudio.AUDIO_TYPE.BATTLE, "b_sprite_skill")
...
--@return 
--==============================--
function BattleSceneNewView:closeup(closeup_bid,group,pos)
    local scene = self.main_layer
    local _config_data = Config.SkillData.data_get_closeup_data[closeup_bid]
    if not _config_data then
        return
    end
    local center_type = _config_data.center -- 移动到中心
    if center_type ==0 then 
        pos = cc.p(0,0)
    end
    
    local  choseup_scale_x  = _config_data.range/10    -- 特写幅度
    local  choseup_scale_y  =  _config_data.range/10     -- 特写幅度
    local  choseup_offset_x = pos.x + _config_data.move_x  -- 偏移x 
    local  choseup_offset_y = _config_data.move_y   -- 偏移y
    local  choseup_time = _config_data.closeup_time/10
    local  stop_time =_config_data.stop_time/10
    if group == BattleGroupTypeConf.TYPE_GROUP_ENEMY then  -- 敌方
        choseup_offset_x = pos.x - _config_data.move_x  
    end 

    if center_type==1 then 
        choseup_offset_y= (pos.y  - self.map_init_y +  _config_data.move_y)*choseup_scale_x
        choseup_offset_x= choseup_offset_x*choseup_scale_x
    end 

   
    self.camera_closeup_pos = cc.p(self.main_layer:getPosition())

    local function returnPos()
        self.is_closeup = false     
        local  choseup_scale  = cc.ScaleTo:create(stop_time, 1, 1)
        local move = cc.MoveTo:create(stop_time,cc.p(SCREEN_WIDTH/2,SCREEN_HEIGHT/2))
        local spawn = cc.Spawn:create(choseup_scale, move)
        scene:runAction(spawn)
    end
    
    -- 去除特写
    if self.is_closeup then 
        returnPos()
        return 
    end    
    if scene.choseup_action then
        scene:stopAllActions() --stopAction(scene.action)
        scene.choseup_action = nil
        self.main_layer:setPosition(cc.p(SCREEN_WIDTH/2,SCREEN_HEIGHT/2))
        self.main_layer:setScale(1)
    end

    self.is_closeup = true

    local pos = cc.p(self.camera_closeup_pos.x + choseup_offset_x, self.camera_closeup_pos.y + choseup_offset_y)

    local move = cc.MoveTo:create(choseup_time,pos)
    -- 缩放到xx 
    local  choseup_scale  = cc.ScaleTo:create(0, choseup_scale_x, choseup_scale_y)
    -- local spawn = cc.Spawn:create(choseup_scale, move)
    local delay = cc.DelayTime:create(stop_time)
    local  choseup_call = cc.Sequence:create(choseup_scale,move,delay)
    scene.choseup_action = choseup_call
    scene:runAction(choseup_call)
...
function BattleSceneNewView:cleanFightView()
    ...
    if self.skill_effect_2 then
        self.skill_effect_2:clearTracks()
        self.skill_effect_2:removeFromParent()
        self.skill_effect_2 = nil
    end
...
function BattleSceneNewView:clearUiLayer(is_reconnect)
    ...
    BattleDramaController:getInstance():setMaplayerShow( true )
    self.call_back = nil
    ...
    MainuiController:getInstance():setIsShowMainUITop(true)
    ...
    if not is_reconnect then
        if not _main_controller:checkIsInDramaUIFight() then
            if self.music_info then
                GlobalAudio:getInstance():playMusic(self.music_info.type, self.music_info.name, self.music_info.is_loop)
            end
        else
            self:playMusic()
        end
...
function BattleSceneNewView:addUnit(vo)
    ...
    EventManager:getInstance():Fire(SceneEvent.SCENE_ADDNPC, vo)
end
