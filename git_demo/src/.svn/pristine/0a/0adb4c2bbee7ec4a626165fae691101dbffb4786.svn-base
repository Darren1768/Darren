...
function CommonScrollViewLayout:createRootWnd()
    ...
    -- self.scroll_view:setBounds(false) 
    self:registerEvent()
end
...
function CommonScrollViewLayout:checkOverShowByHorizontal()
    ...
    -- if self.is_cancelHuiTan then 
    --      if container_x > 0 then 
    --         self.container:setPositionX(0)
    --         return
    --      end 
    --      local size = self.container:getContentSize()          
    --      if size.width + container_x -670 <0 then 
    --         self.container:setPositionX(-size.width + 670)
    --         return
    --      end 
...
--==============================--
--desc:补充需要创建的
--last_x_or_y cur_x_or_y 左右滑动时传x,上下滑动传入y
--@return 
--==============================--
function CommonScrollViewLayout:supplementItemList(item_list, last_x_or_y, cur_x_or_y)
    ...
    if cur_table_num < self.max_sum then
        ...
        if self.dir == ScrollViewDir.horizontal then
            if cur_x_or_y < last_x_or_y then -- 向左,那么就创建到右边
            ...
            if cur_x_or_y > last_x_or_y then -- 向上,那么就创建到下面
                for i=1,(self.max_sum-cur_table_num) do
                    self:createList(self.data_list[max_index+i])
                end
            else
...
function CommonScrollViewLayout:setInnerContainer()
    ...
    if self.dir == ScrollViewDir.horizontal then  -- 水平
        ...
        if not self.is_change then
            container_height = num * self.item_height +  self.start_y + (num - 1) * self.space_y + base_height
        else
            container_height = num * self.item_height + self.start_y + num * self.space_y + base_height
        end
    end
...
function CommonScrollViewLayout:setItemPosition(item, index)
    ...
    local delta_pos = self.setting.delta_pos
    if delta_pos and index <= #delta_pos then
        _x = _x + delta_pos[index].x
        _y = _y + delta_pos[index].y
    end
...
function CommonScrollViewLayout:reloadData()
    if self.data_list == nil or next(self.data_list) == nil then return end
    -- 对数据重新做一次处理
...
function CommonScrollViewLayout:analysisSetting(setting)
    ...
    -- self.is_cancelHuiTan = self.setting.is_cancelHuiTan or false
    self:calculationMaxSum()
end
