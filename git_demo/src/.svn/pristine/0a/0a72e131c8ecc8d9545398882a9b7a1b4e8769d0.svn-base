...
function SignPanel:register_event(  )
	EventManager:getInstance():Bind(WelfareEvent.Update_Sign_Info,self,function ( data )
			self:createList(data)
		end)
	

	EventManager:getInstance():Bind(WelfareEvent.Sign_Success,self,function ( data )
			self:createList(data)
		end)
	...
	-- 引导中不给滑动列表
	EventManager:getInstance():Bind(GuideEvent.Update_Guide_Status_Event,self,function ( in_guide )
			if in_guide then
	            self.item_scrollview:setClickEnabled(false)
	        else
