...
function RoleReportedPanel:open_callback()
    ...
    self.title = self.main_panel:getChildByName("title_con"):getChildByName("win_title")
    ...
    -- 举报原因
    self.key_reported:setFontName('fonts/title.ttf')
    self.key_reported:setFontSize(22)
    self.key_reported:setTextColor(cc.c3b(98,53,43))
    -- 附加说明
    self.key_explain:setFontName('fonts/title.ttf')
    self.key_explain:setFontSize(22)
    self.key_explain:setTextColor(cc.c3b(98,53,43))
    -- 举报证据
    self.key_evidence:setFontName('fonts/title.ttf')
    self.key_evidence:setFontSize(22)
    self.key_evidence:setTextColor(cc.c3b(98,53,43))
    -- 勾选发言记录作为证据
    self.key_evidence2:setFontName('fonts/title.ttf')
    self.key_evidence2:setFontSize(22)
    self.key_evidence2:setTextColor(cc.c3b(71,131,45))
    -- 被举报玩家
    self.key_reported_name:setFontName('fonts/title.ttf')
    self.key_reported_name:setFontSize(22)
    self.key_reported_name:setTextColor(cc.c3b(98,53,43))
    -- 显示的举报玩家名称
    self.reported_name:setFontName('fonts/title.ttf')
    self.reported_name:setFontSize(22)
    self.reported_name:setTextColor(cc.c3b(71,131,45))
    ...
    for i=1,7 do
        ...
        if checkbox then 
            ...
            -- name:setTextColor(cc.c3b(98,53,43))
            -- name:setFontSize(22)
            -- name:align(cc.p(0,0.5),40,14)
            -- name:setFontName('fonts/mini.ttf')
            self.checkbox_list[i] = checkbox
        end
        checkbox:loadTextureFrontCross('hl_common_77.png',1)
    end
    self:setSelectCheckBox()


    local res = PathTool.getResFrame("common","common_99998")
    self.label_content = createRichLabel(22, cc.c3b(0x57,0x95,0x20), cc.p(0,1), cc.p(174,400), 6, nil, 370)
    self.label_content:setString(self.default_content_msg)
    self.main_container:addChild(self.label_content)
    --内容输入框
    self.edit_content = createEditBox(self.main_container, res,cc.size(380,110), nil, 22, nil, 22, "", nil, nil, LOADTEXT_TYPE_PLIST)
    ...
    self.edit_content:setPosition(cc.p(158,403))
    ...
    self.comfirm_btn:getChildByName("label"):setFontName('fonts/mini.ttf')
...
function RoleReportedPanel:register_event()
    ...
    for i,box in ipairs(self.checkbox_list) do
        box:addEventListener(function ( sender,event_type )
            ...
            if i ==self.select_checkbox then 
                return
            end 
...
function RoleReportedPanel:setData(data)
    ...
    -- table.insert(self.data.history,{id=1,channel=2,msg="sgagfdgsdfafasdsdfaerdg"})
    -- table.insert(self.data.history,{id=2,channel=3,msg="sgagfdgsdfafasdsdfaerdg"})
    -- table.insert(self.data.history,{id=3,channel=4,msg="sgagfdgsdfafasdsdfaerdg"})
    -- table.insert(self.data.history,{id=4,channel=3,msg="sgagfdgsdfafasdsdfaerdg"})
    self.show_list = {}
...
function RoleReportedItem:configUI(width, height)
    ...
    self.msg_label = createRichLabel(24, cc.c4b(0x95,0x53,0x22,0xff), cc.p(0,1), cc.p(50, y), 2, nil, 320)
