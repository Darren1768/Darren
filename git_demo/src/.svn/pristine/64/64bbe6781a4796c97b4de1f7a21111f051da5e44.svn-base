...
function HeroModel:updateHeroVo(info, not_show_power, is_detail_info)
    ...
    if is_add then
        ...
        if is_detail_info then
            hero_vo:setInitAttr()
            EventManager:getInstance():Fire(HeroEvent.Hero_Detail_Data_Update, hero_vo)
        else
            EventManager:getInstance():Fire(HeroEvent.Hero_Data_Update, hero_vo)
        end
    end
end
...
function HeroModel:updateHeroVoDetailedInfo(info)
    ...
    if self.hero_list[info.partner_id] then
        ...
        EventManager:getInstance():Fire(HeroEvent.Hero_Vo_Detailed_info, self.hero_list[info.partner_id])
    end
end
...
function HeroModel:delHeroDataList(list)
    ...
    EventManager:getInstance():Fire(HeroEvent.Del_Hero_Event, list)
end
...
function HeroModel:getHeroById(partner_id)
    ...
    if not self.hero_list[partner_id] then 
            if self.show_list then 
                for i,s_data in ipairs(self.show_list) do
                    if partner_id == s_data.partner_id  then
                        return  s_data
                    end
                end 
            end 
...
-- show_list
function HeroModel:setShowHero_list(data)
   self.show_list = data
end 
...
function HeroModel:updateEqm_set_List(hero_vo)
    for i=1,4 do
        self.eqm_set_List[i]=0
    end
    if hero_vo.eqm_list and #hero_vo.eqm_list >1 then 

        for i=1,4 do
            local eqm=hero_vo.eqm_list[i]
             if eqm and eqm.config and eqm.config.eqm_set>0 and eqm.config.eqm_jie>0 then
                self.eqm_set_List[i]=1
             end
            end
        end
end
function HeroModel:getEqm_set_List()
      return  self.eqm_set_List 
  end
function HeroModel:getEquipSuit(hero_vo)
    local suitList = {}  
    print("------------getEquipSuit-------0-------",#hero_vo.eqm_list)

    if hero_vo.eqm_list and #hero_vo.eqm_list >=1 then  
        print("------------getEquipSuit-------1--------")
        for i,eqm in pairs(hero_vo.eqm_list) do
            if eqm.config and eqm.config.eqm_set>0 then
                -- printLuaTable(eqm.config.eqm_set)
                local id=eqm.config.eqm_set
                local find=false
                for k,suit in ipairs(suitList) do
                    if suit[1]==id then
                        suit[2]=suit[2]+1
                        find=true
                        break
                ...
                if find==false then
                    ...
                    new_suit[1]=id
                    new_suit[2]=1
                    suitList[#suitList+1]=new_suit
                end
                end
        end

        print("------------getEquipSuit---------------")
        printLuaTable(suitList)

       -- 判断是否有大于二的
       local _big2 = false
        for a,s in ipairs(suitList) do
            if s[2]>=2 then
                _big2 =true
                break
            end
       ...
       -- 移除没有激活的 
        if  _big2 then 
            ...
            local _mid = {}
            table_sort(suitList, function(a, b) 
                return a[1] > b[1]
            end)
            _mid =suitList[1]
            suitList ={}
            table.insert(suitList ,_mid )
        end 
        ...
        -- end
        -- if #suitList==2 then
        --     if suitList[1][1] > suitList[2][1] then
        --         return suitList[1]
        --     else
        --         return suitList[2]
        --     end
...
function HeroModel:getHeroEquipList(id)
    if self.hero_list and self.hero_list[id] then 
        return self.hero_list[id].eqm_list or {}
    end
    return {}
end
-- 装备战斗力战斗力=atk*（1+atk_per/1000）*2+hp_max *（1+ hp_max_per /1000）*0.4

function HeroModel:getHeroEquipPower(hero_vo,index)
    local eqm=hero_vo.eqm_list[i]

    -- if hero_vo.eqm_list and #hero_vo.eqm_list >1 then 
    --     local eqm=hero_vo.eqm_list[index]
    --     if eqm and eqm.config and eqm.config.id == bid  then
    --        local  power= (eqm.config.atk* (1+ eqm.config.atk_per/1000)*2 +  eqm.config.hp_max*(1+eqm.config.hp_max_per/1000)*0.4 )
    --        return power
    --     end
    -- end
...
function HeroModel:getHeroAllEquipPower(hero_vo)
    local eqm=hero_vo.eqm_list[i]
    ...
    --     for i=1,4 do
    --         local eqm=hero_vo.eqm_list[i]
    --          if eqm and eqm.config  then
    --              print("-------------getHeroAllEquipPower----------------")
    --              printLuaTable(eqm.config)
    --               power= power + (eqm.config.atk* (1+ eqm.config.atk_per/1000)*2 +  eqm.config.hp_max*(1+eqm.config.hp_max_per/1000)*0.4 )
    --          end
    --     end
    -- end
...
function HeroModel:updateHolyEquipmentInfo(data)
    ...
    if self.hero_list[id] then 
        ...
        EventManager:getInstance():Fire(HeroEvent.Holy_Equipment_Update_Event,hero_vo)
    end
end
...
function HeroModel:checkLevelRedPointUpdate()
    EventManager:getInstance():Fire(HeroEvent.Level_RedPoint_Event) 
...
function HeroModel:checkEquipRedPointUpdate()
    ...
    if self.is_equip_redpoint_bag_update and self.is_equip_redpoint_hero_update then
        ...
        EventManager:getInstance():Fire(HeroEvent.Equip_RedPoint_Event)
    end
end
...
function HeroModel:checkUpgradeStarRedPointUpdate()
    ...
    if self.is_upgradestar_redpoint_bag_update and self.is_upgradestar_redpoint_hero_update then
        --清除升星红点记录
        HeroCalculate.clearAllHeroRecordByRedPointType(HeroConst.RedPointType.eRPStar)
        -- EventManager:getInstance():Fire(HeroEvent.UpgradeStar_RedPoint_Event)
    end
end
    ...
    if config then
        for i,v in pairs(config) do
            if v.need_lev > self.record_login_lev and v.need_lev <= lev then
                ...
                EventManager:getInstance():Fire(HeroEvent.Form_RedPoint_Event) 
            end
        end
    end
...
function HeroModel:updatePartnerArtifactList(data)
    ...
    if self.hero_list[id] then 
        ...
        self.hero_list[id]:updateHeroVo(data)
        EventManager:getInstance():Fire(HeroEvent.Artifact_Update_Event,hero_vo)
...
function HeroModel:updateArtifactLuckyRed(  )
    ...
    EventManager:getInstance():Fire(HeroEvent.Artifact_Lucky_Red_Event)
...
function HeroModel:sendAwake(bid,eId)
    local list = {} 
    table_insert(list, {partner_id = bid})
    table_insert(list, {item_id = eId})
    HeroController:getInstance():sender11049(list)
end
function HeroModel:sendStrenth(bid,eId,time)
    local list = {} 
    table_insert(list, {partner_id = bid})
    table_insert(list, {item_id = eId})
    if time > 1 then
    table_insert(list, {count = time})
    HeroController:getInstance():sender11013(list)
  else

    HeroController:getInstance():sender11014(list)
  end
...
function HeroModel:createStar(num, star_con, star_setting, star_width)
    ...
    local _cStar = function(star_count, res, star_list)
        local width = star_width or (29 + 1)
        ...
        for i=1,star_count do
            ...
            star_list[i]:setPositionX(x + (i-1) * width+5)
        end
...
function HeroModel:checkResonateExtractRedpoint(data)
    ...
    --红点有变化
    EventManager:getInstance():Fire(HeroEvent.Hero_Resonate_Extract_Redpoint_Event, self.is_resonate_extract_redpoint)
end
--是否有共鸣精炼经验红点
...
--更新出征按钮红点
function HeroModel:updateFormationRedPoint()
    local model = self
    ...
    RedManager:getInstance():changeStrStatus(MainuiConst.red_str.formation, is_redpoint)
end

-- 得到是否显示队伍支援
function HeroModel:getShowTeamZhiYuanByFormType(form_type)
    local config_list =Config.PartnerData.data_partner_form
    for k,config in pairs(config_list) do
        if config.form_type == form_type then 
            if config.can_employ == 1 then 
               return 1 
            end 
        end 
    end
...
-- 装备判断是否 有评分高的
function HeroModel:checkHeightScore(pos,id,score)
