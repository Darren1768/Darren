...
function GuildModel:updateGuildList(name, data_list)
    ...
    EventManager:getInstance():Fire(GuildEvent.UpdateGuildList, type, guild_list) 
end
...
function GuildModel:clearMyGuildInfo()
    ...
    -- 这里需要做清楚红点操作
    GhostdomController:getInstance():setBuildRedStatus(GhostdomConst.Build_Type.guild)
end
...
function GuildModel:updateMyGuildMemberList(data_list,type)
        ...
        for i,v in ipairs(data_list) do
            ...
            if self.my_guild_assistant[getNorKey(v.rid, v.srv_id)] ~= nil then
                ...
                if role_vo.position ~= GuildConst.post_type.member then
                    EventManager:getInstance():Fire(GuildEvent.UpdateAssistantNumEvent)
                end
            end
        end 
    else
    ...
    EventManager:getInstance():Fire(GuildEvent.UpdateMyMemberListEvent, type)
end
...
function GuildModel:updateDonateInfo(data_list)
    ...
    EventManager:getInstance():Fire(GuildEvent.UpdateDonateInfo)
    -- 设置红点
...
function GuildModel:updateGuildApplyList(data_list)
    ...
    EventManager:getInstance():Fire(GuildEvent.UpdateApplyListInfo)
...
function GuildModel:updateGuildRedStatus(type, status)
    ...
    type ~= GuildConst.red_index.pvp_skill_4 and type ~= GuildConst.red_index.pvp_skill_5 then --公会日志、技能不需要更新公会UI红点
        -- 更新场景红点状态
        GhostdomController:getInstance():setBuildRedStatus(GhostdomConst.Build_Type.guild, {bid = type, status = status})
    end

    -- 事件用于同步更新公会主ui的红点
    EventManager:getInstance():Fire(GuildEvent.UpdateGuildRedStatus, type, status)
end
...
function GuildModel:updataGuildActionRedStatus(data)
    ...
    GhostdomController:getInstance():setBuildRedStatus(GhostdomConst.Build_Type.guild, {bid = GuildConst.red_index.goal_action, status = red})
...
function GuildModel:setDonateBoxStatus(box_id)
    ...
    EventManager:getInstance():Fire(GuildEvent.UpdateDonateBoxStatus, box_id)
    self:checkDonateActivity()
end
...
function GuildModel:updateDonateActivity(value)
    ...
    EventManager:getInstance():Fire(GuildEvent.UpdateDonateBoxStatus)
    self:checkDonateActivity()
end
...
function GuildModel:initGuildNoticeList(data_list)
    ...
    EventManager:getInstance():Fire(GuildEvent.UpdateGuildNoticeList)
end
