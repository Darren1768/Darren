	...
	for _,newData in pairs(data) do
		for k,oldData in pairs(self.challengeRoleData) do
			if newData.idx == oldData.idx then
				...
				EventManager:getInstance():Fire(CrossarenaEvent.Update_Single_Challenge_Role_Event, newData)
				break
			end
		end
	end
...
function CrossarenaModel:updateCrossarenaRedStatus( bid, status )
    ...
    GhostdomController:getInstance():setBuildRedStatus(GhostdomConst.Build_Type.battle, {bid = CrossgroundConst.Red_Type.crossArena, status = arena_status})
    EventManager:getInstance():Fire(CrossarenaEvent.Update_Red_Status_Event, bid, status)
end
