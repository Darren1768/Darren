...
function ArenaChampionRankAwardsWindow:open_callback()
    ...
    if self.view_type == ArenaConst.champion_type.normal then
    	...
    	main_panel:getChildByName("win_title"):setString(TI18N("将灵奖励")) --周冠军赛奖励
    end


    local lb = self.root_wnd:getChildByName("notice_label")
	if lb then
		lb:setString(TI18N("比赛结束后奖品将发送到邮箱")) 
	end
...
function ArenaChampionRankAwardsItem:ctor()
	...
	self.root_wnd = createCSBNote(PathTool.getTargetCSB("arena/arean_champion_awards_item"))
		...
		if data.index ~= nil then
			if data.index <= 3 then
				...
				if data.rank == 0 then
					...
					local res_id = PathTool.getResFrame("common", string.format("hl_common_200%s", data.index-1))
