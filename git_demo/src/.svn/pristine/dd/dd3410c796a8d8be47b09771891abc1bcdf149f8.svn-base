...
function HeroExpeditWindow:__init()
    ...
    self.res_list = {
        ...
        {path = PathTool.getSingleImgForDownLoad("bigbg/heroexpedit","heroexpedit_bg",true), type = ResourcesType.single },
	}
	self.box_index = 0
...
function HeroExpeditWindow:openRootWnd()
	...
	if expeditData or next(expeditData) ~= nil then
		...
		self.expedit_bg:loadTexture(PathTool.getSingleImgForDownLoad("bigbg/heroexpedit","heroexpedit_bg",true), LOADTEXT_TYPE)
