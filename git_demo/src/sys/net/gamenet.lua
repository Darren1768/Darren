...
function GameNet:Connect(timeout, num)
    ...
    print('GameNet:Connect   '..self.server_host_name ..'   '..self.server_host_port)
...
function GameNet:Connected(is_suc, netid)
    ...
    self.is_client_disconnet = false
    self.send_cmd_idx = 0
    EventManager:getInstance():Fire(EventId.CONNECTED)
    -- 确认窗体
    if self.connect_alert ~= nil then
...
function GameNet:Disconnect(netid)
    ...
    self.in_async_connect = false
    self.game_net_id = -1
    EventManager:getInstance():Fire(EventId.DISCONNECT,true)
end
