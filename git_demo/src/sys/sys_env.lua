...
function SysEnv:updateEventKey(key, val)
    if key == SysEnv.keys.music_volume then
        GlobalAudio:getInstance():setMusicVolume(val/100)
    elseif key == SysEnv.keys.audio_volume then
        GlobalAudio:getInstance():setEffectVolume(val/100)
    elseif key == SysEnv.keys.voice_volume then
        GlobalAudio:getInstance():setRecordVolume(val/100)
    elseif key == SysEnv.keys.music_is_open then
        if val == false then
            GlobalAudio:getInstance():setMusicVolume(0)
        elseif val == true then
            GlobalAudio:getInstance():setMusicVolume(SysEnv:getInstance():getNum(SysEnv.keys.music_volume, 100)/100)
        end
    elseif key == SysEnv.keys.audio_is_open then
        if val == false then
            GlobalAudio:getInstance():setEffectVolume(0)
        elseif val == true then
            GlobalAudio:getInstance():setEffectVolume(SysEnv:getInstance():getNum(SysEnv.keys.audio_volume, 100)/100)
        end
    elseif key == SysEnv.keys.voice_is_open then
        if val == false then
            GlobalAudio:getInstance():setRecordVolume(0)
        elseif val == true then
            GlobalAudio:getInstance():setRecordVolume(SysEnv:getInstance():getNum(SysEnv.keys.voice_volume, 100)/100)
        end
