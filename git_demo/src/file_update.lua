...
function FileUpdate:ver_load_start()
    ...
    -- printJavaMsg("========FileUpdate ==ver_load_start")
...
function FileUpdate:initUpdateScene(delay_func)
    ...
    -- printJavaMsg("========FileUpdate ==initUpdateScene")
    -- 创建创景节点
...
function FileUpdate:showFirstPage(force, layout, update_scene, callback)
    ...
    -- 加载进度以及版本号更新提示
    local version = VersionView.new(layout)
...
function FileUpdate:initConfig(layout)
    -- printJavaMsg("=========FileUpdate:initConfig")
    if self.update_scene == nil then return end
    local logo_res = string.format("resource/login/%s/txt_cn_logo.png", PLATFORM_PNG) 
    local bg = createSprite(logo_res, SCREEN_WIDTH*0.5+80, SCREEN_HEIGHT*0.5+80, layout, cc.p(0.5,0.5), LOADTEXT_TYPE)
    bg:setScale(0.5)
...
function FileUpdate:down_load_url_by_fmodex(package_ver)
    ...
    -- printJavaMsg("=========FileUpdate:down_load_url_by_fmodex,服务器地址::"..url)
            ...
            if #plist_list == 0 and downplisting == false then -- 这种表示全部加载完全
                ...
                if downplisting == false then
                    ...
                    if USE_PVR_CCZ == true then
                        prvCcz = string.format("%s.pvr.ccz", plist_file)
                    else
                        prvCcz = string.format("%s.png", plist_file)
                    end
