...
function PathTool.checkRes(path)
    ...
    if isExist then
        ...
        if not MAKELIFEBETTER then
            if string.find(path, ".pvr.ccz") ~= nil or string.find(path, ".png") ~= nil or string.find(path, ".jpg") ~= nil or string.find(path, ".lua") ~= nil and IS_REQUIRE_RES_GY == true then
                print(string_format("============> 警告：资源 %s 不存在，尝试去cdn上下载", path))
                ResourcesLoadMgr:getInstance():checkResouces(path, true)
            end
        else
            if string.find(path, ".lua") ~= nil then
                print(string_format("============> 警告：资源 %s 不存在", path))
            end
        end
...
function PathTool.getNormalSkillBg()
    return PathTool.getResFrame("common", "hl_common_91")
end
...
PathTool.AttrIcon = {
    ["atk"] 		= "hl_common_204", --攻击
    ["atk_per"]		= "hl_common_204", --攻击
    ["hp"] 			= "hl_common_208", --血
    ["hp_max"] 		= "hl_common_208", --血
    ["hp_max_per"]  = "hl_common_208", --血
    ["def"] 		= "hl_common_213", --防御
    ["def_per"] 	= "hl_common_213", --防御
    ["speed"] 		= "hl_common_207", --速度
    ["crit_rate"] 	= "hl_common_205", --暴击率
    ["crit_ratio"] 	= "hl_common_209", --暴伤害
    ["hit_magic"] 	= "hl_common_247", --控制
    ["dodge_magic"] = "hl_common_211", --抗控
    ["tenacity"] 	= "hl_common_212", --抗暴
    ["hit_rate"] 	= "common_90021_2", --命中
    ["res"] 		= "hl_common_213", --免伤
    ["dodge_rate"] 	= "common_90021_4", --闪避
    ["cure"] 		= "common_90021_5", --治疗
    ["be_cure"] 	= "common_90021_6", --被治疗
    ["dam"] 		= "common_90021_7", --伤害加成

    ["dam_p"]       = "hl_common_204", --物伤
    ["dam_s"]       = "hl_common_204", --法伤
    ["res_p"]       = "hl_common_213", --物免
    ["res_s"]       = "hl_common_213", --法免
...
function PathTool.getQualityBg(quality)
    ...
    quality = 300 + quality
    local res_id = "hl_common_" .. quality
...
-- 获取品质框背景(长方的)
function PathTool.getRectQualityBg(quality)
    local quality = quality or 1
    if quality > 5 then
        quality = 5
    end
    ...
    local res_id = "hl_common_" .. quality.."_2"
    return PathTool.getResFrame("common2", res_id)
end
-- 获取布阵左边
function PathTool.getFromQualityBg(quality)
    local quality = quality or 1
    if quality > 5 then
        quality = 5
    end
    if quality ==0 then
    --    return  PathTool.getResFrame("common", "hl_common_2115")  
    return  PathTool.getResFrame("common2", "hl_common_quality_1")  

    end

    local res_id = "hl_common_quality_" .. quality
    return PathTool.getResFrame("common2", res_id)
end


function PathTool.getRectQualityBox(quality)
    local quality = quality or 1
    if quality > 5 then
        quality = 5
    end
    ...
    local res_id = "hl_common_" .. quality.."_1"
    return PathTool.getResFrame("common2", res_id)
end

function PathTool.geHeroS(quality)
    local quality = quality or 1
    if quality > 5 then
        quality = 5
    end
...
function PathTool.geHeroS_bg(quality)
    local quality = quality or 1
    if quality > 5 then
        quality = 5
    end
    local res_id = "hl_common_" .. (2044 + quality)
    return PathTool.getResFrame("common", res_id)
end



function PathTool.geHeroSpineShadow(camp_type)
    local camp_type = camp_type or 1
    if camp_type > 5 then
        camp_type = 5
    end
    if camp_type < 1 then
    ...
    local res_id = "hl_common_211" .. (camp_type -1)
    return PathTool.getResFrame("common", res_id)
end
...
function PathTool.getHeroCampTypeIcon(_type)
    ...
    _type = _type or 1
    -- if _type == HeroConst.CampType.eWater then -- 
    --     return PathTool.getResFrame("common", "hl_common_515")
    -- elseif _type == HeroConst.CampType.eFire then -- 
    --     return PathTool.getResFrame("common", "hl_common_519")
    -- elseif _type == HeroConst.CampType.eWind then  -- 
    --     return PathTool.getResFrame("common", "hl_common_517")
    -- elseif _type == HeroConst.CampType.eLight then  -- 
    --     return PathTool.getResFrame("common", "hl_common_511")
    -- elseif _type == HeroConst.CampType.eLingtDark then  -- 
    --     return PathTool.getResFrame("common", "hl_common_513")
    ...
    elseif _type == HeroConst.CampType.eLight then  -- 
        return PathTool.getResFrame("common", "hl_common_511")
    elseif _type == HeroConst.CampType.eDark then  -- 
        return PathTool.getResFrame("common", "hl_common_519")
    else--if _type == HeroConst.CampType.eDark then
        return PathTool.getResFrame("common", "hl_common_511")
    end
...
function PathTool.getCampGroupIcon( res_id )
    ...
    return PathTool.checkRes(string_format("resource/campicon/hl_campicon_%s.png", res_id))
end
...
function PathTool.getEffectRes(id)
    print("spine:",Config.EffectData.data_effect_info[id]);
...
function PathTool.getTargetRes(packageName, resName, is_plist, is_jpg)
    ...
    if is_plist then
        ...
        if USE_PVR_CCZ == true then
            texture = PathTool.checkRes(string_format("resource/%s/%s.pvr.ccz", packageName, resName))
        end
...
function PathTool.getPlistImgForDownLoad(packageName, resName, is_jpg)
    if is_jpg == true then
        ...
        if USE_PVR_CCZ == true then
            return string_format("resource/%s/%s.pvr.ccz", packageName, resName)
        else
...
--@return 
--==============================--
function PathTool.getSingleImgForDownLoad(packageName, resName, is_jpg)
    if is_jpg == true then
        return string_format("resource/%s/%s.jpg", packageName, resName)
    else
        return string_format("resource/%s/%s.png", packageName, resName)
...
print("CSB====", resName)
    if USE_RES_LUA == true then
        return PathTool.checkRes(string_format("luascript/%s.lua", resName))
    end
...
function PathTool.getResFrame(packageName, resName, is_jpg, plist_file, call_back)
    ...
    if not frame then
        ...
        if display.isSpriteFramesWithFileLoaded(plist) then
            if not string.find(resName,'/') then
                resName = string.format("resource/%s/segment/%s",packageName, resName)
                if cc.SpriteFrameCache:getInstance():getSpriteFrame(resName) then
                    return resName
                end
            end
            print("!!!!!!!!!getResFrame ERROR PNG:!!!!!!!!!!!",resName)
            resName = string_format("%s.png", "common_99999") 
            return resName
        else
...
function PathTool.getSound(sound_type, sound_name)
    ...
    if sound_type == GlobalAudio.AUDIO_TYPE.COMMON then
        path = string_format("sound/%s/%s.mp3", sound_type, sound_name)
    elseif sound_type == GlobalAudio.AUDIO_TYPE.SCENE then
        path = string_format("sound/%s/%s.mp3", sound_type, sound_name)
    elseif sound_type == GlobalAudio.AUDIO_TYPE.BATTLE then
        path = string_format("sound/%s/%s.mp3", sound_type, sound_name)
    elseif sound_type == GlobalAudio.AUDIO_TYPE.DUBBING then
        path = string_format("sound/%s/%s.mp3", sound_type, sound_name)
    elseif sound_type == GlobalAudio.AUDIO_TYPE.Recruit then
        path = string_format("sound/%s/%s.mp3", sound_type, sound_name)
    elseif sound_type == GlobalAudio.AUDIO_TYPE.Drama then
        path = string_format("sound/%s/%s.mp3", sound_type, sound_name)
    end
    return path	
end
...
--==============================--
function PathTool.getSpineByName(spine_name, action_name,is_big)
    local spine_path = ""
    ...
    if is_big then 
        if action_name == nil then
            spine_path = string_format("spine_1/%s/action", spine_name)
        else
            spine_path = string_format("spine_1/%s/%s", spine_name, action_name)
        end
    end
    ...
    if not PathTool.isFileExist(atlas) then
        if is_big then  
            spine_path = string_format("spine_1/%s/role", spine_name)
        else
            spine_path = string_format("spine/%s/role", spine_name)
        end 

        skel = string_format("%s.skel", spine_path)
        atlas = string_format("%s.atlas", spine_path)
        png = string_format("%s.png", spine_path)
