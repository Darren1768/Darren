...
function TimeTool.GetTimeFormatVertical(less_time)
    less_time = tonumber(less_time) or 0
    local hour = math.floor(less_time / 3600)
    local min = math.floor((less_time % 3600) / 60)
    local sec = less_time % 3600 % 60
...
--
function TimeTool.GetTimeFormatDayIIIIIII2(less_time)
    less_time = tonumber(less_time) or 0
    local day = math.floor(less_time/86400)
    local lessT = math.floor(less_time%86400)
    local hour = math.floor(lessT / 3600)
...
-- 周末时间倒计时
function TimeTool.getLastWeekNum()

    local _week = TimeTool.getWeekNum()
    local _last_Day = 7 - _week
    local _time = TimeTool.getOneDayLessTime()

    local last_time = _time + _last_Day*86400
    return last_time
end
